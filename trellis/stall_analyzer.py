...
- external runtime root (`protocol_state.json`) + per-cycle event log in the repo
...
class RuntimeLayout:
    ...
    # (`<repo>/.trellis-history/event-log/`). Recorded for layout
    # completeness; the analyzer keys off protocol_state.json, not the log.
    event_log_dir: Path
    state_dir: Path
