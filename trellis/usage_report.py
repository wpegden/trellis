...
def _event_log_cycle_files(event_log_dir: Path) -> list:
    """Sorted per-cycle event-log files (`cycle-NNNNNN.jsonl`) in global order."""
    if not event_log_dir.is_dir():
        return []
    files = [
        p for p in event_log_dir.iterdir()
...
def _roll_event_log_by_stage(event_log_dir: Path):
    ...
    skipped. Reads the per-cycle files in global index order so deltas span
    cycle boundaries correctly.
    """
    rows = []
...
def report(runtime_root: Path, ledger_path: Optional[Path] = None) -> None:
    ...
    repo = meta.get("repo_path")
    event_log_dir = (
        Path(repo) / ".trellis-history" / "event-log"
        if repo
        else runtime_root / ".event-log-fallback"
    )
    if event_log_dir.is_dir():
        by_stage, n_intervals = _roll_event_log_by_stage(event_log_dir)
