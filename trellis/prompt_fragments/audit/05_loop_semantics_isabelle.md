...
- Isabelle library theorems you've already searched for and didn't find.
...
After the audit produces tasks and the reviewer works through them, the reviewer may set `cleanup_request_reaudit = true` on a Done decision. If `cleanup_audit_round < max_rounds` — the request summary carries both as `audit_round` and `max_rounds` — the kernel transitions back to a fresh audit round. You'll see:
- The task list with terminal-status entries from earlier rounds preserved (Completed/Failed/Dismissed).
- Any leftover Pending tasks from earlier rounds (you may revise them now).
- An empty scratchpad.
- `cleanup_audit_round` incremented by one.

Treat each later round as a second look after workers have swung at the earlier rounds' proposals. This is the time to revisit tasks that depended on each other, or to propose tweaks based on what those workers learned. The kernel-authored views are rebuilt for every round, so they describe the corpus as the completed work has left it.
