...
**Step A (this cycle)**: emit `global_repair_request={proposed_extension_node_ids: [...], reason: "..."}` with `decision=continue`, `reset=none`, `next_mode` kept at the current request's mode, and every action field default (no authorized_nodes, no next_active, no task_blockers). The kernel routes to a StuckMathAudit burst that confirms or declines the request. For process-routing/scope impasses, use this global-repair route.
...
Rate limit: at most one Step A per `stuck_math_audit_dispatch_cooldown_cycles` while a recent decline reason is in scope. A Step A emitted while `pending_global_repair_grant` is pending is legal and supersedes that grant: the kernel drops it and surfaces it to the adjudicating audit as `superseded_global_repair_grant`, so a grant whose envelope misses the repair you now need is replaced by re-requesting with the right scope.
