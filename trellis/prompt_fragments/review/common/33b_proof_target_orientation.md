...
Support surfaces for plans are registered nodes' principal declarations; when the needed fact is an auxiliary declaration inside another node, route the worker to factor it out — move its statement and proof into a new Lean-closed helper node and point the owning node's proof at it.
