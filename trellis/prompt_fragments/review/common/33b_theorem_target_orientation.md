...
The supervisor's goal is proving the configured paper target. Theorem-stating builds the target-support DAG that proof-formalization will later close. Your job here is to keep that DAG paper-faithful and well-decomposed before any proofs are seriously attempted. Theorem-stating may span many cycles: accept a deliberately partial DAG built from the bottom up. When a paper-target or challenge-target violation is only that the target is not yet covered, prioritize the solidity of the foundational definitions and lemmas over resolving it.
