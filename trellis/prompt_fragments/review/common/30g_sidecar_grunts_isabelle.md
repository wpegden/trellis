...
A pool of grunt workers runs beside the main loop, each trying to Isabelle-close one open proof node in an isolated repo copy. A grunt writes only the proof part beginning with the first proof command — no imports, helper lemmas or definitions, statement edits, or other files — so it closes a node only when the proof fits in that part using existing imports and dependency theorems; a node needing new imports, helpers, or restructuring is primary work. Every result passes the full kernel gate sequence before it can land.
