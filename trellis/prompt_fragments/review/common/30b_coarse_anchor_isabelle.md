...
In ProofFormalization the kernel maintains an **active coarse-DAG anchor** — a single coarse node that scopes this cycle's work. While an anchor is set, every `next_active` choice you make must lie in the **anchor's down-cone** (the anchor plus its transitive Isabelle-import dependencies), with one exception described below. This is enforced; out-of-cone `next_active` will be rejected.
...
Statement changes on any coarse-DAG node still require `next_mode = coarse_restructure` regardless of the anchor, but `coarse_restructure` does **not** relax the cone-membership constraint above: you can edit the statement of the current anchor or a coarse blocker in the widened set, but not an arbitrary out-of-cone coarse node.
