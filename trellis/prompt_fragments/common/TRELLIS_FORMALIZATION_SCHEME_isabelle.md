...
Trellis formalizes a natural-language mathematical paper into Isabelle/HOL by maintaining a proof-tablet DAG in `Tablet/`. This is not a one-shot translation workflow. It is a supervised multi-cycle process in which workers propose repository changes, verifiers check specific invariants, and a reviewer decides the next step.

The point of the system is not merely to produce Isabelle code. The point is to build a paper-faithful, verifiable support structure that carries the full mathematical content of the paper and can eventually be closed into a complete Isabelle/HOL development.

## Core objects

- Each node has a paired `Tablet/<Node>.thy` and `Tablet/<Node>.tex`.
- `imports Tablet_<OtherNode>` edges in Isabelle define the support DAG.
- `Tablet/Preamble.thy` is the shared import root. It should contain imports only, not project definitions.
...
- Definition nodes use theory-declaration commands such as `definition`, `abbreviation`, and `type_synonym`, and the data declarations `datatype`, `record`, `fun`, `primrec`, and `inductive`. These do not open an Isar proof; `fun`/`datatype`/`primrec` discharge their obligations internally.
- Proof-bearing nodes use theorem-like commands such as `theorem`, `lemma`, `corollary`, or `proposition`. The statement ends where the proof part begins (`proof`, `by`, `apply`, `using`, `unfolding`, `sorry`, `oops`); there is no separate body marker.

## What the tablet is supposed to represent

The tablet is meant to be a mathematically meaningful decomposition of the paper that is a useful template for Isabelle formalization. In a good tablet:
...
- the DAG gives a faithful scaffold for eventual Isabelle formalization
...
- Every present node has both a Isabelle artifact and an NL artifact.

### Correspondence invariants

- The Isabelle statement and the NL statement for a node must genuinely correspond in mathematical meaning.
...
- Every proof-bearing node must end in one of two acceptable states:
  - a complete Isabelle proof with no `sorry`
...
- In theorem-stating, proof-bearing nodes usually begin with NL proofs and may still contain Isabelle `sorry`s.
- In proof-formalization, those Isabelle `sorry`s are eliminated node by node.

### Definition and import hygiene

- Definitions should have actual bodies, not `sorry`, `axiom`, `opaque`, or placeholder constants. Definitions whose body is just `True` will be autofailed for correspondence by the kernel; this is the preferred placeholder for a Definition body whose form is being deferred as work for a future cycle.
- Library imports should be specific. A node normally needs only `Tablet_Preamble` plus the sibling nodes it genuinely depends on; the warm session base already provides the heavy library (`Main` / `Complex_Main` / `HOL-Probability`), so a node should not re-import it wholesale.
- Project-local definitions should not duplicate existing Isabelle library concepts unnecessarily; search the library (`find_theorems`, `find_consts`) before introducing a new constant.
...
- proof-bearing nodes typically carry detailed NL proofs and may still have Isabelle `sorry`s
...
The goal of proof-formalization is to replace theorem-stating's provisional Isabelle proofs with complete Isabelle proofs, without losing the semantic structure already established.
...
Proof-formalization does not mean “forget the theorem-stating structure.” It means closing the Isabelle side of the already-vetted DAG, or enriching that DAG's structure in authorized ways to make it more amenable to formalization.
...
9. If substantiveness passes, correspondence verifiers check node-level Isabelle-vs-TeX statement alignment.
