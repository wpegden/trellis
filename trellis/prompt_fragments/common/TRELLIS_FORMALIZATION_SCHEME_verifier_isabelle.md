...
- Definition nodes use definition-like theory-declaration commands such as `definition`, `abbreviation`, and `type_synonym`, and the data declarations `datatype`, `record`, `fun`, `primrec`, and `inductive` (whose constructors/fields follow the command's `=` or `where`). Isabelle has no separate body marker: a declaration's statement ends where its proof part begins (`proof`, `by`, `apply`, `using`, `unfolding`, `sorry`, `oops`). A contentless data declaration is the deferred-definition placeholder and is kernel-autofailed for correspondence.
- Proof-bearing nodes use theorem-like Isabelle declarations such as `theorem`, `lemma`, and `instance` (an instance discharges a class's law fields and pairs to a `lemma`-style `.tex` claim). `.tex` labels like `corollary` and `helper` are statement-environment categories, not separate Isabelle declaration keywords.
...
- proof-bearing nodes have either a complete Isabelle proof or a rigorous NL proof at the same level of detail as the paper
...
- Library imports should be specific: a node normally needs only `Tablet_Preamble` plus the sibling nodes it genuinely depends on, since the warm session base already provides `Main`/`Complex_Main`/`HOL-Probability`.
- Project-local definitions should not duplicate existing Isabelle library concepts unnecessarily.
...
The goal of theorem-stating is to construct a faithful, useful support DAG for the paper. In this phase, proof-bearing nodes typically carry detailed NL proofs and may still have Isabelle `sorry`s. Paper-faithfulness is checked first, then per-node substantiveness, then node-level correspondence, and finally soundness on the current active proof target.

### 2. Proof-formalization

The goal of proof-formalization is to replace theorem-stating's provisional Isabelle proofs with complete Isabelle proofs, without losing the semantic structure already established. Work is node-centered.
