...
- **Process memory** — durable cross-burst findings under `process-memory/`
  (`MemoryOperation`: add / supersede / retire). The structural, need-input,
  global-repair, and initial/coverage-planner audit scenarios write
  `memory_operations`; operations from any other lane are rejected
  (`stuck_math_audit_validation_failure`). Workers and reviewers
