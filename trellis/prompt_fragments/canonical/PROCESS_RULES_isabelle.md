...
- **TheoremStating** — statement authoring: workers build the `.tex`+Isabelle statement DAG;
...
`must_close_active` (the active node must arrive Isabelle-closed — sorry-free with its local
...
- claim targets: `target_claim_updates` (paper targets), `challenge_claim_updates`
...
- create new live orphan nodes only while the uncovered-target construction window is open
  ...
  gates reject a valid response that leaves new live orphans; in Cleanup the post-burst
  orphan set must be a subset of the dispatch baseline orphan set;
...
- **Correspondence** (`RequestKind::Corr`) — the node's Isabelle statement matches its own `.tex`
...
- **Soundness** (`RequestKind::Sound`) — the natural-language proof is mathematically
  valid. This is a prose-proof lane; Isabelle closure is a separate mechanical gate:
  `SOUNDNESS.md`.
...
re-planning cadence. The seven scenarios and what each may output:
...
- **Challenge targets** — byte-pinned goal statements
...
- **Blockers** — verifier-lane verdicts and derived conditions
  ...
  plus ChallengeCoverage) whose union (`global_blockers`) gates cleanliness and phase
