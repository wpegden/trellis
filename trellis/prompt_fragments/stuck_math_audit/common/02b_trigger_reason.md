...
- `kernel requested audit (approach): ...` — a kernel counter crossed a reporting threshold; the detail after the colon states the observation. The count is an observation, not a verdict — assess the situation with full judgment and choose the route yourself.
- `coverage re-planning:` — configured targets remain uncovered; scheduled cadence burst to assess the current plan and re-plan the route to full coverage.
