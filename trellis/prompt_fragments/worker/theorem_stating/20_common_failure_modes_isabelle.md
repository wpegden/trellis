...
- Definition nodes use `definition`, `abbreviation`, `type_synonym`, `datatype`, `record`, `fun`, or `primrec`; proof-bearing nodes use `theorem`, `lemma`, `corollary`, or `proposition`.
...
- **Restating an axiom or hypothesis.** A node whose content is identical to a paper-level assumption with no new content. Cite the assumption directly. If a node is redundant, retarget consumers and delete it when unsupported and authorized.
...
- The Isabelle statement must encode every domain assumption the paper's statement carries, including ones the paper's prose leaves implicit.
