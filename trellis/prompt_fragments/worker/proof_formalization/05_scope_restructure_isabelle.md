...
`restructure` lets you change the active node's statement only if the active node is not in `scope_contract.coarse_dag_nodes`, meaning it was introduced during proof-formalization rather than approved during theorem-stating. Statement repairs on coarse-DAG nodes require `coarse_restructure`; if you need that broader scope, return `needs_restructure`.
