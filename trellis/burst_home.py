...
# Top-level entries of each provider dir that are never provider auth/config
# state and must not be seeded into a burst home: they are the operator's
# session transcripts, job scratch and logs. Seeding them made a burst home
# hold millions of hard links (one seed walked 1.5M files for several
# minutes before codex could start) and handed an untrusted worker readable
# copies of every Claude Code / Codex conversation on the host. Names are
# matched with fnmatch against the entry name directly under the provider
# dir; nested paths are untouched.
_PROVIDER_HOME_SEED_PRUNE: dict[str, tuple[str, ...]] = {
    ".codex": (
        "logs_*.sqlite*", "log", "sessions", "archived_sessions",
        "history.jsonl", "db-backups", "tmp", ".tmp", "shell_snapshots",
        # The host's thread/state databases index the operator's own
        # conversations (and point at rollouts under the host's sessions
        # dir). A burst home starts its own; codex creates them on demand.
        "state_*.sqlite*", "thread_history_*", "memories_*", "goals_*", "queue_*",
    ),
    ".claude": (
        "jobs", "projects", "file-history", "sessions", "tasks", "todos",
        "shell-snapshots", "debug", "history.jsonl", "daemon", "daemon.log",
        "uploads", "backups", "session-env", "paste-cache", "statsig",
        "telemetry",
    ),
    ".gemini": ("tmp",),
}
...
def _hardlink_tree(src: Path, dst: Path, *, prune: tuple[str, ...] = ()) -> None:
    ...
    for root, dirs, files in walker:
        ...
        if prune and rel == Path("."):
            dirs[:] = [d for d in dirs if not any(fnmatch.fnmatch(d, pat) for pat in prune)]
            files = [f for f in files if not any(fnmatch.fnmatch(f, pat) for pat in prune)]
        target_dir = dst / rel
        try:
    ...
    *,
    ...
    for name in subdirs:
        ...
        if not src_subdir.exists() or not src_subdir.is_dir():
            continue
        _hardlink_tree(
            src_subdir, target / name, prune=_PROVIDER_HOME_SEED_PRUNE.get(name, ())
        )
