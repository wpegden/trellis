...
from trellis.agents import antigravity as _agy
from trellis.config import SandboxConfig
from trellis.host_runtime import worker_path_env
from trellis.git_trust import launch_failure
from trellis.sandbox import (
    repo_targets_isabelle,
    wrap_command as _project_wrap_command,
)
...
# --- forgotten-done-marker recovery ----------------------------------------
#
# The antigravity (Gemini) worker frequently finishes a burst — writing a
# valid `<id>.raw.json` — but forgets to write the final `<id>.done` marker,
# so the wait loop never sees completion and the run wedges (observed
# repeatedly: connectivity-isa workers 2, 4, 5). The GEMINI.md
# re-read-before-the-marker directive did NOT prevent it (the worker reads it
# and ignores it on long bursts), so the recovery must live in the wait logic.
#
# Recovery fires ONLY when the marker is missing but the raw artifact is a
# settled, valid result: it exists, parses as JSON with a non-empty `outcome`
# field, AND its mtime has been unchanged for at least the threshold below.
# The stability + validity gates are what make this safe — a recent, growing,
# or unparseable raw artifact never triggers it, so the only thing this ever
# rescues is a genuinely-forgotten marker. The threshold is env-tunable via
# TRELLIS_FORGOTTEN_DONE_RECOVERY_SECONDS (default 120s); <= 0 disables it.
def _forgotten_done_recovery_seconds() -> float:
    raw = os.environ.get("TRELLIS_FORGOTTEN_DONE_RECOVERY_SECONDS")
...
def _raw_path_for_done(done_file: Path) -> Path:
    """The `<stem>.raw.json` sibling of a `<stem>.done` marker.

    Mirrors trellis.artifacts.{done_marker_path,raw_json_path}: both live in
    the same staging dir and share the artifact stem.
    """
    return done_file.with_name(f"{done_file.stem}.raw.json")


def _raw_artifact_is_recoverable(raw_path: Path) -> bool:
    """True iff `raw_path` is a finished, valid result safe to mark done.

    Parses the file and requires a non-empty `outcome` field. Any read/parse
    failure, missing file, or empty/missing outcome returns False so an
    in-progress or malformed write is never recovered.
    """
    try:
    ...
    if not isinstance(data, dict):
        return False
    return bool(str(data.get("outcome", "") or "").strip())
...
def burst_launch_path_env(burst_home: Path, work_dir: Optional[Path]) -> str:
    """PATH for an agent burst launch, adding `isabelle` on isabelle_hol runs.

    The bwrap sandbox inherits PATH from this parent-process prefix (no
    `--setenv PATH`). On an isabelle_hol repo the worker/reviewer/verifier
    burst must find `isabelle` on PATH for its interactive proof loop, so we
    prepend `<isabelle_home>/bin` (which `wrap_command` also RO-binds). Gated
    on the repo's `default_target` via `repo_targets_isabelle`; a Lean repo
    yields the unchanged `worker_path_env(burst_home)`, so the Lean launch
    PATH stays byte-identical.
    """
    isabelle = bool(work_dir is not None and repo_targets_isabelle(Path(work_dir)))
    return worker_path_env(burst_home, isabelle=isabelle)
...
def agent_is_busy(screen: str, provider: str) -> bool:
    ...
    if provider == "antigravity":
        return any(m in t for m in _agy.BUSY_MARKERS)
    return False
...
class AgentHandle:
    ...
    preflight_command: Optional[List[str]] = None
...
def wait_until_prompt_ready(h: AgentHandle, *, timeout: float = 60.0, probe_every: float = 0.5) -> bool:
    ...
    while time.monotonic() < deadline:
        ...
        elif h.provider == "antigravity":
            box_empty = _agy.input_box_is_empty(norm)
        else:
            box_empty = False
        if box_empty:
            consecutive_ready += 1
...
def wait_until_idle(
    ...
    *,
    ...
    #
    # The Isabelle branch hardcoded this to False on 2026-06-22 because the 300s
    # zombie-catcher killed long-silent max-effort Claude workers mid-think.
    # Master fixed the same failure mode without losing the catcher: the
    # claude/gemini burst runners now pass apparent_stall_seconds=5400.0 (90 min),
    # which covers an extended think while still catching a truly wedged
    # reasoning thread. That supersedes the blanket disable.
    ...
    # Forgotten-done-marker recovery: track the raw artifact's mtime so we can
    # tell a settled result from an in-progress write. `raw_recover_path` is the
    # `<stem>.raw.json` sibling of the done marker; `raw_recover_seconds` is the
    # stability threshold (<=0 disables). `raw_first_seen_*` remember when the
    # current mtime first appeared so a growing/just-written file resets the
    # window instead of being recovered prematurely.
    raw_recover_seconds = _forgotten_done_recovery_seconds() if done_file is not None else 0.0
    raw_recover_path = _raw_path_for_done(done_file) if done_file is not None else None
    raw_first_seen_mtime_ns: Optional[int] = None
    raw_first_seen_at: float = 0.0
    ...
    while time.monotonic() - last_inactivity_reset < total_timeout:
        ...
        # Forgotten-done-marker recovery (see _raw_artifact_is_recoverable).
        # The worker wrote a valid raw.json but never the .done marker. If the
        # raw artifact has been mtime-stable for `raw_recover_seconds` AND
        # parses with a non-empty `outcome`, write the marker ourselves and
        # proceed as if the worker had — preserving the normal done_file path
        # for every downstream consumer. Stability is what gates out a write
        # still in flight: a growing file bumps mtime, resetting the window.
        if raw_recover_seconds > 0 and raw_recover_path is not None:
            try:
                cur_raw_mtime_ns: Optional[int] = raw_recover_path.stat().st_mtime_ns
            except OSError:
                cur_raw_mtime_ns = None
                raw_first_seen_mtime_ns = None
            if cur_raw_mtime_ns is not None:
                now = time.monotonic()
                if cur_raw_mtime_ns != raw_first_seen_mtime_ns:
                    raw_first_seen_mtime_ns = cur_raw_mtime_ns
                    raw_first_seen_at = now
                elif (
                    now - raw_first_seen_at >= raw_recover_seconds
                    and _raw_artifact_is_recoverable(raw_recover_path)
                ):
                    stable_for = now - raw_first_seen_at
                    try:
                        done_file.write_text("recovered\n", encoding="utf-8")
                    except OSError:
                        # Couldn't write the marker (perms/disk) — fall through
                        # and keep waiting; nothing else changes.
                        pass
                    else:
                        sys.stderr.write(
                            "WARN: recovered forgotten done-marker for "
                            f"{done_file.name} (raw stable {stable_for:.0f}s)\n"
                        )
                        sys.stderr.flush()
                        emit_event(
                            "forgotten_done_recovered",
                            session=h.session,
                            provider=h.provider,
                            detail={
                                "done_marker": done_file.name,
                                "raw_artifact": raw_recover_path.name,
                                "raw_stable_seconds": round(stable_for, 1),
                                "threshold_seconds": raw_recover_seconds,
                            },
                        )
                        return True, "done_file"
...
_CODEX_PRICING_USD_PER_MTOK = {
    ...
    # 2026-07-30 cut: terra -20%, luna -80% (sol unchanged). Cached is
    # 10% of input across this table, and luna's post-cut cached rate is
    # confirmed at 0.02, so terra's follows the same rule.
...
def write_chat_artifacts(
    ...
    *,
    ...
    # Chat-dir naming: when `artifact_prefix` is the kernel canonical artifact
    # name (`trellis_<kind>_<request_id>_<suffix>`), name the dir by it
    # directly — this id-keys the dir so the viewer's request-id resolver
    # (`resolveBurstArtifacts` "Preferred A": `^trellis_<tag>_<rid>`) surfaces
    # the burst, exactly as the codex backend already does. The worker /
    # reviewer-review bursts previously fell back to the id-less session-scope
    # form (`{role}_{phase}:{role}:...`) and so never resolved by request id —
    # the viewer dropped them. Legacy scope-form prefixes keep the `{role}_`
    # prefix for backward compatibility.
    if str(artifact_prefix).startswith("trellis_"):
        dir_name = str(artifact_prefix)
        if request_id is None:
            m = re.search(r"^trellis_[a-z_]+?_(\d+)_", str(artifact_prefix))
            if m:
                request_id = int(m.group(1))
    else:
        dir_name = f"{role}_{artifact_prefix}"
    target = cycle_dir / dir_name
...
def agent_session_transcript_mtime_ns(
    ...
    *,
    ...
    if provider == "antigravity":
        return _agy.latest_conversation_mtime_ns(cwd, home=home)
    return 0
...
def _dialogs_for(provider: str):
    return {
        "claude": _CLAUDE_DIALOGS,
        "gemini": _GEMINI_DIALOGS,
        "antigravity": _agy.DIALOGS,
    }.get(provider, [])
...
def settle_until_ready(
    ...
    *,
    ...
    last_dialog_at = 0.0
    while time.monotonic() < deadline:
        if h.preflight_command and launch_failure(h.preflight_command) is not None:
            return False, seen + ["git_preflight_failed"]
        if pane_dead(h.session):
...
def agent_requires_auth(screen: str, provider: str) -> bool:
    t = strip_ansi(screen)
    if provider == "claude":
        markers: tuple = _CLAUDE_AUTH_MARKERS
    elif provider == "antigravity":
        markers = _agy.AUTH_MARKERS
    else:
        markers = _GEMINI_AUTH_MARKERS
...
def confirm_prompt_delivery(
    ...
    *,
    ...
    if h.provider == "claude":
        ...
        box_empty = _agy.input_box_is_empty(last_norm)
    else:
        box_empty = False
    if box_empty:
...
def run_claude_burst(
    *,
    ...
    for attempt in range(max_restarts + 1):
        ...
        wrapped = ["env", f"PATH={burst_launch_path_env(home_for_path, cwd)}", *wrapped]
        ...
        emit_event("launch", session=session_tmux, provider="claude",
                   ...
                   detail={"kind": kind, "model": model, "effort": effort,
                           "session_id": session_id,
                           "sandbox": sandbox_enabled})
        h = AgentHandle(session=session_tmux, cwd=cwd, provider="claude", session_id=session_id, preflight_command=wrapped)
        ready, dialogs = settle_until_ready(h, total_timeout=600.0)
        emit_event("settle", session=session_tmux, provider="claude",
                   role=role, scope=session_scope, attempt=attempt,
                   detail={"ready": ready, "dialogs": dialogs})
        failure = launch_failure(wrapped)
        if failure is not None:
            status, detail = failure
            kill_session(session_tmux)
            return {"ok": False, "exit_code": status, "captured_output": detail,
                    "duration_seconds": time.monotonic() - started_at,
                    "error": "Burst Git preflight failed"}
        if not ready:
            snap = h.snapshot()
        ...
        if done:
            ...
            write_chat_artifacts(
                cwd, role=role,
                artifact_prefix=artifact_prefix or session_scope or session_id[:8],
                prompt=prompt,
    ...
    *,
    ...
    for attempt in range(max_restarts + 1):
        ...
        env_prefix = [
            ...
            # precedence over the root-owned /usr/bin/gemini, and (on
            # isabelle_hol runs) the isabelle bin dir.
            f"PATH={burst_launch_path_env(home, cwd)}",
        ...
        emit_event("launch", session=session_tmux, provider="gemini",
                   ...
                   detail={"kind": kind, "model": model, "screen_reader": screen_reader,
                           "sandbox": sandbox_enabled})
        h = AgentHandle(session=session_tmux, cwd=cwd, provider="gemini", preflight_command=wrapped)
        ready, dialogs = settle_until_ready(h, total_timeout=600.0)
        ...
        if done:
            ...
            write_chat_artifacts(
                cwd, role=role,
                artifact_prefix=artifact_prefix or session_scope or "gemini",
                prompt=prompt,
                final_pane_text=captured,
                transcript_last_assistant=transcript_msg,
...
def run_antigravity_burst(
    ...
    *,
    ...
    """Antigravity (`agy`) equivalent of run_gemini_burst.

    Drives `agy --dangerously-skip-permissions --model <m> -i <prompt>` in a
    tmux pane inside bwrap and scrapes the TUI. Resume across bursts uses
    `--conversation <id>`, where the id is the most recent agy conversation for
    this cwd (read from history.jsonl); `fresh=True` starts a new conversation.

    agy auth is a single Google OAuth token under ~/.gemini/antigravity-cli/,
    so there's no multi-account rotation / fallback-model machinery (unlike the
    gemini path). Quota probing degrades to a no-op for this provider.
    """
    started_at = time.monotonic()
    started_wall_ms = int(time.time() * 1000)
    cwd = cwd.resolve()
    cwd.mkdir(parents=True, exist_ok=True)
    home = burst_home or Path.home()
    current_model = model or _agy.DEFAULT_MODEL
    # Seed the pane-scrape-friendly TUI settings into the burst HOME (and pin
    # the model so /model and the status line agree).
    try:
        _agy.seed_antigravity_settings(home, model=current_model)
    except Exception:
        pass
    # Quota probe (start-of-burst). probe_for_burst returns None for the
    # antigravity provider (no probe implemented), so this is a graceful no-op
    # — kept for ledger-shape parity and future quota support.
    pre_probe_future = _submit_probe_for_burst(
        "antigravity", cwd, burst_home=burst_home,
    )
    base_name = name_hint or session_name(
        "antigravity", role, session_scope=session_scope
    )
    attempts: list = []
    last_snap = ""
    _emit_burst_launched(
        provider="antigravity", role=role, scope=session_scope, session_id=None,
        model=current_model, effort=None, account=None,
        sandbox=sandbox_enabled,
        prompt_bytes=len(prompt.encode("utf-8")),
    )
    for attempt in range(max_restarts + 1):
        # On a non-fresh task, resume the workspace's latest conversation so
        # context carries across bursts (sticky-session worker pattern). A
        # within-burst restart (attempt>0) also resumes. `fresh=True` on
        # attempt 0 starts a brand-new conversation.
        resume_id: Optional[str] = None
        if attempt > 0 or not fresh:
            resume_id = _agy.latest_conversation_id(cwd, home=home)
        kind = "resume" if resume_id else "fresh"
        session_tmux = base_name + ("" if attempt == 0 else f"-r{attempt}")
        kill_session(session_tmux)
        ...
        agy_argv = [
            ...
            "--dangerously-skip-permissions",
            "--model", current_model,
            # cwd is already agy's workspace; --add-dir makes it explicit.
            "--add-dir", str(cwd),
        ]
        if resume_id:
            agy_argv.extend(["--conversation", resume_id])
        # The prompt is NOT passed on the command line: a real PV worker prompt
        # is ~57KB and tmux new-session rejects argv vectors ≳50KB ("command
        # too long", exit 1). agy launches bare into its interactive input loop
        # (same as claude/gemini), and the prompt is delivered AFTER the TUI is
        # ready via send_prompt's tmux paste-buffer path below. (Bare `agy` and
        # `agy -i` both enter the interactive loop; bare is used so no prompt
        # arg is ever on the command line.)
        ...
        env_prefix = [
            ...
            f"PATH={worker_path_env(home)}",
            "PYTHONDONTWRITEBYTECODE=1",
            # agy is a fixed standalone binary; suppress any self-update.
            "ANTIGRAVITY_DISABLE_UPDATE_CHECK=1",
        ]
        try:
            from trellis.host_runtime import worker_elan_home  # type: ignore
            env_prefix.append(f"ELAN_HOME={worker_elan_home()}")
        ...
        wrapped = env_prefix + wrapped
        new_session(session_tmux, cwd=cwd, cmd=wrapped)
        emit_event("launch", session=session_tmux, provider="antigravity",
                   role=role, scope=session_scope, attempt=attempt,
                   detail={"kind": kind, "model": current_model,
                           "resume_id": resume_id, "sandbox": sandbox_enabled})
        h = AgentHandle(session=session_tmux, cwd=cwd, provider="antigravity")
        ready, dialogs = settle_until_ready(h, total_timeout=600.0)
        emit_event("settle", session=session_tmux, provider="antigravity",
                   role=role, scope=session_scope, attempt=attempt,
                   detail={"ready": ready, "dialogs": dialogs})
        if not ready:
            snap = h.snapshot()
            ...
            kill_session(session_tmux)
            attempts.append({
                "attempt": attempt, "kind": kind, "settle": False,
                "dialogs": dialogs, "auth_expired": auth_expired,
            })
            if auth_expired:
                _emit_burst_failed(
                    provider="antigravity", role=role, scope=session_scope,
                    session_id=None, reason="auth_expired",
                    duration=time.monotonic() - started_at,
                    attempts=attempts,
                    evidence=_evidence_snapshot(session_tmux, snap),
                    retryable=False,
                )
                return to_burst_result_dict(
                    ok=False, reason="auth_expired", started_at=started_at,
                    captured_output=snap, attempts=attempts, usage=None,
                    transcript_path=None, provider="antigravity",
                )
            continue

        # Deliver the prompt explicitly (uniform with the gemini path; also
        # covers resumed conversations where -i may not auto-submit).
        baseline = baseline_snapshot(h)
        send_prompt(h, prompt)
        delivered, _dreason = confirm_prompt_delivery(
            h, prompt_head=prompt, timeout=30.0
        )
        if not delivered:
            send_prompt(h, prompt)
            confirm_prompt_delivery(h, prompt_head=prompt, timeout=30.0)
            ...
            workspace_paths=workspace_paths,
            liveness_probe=lambda: agent_session_transcript_mtime_ns(
                "antigravity", cwd=cwd, burst_home=burst_home,
            ),
            apparent_stall_seconds=apparent_stall_seconds,
        )
        captured = h.snapshot()
        if not done and agent_requires_auth(captured, "antigravity"):
            _emit_burst_failed(
                provider="antigravity", role=role, scope=session_scope,
                session_id=None, reason="auth_expired",
                duration=time.monotonic() - started_at,
                attempts=attempts,
                evidence=_evidence_snapshot(session_tmux, captured),
                retryable=False,
            )
            kill_session(session_tmux)
            return to_burst_result_dict(
        ...
        transcript_msg = _agy.last_assistant_message(strip_ansi(captured))
        attempts.append({
            "attempt": attempt, "kind": kind, "settle": True,
            "dialogs": dialogs, "done": done, "reason": reason,
        })

        # Missing-artifact guard (mirrors gemini/claude): went quiet but no
        # done_file written → retry via resume if a slot remains.
        if (done and reason != "done_file" and done_file is not None
                and not done_file.exists()):
            if attempt < max_restarts:
                emit_event("burst_retry", session=session_tmux,
                           provider="antigravity", role=role,
                           scope=session_scope, attempt=attempt,
                           detail={"reason": "stable_without_done_file",
                                   "stable_reason": reason})
                kill_session(session_tmux)
                continue
        ...
        if done:
            kill_session(session_tmux)
            conv_path = _agy.conversation_db_path(cwd, home=home)
            conv_id = _agy.latest_conversation_id(cwd, home=home)
            post_probe_future = _submit_probe_for_burst(
                "antigravity", cwd, burst_home=burst_home,
            )
            pre_payload = _await_with_short_timeout(pre_probe_future, 5.0)
            ...
            try:
                append_cost_ledger(
                    cwd, provider="antigravity", role=role,
                    scope=session_scope, model=current_model,
                    usage=None, cost_usd=None,
                    duration_seconds=time.monotonic() - started_at,
                    attempts=len(attempts), ok=True, reason=reason,
                    account=None, session_id=conv_id,
                    ts_start=time.time() - (time.monotonic() - started_at),
                    quota_pre=_project_quota_for_ledger(
                        pre_payload, "antigravity"),
                    quota_post=_project_quota_for_ledger(
                        post_payload, "antigravity"),
                )
            except Exception:
                pass
            write_chat_artifacts(
                cwd, role=role,
                artifact_prefix=artifact_prefix or session_scope or "antigravity",
                prompt=prompt,
                final_pane_text=captured,
                transcript_last_assistant=transcript_msg,
                transcript_path=conv_path,
                ...
                ended_at_ms=int(time.time() * 1000),
                scope=session_scope,
                persistent_transcript_name="transcript.db",
            )
            _emit_burst_succeeded(
                provider="antigravity", role=role, scope=session_scope,
                session_id=conv_id, model=current_model, account=None,
                reason=reason, duration=time.monotonic() - started_at,
                cost_usd=None, transcript_len=len(transcript_msg or ""),
                attempts=len(attempts),
            )
            return to_burst_result_dict(
                ok=True, reason=reason, started_at=started_at,
                captured_output=captured, attempts=attempts, usage=None,
                transcript_path=conv_path, provider="antigravity",
                session_id=conv_id,
                extra={
                    "transcript_last_assistant": transcript_msg,
                    "screen_last": captured.splitlines()[-10:],
        ...
        emit_event("burst_retry", session=session_tmux, provider="antigravity",
                   role=role, scope=session_scope, attempt=attempt,
                   detail={"reason": reason, "done": done,
                           "source": "post_idle_fallthrough"})
        last_snap = captured or last_snap
        kill_session(session_tmux)

    final_reason = (
        attempts[-1].get("reason", "no_settle") if attempts else "no_attempts"
    )
    _emit_burst_failed(
        provider="antigravity", role=role, scope=session_scope,
        session_id=None, reason=final_reason,
    ...
    return to_burst_result_dict(
        ok=False, reason=final_reason, started_at=started_at,
        captured_output=last_snap, attempts=attempts, usage=None,
        transcript_path=None, provider="antigravity", burst_home=burst_home,
    )
...
def run(
    ...
    *,
    ...
    if fresh and config.provider in {"claude", "gemini", "antigravity"}:
    ...
    if config.provider == "claude":
        ...
        result = run_antigravity_burst(
            ...
            fresh=fresh,
            apparent_stall_seconds=5400.0,    # 90 min — see run_claude_burst /
                                              # run_gemini_burst; Gemini 3.1 Pro
                                              # can stay in a single thinking
                                              # phase 30-45 min without FS or
                                              # transcript writes.
            stable_seconds=5400.0,
            wait_timeout=max(float(timeout or 0), 7200.0),
            max_restarts=0,
