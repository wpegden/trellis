...
DEFAULT_ISABELLE_HOME = os.path.expanduser("~/Isabelle2025-2")
DEFAULT_ISABELLE_HOME_USER = os.path.expanduser("~/.isabelle/Isabelle2025-2")


def worker_isabelle_home() -> Path:
    """Isabelle distribution root for isabelle_hol worker/reviewer/verifier bursts.

    Resolves `TRELLIS_ISABELLE_HOME` (operator override) or the default
    `~/Isabelle2025-2` install. The Lean toolchain analogue is
    `worker_elan_home`. This path is RO-bound into the sandbox (and its
    `bin/` prepended to PATH) only on isabelle runs; a Lean run never reads
    it, so Lean bwrap binds and PATH stay byte-identical.
    """
    raw = os.environ.get("TRELLIS_ISABELLE_HOME", DEFAULT_ISABELLE_HOME).strip() or DEFAULT_ISABELLE_HOME
...
def worker_isabelle_home_user() -> Path:
    """Per-user Isabelle state dir holding the warm session heaps (~274MB).

    Defaults to `~/.isabelle/Isabelle2025-2` (the `$ISABELLE_HOME_USER` the
    distribution computes for this version). RO-bound into the sandbox so an
    in-burst `isabelle build` finds the prebuilt `HOL` image without being
    able to rebuild/clobber it (the read-only mount makes a rebuild
    physically impossible — the worker's interactive Draft→Sketch→Prove loop
    only needs to *read* the heap).
    """
    raw = os.environ.get("TRELLIS_ISABELLE_HOME_USER", DEFAULT_ISABELLE_HOME_USER).strip() or DEFAULT_ISABELLE_HOME_USER
    path = Path(raw).expanduser()
    try:
        return path.resolve(strict=False)
    except OSError:
...
def worker_path_env(burst_home: Optional[Path] = None, *, isabelle: bool = False) -> str:
    ...
    if burst_home is None:
        # Legacy no-burst-user callers get DEFAULT_WORKER_PATH unchanged, except
        # an isabelle_hol run still needs `isabelle` resolvable on PATH — prepend
        # its bin dir (if present) ahead of the system path. isabelle=False (every
        # Lean caller) keeps the byte-identical legacy return.
        if isabelle:
            isa_bin = str(worker_isabelle_home() / "bin")
            if Path(isa_bin).is_dir():
                return ":".join([isa_bin, DEFAULT_WORKER_PATH])
    ...
    # GATE I (isabelle): on isabelle_hol runs prepend `<isabelle_home>/bin` so
    # the burst's bare `isabelle` invocation (the worker skill's
    # Draft→Sketch→Prove loop + the probe's `command -v isabelle`) resolves on
    # PATH. The dir is bound RO into the sandbox by `wrap_command`. Gated on the
    # caller's `isabelle` flag (computed from `default_target`), so a Lean burst
    # never sees this dir and its PATH stays byte-identical. Additive: inserted
    # ahead of the existing candidates, all still preserved.
    if isabelle:
        candidate_dirs.insert(0, str(worker_isabelle_home() / "bin"))
    seen: set[str] = set()
...
def _node_modules_runtime_root(path: Path) -> Path | None:
    """Include npm package data and both nested and hoisted dependencies.

    A launcher in ``node_modules/@openai/codex/bin`` is not a standalone
    runtime: Codex resolves its native executable through an optional npm
    dependency. Mount the outer module tree, not just ``bin`` or the CLI's
    own package, since Node also resolves dependencies in ancestor module
    trees. Keep the enclosing install prefix and home out of this bind.
    """
...
def _runtime_roots_from_path(raw_path: Path, resolved: Path) -> List[Path]:
    ...
    for resolver in (_nvm_version_root, _claude_runtime_root, _node_modules_runtime_root):
    ...
    *,
    ...
    if burst_home is not None:
        ...
        if candidate is not None:
            return _runtime_roots_from_path(candidate, candidate.resolve())
