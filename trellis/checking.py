...
def _stuck_math_audit_probe_path_errors(
    ...
    *,
    ...
    request_id = str(audit_request["id"]) if audit_request.get("id") not in (None, "") else "unknown"
    cycle = str(audit_request["cycle"]) if audit_request.get("cycle") is not None else "unknown"
    scratch = (
        repo
        / ".trellis"
        / "stuck-math-audit"
...
def write_scripts(
    ...
    # Advisory warm-server pre-check. The module is self-contained (no trellis
    # package imports) so it runs standalone inside the worker sandbox without
    # the package on sys.path; provision it by copying the source verbatim.
    incremental_src = Path(__file__).resolve().parent / "incremental_check.py"
    incremental_dst = scripts_dir / "incremental_check.py"
    ...
    print(
        ...
        + ", ".join(
            sorted(
                ...
                | {"sync-supervisor-workspace", "node", "tablet"}
                | _ARTIFACT_COMMANDS
            )
        )
    )
