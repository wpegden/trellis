...
class CheckerServer:
    ...
    def start(self) -> None:
        ...
        # and its provenance sidecar matches the current source-closure hash
        # (``observations.olean_is_content_current``) — exactly the same
        # content gate ``_compute_compile_cache_subset`` uses on the cache-hit
        # path. mtime is never consulted, so an operator ``touch`` / ``git
        # reset`` cannot prewarm a content-stale olean as current.
    ...
    def _prewarm_oleans_known_current(self) -> None:
        ...
        Currency is decided by SOURCE-CLOSURE CONTENT HASH
        (``observations.olean_is_content_current``), not mtime: on a server
        restart after an operator ``touch`` / ``git reset``, an mtime gate
        would seed a content-stale (e.g. ``sorry``-bearing) olean as current
        and route a later request to a full cache hit that never rebuilds it.
        The content gate fails closed on any node lacking a matching
        provenance sidecar, so the worst case is a rebuild, never a stale
        serve.
        """
        ...
        with self._oleans_lock:
            for src in sources:
                ...
                if not NODE_NAME_REGEX.fullmatch(stem):
                    continue
                try:
                    if observations.olean_is_content_current(
                        self.supervisor_repo, stem
                    ):
    ...
    def _try_local_closure_axioms_cache_hit(
        ...
        # FIX 2 coverage: the ``--scan-only`` owner-file authoring scan is
        # parse-only and cheap; it is NOT olean-keyed (it never imports the
        # node's module), so it bypasses the sidecar cache entirely. Fall
        # through to the live (lock-guarded) probe rather than risk serving
        # or polluting the full-probe cache space with a scan-only result.
        if bool(request.raw.get("scan_only", False)):
            return None
    ...
    def _compute_compile_cache_subset(
        ...
        for name in names:
            if name not in candidate:
                continue
            try:
                current = observations.olean_is_content_current(
    ...
    def _handle_local_closure_axioms(
        ...
        no_axcheck = bool(request.raw.get("no_axcheck", False))
        # FIX 2 coverage: ``--scan-only`` runs ONLY the owner-file
        # authored-name scan (no closure walk, no axcheck). It is the
        # universal, all-node-kind authoring gate the Rust kernel runs on
        # every changed/new node file (Definition-kind nodes included,
        # which the full closure probe deliberately skips).
        ...
        sync_cache = load_fingerprint_cache(self.fingerprint_cache_path)
        cache_key: Optional[str] = None
        # FIX 2 coverage: scan-only is uncached (see ``script_args`` note).
        # Leaving ``cache_key`` None makes both the lock-acquired load
        # double-check and the post-probe store below no-ops.
        if not scan_only and local_closure_script_sha and toolchain_sha:
            try:
                cache_key_base = compute_semantic_payload_cache_key(
                    self.supervisor_repo,
                    node_name,
    ...
    def _get_or_create_isabelle_session(self) -> Any:
        ...
        with self._isabelle_lock:
            if self._isabelle_session is None:
                session = IsabelleSession(name=self._isabelle_server_name)
                session.start()
    ...
    def _isabelle_build_session(
        ...
        cmd = [isabelle_bin(), "build", "-b", "-D", str(session_dir)]
        try:
            proc = subprocess.run(
                cmd,
                capture_output=True,
