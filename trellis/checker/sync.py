...
sync to ``*.lean`` / ``*.thy`` (the Isabelle worker theory analogue) /
``*.tex`` plus the auto-managed Tablet docs.
...
# change to ``scripts/lean_semantic_fingerprint.lean``'s output format or to
# the capability required before its output may be trusted.
...
# precision.
# Replay is not a payload-schema change. Cache reuse is instead guarded by a
# current per-olean replay attestation in ``server.py``; a byte-identical v5
# observation becomes safe as soon as those exact oleans have been replayed.
# Keeping v5 avoids a mass re-probe on first deployment without weakening the
# prerequisite.
SEMANTIC_PAYLOAD_CACHE_VERSION = 5
...
# whenever the persisted record shape OR the generated probe semantics
# changes; ``load_print_axioms`` will treat older sidecars as cold misses on
# the next request. Replay is enforced by the olean-attestation consistency
# guard rather than a namespace bump. v2 root-qualifies the principal
# declaration so v1's potentially ambiguous unqualified verdict is never
# replayed after upgrade.
PRINT_AXIOMS_CACHE_VERSION = 2
...
# as print_axioms — node source + transitive artifact-bundle closure + toolchain pin +
...
# stored-schema change. Replay is enforced by the olean-attestation
# consistency guard rather than a namespace bump. Version 3 binds every
# ordered exported/server/private part rather than only the public olean;
# version 4 carries authoritative ModuleData.imports into certificate issuance.
LOCAL_CLOSURE_AXIOMS_CACHE_VERSION = 4
...
def _is_synced_relpath(rel: Path) -> bool:
    """Filter sync to ``*.lean`` / ``*.thy`` (any depth) and the auto-managed
    docs at the Tablet root. ``*.thy`` is the Isabelle worker theory analogue
    of ``*.lean`` — the worker authors ``Tablet/<Node>.thy`` and the checker
    mirrors it supervisor-side exactly as it does the Lean node files (the
    session-dir projection then renames it to ``isabelle/Tablet_<Node>.thy``).
    Mirror ``*.tex`` files too — workers author them and reviewers expect them
    to round-trip — but exclude ``*.tmp.*`` sync-bait (caught by the suffix
    check) and any dotfiles. Mitigation 5 of the threat model: drop the
    ``.tmp.``-substring class of sync bait by enforcing positive suffix
    matching here rather than negative-prefix filtering at the walk."""
    name = rel.name
    if name.startswith("."):
        return False
    if rel.suffix == ".lean":
...
def sync_tablet_dir(
    ...
    if worker_tablet.is_dir():
        ...
        # at /path/to/trellis and trick us into copying the whole homedir.
    ...
    for rel_str in list(prior_cache):
        if rel_str in seen_rels:
            continue
        if rel_str in rejected:
            # Rejection is not evidence of deletion. Preserve both the last
            # trusted mirror bytes and their cache row so a later genuine
            # disappearance can still be distinguished from this fail-closed
            # source substitution.
            new_cache[rel_str] = dict(prior_cache[rel_str])
            continue
        rel = Path(rel_str)
        try:
...
@telemetry.measured("semantic_cache_key")
def compute_semantic_payload_cache_key(
    ...
    artifact_bundles: list[tuple[str, list[dict[str, Any]]]] = []
    self_bundle = _artifact_bundle(supervisor_repo, node_name)
    if self_bundle is None:
        return None
    artifact_bundles.append((node_name, self_bundle))
    for dep in closure:
        if dep == node_name:
            continue
        ...
        dep_bundle = _artifact_bundle(supervisor_repo, dep)
        if dep_bundle is None:
            return None
        artifact_bundles.append((dep, dep_bundle))
    deps_sorted.sort(key=lambda pair: pair[0])
    artifact_bundles.sort(key=lambda pair: pair[0])
    ...
    for artifact_name, bundle in artifact_bundles:
        for part in bundle:
            blob.extend(
                (
                    f"artifact={artifact_name}={part['level']}="
                    f"{part['sha256']}={part['size_bytes']}\n"
                ).encode("utf-8")
            )
