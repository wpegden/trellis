...
def project_event_log_dir(repo_path: Path) -> Path:
    """Per-cycle event-log directory inside the tracked repo tree. Mirrors the
    kernel's `event_log_dir_for` (runtime.rs): `cycle-NNNNNN.jsonl` files live
    here, replacing the legacy monolithic `<runtime>/event_log.jsonl`."""
    return project_history_dir(repo_path) / "event-log"
