...
CODEX_WORKER_HOME = Path("/home/<sandbox-user>")
...
def _default_ledger_path() -> Path:
    ...
    return Path("/path/to/trellis/math/example-run/.trellis/logs/cost-ledger.jsonl")
...
def _row_models(row: dict) -> tuple[Optional[str], Optional[str]]:
    """(configured, actual) model for a ledger row.

    The ``model`` column is what the CONFIG asked for; a lane that pins no
    model records ``""`` there and only ``usage.model`` says what the
    provider CLI actually ran. Keeping both apart matters: the gap between
    them is precisely "the CLI picked a default for us", which changes
    under the provider's own upgrades with nothing in the config moving.
    """
    configured = str(row.get("model") or "").strip() or None
    usage = row.get("usage")
    actual = None
    if isinstance(usage, dict):
        actual = str(usage.get("model") or "").strip() or None
    return configured, actual


def config_models(config_path: Path) -> set[str]:
    """Every model any lane in a config can dispatch.

    Generic walk, deliberately: a hand-listed set of roles is what let a
    whole verifier pool dispatch on a stale model unnoticed. Over-collecting
    is the safe direction — this set decides what counts as EXPECTED, so a
    miss here would raise a false alarm, not hide a real one.
    """
    try:
    ...
    def walk(node, depth: int = 0) -> None:
        ...
        if isinstance(model, str) and model.strip():
            found.add(model.strip())
        # The sidecar names its model `sidecar.model.name`, not `.model`.
        elif isinstance(model, dict) and isinstance(model.get("name"), str):
            if model["name"].strip():
                found.add(model["name"].strip())
        for key in ("fallback_models", "models"):
            value = node.get(key)
            if isinstance(value, list):
                found.update(m.strip() for m in value if isinstance(m, str) and m.strip())
...
def _cmd_model_mix(args: argparse.Namespace) -> int:
    """Report which models actually ran, split by lane, and flag surprises.

    This exists because of a specific failure: a run configured for
    gpt-5.6-sol dispatched 107 of 141 reviewer-role bursts on gpt-5.5 for
    34 cycles (~$91) with nothing anywhere reporting it. Every verifier
    pool logs as role ``reviewer``, so a role tally showed nothing wrong,
    and no surface broke the ledger down by model. The data was already
    in the ledger the whole time — only the question was missing.

    ``unexpected`` is the alarm: a model that ran but that no lane in the
    config names. Reported per (category, model) so the answer to "which
    lane is wrong" comes with it.
    """
    ledger_path = Path(args.ledger) if args.ledger else _default_ledger_path()
    rows = _filter_since(_read_ledger(ledger_path), args.since)

    if args.config:
        config_path = Path(args.config)
    else:
        # <repo>/.trellis/logs/cost-ledger.jsonl -> <repo>/trellis.config.json
        config_path = ledger_path.parent.parent.parent / "trellis.config.json"
    expected = config_models(config_path)

    buckets: dict[tuple[str, str], dict] = {}
    drift: list[dict] = []
    for row in rows:
        configured, actual = _row_models(row)
        model = actual or configured or "(unrecorded)"
        category = _category_for_row(row)
        bucket = buckets.setdefault((category, model), {
            "category": category, "model": model, "n": 0, "cost_usd": 0.0,
        ...
        try:
            ...
            pass
        bucket["roles"].add(str(row.get("role") or "?"))
        # Recency is what makes an unexpected model actionable. A finished
        # run whose config was later upgraded has every pre-upgrade row
        # "unexpected" forever — true, but not a thing to go fix. The
        # operator needs to tell that from a lane running wrong right now.
        try:
        ...
        if ts > 0:
            ...
            if bucket["last_ts"] is None or ts > bucket["last_ts"]:
                bucket["last_ts"] = ts
        # A lane that pinned nothing and got something: the CLI's default.
        if actual and not configured:
            drift.append({"category": category, "ran": actual,
                          "scope": str(row.get("scope") or "")})
    ...
    out_buckets = []
    for bucket in sorted(buckets.values(), key=lambda b: (-b["n"], b["category"], b["model"])):
        entry = dict(bucket)
        entry["roles"] = sorted(entry["roles"])
        entry["cost_usd"] = round(entry["cost_usd"], 4)
        entry["first_seen"] = _iso(entry.pop("first_ts"))
        entry["last_seen"] = _iso(entry.pop("last_ts"))
        # An unreadable config makes EVERYTHING expected rather than
        # everything suspect: this reports, it does not gate, and a check
        # that cannot read its baseline has nothing to say. Same posture as
        # the viewer's auth preflight, which fails open by design.
        entry["expected"] = (not expected) or entry["model"] in expected
        out_buckets.append(entry)

    unexpected = [b for b in out_buckets if not b["expected"] and b["model"] != "(unrecorded)"]
    result = {
        "ledger": str(ledger_path),
        "config": str(config_path),
        "config_models": sorted(expected),
        "config_readable": bool(expected),
        "n_rows": len(rows),
        "by_category_model": out_buckets,
        "unexpected": unexpected,
        "unpinned_lane_defaults": drift[:20],
        "ok": not unexpected,
    }
    print(json.dumps(result, indent=2))
    # `--strict` is for cron/CI. Default stays 0 so an operator running
    # this by hand gets the report, not a shell error.
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    mix = sub.add_parser(
        "model-mix",
        help=(
            "JSON per-(category, model) burst counts and spend, with any "
            "model that ran but is named by no lane in the config flagged "
            "as unexpected. Answers 'is anything dispatching on the wrong "
            "model?' — which a role tally cannot, since every verifier "
            "pool logs as role 'reviewer'."
        ),
    )
    mix.add_argument("--since")
    mix.add_argument("--ledger")
    mix.add_argument("--config", help="Config to read expected models from "
                                      "(default: derived from the ledger path).")
    mix.add_argument("--strict", action="store_true",
                     help="Exit 1 when an unexpected model is found (for cron/CI).")
    mix.set_defaults(func=_cmd_model_mix)
