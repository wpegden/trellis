    ...
    **_kwargs,
    ...
    def _run():
        ...
        if config.provider == "codex":
            ...
            return run(config, prompt, role=sandbox_role, session_name=session_name,
                      work_dir=work_dir,
        ...
        if config.provider in {"claude", "gemini", "antigravity"}:
...
# 30s, not 10s: under a swap storm the tmux server can stall for tens of
# seconds, and a timeout here surfaces as a fatal adapter error that takes the
# supervisor down. On 2026-08-01 a `display-message #{pane_dead}` probe timed
# out at 10s while several multi-GB Lean elaborations were paging, and the run
# stopped for ~40 minutes. These are local IPC calls — a slow answer means a
# loaded box, which is exactly when killing the run is worst.
def tmux_cmd(*args: str, check: bool = False, timeout: int = 30):
    import subprocess
...
def tmux_kill_session(session: str) -> None:
    """Tear down an entire per-burst tmux session, best-effort.

    Dispatch is serial and session names are per-request-unique, so a burst's
    session is owned solely by the one run() that created it. Killing the
    window alone leaves the session's default idle-bash window 0 alive, so the
    session lingers forever as a husk (~357 had accumulated). Killing the
    session at teardown reaps window 0 too. check=False keeps this a no-op when
