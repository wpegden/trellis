...
SYSTEM_FEEDBACK_DESIGN_GAP_LOG_FILENAME = "system_feedback_design_gap_log.jsonl"
...
def _append_system_feedback_design_gap_log(
    runtime_root: Path,
    *,
    request: SingleAgentRequest,
    artifact_name: str,
    ...
    emission. Best-effort: a failed write is swallowed (the design-gap log
    is a forensic layer, not authoritative). Matches the Rust
    `append_system_feedback_design_gap_log` field set.
    """
    record = {
        "schema_version": 1,
    ...
    try:
        path = runtime_root / SYSTEM_FEEDBACK_DESIGN_GAP_LOG_FILENAME
...
def _write_system_feedback_halt_marker(
    ...
    *,
    ...
    # CONSERVATIVE ack policy — the ONLY branch that diverges from the
    # historical fail-loudly behavior, and it can only fire when an
    # operator has explicitly acknowledged THIS fingerprint. With the
    # default (no ack store on disk / empty store), membership is always
    # False and control falls through to the byte-for-byte original halt
    # path below: a pure superset. Checked BEFORE the marker-exists guard
    # to mirror the Rust ordering (an acked recurrence is logged even if an
    # earlier, unrelated marker is still present).
    if fingerprint in _load_system_feedback_ack_store(runtime_root):
        _append_system_feedback_design_gap_log(
            runtime_root,
            request=request,
            artifact_name=artifact_name,
            system_feedback=system_feedback,
