...
def extract_compiler_errors(
    ...
    undiagnosable. (live run, node `twisty`, 2026-07-31.)
...
def rollout_usage_fallback(codex_home: Optional[Path], thread_id: Optional[str]) -> Dict[str, Any]:
    ...
    if not codex_home:
        return {}
    sessions = grunt_codex_home(codex_home) / "sessions"
    ...
    *,
    ...
    if wall_seconds:
        persistence = (
            ...
            # Observed give-up mode (live run, node `twisty`): the agent hit a
...
def _last_refresh(path: Path) -> str:
    """``last_refresh`` from a codex ``auth.json``, or ``""`` when absent.

    The field is an ISO-8601 UTC stamp, so string order is time order. An
    unreadable or unstamped file sorts oldest, which makes the caller's
    comparison fail safe toward copying.
    """
    try:
        return str(json.loads(path.read_text()).get("last_refresh") or "")
...
def refresh_grunt_credential(codex_home: Path) -> bool:
    ...
    ``codex_home`` is the grunt's burst home; the state codex reads sits one
    level down (`grunt_codex_home`).

    The copy is refused when the target's ``last_refresh`` is NEWER than the
    master's. A refresh token is single-use, so whichever process refreshes
    mints the successor and the other copy's token becomes spent. Copying
    backwards over a target that refreshed more recently would discard the
    only live token and strand the lineage.
    """
    master = Path.home() / ".codex" / "auth.json"
    target = grunt_codex_home(codex_home) / "auth.json"
    try:
        ...
        if target.exists() and target.read_bytes() == payload:
            return False
        if target.exists() and _last_refresh(target) > _last_refresh(master):
            return False
...
def _attempt_round(
    *,
    ...
    # The split behind the two scalars above, kept for the ledger's cost
    # accounting: without it the cached share of the prompt was discarded
    # here, and an attempt could only ever be priced to within the
    # cached-vs-uncached spread. Captured raw from the same usage dict.
