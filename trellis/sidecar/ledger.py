...
``detail``, ``snapshot_sha`` and the attribution pair alone.

``provider``/``model`` name the generator that ran the attempt, on
EVERY outcome. They were absent originally, which left the kernel's
closure provenance as the only record of who ran what — and that
covers only attempts that CLOSED a node, so every failure and cancel
was permanently unattributable. Rows written before this field
existed carry neither; readers must treat both as optional and fall
back to the closure provenance (joined on ``attempt_id``) for
historical rows.

``cached_input_tokens`` / ``reasoning_output_tokens`` / ``cost_usd``
(main-lane cost-ledger parity) are the token split pricing needs:
``prompt_tokens`` is GROSS prompt (cached portion included) and
``completion_tokens`` is output+reasoning merged — those two scalars
keep exactly that meaning, and ``rolling_token_totals`` below sums
only them, so the split fields are purely additive. ``cost_usd`` is
computed at append time from the split via the main lanes'
``codex_cost_usd`` (one pricing table for all lanes) and is present
only when the model was priceable. All three are optional: rows
written before they existed carry none of them, and every reader must
