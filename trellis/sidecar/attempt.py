...
from trellis.sidecar.workspace import default_grunt_burst_home
...
def _resolve_burst_home(config, runtime_root: Path, grunt: int) -> Path:
    """The grunt's burst home: the configured one, or a derived default.

    Created here rather than left to the sandbox, because `wrap_command`
    binds it and a missing directory would fail inside bwrap with a far
    less legible error than a mkdir failure here. A configured value is
    honoured verbatim — an operator pointing grunts at a prepared home
    (pre-seeded codex credentials, say) must not be second-guessed.
    """
    raw = str(getattr(config, "codex_home", "") or "").strip()
    home = Path(raw) if raw else default_grunt_burst_home(runtime_root, grunt)
    home.mkdir(parents=True, exist_ok=True)
    return home


def _prior_kernel_feedback(runtime_root: Path, node: str, attempt_id: str) -> str:
    """Feedback a new grunt must see before it repeats a refused closure."""
    rows = [
        row
        for row in spool_mod.iter_kernel_feedback(runtime_root)
        if str(row.get("node", "")) == node
        and str(row.get("attempt_id", "")) != attempt_id
        and str(row.get("status", "")) == "rejected"
    ]
    rows.sort(key=lambda row: (int(row.get("cycle", 0) or 0), str(row.get("feedback_id", ""))))
    if not rows:
        return ""
    lines = [
        "KERNEL FEEDBACK FROM PRIOR ATTEMPTS (do not blindly repeat the refused body):"
    ]
    for row in rows[-3:]:
        lines.append(
            f"- cycle {int(row.get('cycle', 0) or 0)}: "
            f"{str(row.get('detail', ''))[:500]}"
        )
    return "\n".join(lines)
...
def run_grunt_attempt(
    ...
    *,
    ...
    try:
        ...
        prior_feedback = _prior_kernel_feedback(runtime_root, node, attempt_id)
        if prior_feedback:
            log("including prior kernel rejection feedback in this attempt's prompt")
        ...
        result = run_attempt_codex(
            ...
            # An unset `codex_home` means "derive one", never `None` —
            # `wrap_command` refuses role="grunt" without a burst home, so
            # None crashed every attempt (see `default_grunt_burst_home`).
            codex_home=_resolve_burst_home(config, runtime_root, grunt),
            log=log,
        )
        ...
        outcome: Dict[str, Any] = {
            ...
            # Token split (cost accounting): the cached share of
            # prompt_tokens and the reasoning share of completion_tokens.
            # Additive — the two scalars above keep their merged meaning.
        ...
        try:
            ...
            # kernel DEFERS. A live run lost `summary5` (a genuine closure
...
def main(argv=None) -> int:
    ...
    try:
        ...
        # `detail` reaches trusted readers (daemon ledger / spool outcome);
        # an exception message can carry newlines that break the reviewer's
        # one-row-per-line table, so collapse it through `sanitize_detail`.
        ...
        outcome = {
            ...
            "detail": sanitize_detail(
                f"attempt runner crashed: {type(exc).__name__}: {exc}"
            ),
        }
