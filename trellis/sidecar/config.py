...
DEFAULT_MODEL_NAME = "labs-leanstral-1-5"
...
# proofs). "" / "none" => the field is not sent. A server that rejects
# the field is handled gracefully once by ModelClient (logged fallback).
...
class SidecarConfig:
    ...
    # Leanstral trained-regime effort ("high" default; "" / "none" => the
    # field is not sent). ModelClient falls back gracefully once if the
    # endpoint rejects it.
