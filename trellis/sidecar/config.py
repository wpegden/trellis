...
# 60 minutes. The wall is the SOLE per-attempt budget (max_iterations was
# retired in favour of wall-clock), and it is a hard kill: the attempt is
# spawned with start_new_session=True, so hitting it destroys that attempt's
# work entirely — which is why the number is a real trade rather than a
# safety margin. Too high and every dead end holds a pool slot for the full
# duration; too low and genuinely hard nodes are killed just short of a
# proof, losing everything they had built. 90 minutes was too generous for
# the first, 45 risked the second on the harder nodes a grunt is worth
# spending on at all.
DEFAULT_ATTEMPT_WALL_SECONDS = 3600.0
