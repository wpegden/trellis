...
def release_workspace(runtime_root: Path, grunt: int) -> bool:
    """Delete grunt ``k``'s checkout, keeping its attempt logs/results.

    Only ``grunts/<k>/repo`` goes: it is a CACHE (module docstring —
    bootstrap re-clones it and refresh resets it, so losing it costs
    bootstrap time only) and it is where all the bytes are — the sibling
    ``attempt-*.log`` / ``result-*.json`` files are a few KB each and are
    the only forensic record of what the pool did, so they stay.

    Returns True when a checkout was removed. Never raises: a workspace
    that cannot be deleted is disk to reclaim by hand, not a reason to
    keep a daemon alive."""
...
def prewarm_node_closure(repo: Path, node: str, config, *, timeout: float = 1800.0):
    """Build `node`'s closure BEFORE the agent's turn, off its wall.

    The purge leaves holes the agent then pays to fill out of its own
    900s budget, and on a large closure that is the whole budget:
    `bicographs0` spent 89% of its wall compiling, launched four
    `lake build`s, completed NONE of them (it `pkill`ed one at 281s),
    and shipped an uncompiled body. Worse, an agent that kills its own
    build leaves the closure half-made — which is where
    `object file '.../Tablet/nonsat.olean' does not exist` came from,
    and it breaks the warm-server fast path for every later attempt.

    `run_attempt_codex` starts its clock internally, so anything done
    before that call is outside the wall by construction — the same
    treatment the harness's verification compile already gets.

    Non-fatal on rc != 0: the TARGET node's body may legitimately not
    compile (that is the grunt's job). What matters is that its
    dependencies are on disk.

    Returns (ok, seconds, tail).
    """
    import subprocess
    ...
    try:
        ...
        return proc.returncode == 0, _time.monotonic() - started, out[-400:]
    except subprocess.TimeoutExpired:
        return False, _time.monotonic() - started, f"prewarm timeout after {timeout:.0f}s"
    except OSError as exc:
        return False, _time.monotonic() - started, f"prewarm failed: {exc}"


def restamp_rebuilt_oleans(repo: Path, node: str, purged: Sequence[str]) -> int:
    """Re-write the srcclosure sidecar for each purged dep whose olean is
    back on disk. Returns how many were stamped.

    THE RATCHET THIS FIXES. `_purge_olean_artifacts` deletes the
    `.olean.srcclosure` sidecar along with the olean, and nothing in the
    sidecar path ever wrote one back. `olean_is_content_current` fails
    closed on a missing sidecar, so the next attempt purged the same node
    again — and the next, forever, on a workspace that never went stale.
    Measured on the live grunts: of 118 purged oleans, 85 were this
    ratchet and only 15 were genuine source staleness. `smallbico` purged
    16 nodes of which zero were stale, and the rebuilt files were
    bit-identical to the live repo's.

    The kernel's `materialize_tablet_oleans` is purge -> build -> STAMP.
    This is the missing third step; without it the first two are a
    treadmill.

    PROVENANCE IS OBSERVED, NOT INFERRED, which is what makes this safe.
    Only nodes THIS attempt deleted are eligible, so an olean present now
    can only have been produced by lake compiling the sources on disk
    during this attempt. Nothing is ever stamped on lake's own
    up-to-date judgement — that inference is exactly what the content-hash
    gate exists to refuse, and it is still refused. The target node is
    never stamped: its olean comes from a body the agent is about to
    replace.

    The residual hazard is a grunt editing a dependency, building, and
    reverting it, so the stamp vouches for an olean built from edited
    source. Two guards: the caller skips the restamp entirely if the
    worktree is dirty beyond the target node, and a stamp is skipped for
    any file whose (size, mtime_ns) does not match the snapshot taken
    right after the prewarm build. Beyond that, the kernel re-verifies
    every published body independently in the LIVE repo under its own
    purge/build/stamp, so a corrupted grunt workspace cannot land a proof.
    """
    stamped = 0
    for name in purged:
        if name == node:
            continue
        olean = _tablet_olean_path(Path(repo), name)
        try:
            if not olean.is_file() or olean.stat().st_size == 0:
                continue
        except OSError:
            continue
        closure_hash = tablet_source_closure_hash(Path(repo), name)
        if not closure_hash:
            continue
        write_olean_srcclosure(Path(repo), name, closure_hash)
        stamped += 1
    return stamped
...
def default_grunt_burst_home(runtime_root: Path, grunt: int = 0) -> Path:
    """Grunt `k`'s burst home when the config does not name one.

    `wrap_command` REFUSES `role="grunt"` without an explicit `burst_home`,
    because falling back to `Path.home()` would bind the operator's home
    read-write into an untrusted agent (see 6a0b50b7). But `SidecarConfig`
    defaults `codex_home` to `""`, so a sidecar block that merely says
    `enabled: true` — which is every shipped template and every config the
    create wizard writes — produced `burst_home=None` and crashed EVERY
    attempt with "role 'grunt' requires an explicit burst_home". The daemon
    then suspended assignments for 30 minutes at a time, so the feature
    looked like a queue that never drains. It had been dead since
    2026-07-31; nothing exercised it in the interim.

    "Unset" must therefore mean a safe derived path, not `None`: refusing to
    guess is right about `$HOME` and wrong about a directory we already own.
    Grunt `k` already owns `sidecar/grunts/<k>/` (its repo checkout lives
    there), so its home goes beside it — inside the runtime root, per-grunt
    so two grunts never share codex state, and never the operator's home.
    """
    return Path(runtime_root) / "sidecar" / "grunts" / str(int(grunt)) / "burst-home"


def grunt_codex_home(burst_home) -> Path:
    """The directory codex reads its state from, given a grunt burst home.

    codex resolves `$HOME/.codex`, and the grunt bwrap rewrites HOME to
    `burst_home`, so `auth.json`, `config.toml` and `sessions/` all live one
    level down. Every reader and writer of that state resolves it through
    here, so the launcher's `--setenv CODEX_HOME` and the credential
    re-seed cannot name different files: on 2026-08-11 they did, and the
    grunt arm spent an evening authenticating against a ten-day-old token
    while a re-seed kept a flat copy beside it perfectly fresh.
    """
    return Path(burst_home) / ".codex"
...
def build_codex_agent_command(
    ...
    *,
    ...
    """Sandbox wrapping for the codex-CLI grunt agent.

    The agent is a SHELL agent running on the host with approvals bypassed
    (``--dangerously-bypass-approvals-and-sandbox``), so this bwrap is the
    ONLY boundary around it.

    The role is ``grunt`` — untrusted. It was ``worker`` until now, which
    gave a speculative agent the full trusted surface: all three provider
    credential trees, the checker socket and its live per-burst token, the
    writable ``reference/`` deviation dir, the
    worker kernel cache inside the live runtime root, and the hardlinked
    package build closures. None of that is needed to Lean-close a node
    whose statement the trusted process already wrote, which is the only
    thing a grunt is permitted to do.

    Compile steps keep ``config.sandbox_role`` (``lake_compiler``); only
    the agent runs as ``grunt``.

    ``burst_home`` is the agent's ``CODEX_HOME``: writable, isolated from
    the live run's own codex state, and REQUIRED — ``wrap_command`` refuses
    ``role="grunt"`` without one, because falling back to ``Path.home()``
    would bind the operator's home read-write.
    """
    inner_cmd = _wrapped_inner_command(
        repo,
        inner,
    ...
    # bwrap rewrites HOME to `burst_home`, and codex resolves its auth
    # from `$HOME/.codex` unless told otherwise. Pin CODEX_HOME explicitly
    # so the layout is stated rather than inherited: auth lives at
    # `<burst_home>/.codex/auth.json`, and a flat copy beside it — the
    # obvious mistake — fails loudly instead of silently unauthenticating
    # every grunt. Threaded as `--setenv` right after argv[0], the same
    # way `_wrapped_inner_command` threads LEAN_NUM_THREADS.
    if burst_home and inner_cmd and inner_cmd[0] != inner[0]:
        codex_home = grunt_codex_home(burst_home)
        codex_home.mkdir(parents=True, exist_ok=True)
        inner_cmd = (
            [inner_cmd[0], "--setenv", "CODEX_HOME", str(codex_home)]
            + inner_cmd[1:]
        )
    return ["nice", "-n", "15", "ionice", "-c3", *inner_cmd]
