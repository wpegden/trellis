...
def feedback_dir(runtime_root: Path) -> Path:
    return Path(runtime_root) / "sidecar" / FEEDBACK_DIR


def iter_kernel_feedback(runtime_root: Path) -> Iterator[Dict[str, Any]]:
    """Yield durable kernel dispositions in stable order.

    Feedback is intentionally retained rather than consumed: it is both the
    recovery source for the result-write race and the bounded input a later
    grunt uses to learn why an earlier closure was refused.
    """
    directory = feedback_dir(runtime_root)
    if not directory.is_dir():
        return
    for path in sorted(directory.glob("feedback-*.json")):
        try:
            record = json.loads(path.read_text(encoding="utf-8"))
        except (OSError, ValueError):
            continue
...
def _write_json_atomic(path: Path, value: Any) -> None:
    tmp = path.with_suffix(".json.kernel-feedback.tmp")
    tmp.write_text(json.dumps(value, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    os.replace(tmp, path)


def deliver_kernel_feedback(runtime_root: Path) -> int:
    """Merge new kernel dispositions into each grunt's own result and log.

    Returns the number of newly delivered feedback rows. Delivery is
    idempotent by ``feedback_id``. If the kernel won the race with the attempt
    child's result write, this pass simply finds no result; a later pass tries
    again because the durable feedback file remains.
    """
    grunts = Path(runtime_root) / "sidecar" / "grunts"
    if not grunts.is_dir():
        return 0
    delivered = 0
    grunt_dirs = [path for path in sorted(grunts.iterdir()) if path.is_dir()]
    for feedback in iter_kernel_feedback(runtime_root):
        attempt_id = str(feedback.get("attempt_id", ""))
        feedback_id = str(feedback.get("feedback_id", ""))
        if not attempt_id or not feedback_id:
            continue
        expected_result = f"result-{attempt_id}.json"
        expected_log = f"attempt-{attempt_id}.log"
        for grunt_dir in grunt_dirs:
            result_path = next(
                (path for path in grunt_dir.iterdir() if path.name == expected_result),
                None,
            )
            ...
            if not isinstance(result, dict):
                continue
            history = result.get("kernel_feedback")
            if not isinstance(history, list):
                history = []
            already_in_result = any(
                isinstance(row, dict) and row.get("feedback_id") == feedback_id
                for row in history
            )
            if not already_in_result:
                ...
                result["node"] = str(feedback.get("node", ""))
                result["status"] = str(feedback.get("status", ""))
                result["detail"] = str(feedback.get("detail", ""))
                history.append(dict(feedback))
                result["kernel_feedback"] = history
            try:
                log_path = next(
                    (path for path in grunt_dir.iterdir() if path.name == expected_log),
                    grunt_dir / expected_log,
                )
                log_marker = f"[{feedback_id}]"
                try:
                    log_text = log_path.read_text(encoding="utf-8")
                except FileNotFoundError:
                    log_text = ""
                already_in_log = log_marker in log_text
                if already_in_result and already_in_log:
                    continue
                if not already_in_result:
                ...
                if not already_in_log:
                    with log_path.open("a", encoding="utf-8") as log:
                        log.write(
                            f"sidecar[{attempt_id}]: kernel {result['status']}: "
                            f"{result['detail']} {log_marker}\n"
                        )
            except OSError:
                continue
            delivered += 1
...
def reviewer_export_path(runtime_root: Path) -> Path:
    ...
    inside the reviewer bwrap, which is the asymmetry that halted a
    live run at cycle 473. Returns the preferred name when nothing
