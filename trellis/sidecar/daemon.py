...
The export's queue order IS the schedule: the manager holds N grunt
...
TWO LANES, one walk (``assignable_lanes``): the reviewer's ``queue``
comes first, then the kernel's ``kernel_queue`` — a standing ranked
list of every eligible node the kernel refills at each boundary. A
free grunt idles only when BOTH are exhausted; without the second lane
the pool drains a cycle's reviewer queue in minutes and then sits idle
until the next boundary, one to two hours later (measured: 5.1%
utilisation over 13.2 h). The rows carry the identical assignable
shape, so nothing below the walk knows which lane it took.
...
END OF WORK (``terminal_export_phase`` / ``_wind_down_for_phase``): the
manager also decides for itself when the run has finished with it. The
export already carries the run's ``phase``; once that phase is one the
sidecar can never be given work in (``Cleanup``, ``Complete`` — every
accepted node is Lean-closed by construction and the engine's apply gate
refuses grunt closures there), a pass takes the STOP path, releases the
grunt checkouts and exits. The decision is the DAEMON's precisely
because it is the daemon's own lifecycle: the kernel would have to
become a second writer under ``sidecar/`` to signal it, which is the one
ownership rule the spool is built around. Nothing about the wind-down is
permanent — it is an exit, and a run revived into formalization
(add-targets) is served by starting the daemon again.
"""
...
# `status.json` is `--ro-bind`ed into the TRUSTED reviewer's sandbox
# (trellis/sandbox.py `_reviewer_sidecar_readonly_files`), and the review
# fragment instructs the reviewer to open it. The attempted-set rows this
# file surfaces carry a `detail` field lifted verbatim from the Lean
# compiler log, which quotes the untrusted grunt's proof body — an
# untrusted->trusted prompt-injection channel aimed at the reviewer's
# flip/give-up/routing decisions. So the reviewer-facing row is rebuilt
# from a fail-closed ALLOWLIST of kernel/daemon-controlled fields
# (enums, identifiers, counts, timestamps); `detail` and any field not
# named here never cross into the reviewer's sandbox. `attempted.json`
# and the ledger keep the full row for operators — only this exported
# view is redacted.
STATUS_ATTEMPT_PUBLIC_FIELDS: Tuple[str, ...] = (
    "entry_seq",
    "attempt_id",
    "status",
    "iterations",
...
def _redact_attempt_row_for_status(row: Any) -> Dict[str, Any]:
    """Keep only the kernel/daemon-controlled fields from an attempted-set
    row (drops the grunt-authorable `detail`); fail-closed by allowlist."""
    if not isinstance(row, Mapping):
        return {}
    return {k: row[k] for k in STATUS_ATTEMPT_PUBLIC_FIELDS if k in row}
...
# The two DELIBERATE exits. Written by the exit paths themselves so a
# reader of `status.json` can tell "this daemon was stopped on purpose"
# from "this daemon died" — until now every exit left `phase: "running"`
# behind, because `run_once` rewrites the status after `_run_once_inner`
# returns and the rewrite carried the default marker. A stopped daemon
# and a crashed one were byte-indistinguishable except for
# `last_pass.status`.
PHASE_STOPPED = "stopped"
PHASE_DRAINED = "drained"

# RUN phases (the kernel's `Phase` debug spelling, carried in the
# export's `phase` field) in which the closure sidecar can never be
# given work: `sidecar_window_open_with_config` returns false for them
# unconditionally (kernel/src/sidecar.rs, the `_ => false` arm), and the
# engine's apply gate refuses a grunt closure arriving in them. Every
# node accepted by a run that reached `Cleanup` is Lean-closed by
# construction, so there are no `sorry`s left for the pool to close.
#
# NOT `window_shut`, which is a different and TRANSIENT thing (an
# active_node/held-target/pending-task block, or an operator's
# per-phase config toggle) and must leave the daemon polling.
#
# Compared case- and underscore-insensitively, so both the `{:?}`
# spelling the export writes (`Cleanup`) and the serde aliases
# (`cleanup`) match.
TERMINAL_RUN_PHASES = frozenset({"cleanup", "complete"})


def terminal_export_phase(export: Optional[Mapping[str, Any]]) -> str:
    """The export's run phase when it is one the sidecar can never work
    in, else "" — fail-OPEN on anything unrecognised.

    A missing, non-string or unknown `phase` keeps the daemon running:
    the wind-down kills in-flight attempts and deletes grunt workspaces,
    so it must fire only on a phase this version positively knows is
    past formalization. An export from an older kernel that omits the
    field simply never triggers it."""
    if not isinstance(export, Mapping):
        return ""
    phase = export.get("phase")
    if not isinstance(phase, str) or not phase.strip():
        return ""
    key = phase.strip().lower().replace("_", "").replace("-", "")
    return phase.strip() if key in TERMINAL_RUN_PHASES else ""
...
def reconcile_attempt_charges(runtime_root: Path) -> int:
    """Mark authority/infrastructure discards as non-chargeable.

    ``attempted.json`` remains the daemon's raw generation-dedupe and
    forensic history.  The kernel's dispatch reader excludes only rows with
    ``chargeable: false`` from ranking and the mathematical-attempt ceiling.
    A terminal ``not_queued`` disposition is such a row: the kernel refused
    authority before judging the proof, whether the absence came from the
    lane's own prune or from a reviewer removal.

    Feedback is kernel-owned and durable.  Missing/malformed feedback leaves
    the row charged (the conservative direction), and matching by BOTH node
    and attempt id prevents a malformed cross-node disposition from moving
    accounting.  Dedupe still sees the row regardless of this marker, so the
    same generation is never re-run.
    """
    discarded: Set[Tuple[str, str]] = set()
    for feedback in spool_mod.iter_kernel_feedback(runtime_root):
        if str(feedback.get("status", "")) != "rejected":
            continue
        detail = str(feedback.get("detail", ""))
        gate = detail.split(maxsplit=1)[0].split("(", 1)[0] if detail else ""
        if gate != "not_queued":
            continue
        node = str(feedback.get("node", ""))
        attempt_id = str(feedback.get("attempt_id", ""))
        if node and attempt_id:
            discarded.add((node, attempt_id))
    if not discarded:
        return 0
    ...
    for node, rows in attempted.items():
        ...
        for row in rows:
            if not isinstance(row, dict):
                continue
            key = (str(node), str(row.get("attempt_id", "")))
            if key in discarded and row.get("chargeable") is not False:
...
def _lane_rows(export: Mapping[str, Any], key: str) -> List[Mapping[str, Any]]:
    rows = export.get(key)
    if not isinstance(rows, list):
        return []
...
def assignable_lanes(export: Mapping[str, Any]) -> List[Mapping[str, Any]]:
    """The daemon's work list: the REVIEWER lane (``queue``) first, then
    the kernel's ranked standing list (``kernel_queue``).

    Owner's rule: "any time grunts are free, they're fed from the
    reviewer queue if it's available, otherwise from the kernel queue".
    Concatenation implements it exactly — the walk is front to back and
    only reaches a kernel row once every reviewer row has been skipped
    (busy, spent, blocked, or awaiting ingest).

    ``kernel_queue`` is absent from a schema-2 export, in which case
    this is the reviewer lane alone and the daemon behaves as it always
    did. Rows in both lanes carry the identical assignable shape, so
    nothing downstream needs to know which lane a row came from."""
    return _lane_rows(export, "queue") + _lane_rows(export, "kernel_queue")
...
def _grunt_workspace_indices(runtime_root: Path) -> List[int]:
    """Grunt indices whose CHECKOUT is present on disk — which is NOT
    the configured pool.

    Keyed on ``grunts/<k>/repo`` rather than on ``grunts/<k>``, because
    the checkout is both the thing that costs the disk and the thing the
    pool needs: a released grunt keeps its directory for the few KB of
    attempt logs, and counting that as a workspace would report a
    wound-down daemon as still holding everything it just deleted.

    ``status.json``'s ``grunts`` is ``len(self.slots)``, the CONFIGURED
    pool width, and nothing has ever removed the workspace of an index
    the config later dropped. One live run reported ``grunts: 1``
    while three Lean checkouts (16G + 12G + 9.9G) sat under
    ``sidecar/grunts/``, two of them orphaned by a 3 → 1 config change:
    the accounting and the filesystem disagreed, and only the filesystem
    was paying for it.

    A pure path scan, deliberately not the workspace module's business:
    this runs on the status path every pass, and importing
    ``trellis.sidecar.workspace`` there would pull the observation/lake
    machinery into a heartbeat write. Never raises."""
    base = Path(runtime_root) / "sidecar" / "grunts"
    try:
        entries = list(base.iterdir())
    except OSError:
        return []
    # `isdecimal` rather than `isdigit`: the latter admits non-decimal Unicode
    # digits that `int()` then rejects, and this runs unguarded on the status
    # path, so such a directory name would raise on every pass.
    return [
        int(entry.name)
        for entry in entries
        if entry.name.isdecimal() and (entry / "repo").is_dir()
    ]
...
def slot_record(slot: "GruntSlot") -> Dict[str, Any]:
    ...
    return {
        ...
        "node_file_sha256": slot.node_file_sha256,
    }
...
def slot_from_record(record: Mapping[str, Any], proc: Any) -> Optional["GruntSlot"]:
    ...
    return GruntSlot(
        ...
        adopted=True,
        node_file_sha256=str(record.get("node_file_sha256", "") or ""),
    )
...
class GruntSlot:
    ...
    adopted: bool = False
    # ``node_file_sha256`` of the export row this assignment was made
    # from — the content the attempt is AGAINST. Stamped into every
    # ``attempted.json`` row so the kernel's attempt cap can count
    # attempts per content (a repaired node is a new problem). "" when
    # unknown (adoption by /proc scan, a pre-stamping journal row, or a
    # ``blocked:unsplittable`` export row); the kernel counts hash-less
    # rows toward the cap unconditionally.
    node_file_sha256: str = ""
...
class DaemonState:
    ...
    # Set by an exit path (stop sentinel, drain sentinel, terminal run
    # phase) to describe THAT exit. Once set it overrides the
    # `status.json` phase marker for every later write, so the last
    # status the daemon leaves behind says why it went away. None while
    # the daemon is working normally.
...
class SidecarDaemon:
    ...
    def _startup_inner(self, dirs: "spool_mod.SpoolDirs") -> None:
        ...
        if self.bootstrap_fn is None:
            return
        # Do not build what the first pass will delete. A daemon started
        # (or restarted) while the run is already past formalization
        # would otherwise spend minutes and tens of GB re-cloning every
        # grunt workspace, and then wind down and release them. Adoption
        # and the inflight sweep above still run — orphans must be
        # settled whatever the phase.
        run_phase = terminal_export_phase(self._fresh_export())
        if run_phase:
            self.log(
                f"sidecar: export says the run is in phase {run_phase}; "
                "skipping the grunt workspace bootstrap — the first pass "
                "winds this daemon down"
            )
            return
    ...
    def _run_once_inner(self) -> str:
        ...
        self._reap()
        delivered = spool_mod.deliver_kernel_feedback(self.runtime_root)
        if delivered:
            self.log(
                f"sidecar: delivered {delivered} kernel disposition(s) to grunt results"
            )
        uncharged = reconcile_attempt_charges(self.runtime_root)
        if uncharged:
            self.log(
                f"sidecar: exempted {uncharged} infrastructure-discarded "
                "attempt(s) from dispatch accounting"
            )
        ...
        if drain_sentinel_path(self.runtime_root).exists():
            ...
            self.state.stopped = {
                "phase": PHASE_DRAINED,
                "reason": "drain_sentinel",
                "at_ms": int(now * 1000),
                "left_running": len(self._busy()),
            }
            self._write_status(now)
            return "drain"

        # AUTOMATIC WIND-DOWN: the run has left the phases the sidecar
        # can work in, so there is nothing left for it to do — ever, on
        # this phase. Takes the STOP path (cancel each attempt as a
        # process group, bookkeep it, exit), then releases the grunt
        # checkouts.
        #
        # ORDERING, all three constraints load-bearing:
        #   * AFTER the reap and the rewind guard, so finished work is
        #     bookkept and a rewind is handled as a rewind;
        #   * AFTER both sentinels, so an operator's explicit intent
        #     wins. `drain` in particular MUST win: draining for a
        #     redeploy at the very boundary that flips the phase would
        #     otherwise delete the workspaces of children that are still
        #     compiling in them;
        #   * BEFORE the cancel-watch, which would otherwise cancel the
        #     same attempts under the vaguer "entry left the queue" and
        #     spend a workspace reset on trees about to be deleted.
        #
        # A STALE export cannot trigger it (the A2 rule: never act on a
        # frozen view). The trigger is a transition, and the export that
        # carries the transition is written at that boundary — fresh by
        # construction, seen within one poll.
        if not stale:
            run_phase = terminal_export_phase(export)
            if run_phase:
                return self._wind_down_for_phase(run_phase, export, now)
        ...
        if not queue:
            # Owner decision 1: nothing queued in EITHER lane ⇒ grunts
            # idle.
            return "queue_empty"
    ...
    def run_forever(self) -> None:
        ...
        while True:
            ...
            if status == "phase_complete":
                # No sentinel to unlink: the trigger is the kernel's own
                # export, and writing one would be a file the next
                # operator has to reason about (and that
                # `clear_startup_sentinels` would delete anyway).
                self.log(
                    "sidecar: the run is past proof formalization; exiting. "
                    "Start the daemon again if the run re-enters a phase the "
                    "sidecar works in (add-targets revives a Complete run "
                    "into RevisionStating); the grunt workspaces are "
                    "re-cloned at that start."
                )
                return
    ...
    def _fresh_export(self) -> Optional[Dict[str, Any]]:
        """The export, or None when it is missing or STALE — the same
        frozen-view rule the pass applies (A2), in the one form the
        startup path needs."""
        if export_is_stale(
            self.runtime_root, self.config.export_stale_after_seconds, now=self.now()
        ):
            return None
        return spool_mod.read_candidates(self.runtime_root)
    ...
    def _attempt_cost_usd(self, row: Mapping[str, Any]) -> Optional[float]:
        """Price a ledger row from its own token split, in USD.

        Reuses the main lanes' ``codex_cost_usd`` — the one pricing
        table in the codebase — rather than duplicating rates into a
        second subsystem that would drift on the next price change. The
        import is lazy and every failure returns None: pricing is
        telemetry, and the ledger append it decorates is itself
        never-fatal. ``completion_tokens`` here is output+reasoning
        merged (the throttle's historical scalar), while codex prices
        gross ``output_tokens`` (reasoning already included), so the raw
        output count is recovered by subtracting the reasoning share.
        None — and thus no ``cost_usd`` on the row — whenever the
        provider is not codex or the model is not in the price table."""
        if str(self.config.provider or "") != "codex":
            return None
        try:
            from trellis.agents.tmux_backend import codex_cost_usd

            completion = int(row.get("completion_tokens", 0) or 0)
            reasoning = int(row.get("reasoning_output_tokens", 0) or 0)
            return codex_cost_usd(
                {
                    "model": str(row.get("model", "") or ""),
                    "input_tokens": int(row.get("prompt_tokens", 0) or 0),
                    "cached_input_tokens": int(
                        row.get("cached_input_tokens", 0) or 0
                    ),
    ...
    def _bookkeep_outcome(self, slot: GruntSlot, outcome: Mapping[str, Any]) -> None:
        ...
        row: Dict[str, Any] = {
            ...
            # Token split (main-lane cost-ledger parity): the cached
            # share of prompt_tokens and the reasoning share of
            # completion_tokens. This row build is a de-facto allowlist —
            # a field the runner reports but this dict omits is silently
            # stripped — so the split MUST be named here to survive.
            "cached_input_tokens": outcome.get("cached_input_tokens", 0),
            "reasoning_output_tokens": outcome.get("reasoning_output_tokens", 0),
            "wall_secs": outcome.get("wall_secs", 0.0),
            "snapshot_sha": outcome.get("snapshot_sha", ""),
            # Attribution, recorded on EVERY outcome — not just successes.
            # Without it the only record of which model ran an attempt was
            # the kernel's closure provenance, which exists solely for
            # attempts that CLOSED something: every failed and cancelled
            # attempt was permanently unattributable, and the viewer's
            # ledger table could only ever show "—" for them. Cheap
            # (two short strings) and it makes the failures — the rows you
            # actually want to compare across models — answerable.
        ...
        cost = self._attempt_cost_usd(row)
        if cost is not None:
            row["cost_usd"] = cost
        ...
        record_attempted(
            ...
            slot.node,
            {
                ...
                "wall_secs": outcome.get("wall_secs", 0.0),
                "ts": self.now(),
                # Content this attempt was made against — the kernel's
                # attempt cap counts attempts per node_file_sha256.
                "node_file_sha256": slot.node_file_sha256,
            },
    ...
    def _cancel_slot(
        ...
        record_attempted(
            ...
            slot.node,
            {
                ...
                "detail": reason,
                "ts": self.now(),
                # A cancelled attempt still spent a generation, so it
                # counts toward the kernel's attempt cap like any other
                # published spend — intentional, not an oversight.
                "node_file_sha256": slot.node_file_sha256,
            },
        ...
        ledger_mod.append_ledger_row(
            self.runtime_root,
            {
                ...
                # Same attribution as every other outcome: a cancel spent
                # a generation on a specific model, so it is comparable.
                "provider": self.config.provider,
                "model": self.config.model_name,
            },
        )
    ...
    def _cancel_removed(self, export: Mapping[str, Any]) -> None:
        # BOTH lanes: a live attempt's generation is still live whether
        # the entry sits in the reviewer lane or the kernel lane, and a
        # reviewer add PROMOTES a kernel entry across lanes keeping its
        # `entry_seq` — so a reviewer-lane-only key set would cancel a
        # running attempt the instant the reviewer prioritised it.
        live_keys = {
            (str(row.get("node", "")), row.get("entry_seq"))
            for row in assignable_lanes(export)
        }
    ...
    # -- automatic wind-down (terminal run phase) ------------------------------

    def _wind_down_for_phase(
        self, run_phase: str, export: Optional[Mapping[str, Any]], now: float
    ) -> str:
        """The run has left proof formalization: take the stop path and
        go away, leaving state that reads as INTENTIONAL.

        Not a new shutdown route — it is the `stop` sentinel's route
        reached by a different trigger: the same `_cancel_all` (killpg by
        process group, ledger row, attempted row, published spent
        generation) and the same "return a status the loop exits on".
        What it adds is releasing the grunt checkouts, which the operator
        sentinels deliberately do not do (they are restart paths; this
        one is an end-of-work path).

        Cancelling in flight rather than draining is the right side of
        that trade HERE, and only here: in a terminal phase the engine's
        apply gate refuses a grunt closure, so an attempt allowed to run
        on would spend its whole wall producing something the kernel will
        reject — and it would do so unsupervised, since this daemon is
        the last one. Spending those generations costs nothing that can
        be recovered: the entries are moot in a phase whose window can
        never open, and a run revived into formalization later (the
        add-targets path) re-mints the kernel lane from scratch.

        IDEMPOTENT by construction: the process exits, so it runs at most
        once. A daemon restarted while the run is still in a terminal
        phase repeats it — cancel nothing, release nothing already gone,
        exit — which is the same end state, reached again."""
        self.log(
            f"sidecar: the run is in phase {run_phase} — past proof "
            "formalization, where the kernel's apply gate refuses grunt "
            "closures and the window can never reopen. Winding down: "
            "cancelling in-flight attempts, releasing the grunt "
            "workspaces, exiting."
        )
        cancelled = len(self._busy())
        self._cancel_all(
            f"run left the formalization phase ({run_phase})",
            self._snapshot_of(export),
        )
        released = self._release_grunt_workspaces()
        self.state.stopped = {
            "phase": PHASE_STOPPED,
            ...
            "workspaces_released": released,
            # The wind-down is a daemon exit, NOT a disablement: nothing
            # it writes stops a future daemon from starting over this
            # runtime root, and the workspaces it deleted are re-cloned
            # by `startup()`'s bootstrap. Stated in the file because the
            # question ("has this run's sidecar been retired for good?")
            # is asked of the file, months later.
            "restartable": True,
        }
        self._write_status(now)
        return "phase_complete"

    def _release_grunt_workspaces(self) -> List[int]:
        """Delete every grunt CHECKOUT on disk; return the indices
        released.

        EVERY index the directory holds, not ``range(config.grunts)`` —
        the orphans a lowered ``daemon.grunts`` left behind are exactly
        the ones nothing else will ever reclaim.

        Fail-closed against a live attempt, twice over: `_cancel_all` has
        already taken this daemon's children down as process groups and
        waited on them, and then the /proc scan (`scan_attempt_processes`
        — the documented attempt-child predicate, not a name match) is
        consulted for anything still alive in a workspace, which is
        skipped. An unreadable /proc means "cannot prove nothing is
        running", so nothing is deleted.

        Best-effort throughout: a workspace that will not delete is disk
        to reclaim by hand, never a reason to keep a daemon alive."""
        indices = sorted(_grunt_workspace_indices(self.runtime_root))
        if not indices:
            return []
        # `scan_attempt_processes` reports an unreadable /proc as an EMPTY
        # result rather than by raising, so the guard below never sees it and
        # an empty `live` would read as "nothing is running" — releasing every
        # checkout on exactly the evidence we failed to gather. Check
        # readability first, as `adopt_orphans` does for the same condition.
        if not Path(self.proc_root).is_dir():
            self.log(
                f"sidecar: not releasing grunt workspaces — {self.proc_root} "
                f"is unreadable; reclaim them by hand"
            )
            return []
        try:
            live = {
                identity.grunt
                for _, identity in scan_attempt_processes(
                    self.runtime_root, proc_root=Path(self.proc_root)
                )
            }
        except Exception as exc:  # noqa: BLE001 — fail-closed, never fatal
            self.log(
                f"sidecar: not releasing grunt workspaces — cannot scan "
                f"{self.proc_root} for live attempts ({type(exc).__name__}: "
                f"{exc}); reclaim them by hand"
            )
            return []
        from trellis.sidecar.workspace import release_workspace

        released: List[int] = []
        for k in indices:
            if k in live:
                self.log(
                    f"sidecar: keeping grunt {k}'s workspace — an attempt "
                    "process is still running in it"
                )
                continue
            try:
                if release_workspace(self.runtime_root, k):
                    released.append(k)
            except Exception as exc:  # noqa: BLE001 — disk, never fatal
                self.log(
                    f"sidecar: grunt {k} workspace release failed "
                    f"({type(exc).__name__}: {exc}); reclaim it by hand"
                )
        if released:
            self.log(
                "sidecar: released the grunt checkout(s) "
                + ", ".join(str(k) for k in released)
                + " (rebuilt by the next daemon's bootstrap if the run "
                "re-enters formalization)"
            )
        return released
    ...
    def _write_status(self, now: float, phase: str = PHASE_RUNNING) -> None:
        ...
        attempts = {
            node: [
                _redact_attempt_row_for_status(row)
                for row in rows[-STATUS_ATTEMPTS_PER_NODE:]
            ]
            for node, rows in attempted.items()
        ...
        # An exit path's marker WINS over the caller's, because
        # ``run_once`` rewrites the status one more time after
        # ``_run_once_inner`` has already recorded the exit — with the
        # default ``running``. That last write is why a deliberately
        # stopped daemon has always left ``"phase": "running"`` on disk.
        stopped = self.state.stopped
        if isinstance(stopped, Mapping) and stopped.get("phase"):
            phase = str(stopped["phase"])
        _write_json_atomic(
            status_path(self.runtime_root),
            {
                ...
                "pid": os.getpid(),
                "phase": phase,
                # Present ONLY on a deliberate exit (§4): the reason, the
                # run phase that caused it, and what the exit did. Absent
                # on every ordinary pass, so an older reader is
                # unaffected.
                **({"stopped": dict(stopped)} if isinstance(stopped, Mapping) else {}),
                # Grunt workspaces present on disk, which is NOT
                # ``grunts`` (the configured pool width): lowering
                # ``daemon.grunts`` leaves the dropped indices' Lean
                # checkouts behind forever. Reported so the two numbers
                # disagreeing is visible instead of being a filesystem
                # discovery.
                "workspaces": sorted(_grunt_workspace_indices(self.runtime_root)),
