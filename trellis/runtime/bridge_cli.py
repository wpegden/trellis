...
def main() -> int:
    try:
        ...
        # ALSO persist to a sidecar file: the kernel adapter discards this
        # stdout JSON on error paths, so without the file the stack is lost.
        try:
            import os, time
            with open(os.path.join(os.environ.get("TMPDIR", "/tmp"),
                                   "trellis-bridge-last-error.json"), "w") as fh:
                json.dump({"ts": time.time(), "error": str(exc),
