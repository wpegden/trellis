...
def _acceptance_source_identity(snapshot: Path) -> str:
    digest = hashlib.sha256()
    digest.update(b"trellis-worker-acceptance-v1\0")
    try:
        for relative in _ACCEPTANCE_SOURCE_FILES:
    ...
    return f"trellis-worker-acceptance-v1:sha256:{digest.hexdigest()}"
...
def _tablet_target_for_repo(repo_path: Path) -> str:
    """Resolve the tablet backend target for a repo.

    Mirrors the kernel `worker_normalization::tablet_target_for_repo`: reads
    `workflow.default_target` from `trellis.config.json` and returns the
    backend id string (`"lean"` / `"isabelle_hol"`). Defaults to `"lean"`
    when the config is absent, unreadable, or the key is missing, so a repo
    that predates the field — and the all-Lean live path — selects the Lean
    backend, keeping the rendered prompt byte-identical.
    """
...
def _filespec_path(repo_path: Path) -> Path:
    """Resolve the spec every prompt fragment calls `FILESPEC.md`.

    The run repo holds one filespec, at the canonical name, carrying the
    backend's content (`setup_repo.sh`). The runtime snapshot instead mirrors
    the source tree, where the Isabelle spec keeps its `_isabelle` suffix, so
    the backend name applies only to that fallback.
    """
    ...
    if repo_local.exists():
        return repo_local
    snapshot_name = (
        "FILESPEC_isabelle.md"
        if _tablet_target_for_repo(repo_path) == "isabelle_hol"
        else "FILESPEC.md"
    )
    runtime_copy = repo_path / ".trellis" / "runtime" / "src" / snapshot_name
...
def _scheme_reference_path(repo_path: Path) -> Path:
    ...
    repo_path = repo_path.resolve()
    # BACKEND-ROUTED, exactly like `_filespec_path`. This slot used to hardcode
    # the Lean scheme, so an Isabelle run's worker prompt opened with "Trellis
    # formalizes ... into Lean" and went on to describe `Tablet/<Node>.lean`,
    # `import Tablet.<Other>`, `Preamble.lean`, `def`/`abbrev`/`structure`, the
    # `-- BODY` marker and Mathlib import hygiene — none of which exist on an
    # Isabelle tablet. Live-observed: a worker on an Isabelle run reported "the
    # target's Lean proof remains explicitly open", reading that vocabulary
    # straight out of its own prompt while the correct Isabelle FILESPEC sat
    # further down the same prompt.
    scheme_name = (
        "TRELLIS_FORMALIZATION_SCHEME_isabelle.md"
        if _tablet_target_for_repo(repo_path) == "isabelle_hol"
        else "TRELLIS_FORMALIZATION_SCHEME.md"
    )
    runtime_copy = (
        repo_path
    ...
    if runtime_copy.exists():
        return runtime_copy
    source_copy = PROMPT_SCHEME_REFERENCE_PATH.parent / scheme_name
...
# The reviewer authors trust-critical decisions (`set_live_polarity`,
# `give_up`, `next_active`) from this block, and everything it renders is
# either ro-bound into the reviewer sandbox (`status.json`) or
# interpolated into the reviewer prompt. Any grunt-authorable prose in
# that context is an untrusted->trusted injection channel. So every value
# rendered here is a kernel/daemon-controlled enum, identifier, or count
# passed through this fail-closed filter: it keeps only an
# identifier/gate-name charset and hard-caps the length, so an
# unanticipated free-text field cannot smuggle instructions in. The grunt
# proof body reaches `detail`/the compiler log verbatim, so `detail` is
# not rendered at all — the reviewer still sees WHAT happened through the
# kernel `status`/`reason` enum and the attempt counts.
_PROMPT_TOKEN_CAP = 48


def _sanitize_prompt_token(value: Any, *, cap: int = _PROMPT_TOKEN_CAP) -> str:
    """Reduce an agent-influenced field to a bounded status token: keep
    only the gate/enum charset, collapse everything else to ``-``, cap
    the length. Fail-closed by construction — no path renders raw text."""
...
def _attempt_digest(attempts: Any, node: str) -> str:
    ...
    ``attempted x2, last: failed``.

    The last attempt's kernel/daemon `status` (a gate-name enum such as
    ``rejected:evaluate_node`` / ``budget_exhausted``) and the count are
    rendered; the free-text `detail` (which quotes the grunt proof body
    verbatim via the Lean compiler log) is deliberately NOT rendered."""
    ...
    status = _sanitize_prompt_token(last.get("status", "?")) or "?"
    return f"attempted x{len(rows)}, last: {status}"
    ...
    *,
    ...
    if runtime_root is None:
        return (
            ...
            "<runtime>/sidecar/reviewer_candidates.json",
            "<runtime>/sidecar/status.json",
            str(max_lines),
        )
    sidecar_dir = Path(runtime_root) / "sidecar"
    # The REVIEWER's copy of the export — the kernel's own
    # `candidates.json` additionally carries `kernel_queue`, which the
    # reviewer is not shown. Resolved through the shared
    # `spool.reviewer_export_path` so this and the reviewer sandbox's
    # bind list cannot name different files.
    candidates_path = spool_mod.reviewer_export_path(Path(runtime_root))
    ...
    if not status:
        ...
        # A daemon that stopped ITSELF because the run left the phases the
        # sidecar works in is not an outage, and must not read as one: the
        # reviewer's block is the surface where "NOT RUNNING" otherwise
        # invites a restart of a daemon that would only wind down again.
        # The marker is written by the daemon's own exit path.
        stopped = status.get("stopped")
        stopped = stopped if isinstance(stopped, Mapping) else {}
        if str(stopped.get("reason", "")) == "phase_complete":
            run_phase = _sanitize_prompt_token(stopped.get("run_phase", "?")) or "?"
            lines.append(
                f"Sidecar daemon: STOPPED{pid_note} — it wound itself down "
                f"when the run reached {run_phase}, which is past the phases "
                "the sidecar works in, and released its grunt workspaces. "
                "This is deliberate, not an outage; there is no grunt "
                "capacity and nothing below claims any."
            )
        else:
            lines.append(
    ...
    if queue:
        lines.append(f"Your queue ({len(queue)} entries, submission order):")
        ...
        if free_slots and not attemptable:
            lines.append(
                "Your queue holds no assignable entry (every entry is spent, "
                f"in flight, or blocked), so the {free_slots} free grunt(s) "
                "are drawing on the kernel's own ranked list instead."
            )
        elif not healthy:
            lines.append(
        ...
        lines.append(
            "Your queue: (empty — the grunts are being fed from the kernel's "
            "own ranked list)"
        )
    # The standing fact, stated whether or not the reviewer has queued
    # anything: an empty reviewer queue is NOT an idle pool, and a
    # reviewer that believes otherwise queues work to keep grunts busy
    # rather than to express priority.
    lines.append(
        "The kernel keeps its own ranked list of every eligible node and "
        "dispatches from it whenever your queue has nothing assignable, so "
        "the pool never idles for want of work. Queue a node to put it "
        "AHEAD of that list."
    )
    ...
    if eligible:
        ...
        # cap on a live run at cycle 478, so not one of the 74
    ...
    if pruned:
        ...
        def _prune_line(row: Mapping[str, Any]) -> str:
            node = str(row.get("node", "?"))
            digest = _attempt_digest(attempts, node)
            # `reason` is kernel-authored (`attempt_spent:<status>` /
            # `rejected:<gate>` / a lifecycle enum) but uncapped; bound it
            # through the same fail-closed filter before it reaches the
            # reviewer prompt.
            reason = _sanitize_prompt_token(row.get("reason", "?")) or "?"
            return (
                f"- {node} [seq {row.get('entry_seq')}] "
                f"pruned: {reason} (cycle {row.get('cycle')})"
    ...
    if closures:
        ...
        for row in closures:
            ...
            if len(shown) >= max_lines:
                break
            # `model` is the configured model NAME (operator config, not
            # grunt-authored), but it is rendered raw into the reviewer
            # prompt, so pass it through the same identifier filter.
            model = _sanitize_prompt_token(row.get("model", ""))
...
ISABELLE_FRAGMENT_SUFFIX = "_isabelle"


def _resolve_prompt_fragment(
    fragment_id: str, *, backend: Optional[str] = None
) -> Path:
    """Resolve a fragment id to a file, preferring the backend's variant.

    On an ``isabelle_hol`` run, ``foo/bar.md`` resolves to ``foo/bar_isabelle.md``
    when that file exists, and falls back to ``foo/bar.md`` otherwise.

    This is the STRUCTURAL fix for backend leakage. Every ``_isabelle`` variant
    used to be routed by a hand-written conditional at its own call site in the
    kernel's contract builder, so adding a fragment and forgetting the
    conditional silently shipped Lean text to an Isabelle run — with no error,
    because the Lean fragment exists and renders fine. That is how an Isabelle
    worker came to be told "Trellis formalizes ... into Lean", to read about
    ``Tablet/<Node>.lean``, ``def``/``abbrev``/``structure``, the ``-- BODY``
    marker and Mathlib import hygiene, and to report back that "the target's
    Lean proof remains explicitly open" — vocabulary it took from its own prompt.

    With resolution centralized here, adding the variant file is SUFFICIENT: it
    is picked up automatically, the existing per-call-site conditionals become
    redundant rather than load-bearing, and a missing variant degrades to the
    shared text instead of failing. The Lean path is unchanged — ``backend`` is
    ``None``/``"lean"`` there, so the original id resolves exactly as before.
    """
    root = PROMPT_FRAGMENT_ROOT.resolve()

    def _checked(candidate_id: str) -> Path:
        candidate = (PROMPT_FRAGMENT_ROOT / candidate_id).resolve()
        if root not in candidate.parents:
            raise ValueError(f"prompt fragment escapes fragment root: {candidate_id}")
        if candidate.suffix != ".md":
            raise ValueError(f"prompt fragment must be markdown: {candidate_id}")
        return candidate

    path = _checked(fragment_id)
    if backend == "isabelle_hol":
        variant = path.with_name(f"{path.stem}{ISABELLE_FRAGMENT_SUFFIX}{path.suffix}")
        # Re-check the variant through the same guard: the id is kernel-supplied,
        # never worker text, but the escape check stays on every resolved path.
        if variant.is_file() and root in variant.resolve().parents:
            return variant
    if not path.is_file():
...
def _render_prompt_fragment(
    fragment_id: str, context: Mapping[str, str], *, backend: Optional[str] = None
) -> str:
    template = _strip_prompt_fragment_comments(
        _resolve_prompt_fragment(fragment_id, backend=backend).read_text(encoding="utf-8")
    )
...
def render_prompt_sections(
    ...
    *,
    ...
    for fragment_id in fragment_ids:
        rendered = _render_prompt_fragment(fragment_id, context, backend=backend)
        if not rendered:
            continue
        sections.append(
            {
                ...
                "source_path": str(
                    _resolve_prompt_fragment(fragment_id, backend=backend)
                ),
                "text": rendered,
            }
        )
...
def _render_prompt_from_fragments(
    fragment_ids: Iterable[str],
    context: Mapping[str, str],
    *,
    backend: Optional[str] = None,
) -> str:
    sections = render_prompt_sections(fragment_ids, context, backend=backend)
...
def build_correspondence_prompt(
    *,
    ...
    return _render_prompt_from_fragments(
        ...
        context,
        backend=_tablet_target_for_repo(repo_path),
    )
...
def _render_cleanup_audit_tasks(tasks: Any) -> Any:
    """Prompt-side view of `cleanup_audit_tasks` for the audit burst.

    Two adjustments, both confined to this one rendered field:

    1. Every entry carries an explicit `task_index` equal to its
       zero-based position in the kernel's `cleanup_audit_tasks` array,
       attached BEFORE any reordering. `task_index` is the same field
       the audit emits in `task_modifications`, and the kernel validates
       an inbound index only for range and Pending status — so if
       positions were the only index signal, a reordered rendering would
       silently dismiss the wrong task. (The kernel already stamps
       `task_index` in `audit_contract_payload`; the position fallback
       here keeps the invariant even against an older kernel binary.)

    2. Pending tasks are ordered first, so `_json_fence` truncation
       (threshold 15) drops resolved history (Completed / Failed /
       Dismissed) rather than the live Pending proposals the audit needs
       to avoid duplicating. Non-Pending tasks form a single trailing
       group; within each group the underlying array order is preserved,
       so the output is deterministic.

    Prompt-side only: the rendered `task_index` never reaches inbound
    artifact validation (`validate_trellis_audit_result_data` rebuilds
    `new_tasks` entries from an explicit field allowlist).
    """
    if not isinstance(tasks, list):
        return tasks

    def _is_pending(entry: Any) -> bool:
        if not isinstance(entry, Mapping):
            return False
        status = entry.get("status")
        return isinstance(status, Mapping) and status.get("kind") == "pending"

    indexed: List[Any] = []
    for position, entry in enumerate(tasks):
        if isinstance(entry, Mapping):
            rendered = dict(entry)
            rendered.setdefault("task_index", position)
            indexed.append(rendered)
        else:
            indexed.append(entry)
    return sorted(indexed, key=lambda entry: 0 if _is_pending(entry) else 1)
...
def build_audit_prompt(
    *,
    ...
    context = {
        ...
        "cleanup_audit_tasks_json": _json_fence(_render_cleanup_audit_tasks(cleanup_audit_tasks)),
...
def build_review_prompt(
    *,
    ...
    if snapshot and source_sha:
        ...
        judging_identity = str(
            request_summary.get("acceptance_logic_identity", "") or ""
        ).strip()
        source_identity = _acceptance_source_identity(Path(snapshot))
        context["acceptance_logic_identity"] = judging_identity or "unavailable"
        context["reviewer_source_acceptance_identity"] = source_identity
        context["reviewer_source_matches_acceptance"] = str(
            bool(judging_identity) and source_identity == judging_identity
        ).lower()
