...
class WorkingTreeSnapshot:
    ...
    def list_tablet_node_names(self) -> List[str]:
        return _list_repo_tablet_nodes(self.repo_path, self.node_ext)


# --- Backend descriptor (Lean <-> Isabelle/HOL) -----------------------------
# The viewer is the same three layers for either backend; only a handful of
# source-format facts differ (file extension, whether proofs are split by a
# `-- BODY` marker, the certificate-violation vocabulary, the pane labels).
# The descriptor captures exactly those facts so the adapter + frontend can
# route on data rather than branch on the backend name everywhere. It is
# resolved once per run from `trellis.config.json` `workflow.default_target`
# (`isabelle_hol` -> Isabelle arm; absent / `lean` -> Lean arm) and surfaced
# to the frontend as `payload["backend"]`.
#
# The LEAN ARM IS THE CURRENT BEHAVIOUR VERBATIM: every label is the literal
# string the frontend renders today, `nodeExt` is "lean", `hasBodyMarker` is
# true. So a run with no `default_target` (every existing Lean run) renders
# byte-identically — each routed site reduces to today's path.
_LEAN_BACKEND_DESCRIPTOR: Dict[str, Any] = {
    "backend": "lean",
    "nodeExt": "lean",
    "hasBodyMarker": True,
    "certKind": "lean_axioms",
    "libSearch": "loogle",
    "labels": {
        "sourceImports": "Lean Imports",
        "sourceStatement": "Lean statement",
        "sourceProof": "Lean Proof",
        "closedVerdict": "Lean closed",
        "certViolations": "axioms",
        "progressClosed": "lean-closed (shallow)",
        "proofChars": "lean proof chars",
    },
}

_ISABELLE_BACKEND_DESCRIPTOR: Dict[str, Any] = {
    "backend": "isabelle_hol",
    "nodeExt": "thy",
    "hasBodyMarker": False,
    "certKind": "isabelle_oracles",
    "libSearch": "find_theorems",
    "labels": {
        "sourceImports": "Isabelle Imports",
        "sourceStatement": "Isabelle theory",
        "sourceProof": "Isabelle Proof",
        "closedVerdict": "Isabelle closed",
        "certViolations": "oracles",
        "progressClosed": "isabelle-closed (shallow)",
        "proofChars": "isabelle proof chars",
    },
}


def _read_default_target(repo_path: Path) -> str:
    """Return `workflow.default_target` from the project config, lowercased.

    Read directly from the raw JSON rather than via `load_config`: the strict
    loader does not parse `default_target` into `WorkflowConfig`, and the
    descriptor must work even on configs the strict loader would reject. An
    absent / unreadable config yields "" (the Lean default arm).
    """
    ...
    if not isinstance(workflow, dict):
        return ""
    return str(workflow.get("default_target", "") or "").strip().lower()


def _resolve_backend_descriptor(repo_path: Path) -> Dict[str, Any]:
    """Resolve the backend descriptor for a run from its config.

    `isabelle_hol` -> Isabelle descriptor; anything else (absent, `lean`) ->
    the Lean descriptor (current behaviour verbatim).
    """
    if _read_default_target(repo_path) == "isabelle_hol":
        return dict(_ISABELLE_BACKEND_DESCRIPTOR)
    return dict(_LEAN_BACKEND_DESCRIPTOR)
...
def _list_repo_tablet_nodes(repo_path: Path, node_ext: str = "lean") -> List[str]:
    tablet_dir = repo_path / "Tablet"
    if not tablet_dir.is_dir():
        return []
    # Accept the backend's source extension plus `.tex` (the paper-side
    # statement always present) and `.lean` (Lean is the default, and an
    # Isabelle run keeps a `Preamble.lean` placeholder). For a Lean run
    # `node_ext` is "lean", so the set is unchanged from the historical
    # `{.lean, .tex}` and the listing is byte-identical.
    suffixes = {f".{node_ext}", ".lean", ".tex"}
    names: set[str] = set()
...
def _build_nodes_from_snapshot(
    *,
    ...
    descriptor = descriptor or _LEAN_BACKEND_DESCRIPTOR
    node_ext = str(descriptor.get("nodeExt") or "lean")
    has_body_marker = bool(descriptor.get("hasBodyMarker", True))
    ...
    if "Preamble" not in node_names and snapshot.read_text(f"Tablet/Preamble.{node_ext}"):
        node_names.insert(0, "Preamble")

    # Kernel-authoritative open/present sets (committed tier, with a live-tier
    # fallback) — used to source the per-node closure status for backends
    # whose source format we don't sorry-scan (Isabelle, sign-off #3). The
    # committed mirror is preferred so the status stays on the same tier as
    # the DAG closure coloring (`augmentViewerStateClosure`).
    committed = protocol_state.get("committed") if isinstance(protocol_state.get("committed"), dict) else {}
    kernel_present = committed.get("present_nodes")
    if not isinstance(kernel_present, list):
        kernel_present = live.get("present_nodes") or []
    kernel_open = committed.get("open_nodes")
    if not isinstance(kernel_open, list):
        kernel_open = live.get("open_nodes") or []
    kernel_present_set = {str(n) for n in kernel_present}
    kernel_open_set = {str(n) for n in kernel_open}

    def _node_is_open(name: str, content: str) -> bool:
        if has_body_marker:
            # Lean: trust the cheap sorry-scan over the file content (current
            # behaviour verbatim).
            return (not content) or _file_has_real_sorry(content)
        # Isabelle (sign-off #3): trust the kernel. A node the kernel tracks
        # and does not list as open is closed; a node in the open list, or a
        # not-yet-tracked draft on disk, is open.
        if name in kernel_open_set:
            return True
        return name not in kernel_present_set
    ...
    challenge_claims = (
        protocol_state.get("challenge_claims")
        if isinstance(protocol_state.get("challenge_claims"), dict)
        else {}
    )
    # D6 re-key (mirrors the kernel's `challenge_claim_waives_lanes`): a
    # claim waives the corr/substantiveness lanes only when NO claimed
    # target's spec is `worker_authored` (absent spec / absent field default
    # to seed_pinned). Filter here so `_challenge_claims_node` — the sole
    # consumer — keeps its claim-presence signature.
    _challenge_registry = (
        protocol_state.get("configured_challenge_targets")
        if isinstance(protocol_state.get("configured_challenge_targets"), dict)
        else {}
    )

    def _claims_waive(claims: Any) -> bool:
        if not isinstance(claims, list):
            return False
        for target in claims:
            spec = _challenge_registry.get(str(target))
            provenance = (
    ...
    challenge_claims = {
        node: claims for node, claims in challenge_claims.items() if _claims_waive(claims)
    }
    node_role = (
        protocol_state.get("node_role")
        if isinstance(protocol_state.get("node_role"), dict)
        else {}
    )
    ...
    for node_name in node_names:
        lean_content = snapshot.read_text(f"Tablet/{node_name}.{node_ext}")
        tex_content = snapshot.read_text(f"Tablet/{node_name}.tex")
        node_is_open = _node_is_open(node_name, lean_content)
        ...
        verification = {
            ...
            "nl_proof": _viewer_nl_status(
                ...
                # The NL-proof lane's cheap "source has no `sorry` token => pass"
                # shortcut is Lean-specific; suppress it for backends we don't
                # sorry-scan (sign-off #3) so the lane falls through to the
                # kernel soundness fingerprint state instead.
                lean_content=lean_content if has_body_marker else "",
            ),
        }
        if substantiveness_present:
            verification["substantiveness"] = _viewer_substantiveness_status(
                ...
                challenge_claims=challenge_claims,
                node_role=node_role,
                phase=str(protocol_state.get("phase") or ""),
            )
        ...
        nodes[node_name] = {
            "status": "open" if node_is_open else "closed",
            "kind": viewer_kind,
            "imports": [str(dep) for dep in imports],
            "difficulty": difficulty or "hard",
            "title": title or node_name,
            "texEnv": tex_env,
            # `hasSorry` is the Lean cheap-scan signal; for backends we don't
            # sorry-scan (sign-off #3) report False (no source-level detector
            # yet — closure is kernel-sourced via `status`).
            "hasSorry": _file_has_real_sorry(lean_content) if has_body_marker else False,
...
def _build_live_nodes(
    *,
    ...
    return _build_nodes_from_snapshot(
        snapshot=WorkingTreeSnapshot(repo_path=repo_path, node_ext=node_ext),
...
def _normal_role(raw: Any) -> str:
    return str(raw or "").strip().lower().replace("-", "_")


def _challenge_claims_node(claims_map: Dict[str, Any], node_name: str) -> bool:
    claims = claims_map.get(node_name)
    return isinstance(claims, list) and len(claims) > 0


def _pv_review_suppressed(node_role: Dict[str, Any], node_name: str) -> bool:
    return _normal_role(node_role.get(node_name)) == "extraction_model"


def _under_model_assumptions_waived_for_substantiveness(
    node_role: Dict[str, Any], node_name: str
) -> bool:
    return (
        node_name == _ASSUMPTIONS_NODE
        and _normal_role(node_role.get(node_name)) == "under_model_assumptions"
    )


def _substantiveness_lane_dormant(phase: str) -> bool:
    normalized = _legacy_phase_name(phase)
...
def _sound_status_from_assessment(
    *,
    ...
    if stored_own and current_own and stored_own != current_own:
        # Pin fail-ish verdicts through own-tex drift, mirroring the
        # dep-drift and combined-drift branches below. The kernel's
        # sound_status stays Fail while repairs iterate, so the badge
        # should too — the previous unconditional "?" made every fail
        # invisible the moment its repair burst edited the proof text
        # (in the dec2flt example, fails were viewer-visible for under one cycle).
...
def _viewer_corr_status(
    *,
    ...
    challenge_claims: Dict[str, Any],
    node_role: Dict[str, Any],
) -> str:
    if node_name not in present_nodes or node_name in touched_nodes:
        return "?"
    if _pv_review_suppressed(node_role, node_name):
        return "pass"
    if _challenge_claims_node(challenge_claims, node_name):
        return "pass"
    return _kernel_current_state(
        node_name=node_name,
...
def _viewer_substantiveness_status(
    *,
    ...
    if _challenge_claims_node(challenge_claims, node_name):
        return "pass"
    if _substantiveness_lane_dormant(phase):
        return "pass"
    if node_name == "Preamble":
        return "pass"
    if _under_model_assumptions_waived_for_substantiveness(node_role, node_name):
        return "pass"
    return _kernel_current_state(
        node_name=node_name,
...
def _load_history_state(snapshot: GitSnapshot | WorkingTreeSnapshot) -> Dict[str, Any]:
    payload = decode_shared_state(
        _read_snapshot_json(snapshot, f".trellis-history/{SUPERVISOR_STATE_FILENAME}")
    )
...
def _build_live_viewer_state(repo_path: Path) -> Dict[str, Any]:
    descriptor = _resolve_backend_descriptor(repo_path)
    has_body_marker = bool(descriptor.get("hasBodyMarker", True))
    ...
    state = {
        ...
        # Which human gate is open, and (for the fail-loud NeedInput
        # escalation only) why. See `_gate_view`.
        "gate": _gate_view(protocol_state),
        ...
        # Challenge-target registry (id → spec) and the live challenge
        # coverage map, mirroring configured_targets/coverage above.
        # Empty for paper-only runs.
        "configured_challenge_targets": dict(
            protocol_state.get("configured_challenge_targets") or {}
        ),
        "challenge_coverage": {
            str(target): list(nodes or [])
            for target, nodes in (live_snapshot.get("challenge_coverage") or {}).items()
        },
        "live": {
            "present_nodes": list(live_snapshot.get("present_nodes") or []),
            # Lean: recompute open_nodes from the on-disk file sorry-scan (the
            # kernel's own `has_sorry` over-flags the macro-rules trick).
            # Isabelle (sign-off #3): trust the kernel's `open_nodes` verbatim —
            # we don't sorry-scan `.thy`, and `_recompute_open_nodes_from_repo`
            # would read `.lean` (absent) and mark every node open.
            "open_nodes": (
                _recompute_open_nodes_from_repo(
                    live_snapshot.get("present_nodes") or [], repo_path
                )
                if has_body_marker
                else list(live_snapshot.get("open_nodes") or [])
            ),
        },
        "committed": {
            ...
            "open_nodes": (
                ...
                if has_body_marker
                else list(committed_snapshot.get("open_nodes") or [])
            ),
        },
...
def _normalized_gate_kind(value: Any) -> str:
    """`GateKind` -> snake_case. Empty string when the field is absent.

    An empty result is the "this runtime predates the field" signal and is
    distinct from `"none"` (no gate is open): the viewer falls back to its
    legacy `last_review.decision` heuristic only on the former.
    """
    raw = str(value or "").strip()
    if not raw:
        return ""
...
def _gate_view(protocol_state: Dict[str, Any]) -> Dict[str, Any]:
    """Authoritative "which human gate is open, and why" projection.

    `gate_kind` is the kernel's own discriminator (`ProtocolState::gate_kind`,
    invariant: `stage == HumanGate` iff `gate_kind != None`), so the viewer
    never has to guess whether a stopped run is at a routine phase-advance
    gate or at the fail-loud `NeedInput` escalation. The two are semantic
    opposites and the reviewer-decision string the viewer used to key off is
    only a proxy for the latter.

    The escalation diagnosis is populated ONLY for `need_input`: the reason
    carriers (`latest_stuck_math_audit_rejection_reason`, the audit's
    `need_input_audit`, `gap_human_escalation`) are not cleared on every
    transition, so surfacing them at a routine gate would decorate it with a
    stale incident. `reason_source` is the protocol-state key the text came
    from, so the operator gets a pointer to what to inspect.
    """
    kind = _normalized_gate_kind(protocol_state.get("gate_kind"))
    if not kind:
        request = protocol_state.get("in_flight_request")
        if isinstance(request, dict) and _normalized_request_kind(request.get("kind")) == "human_gate":
            kind = _normalized_gate_kind(request.get("gate_kind"))
    gate: Dict[str, Any] = {
        "kind": kind,
        "from_invalid_attempt": bool(protocol_state.get("gate_from_invalid_attempt")),
        "reason": "",
        "reason_source": "",
        "question": "",
        "unblocking_input": "",
        "ruled_out": [],
        "escalated_at_cycle": None,
    }
    if kind != "need_input":
        return gate

    audit = protocol_state.get("stuck_math_audit")
    need_input_audit = audit.get("need_input_audit") if isinstance(audit, dict) else None
    escalation = protocol_state.get("gap_human_escalation")
    candidates = [
        (
            "latest_stuck_math_audit_rejection_reason",
            _string_or_empty(protocol_state.get("latest_stuck_math_audit_rejection_reason")),
        ),
        (
            "stuck_math_audit.need_input_audit.reviewer_reason",
            _string_or_empty(need_input_audit.get("reviewer_reason"))
            if isinstance(need_input_audit, dict)
            else "",
        ),
        (
            "gap_human_escalation.reason",
            _string_or_empty(escalation.get("reason")) if isinstance(escalation, dict) else "",
        ),
    ]
    for source, text in candidates:
        if text:
            gate["reason"] = text
            gate["reason_source"] = source
            break

    if isinstance(escalation, dict):
        gate["question"] = _string_or_empty(escalation.get("precise_question_tex"))
        gate["unblocking_input"] = _string_or_empty(escalation.get("minimal_unblocking_input"))
        gate["ruled_out"] = [
            _string_or_empty(item) for item in (escalation.get("what_was_ruled_out") or [])
        ]
        cycle = escalation.get("escalated_at_cycle")
        gate["escalated_at_cycle"] = int(cycle) if isinstance(cycle, int) else None
    return gate
...
def _list_cycles(repo_path: Path) -> List[Dict[str, Any]]:
    # `--merged HEAD`: only enumerate checkpoint tags whose commits are reachable
    # from the current HEAD. After a clean restart (`git reset --hard <seed>` then
    # a fresh run), the PRIOR run's `supervisor2/checkpoint-*` tags still exist but
    # point to commits on the abandoned line — not ancestors of the new HEAD — so
    # this filter excludes them automatically. Without it the viewer mixed the new
    # run's cycles (1, 2) with stale old-run cycles (…17, 18, 19). On a transient
    # git failure we fall back to the unfiltered list rather than show nothing.
    try:
        tags = _git(
            repo_path, "tag", "-l", "supervisor2/checkpoint-*",
            "--merged", "HEAD", "--sort=refname",
        ).stdout.splitlines()
    except Exception:
        try:
...
def _build_historical_viewer_state(repo_path: Path, cycle: int) -> Dict[str, Any]:
    ...
    for node_name in node_names:
        ...
        # No kernel protocol_state at this cycle. Lean: cheap sorry-scan
        # (closed iff no real sorry). Isabelle (sign-off #3, no sorry-scan):
        # no verdict available, so a present draft is reported open.
        node_closed = (not _file_has_real_sorry(lean_content)) if has_body_marker else False
        nodes[node_name] = {
            "status": "closed" if node_closed else "open",
            "kind": viewer_kind,
            "imports": _extract_imports(lean_content),
            "difficulty": "hard",
            "title": title or node_name,
    ...
    return {
        "state": {
            ...
            # Git-tag reconstruction: no protocol state, so no gate.
            "gate": _gate_view({}),
            "cleanup_last_good_commit": "",
            "agent_token_usage": {},
            "resume_from": "",
        ...
        "nodes": nodes,
        "backend": _resolve_backend_descriptor(repo_path),
    }
...
def _feedback_post(repo_path: Path, *, action: str, feedback: str) -> Dict[str, Any]:
    ...
    # Stamp the response with the current protocol cycle (Defect 1). The
    # bridge / kernel reject a response whose `cycle` does not match the
    # in-flight gate's cycle, so a stale approve cannot auto-clear a later
    # gate. Compute the cycle BEFORE writing the response so the stamp is
    # embedded in `human_gate_response.json` itself (not only the sidecar).
    try:
        protocol_state = _read_json(runtime_info.protocol_state_path)
    except Exception:
        protocol_state = {}
    ...
    response_path.write_text(
        json.dumps({"choice": normalized_action, "cycle": approved_cycle}, indent=2)
        + "\n",
        encoding="utf-8",
    )
    ...
    # bridge consume only `human_gate_response.json`; this sidecar is read by
    # `_human_gate_response_stale` for the dialog-dismiss heuristic.
...
def _live_chats(repo_path: Path, cycle: int) -> Dict[str, Any]:
    # An empty map (no event-log dir yet) must pass `None` so live chats are
    # not filtered out; only a populated map drives request→cycle filtering.
    request_cycles = _request_cycles_from_event_log(project_event_log_dir(repo_path)) or None
...
def _semantic_closure_for_nodes(
    ...
    names (Tablet.* per the script's artifact-local provider index) whose
    ...
    for node in requested:
        ...
        # Each `const|<full-name>|node=<owner>|...` line carries the owning
        # NODE ID computed by the fingerprint script's declaring-module rule;
        # the closure surface for human reviewers is the set of owner nodes.
        # A line without the owner field (pre-format payload) is skipped —
        # deriving the owner from the dotted name here misattributes
        # namespaced corpora.
        ...
        for chunk in payload_text.split("||"):
            ...
            if not chunk.startswith("const|"):
                continue
            fields = chunk[len("const|"):].split("|")
            owner = ""
            if len(fields) >= 2 and fields[1].startswith("node="):
                owner = fields[1][len("node="):].strip()
            if not owner:
                continue
