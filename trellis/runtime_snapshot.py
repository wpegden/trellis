...
DOC_SOURCES = (
    ...
    SOURCE_ROOT / "FILESPEC_isabelle.md",
)
SCRIPT_SOURCES = (
    SOURCE_ROOT / "scripts" / "lean_module_manifest.lean",
...
def _should_ignore_snapshot_name(name: str) -> bool:
    return (
        ...
        # Cargo target dirs are named for their worktree in this project
        # (`target-worktree`, and per-worktree variants), so the bare
        # `target` entry above missed them: the live run's snapshot was
        # carrying 11 GB of Rust build artifacts into
        # `<repo>/.trellis/runtime/src/kernel/`, copied afresh on every
        # materialize. Nothing downstream reads them — `check.py` pins the
        # kernel to `runtime/bin/`, which is staged separately.
...
def _tablet_target_for_repo(repo_path: Path) -> str:
    """The run's backend, from its config; absent or unreadable means Lean."""
    try:
        raw = json.loads((repo_path / "trellis.config.json").read_text(encoding="utf-8"))
...
def materialize_project_runtime(repo_path: Path, state_dir: Path) -> None:
    ...
    # The snapshot dir above mirrors the source tree, where the Isabelle spec
    # keeps its `_isabelle` suffix. The REPO ROOT instead gets one filespec, at
    # the canonical name every prompt fragment cites, carrying this backend's
    # content — the same rule `setup_repo.sh` follows. Mirroring both names here
    # put the other backend's spec at the root of every run repo.
    repo_filespec = repo_path / "FILESPEC.md"
    if not repo_filespec.exists():
        declared = {doc.name: doc for doc in DOC_SOURCES}
        source_name = (
            "FILESPEC_isabelle.md"
            if _tablet_target_for_repo(repo_path) == "isabelle_hol"
            else "FILESPEC.md"
        )
        doc_src = declared.get(source_name) or declared.get("FILESPEC.md")
        if doc_src is not None:
            repo_filespec.parent.mkdir(parents=True, exist_ok=True)
            shutil.copy2(doc_src, repo_filespec)
