...
# Canonical TeX statement environments, mirroring `TEX_STATEMENT_ENVS` in
# kernel/src/paper_targets.rs (the kernel stays the authority and re-validates;
# this copy exists so a bad `workflow.main_result_envs` is named at config-load
# time). tests/test_config.py pins the two lists equal.
TEX_STATEMENT_ENVS: Tuple[str, ...] = (
    "theorem",
    "lemma",
    "definition",
    "corollary",
...
class WorkflowConfig:
    ...
    # Main-result environment set for this run; empty means the kernel default
    # (theorem, corollary).
    main_result_envs: List[str] = field(default_factory=list)
    # Imported challenge-target registry (challenge_targets.json produced
    # by scripts/import_challenge_targets.py and copied into the run repo
    # by setup_repo.sh --challenge-targets). None for paper-only runs.
    challenge_targets_path: Optional[Path] = None
...
@dataclass
class HistoryConfig:
    """Format switch for the git-tracked `.trellis-history/` artifacts.

    `shared_state` selects the on-disk form of `supervisor_state.json`:
    `trellis-shared-state/1` (structural sharing + string interning) when
    true, the plain snapshot when false. Every reader decodes both forms
    structurally, so this is a pure write-side switch and flipping it costs
    nothing but the next checkpoint's bytes -- rollback is a config edit, not
    a rebuild or a migration.
    """
    shared_state: bool = True


# Known `effort` tiers per provider. ADVISORY — a discoverability aid, never
# a validation gate: efforts are provider-defined and grow with CLI releases,
# so nothing anywhere checks a configured effort against this mapping (the
# create flow's charset checks in viewer/server.js `validatedOverrides` and
# scripts/trellis_create_run.sh `validate_overrides` stay charset checks).
# It exists so every surface that wants to SUGGEST an effort — the create
# wizard's dropdown inputs via `python3 -m trellis.create_options` — reads one
# registry instead of hardcoding a copy that rots.
#
# The backends pass whatever string the config carries straight through
# (codex: `-c reasoning_effort=…` in agents/codex_headless.py; claude:
# `--effort …` in agents/tmux_backend.py; gemini: ignored — see
# INSTALLATION.md "How `effort` is passed"). Evidence for each entry:
# - codex "xhigh": every shipped template (examples/*.config.json), the
#   live runs' configs, and the field comments below.
# - codex "high": the sidecar's trained-regime default
#   (trellis/sidecar/config.py DEFAULT_REASONING_EFFORT), passed to the same
#   codex CLI flag by sidecar/codex_driver.py.
# - claude "low"/"medium"/"high"/"max": the CLI contract recorded on
#   ProviderConfig.effort (trellis/adapters.py); "max" is what live claude
#   workers run (the max-effort zombie-catcher note in agents/tmux_backend.py).
KNOWN_EFFORTS: Dict[str, Tuple[str, ...]] = {
    "codex": ("high", "xhigh"),
...
class CorrespondenceAgentConfig:
    ...
    effort: Optional[str] = None  # any string the CLI accepts; known tiers: KNOWN_EFFORTS
...
class VerificationConfig:
    """The verifier agent pools.

    The three `*_agents` lists are the whole of what reaches dispatch:
    the kernel's verification struct holds exactly these and nothing
    else. The scalar fields below are legacy and inert — they are parsed
    so that configs already on disk keep loading, but no dispatch path
    has ever read them (see `_parse_verification_config`). Do not add a
    consumer for them; put per-lane selection in the agent pools, which
    is where the kernel looks.
    """
    provider: str = "claude"
    model: str = "claude-opus-4-6"
    extra_args: List[str] = field(default_factory=list)
    ...
    substantiveness_agents: List[CorrespondenceAgentConfig] = field(default_factory=list)
...
def normalize_main_result_envs(raw: Any, label: str) -> Optional[List[str]]:
    """Normalize `workflow.main_result_envs` (trim + lowercase, order-preserving
    dedupe) and validate it as a subset of `TEX_STATEMENT_ENVS`. Returns None
    for an absent key, meaning the kernel default (theorem, corollary).

    The subset rule is load-bearing: verifier-side and paper-diff extraction
    always scan the full canonical set, so a main-result env outside it would be
    invisible to them. A non-canonical author env reaches candidacy through
    alias normalization (scripts/normalize_paper_envs.py) instead.
    """
    if raw is None:
        return None
    if not isinstance(raw, list):
        raise ConfigError(f"{label} must be a list of environment-name strings")
    envs: List[str] = []
    for entry in raw:
        if not isinstance(entry, str) or not entry.strip():
            raise ConfigError(f"{label} must be a list of non-empty environment-name strings")
        env = entry.strip().lower()
        if env not in TEX_STATEMENT_ENVS:
            raise ConfigError(
                f"{label} entry {entry.strip()!r} is not a canonical TeX statement environment "
                f"(allowed: {', '.join(TEX_STATEMENT_ENVS)}). Map a non-canonical environment "
                "onto a canonical one with scripts/normalize_paper_envs.py rather than widening "
                "this set."
            )
        if env not in envs:
            envs.append(env)
    if not envs:
        raise ConfigError(f"{label} is empty; omit it entirely to get the default set")
    return envs
...
def resolve_main_result_targets_via_kernel(
    *,
    ...
    raw_labels: Any = None,
    main_result_envs: Optional[Sequence[str]] = None,
) -> Dict[str, Any]:
    payload: Dict[str, Any] = {
        ...
        "main_result_envs": list(main_result_envs) if main_result_envs is not None else None,
    }
    try:
...
def _parse_provider_config(raw: Any, label: str) -> ProviderConfig:
    ...
    if provider not in ("claude", "codex", "gemini", "antigravity"):
        raise ConfigError(
            f"{label}.provider must be claude, codex, gemini, or antigravity, "
            f"got {provider!r}"
        )
...
def _parse_verifier_agents(raw: Any) -> List[CorrespondenceAgentConfig]:
    """Parse one verifier agent pool.

    All three pools (`correspondence_agents`, `soundness_agents`,
    `substantiveness_agents`) share one entry format, so they share one
    parser. They used to be parsed by copy-pasted blocks, which is how
    `substantiveness_agents` came to be dropped here for the entire time
    the kernel was dispatching from it.
    """
    agents: List[CorrespondenceAgentConfig] = []
    if not isinstance(raw, list):
        return agents
    for agent_raw in raw:
        ...
        agents.append(CorrespondenceAgentConfig(
            provider=provider,
            model=model,
...
def _parse_verification_config(raw: Any) -> VerificationConfig:
    # NOTE: this parse does NOT decide dispatch. The kernel re-reads the
    # raw config per request (`bridge_verifier_bindings.rs`) and stamps
    # the binding into the request; Python only executes what the request
    # says. Everything here feeds tooling (provider preflight), so it must
    # track the kernel's schema or that tooling silently covers less than
    # the run actually dispatches.
    if not isinstance(raw, dict):
        return VerificationConfig()

    return VerificationConfig(
        # `provider` / `model` / `thinking_budget` / `max_context_tokens`
        # are accepted-and-ignored: the kernel's verification struct has
        # no such fields, so they never reached dispatch. Still parsed
        # (never rejected) because configs on disk in live runs carry
        # them — rejecting would break those runs on resume.
        ...
        correspondence_agents=_parse_verifier_agents(raw.get("correspondence_agents", [])),
        soundness_agents=_parse_verifier_agents(raw.get("soundness_agents", [])),
        # Kernel: a dedicated pool for the substantiveness lane, falling
        # back to `correspondence_agents` when empty
        # (`bridge_verifier_bindings.rs`). Mirror that fallback at the
        # point of use, not here, so an empty list stays distinguishable
        # from an absent key.
        substantiveness_agents=_parse_verifier_agents(raw.get("substantiveness_agents", [])),
    )
...
def load_config(path: Path) -> Config:
    ...
    # goal_file. KEEP IN SYNC: this default is mirrored by the kernel's
    # `goal_file_from_config` (kernel/src/bin/runtime_cli.rs) for the PV
    # mode-A initial-planning packet; key and default must agree.
    ...
    # Same key `add_paper_targets_action` reads, so every resolution site for a
    # run resolves under one pinned env set.
    main_result_envs = normalize_main_result_envs(
        wf_raw.get("main_result_envs"), "config.workflow.main_result_envs"
    )
    main_result_resolution = resolve_main_result_targets_via_kernel(
        ...
        main_result_envs=main_result_envs,
    )
    ...
    challenge_targets_raw = wf_raw.get("challenge_targets_path")
    challenge_targets_path: Optional[Path] = None
    if challenge_targets_raw:
        if not isinstance(challenge_targets_raw, str):
            raise ConfigError("config.workflow.challenge_targets_path must be a string path")
        candidate = Path(challenge_targets_raw)
        challenge_targets_path = (
            candidate.resolve() if candidate.is_absolute() else (repo_path / candidate).resolve()
        )
        if not challenge_targets_path.is_file():
            raise ConfigError(
                f"config.workflow.challenge_targets_path points at {challenge_targets_path}, which is not a readable file"
            )
        try:
            registry = json.loads(challenge_targets_path.read_text())
        except json.JSONDecodeError as exc:
            raise ConfigError(
                f"config.workflow.challenge_targets_path file {challenge_targets_path} is not valid JSON: {exc}"
            ) from exc
        targets = registry.get("targets") if isinstance(registry, dict) else None
        if not isinstance(targets, list) or not targets:
            raise ConfigError(
                f"config.workflow.challenge_targets_path file {challenge_targets_path} must contain a non-empty `targets` array"
            )
    # Phase overrides
    ...
    workflow = WorkflowConfig(
        ...
        main_result_envs=list(main_result_envs or []),
        challenge_targets_path=challenge_targets_path,
    ...
    if not isinstance(history_raw, dict):
        raise ConfigError("config.history must be a dict")
    shared_state_raw = history_raw.get("shared_state", True)
    if not isinstance(shared_state_raw, bool):
        raise ConfigError(
            f"config.history.shared_state must be a bool, got {shared_state_raw!r}"
        )
    history = HistoryConfig(shared_state=shared_state_raw)
