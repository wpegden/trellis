...
from trellis.runtime.bridge_prompts import (
    ...
    _contract_prompt_facing_view,
    _corr_contract_with_prompt_hints,
    _corr_previous_findings_for_prompt,
    _corr_verify_nodes,
    _correspondence_scratch_path,
    _filespec_path,
    _json_fence,
    _loogle_helper_path,
    _paper_tex_path,
    _prompt_facing_artifact_delivery,
    _prompt_facing_corr_contract,
    _scheme_reference_path,
    _sound_lane_scoped_contract_json,
    _structured_request_path_for,
...
def _paper_bundle(*, request: Dict[str, Any], repo_path: Path, lane_id: str, raw_output_path: Path, done_path: Path) -> Dict[str, Any]:
    ...
    node_basis_inputs = substantiveness_basis_inputs(contract)
    deviation = contract.get("deviation") if isinstance(contract, Mapping) else None
    ...
    if is_per_node_scenario:
        previous_own_findings_raw = contract.get("previous_own_findings")
        previous_own_findings = (
            dict(previous_own_findings_raw)
    ...
    previous_own_findings_block = (
        _json_fence(previous_own_findings)
        if is_per_node_scenario
        else _json_fence(previous_own_findings.get(lane_id))
    )
    context = {
        "repo_path": str(repo_path),
        "lane_id": lane_id,
        ...
        "contract_json": _json_fence(_contract_prompt_facing_view(contract)),
...
def _corr_bundle(*, request: Dict[str, Any], repo_path: Path, lane_id: str, raw_output_path: Path, done_path: Path) -> Dict[str, Any]:
    ...
    request_summary = contract_request_summary(contract)
    previous_own_findings = contract_previous_own_findings_by_lane(contract)
    active_nodes = _corr_verify_nodes(request_summary=request_summary)
