...
_REVIEWER_SIDECAR_READONLY_FILENAMES = ("candidates.json", "status.json")
...
def _reviewer_sidecar_readonly_files() -> List[Path]:
    ...
    the kernel's `candidates.json` export and the daemon's `status.json`,
    then names both absolute paths for the omitted tail. Those files live
    ...
    what halted the `perfect` run at cycle 473). Same class of bug, and
    ...
    return [
        sidecar_dir / name
        for name in _REVIEWER_SIDECAR_READONLY_FILENAMES
        if (sidecar_dir / name).is_file()
    ]
