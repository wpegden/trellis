...
from trellis.git_trust import _inject_git_safe_directories
...
@telemetry.measured("lake_command")
    ...
    try:
        ...
        with _Wait4Popen(
            ...
            if sampler is not None:
                sampler.start()
            telemetry_child = telemetry.child_started(
                process, str(args[0]) if args else "lake", started_ns
            )
            try:
                try:
                ...
                telemetry.child_finished(telemetry_child, process)
                if sampler is not None:
...
@telemetry.measured("hash_bytes", value_arg=(0, "data"))
def _sha256_bytes(data: bytes) -> str:
    return hashlib.sha256(data).hexdigest()


@telemetry.measured("strip_lean_comments", value_arg=(0, "content"))
...
class _LeanImportMemo:
    """Process-local LRU of a pure parser, never of filesystem observations.

    Keys are exact decoded source strings. Every caller still reads disk on
    every use; decoding and all raw-byte provenance hashes are unchanged.
    Values are immutable, and callers receive a new list. Eviction affects
    performance only. The lock protects cache bookkeeping, not source state;
    parsing outside it allows concurrent misses to do harmless duplicate work.

    Retained source/result objects plus a conservative 256-byte per-entry
    bookkeeping allowance are limited to 8 MiB, with at most 2048 entries.
    OrderedDict's index allocation is additionally bounded by the entry cap.
    Oversized entries are parsed normally but never retained. Temporary
    parser allocations and caller-owned return lists are not cache retention.
    """

    def __init__(self, max_bytes: int = 8 << 20, max_entries: int = 2048):
        self.max_bytes = max_bytes
        self.max_entries = max_entries
        self._entries: OrderedDict[str, tuple[tuple[str, ...], int]] = OrderedDict()
        self._bytes = 0
    ...
    def parse(self, content: str) -> list[str]:
        with self._lock:
            entry = self._entries.get(content)
            if entry is not None:
                self._entries.move_to_end(content)
                self._hits += 1
                return list(entry[0])
            self._misses += 1
        modules = _lean_import_modules_uncached(content)
        value = tuple(modules)
        size = (sys.getsizeof(content) + sys.getsizeof(value)
                + sum(sys.getsizeof(module) for module in value) + 256)
        if size > self.max_bytes or self.max_entries <= 0:
            return modules
        with self._lock:
            # A concurrent miss may already have installed the same result.
            ...
            while self._entries and (
                ...
                self._evictions += 1
            self._entries[content] = (value, size)
            self._bytes += size
        return modules

    def info(self) -> dict[str, int]:
        """Bound/usage counters for offline measurement; no source exposure."""
        with self._lock:
            return dict(entries=len(self._entries), charged_bytes=self._bytes,
                        index_bytes=sys.getsizeof(self._entries),
                        max_bytes=self.max_bytes, max_entries=self.max_entries,
                        hits=self._hits, misses=self._misses,
                        evictions=self._evictions)


_lean_import_memo = _LeanImportMemo()


@telemetry.measured("lean_import_modules")
def _lean_import_modules(content: str) -> list[str]:
    """Parse freshly read content, reusing only exact-content parser results."""
    return _lean_import_memo.parse(content)


def _lean_import_modules_uncached(content: str) -> list[str]:
...
@telemetry.measured("source_closure_hash")
...
@telemetry.measured("artifact_bundle")
...
@telemetry.measured("replay_manifests")
...
@telemetry.measured("olean_is_content_current")
...
@telemetry.measured("kernel_replay")
...
@telemetry.measured("materialization")
