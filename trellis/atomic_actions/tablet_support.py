...
def _safe_write(path: Path, content: str) -> None:
    ...
    # Write-if-changed: compare BEFORE any tempfile creation. Non-worker
    # roles (corr/sound/reviewer/audit) run under a bwrap with the repo
    # ro-bound, where even `mkstemp` dies with EROFS; when the rendered
    # content already matches the on-disk bytes (the steady state — the
    # render is a pure function of the repo tree) there is nothing to
    # write, so the sync must be a read-only no-op. When content genuinely
    # differs the write proceeds and, under an ro mount, still fails
    # loudly: a stale index that cannot be refreshed is a real error.
    #
    # Race note: the read-then-skip does not weaken the concurrent-writer
    # story above — a reader never unlinks or replaces anything, and a
    # skipped write is indistinguishable from losing the last-writer-wins
    # race to a writer carrying identical bytes.
    try:
        if path.read_bytes() == content.encode("utf-8"):
            return
    except OSError:
        # Missing or unreadable: fall through to the write attempt, which
        # either repairs the file or raises the honest error.
        pass
