...
def client_lean_semantic_payloads(
    ...
    *,
    ...
    if principal_names is not None:
        request["principal_names"] = {
            node: str(principal_names.get(node, "")).strip()
            for node in validated_nodes
            if str(principal_names.get(node, "")).strip()
        }
...
def client_print_axioms(
    ...
    *,
    ...
    """Run ``#print axioms _root_.<node_name>``.
...
def client_local_closure_axioms(
    ...
    *,
    ...
    ``scan_only``: when True, the server appends ``--scan-only`` so the Lean
    script runs only the source-based worker-authoring policy check. It emits
    no certificate evidence and reports ``ok``, ``policy_rejection``, or
    ``internal_error``. Default False (run the artifact certificate probe).

    ``module_owner`` requests the manifest-complete Preamble probe, whose
    owner has no ordinary FILESPEC principal. The server accepts this mode
    only for ``Preamble`` and rejects combining it with ``principal_name``.
    ...
    if scan_only:
        request["scan_only"] = True
    if module_owner:
        if name != "Preamble" or principal_name is not None:
            raise ValueError(
                "module_owner is reserved for Preamble and cannot be combined with principal_name"
            )
        request["module_owner"] = True
    if principal_name is not None:
        if not isinstance(principal_name, str) or not principal_name:
            raise ValueError("principal_name must be a non-empty string")
        if len(principal_name) > 1024 or any(ch.isspace() or ord(ch) < 0x20 for ch in principal_name):
            raise ValueError("principal_name must be a whitespace-free Lean name of at most 1024 characters")
        request["principal_name"] = principal_name
    return _round_trip(sock_path, request, timeout_secs=coerced_timeout)


def _client_isabelle_node_op(
    op: str,
    socket_path: Any,
    node_name: Any,
    *,
    ...
    """Shared round-trip for the three node-keyed Isabelle ops.

    Like ``client_local_closure_axioms`` (and unlike the lean ``client_*``
    helpers), the Isabelle node ops take NO ``repo`` argument: the server
    derives the supervisor repo from the socket runtime root and the raw
    Isabelle TCP port + password are held internally by the server. The
    worker side must not even pretend to have a say in the repo.
    """
    sock_path = _validate_socket_path(socket_path)
    name = _validate_node_name(node_name)
    coerced_timeout = _validate_timeout(timeout_secs)
...
def client_isabelle_check_node(
    socket_path: Any,
    node_name: Any,
    *,
    timeout_secs: float = 3600.0,
) -> Mapping[str, Any]:
    """Run ``isabelle-check-node`` (the ``lean-compile-node`` analogue).

    Returns the ``ExternalCommandObservation``-shaped envelope
    (``returncode``/``stdout``/``stderr``/``timed_out``/``spawn_error``)
    plus ``node``. ``returncode`` is 0 iff the proof checked.
    """
    return _client_isabelle_node_op(
        "isabelle_check_node", socket_path, node_name, timeout_secs=timeout_secs
    )


def client_isabelle_thm_oracles(
    socket_path: Any,
    node_name: Any,
    *,
    timeout_secs: float = 3600.0,
) -> Mapping[str, Any]:
    """Run ``isabelle-thm-oracles`` (the ``print-axioms`` analogue)."""
    return _client_isabelle_node_op(
        "isabelle_thm_oracles", socket_path, node_name, timeout_secs=timeout_secs
    )


def client_isabelle_thm_deps(
    socket_path: Any,
    node_name: Any,
    *,
    timeout_secs: float = 3600.0,
) -> Mapping[str, Any]:
    """Run ``isabelle-thm-deps`` (the ``local-closure-axioms`` analogue).

    Returns the ``LocalClosureProbeOutput``-shaped cert envelope
    (``status``/``kernel_axioms``/``oracles_used``/``boundary_theorems``/
    ``strict_*_deps``/``errors``) plus the transport fields.
    """
    return _client_isabelle_node_op(
        "isabelle_thm_deps", socket_path, node_name, timeout_secs=timeout_secs
    )


def client_isabelle_warm_advisory(
    socket_path: Any,
    node_name: Any,
    *,
    timeout_secs: float = 3600.0,
) -> Mapping[str, Any]:
    """Run ``isabelle-warm-advisory`` — the worker inner-loop warm pre-check.

    The Isabelle analogue of the Lean warm ``lean --server`` advisory: warm-
    elaborate ONLY the in-flight node against the warm accepted prefix and
    return ``{ok, seconds, errors, advisory_unavailable}`` — NO ``thm_deps``
    cert, NO cold cross-check. Green is necessary-not-sufficient; the
    deterministic node-check (``isabelle-thm-deps``) remains the only sign-off.

    ``advisory_unavailable`` is True when the warm capability is off, the node
    is not synced yet, or the warm session hit an anomaly — the worker then
    falls back to ``isabelle build``. Node-keyed like the other Isabelle node
    ops: no ``repo`` argument (the server derives it from the socket root).
    """
    return _client_isabelle_node_op(
        "isabelle_warm_advisory", socket_path, node_name, timeout_secs=timeout_secs
    )


def _client_isabelle_workspace_op(
    op: str,
    socket_path: Any,
    *,
    timeout_secs: float,
    ...
    coerced_timeout = _validate_timeout(timeout_secs)
    rid = _next_request_id()
    request = {"op": op, "request_id": rid, "timeout_secs": coerced_timeout}
    return _round_trip(sock_path, request, timeout_secs=coerced_timeout)


def client_isabelle_build_session(
    socket_path: Any,
    *,
    timeout_secs: float = 3600.0,
) -> Mapping[str, Any]:
    """Run ``isabelle-build-session`` (the ``prepare-compiled-support`` analogue)."""
    return _client_isabelle_workspace_op(
        "isabelle_build_session", socket_path, timeout_secs=timeout_secs
    )


def client_isabelle_sync_session(
    socket_path: Any,
    *,
    timeout_secs: float = 3600.0,
) -> Mapping[str, Any]:
    """Run ``isabelle-sync-session`` (the ``sync-tablet-support`` analogue)."""
    return _client_isabelle_workspace_op(
        "isabelle_sync_session", socket_path, timeout_secs=timeout_secs
    )
