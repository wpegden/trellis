...
def client_local_closure_axioms(
    ...
    *,
    ...
    ``scan_only`` (FIX 2 coverage): when True, the server appends
    ``--scan-only`` so the Lean script runs ONLY the owner-file
    authored-name scan (no closure walk, no axcheck) and bypasses the
    olean-keyed sidecar cache. The response carries empty dep/axiom
    arrays and ``status`` ``ok``/``internal_error`` for the authoring
    invariant only. Default False (run the full closure probe).
