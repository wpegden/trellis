...
#                          (matches `lake build` here, which the Tablet
#                          leanOptions also set).
...
def _inject_options(text: str) -> Tuple[str, int, int]:
    ...
    lines = text.split("\n")
    n = len(_INJECTED_OPTIONS)
    new_lines = lines[:insert_at] + list(_INJECTED_OPTIONS) + lines[insert_at:]
...
class _Broker:
    ...
    def handle_request(self, req: Mapping[str, Any]) -> Mapping[str, Any]:
        ...
        with self._lock:
            try:
                return self._handle_locked(node_name, lean_path)
    ...
    def _handle_locked(self, node_name: str, lean_path: Path) -> Mapping[str, Any]:
        ...
        # geometry so diagnostics map back to real-file coordinates.
        text, insert_line, num_injected = _inject_options(raw_text)
        ...
        verdict: Mapping[str, Any] = _build_verdict(failed, error_lines, sorry_lines)
...
def _lake_build_fallback(repo: Path, node_name: str, *, reason: str) -> int:
...
def _run_incremental(repo: Path, node_name: str) -> int:
    ...
    if giant:
        return _lake_build_fallback(
            repo, node_name, reason=f"node too large to warm ({giant_reason})"
        )
...
def _run_prewarm(repo: Path, node_name: str) -> int:
    ...
    try:
        ...
        if giant:
            sys.stderr.write(
                f"incremental-check: prewarm skipped, Tablet/{node_name}.lean "
                f"is too large to warm ({giant_reason}); the real check will "
                f"fall back to lake build.\n"
            )
            return 0
        result = _broker_check(repo, node_name)
        if result is None:
