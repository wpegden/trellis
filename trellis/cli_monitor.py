...
def _is_project_repo(candidate: Path) -> bool:
    """True when `candidate` really is a trellis project repo.

    The project config is the discriminator, matching the web viewer's
    `repoTypeForRepo()`; the state dir is what this monitor actually reads.
    A bare `.trellis/` is not evidence on its own -- stray empty ones exist
    (setup bugs, the source checkout), and accepting one binds the monitor to
    a non-repo and renders an empty run instead of failing.
    """
    return project_config_path(candidate).is_file() and project_state_dir_for_repo(candidate).is_dir()


def _repo_candidates(start: Path) -> List[Path]:
    """The ancestor chain `_find_repo` considers, nearest first."""
    cur = start.resolve()
    seen: List[Path] = []
...
def _find_repo(start: Path) -> Optional[Path]:
    """Nearest ancestor of `start` that is a trellis project repo, else None."""
    for candidate in _repo_candidates(start):
        if _is_project_repo(candidate):
            return candidate
    return None
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    parser.add_argument("--repo", type=Path, default=Path.cwd(), help=f"Repo path (default: cwd; walks up for a dir holding {PROJECT_CONFIG_FILENAME} + {PROJECT_STATE_DIRNAME}/)")
    ...
    found = _find_repo(args.repo)
    if found is None:
        start = Path(args.repo).resolve()
        print(f"error: no trellis repo found from {start}", file=sys.stderr)
        print(
            f"  looked for a directory holding both {PROJECT_CONFIG_FILENAME} and {PROJECT_STATE_DIRNAME}/",
            file=sys.stderr,
        )
        print("  searched:", file=sys.stderr)
        for candidate in _repo_candidates(start):
            print(f"    {candidate}", file=sys.stderr)
        print("  pass --repo PATH to point at the run repo explicitly", file=sys.stderr)
        return 2
    repo = found
