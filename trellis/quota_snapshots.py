...
# ----- codex quota (read-only access-token query, no CLI state) ------------
...
def probe_codex(*, timeout_seconds: float = 60.0) -> Dict[str, Any]:
    """Read account quota using the current access token, without running Codex.

    This cosmetic probe never uses or rotates a refresh token and never
    writes provider state. An expired access token is a failed snapshot; the
    operator's Codex remains responsible for refresh. There is deliberately no
    TUI fallback: its per-invocation trust entries polluted the operator home.
    """
    from trellis.codex_quota_api import read_codex_usage

    try:
        payload, email = read_codex_usage(timeout_seconds=timeout_seconds)
        out = _normalize_codex_usage(payload)
        if email:
            out["account"] = email
        return out
    except Exception as exc:
        # Neither response bodies nor exception messages are safe to log here:
        # they can contain credentials. Quota failure must not stop a burst.
        return {"provider": "codex", "ok": False,
                "error": f"read-only quota query failed ({type(exc).__name__})"}


def _normalize_codex_usage(payload: Dict[str, Any]) -> Dict[str, Any]:
    """Keep the existing ledger shape; ignore model-specific rate limits."""
    import math
    ...
    for key in ("primary_window", "secondary_window"):
        ...
        name = {5 * 3600: "five_hour", 7 * 24 * 3600: "weekly"}.get(seconds)
        if name is None:
            continue
        ...
        if reset is None and row.get("reset_after_seconds") is not None:
            reset = now + int(row["reset_after_seconds"])
        reset = int(reset) if reset is not None else None
        frac = _frac_of_month(seconds)
        windows.append({
            "name": name, "pct_used": pct, "pct_used_kind": "exact",
            "resets_at": reset,
            "resets_at_repr": (datetime.fromtimestamp(reset, timezone.utc).isoformat()
                               if reset is not None else None),
            "resets_in_seconds": reset - now if reset is not None else None,
            "window_seconds": seconds, "fraction_of_month": round(frac, 6),
            "monthly_burn_pct": round(pct * frac, 4),
        })
    out = {"provider": "codex", "ok": bool(windows), "windows": windows}
    if payload.get("plan_type"):
        out["plan_tier"] = payload["plan_type"]
    ...
    if isinstance(credits, dict) and credits.get("balance") is not None:
        try:
            ...
            pass
    if not windows:
        out["error"] = "no 5h/Weekly quota windows returned"
    return out
