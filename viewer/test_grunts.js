...
// ---------------------------------------------------------------------
// A fully-populated run, shaped after a live run's sidecar.
function populated(name, extra) {
  ...
  writeJson(path.join(rt, 'protocol_state.json'), Object.assign({
    ...
    sidecar_queue_seq: 4,
    // The kernel's durable map, in its REAL shape. This fixture used to
    // model a predecessor key (`sidecar_closures`, flat) that the kernel
    // had already replaced — so these tests passed against a reader that
    // could never match a live run, and the model column on the grunts
    // page was blank in production while green here.
    //
    // Two things the real shape does that the old one didn't: it records
    // worker closures alongside grunt ones (tagged by `closed_by`), and
    // it nests the attempt identity under `sidecar`. `WorkerClosed` is
    // here to hold the filter honest — a worker closure must never be
    // counted as grunt work.
    closure_provenance: {
      Zeta: { closed_by: 'sidecar', cycle: 308,
        sidecar: { attempt_id: 'sc-z', provider: 'mistral', model: 'leanstral', wall_ms: 164500 } },
      Eta: { closed_by: 'sidecar', cycle: 301,
        sidecar: { attempt_id: 'sc-e', provider: 'mistral', model: 'leanstral', wall_ms: 90000 } },
      WorkerClosed: { closed_by: 'worker', cycle: 307 },
    },
  }, (extra || {}).state || {}));
  spool(rt, 'pending', 2);
  spool(rt, 'claimed', 1);
...
test('closures: durable kernel map preferred, newest cycle first', () => {
  ...
  // 2, not 3: `WorkerClosed` is in the same map but was closed by a
  // worker, and this is the grunt surface.
  ...
  // Read out of the nested `sidecar` arm, not the entry's top level.
...
test('closures: a worker closure is never reported as grunt work', () => {
  // Guards the `closed_by` filter directly: with only worker closures in
  // the map the grunt surface must fall through to the export mirror, not
  // claim the worker's node. Cheap, and the failure it prevents —
  // crediting a grunt with a worker's closure — is one the predecessor
  // map's "was EVER grunt-closed" semantics actually shipped.
  const rt = populated('closures-worker-only', {
    state: { closure_provenance: { OnlyWorker: { closed_by: 'worker', cycle: 300 } } },
  });
  const d = gruntsStateForRuntimeRoot(rt, { now: 1_060_000 });
  assert.ok(!d.recent_closures.some((r) => r.node === 'OnlyWorker'));
...
test('closures: export mirror used when kernel state has no closure map', () => {
  const rt = populated('closures-export', { state: { closure_provenance: {} } });
  const d = gruntsStateForRuntimeRoot(rt, { now: 1_060_000 });
...
test('malformed: non-object / wrong-typed fields degrade per-section', () => {
  ...
  writeJson(path.join(rt, 'protocol_state.json'), { sidecar_queue: 'nope', closure_provenance: 5 });
  const d = gruntsStateForRuntimeRoot(rt, { now: 1_060_000 });
  assert.strictEqual(d.enabled, true);
  assert.deepStrictEqual(d.errors, []);
  assert.deepStrictEqual(d.queue, []);
