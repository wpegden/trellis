...
function main() {
  ...
  // Launch presets: one selection sets every role; guided presets touch the
  // audit lane only. astra/sol run xhigh, luna runs high.
  assert.deepStrictEqual(CREATE_PRESETS.map(preset => preset.key),
    ['astra-guided-sol', 'all-sol', 'sol-guided-luna', 'all-luna']);
  assert.strictEqual(CREATE_DEFAULT_PRESET, 'all-sol');
  for (const preset of CREATE_PRESETS) {
    for (const lane of [preset.base, preset.guide]) {
      assert.strictEqual(lane.effort, /luna/.test(lane.model) ? 'high' : 'xhigh',
        `${preset.key}: ${lane.model} effort`);
    }
  }
  assert.deepStrictEqual(launcherGuideRoles(launcherRoleOptions()), ['stuck_math_audit'],
    'the math launcher guide lane is the stuck-math audit alone');
  assert.deepStrictEqual(launcherGuideRoles(['worker', 'reviewer']), []);

  const landingSource = fs.readFileSync(path.join(__dirname, 'public', 'landing.html'), 'utf-8');
  const resolverStart = landingSource.indexOf('function resolvePresetRoleSettings(');
  const resolverEnd = landingSource.indexOf('\nfunction describePreset(', resolverStart);
  assert.ok(resolverStart >= 0 && resolverEnd > resolverStart);
  // eslint-disable-next-line no-new-func
  const resolvePresetRoleSettings = new Function(
    `${landingSource.slice(resolverStart, resolverEnd)}\nreturn resolvePresetRoleSettings;`,
  )();
  const mathRoles = launcherRoleOptions().map(role => ({
    ...role, provider: 'codex', model: 'gpt-5.6-sol', effort: 'xhigh',
  }));
  const guided = resolvePresetRoleSettings(CREATE_PRESETS[0], mathRoles, ['stuck_math_audit']);
  assert.deepStrictEqual(guided.stuck_math_audit, { model: 'gpt-6-astra', effort: 'xhigh' });
  for (const key of ['worker', 'easy_worker', 'hard_worker', 'reviewer',
    'verification.correspondence_agents', 'verification.soundness_agents',
    'verification.substantiveness_agents']) {
    assert.deepStrictEqual(guided[key], { model: 'gpt-5.6-sol', effort: 'xhigh' }, key);
  }
  const solLuna = resolvePresetRoleSettings(CREATE_PRESETS[2], mathRoles, ['stuck_math_audit']);
  assert.deepStrictEqual(solLuna.stuck_math_audit, { model: 'gpt-5.6-sol', effort: 'xhigh' });
  assert.deepStrictEqual(solLuna.worker, { model: 'gpt-5.6-luna', effort: 'high' });
  const allLuna = resolvePresetRoleSettings(CREATE_PRESETS[3], mathRoles, ['stuck_math_audit']);
  assert.ok(Object.values(allLuna).every(selected => selected.model === 'gpt-5.6-luna'
