...
try {
  ...
  // Liveness rides in from the pause surface. The markers say why the kernel
  // stopped; whether the process is still there is a separate fact, and the
  // attention rows have to carry the resume control once it is gone.
  fs.unlinkSync(checkerPath);
  fs.writeFileSync(systemPath, JSON.stringify({ cycle: 847, burst_role: 'stuck_math_audit' }));
  const supervisor = supervisorContextFromStatus({
    state: 'down', wrapper_pid: null, resumable: true, launch_env: { present: true },
  });
  state = haltStateForRuntimeRoot(root, supervisor);
  assert.strictEqual(state.attention.length, 1);
  assert.strictEqual(state.attention[0].id, 'system_feedback_halt');
  assert.strictEqual(state.attention[0].supervisor_state, 'down');
  assert.strictEqual(state.attention[0].resumable, true);
  assert.match(state.attention[0].summary, /process has exited/);
} finally {
