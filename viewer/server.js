...
// Async twin of getCachedHistoricalViewerState for the state-at routes: the
// first request for a cycle spawns the python adapter off the event loop and
// concurrent requests for the same cycle share that one spawn.
async function getCachedHistoricalViewerStateAsync(projectInfo, cycle) {
  const key = cycleEntryKey(projectInfo, cycle);
  if (cycleStateCache.has(key)) return cycleStateCache.get(key);
  if (projectInfo.repoType !== 'trellis') return getCachedHistoricalViewerState(projectInfo, cycle);
  return _adapterInFlightOnce(`state-at::${key}`, async () => {
    if (cycleStateCache.has(key)) return cycleStateCache.get(key);
    const data = await trellisAdapterAsync(projectInfo, 'state-at', [String(cycle)]);
    if (typeof augmentViewerStateClosure === 'function') augmentViewerStateClosure(data);
    if (typeof augmentViewerStateAttention === 'function') augmentViewerStateAttention(data);
    if (typeof thinViewerStatePayload === 'function') {
      thinViewerStatePayload(data, projectCacheKey(projectInfo));
    }
    cycleStateCache.set(key, data);
...
// Sync accessor for callers on polled paths (external-codex, pause state,
// landing alarms). A stale entry is returned immediately while an async
// refresh runs in the background, so a 30s TTL expiry never blocks the
// event loop behind a multi-second python adapter spawn. Only the very
// first read for a project (no entry at all) pays the synchronous cost.
function readLiveViewerStateCached(projectInfo) {
  ...
  if (entry && projectInfo.repoType === 'trellis') {
    readLiveViewerStateCachedAsync(projectInfo).catch(() => {});
    return entry.value;
  }
...
app.get(`${BASE}/api/state-at/:cycle`, async (req, res) => {
  ...
  try {
    const projectInfo = resolveRepoPath(defaultProjectSlug());
    res.json(await getCachedHistoricalViewerStateAsync(projectInfo, cycle));
  } catch (e) {
    res.status(404).json({ error: `Cycle ${cycle} not found: ${e.message}` });
  }
});

app.get(`${BASE}/:project/api/state-at/:cycle`, async (req, res) => {
  const cycle = parseInt(String(req.params.cycle).replace(/\.json$/, ''), 10);
  if (isNaN(cycle)) return res.status(400).json({ error: 'Invalid cycle' });
  try {
    const projectInfo = resolveRepoPath(projectFromRequest(req));
...
// Launch presets: one selection sets every role. A "guided" preset puts the
// guide model on the audit lanes only (see launcherGuideRoles); every role
// that runs each cycle (workers, reviewer, verifiers) takes the base model.
// astra and sol always run xhigh; luna always runs high.
const CREATE_PRESETS = [
  {
    key: 'astra-guided-sol',
    label: 'astra-guided sol',
    base: { model: 'gpt-5.6-sol', effort: 'xhigh' },
    guide: { model: 'gpt-6-astra', effort: 'xhigh' },
  },
  {
    key: 'all-sol',
    label: 'all sol',
    base: { model: 'gpt-5.6-sol', effort: 'xhigh' },
    guide: { model: 'gpt-5.6-sol', effort: 'xhigh' },
  },
  {
    key: 'sol-guided-luna',
    label: 'sol-guided luna',
    base: { model: 'gpt-5.6-luna', effort: 'high' },
    guide: { model: 'gpt-5.6-sol', effort: 'xhigh' },
  },
  {
    key: 'all-luna',
    label: 'all luna',
    base: { model: 'gpt-5.6-luna', effort: 'high' },
    guide: { model: 'gpt-5.6-luna', effort: 'high' },
  },
];
const CREATE_DEFAULT_PRESET = 'all-sol';

// The guide lanes of a launch: the stuck-math audit plus any Phase-0 audit
// lane the flow declares. Nothing that runs every cycle belongs here.
function launcherGuideRoles(roles, independence) {
  const keys = (roles || []).map(role => (typeof role === 'string' ? role : role.key));
  const out = keys.filter(key => String(key).split('.').pop() === 'stuck_math_audit');
  for (const key of (independence && independence.audit_roles) || []) {
...
registerControlReadRoute('create-jobs.json', (_req, res) => {
  try {
    res.json({
      ...
      guide_roles: launcherGuideRoles(launcherRoleOptions()),
