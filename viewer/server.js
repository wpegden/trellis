...
function handleDownloadTablet(res, project) {
  ...
  if (fs.existsSync(configPath)) {
    try {
      ...
      // PV runs carry a `pv_tablet` block (extractor stack, goal mode,
      // pinned target statements). The viewer-state projection has no PV
      // fields, so the config is the keying source for the PV README.
      const rawPv = (config || {}).pv_tablet;
      if (rawPv && typeof rawPv === 'object') {
        pvTablet = rawPv;
      }
    } catch (_err) {
      mainResultTargets = [];
  ...
  // PV README facts (generated fresh per download):
  // - livePolarity: the kernel's effective per-pair Decide polarity, read from
  //   the runtime protocol_state (the viewer-state projection has no PV
  //   fields). Absent/unreadable -> empty map -> goal lines render without a
  //   polarity annotation (never a guessed one).
  // - assumptionsAll: every under-model assumption in Tablet/Assumptions.lean
  //   (marker-parsed), cross-referenced against APPROVED_AXIOMS.json
  //   (ratified) and PROPOSED_ASSUMPTIONS.json (pending metadata).
  let livePolarity = {};
  try {
    const rt = runtimeRootForProject(projectInfo);
    if (rt) {
      const ps = JSON.parse(fs.readFileSync(path.join(rt, 'protocol_state.json'), 'utf-8'));
      if (ps && typeof ps.pv_live_polarity === 'object' && ps.pv_live_polarity) {
        livePolarity = ps.pv_live_polarity;
      }
    }
  } catch (_err) { livePolarity = {}; }
  let assumptionsAll = [];
  try {
    const lean = fs.readFileSync(path.join(tabletDir, 'Assumptions.lean'), 'utf-8');
    const approved = new Set();
    try {
      const appr = JSON.parse(fs.readFileSync(path.join(repoPath, 'APPROVED_AXIOMS.json'), 'utf-8'));
      for (const n of (appr.global || [])) approved.add(String(n));
    } catch (_e) { /* absent -> nothing ratified */ }
    const pendingMeta = {};
    try {
      const prop = JSON.parse(fs.readFileSync(path.join(repoPath, 'PROPOSED_ASSUMPTIONS.json'), 'utf-8'));
      for (const a of (prop.assumptions || [])) pendingMeta[String(a.axiom_name || '')] = a;
    } catch (_e) { /* absent -> no pending metadata */ }
    const re = /^-- BEGIN UNDERMODEL ASSUMPTION (\S+)$[\s\S]*?^axiom (\S+)/gm;
    let m;
    while ((m = re.exec(lean)) !== null) {
      const [, id, axiom] = m;
      const meta = pendingMeta[axiom] || null;
      assumptionsAll.push({
        id,
        axiom,
        ratified: approved.has(axiom),
        claimClass: meta ? String(meta.claim_class || '') : '',
        neededBy: meta && Array.isArray(meta.needed_by) ? meta.needed_by : [],
      });
    }
  } catch (_err) { assumptionsAll = []; }
...
`;
  ...
  // ── PV-mode README ────────────────────────────────────────────────────
  // A PV run's expert task is NOT paper-correspondence review: extraction-
  // model nodes are provenance-trusted (never expert-read), goals are
  // either byte-pinned at seed (goal_mode 'spec' with pinned target
  // statements) or worker-derived from GOAL.md prose (goal_mode 'prose'),
  // and the artifact that genuinely needs a human is the trusted base —
  // the under-model assumption set and the TCB disclosures. Keyed on the
  // config's `pv_tablet` block; math-mode output above stays byte-identical.
  if (pvTablet) {
    const pvGoalMode = String(pvTablet.goal_mode || 'spec').toLowerCase();
    const pvTargets = Array.isArray(pvTablet.verification_target_statements)
      ? pvTablet.verification_target_statements
      : [];
    const pvPinned = pvGoalMode === 'spec' && pvTargets.length > 0;
    const pvLang = String(pvTablet.language || 'source');
    const pvStack = Array.isArray(pvTablet.extractor_stack) && pvTablet.extractor_stack.length
      ? pvTablet.extractor_stack.map((s) => `\`${s}\``).join(' + ')
      : 'the pinned extractor stack';
    const decideIntro = pvTargets.some((t) => String((t || {}).resolution || '').trim() === 'decide')
      ? 'A `decide` goal is resolved in either direction: prove the statement, or prove its kernel-pinned `<name>__Refutation` negation. `decide → prove|disprove` below is the currently live direction.\n\n'
      : '';
    const pvGoalSection = pvPinned
      ? decideIntro + pvTargets.map((t) => {
          const name = String((t || {}).name || '').trim() || '(unnamed)';
          const res = String((t || {}).resolution || '').trim();
          const informal = String((t || {}).informal || '').split('\n')[0].trim();
          let resTag = res;
          if (res === 'decide' && Object.keys(livePolarity).length) {
            resTag = `decide → ${String(livePolarity[name] || 'prove')}`;
          }
          return `- \`${name}\`${resTag ? ` (${resTag})` : ''}${informal ? ` — ${informal}` : ''}`;
        }).join('\n')
      : '- Goals are derived by the run from the prose intent in `GOAL.md`; the current target-coverage map is in the viewer.';
    const pvProtected = `${protectedIntro}\n\n${protectedSection}${protectedClosureExplanation}`
      .replace('paper-faithfulness fingerprint', 'per-target fingerprint');
    const assumptionLines = assumptionsAll.map((a) => {
      const extras = [];
      if (a.claimClass) extras.push(a.claimClass);
      if (a.neededBy.length) extras.push(`needed by ${a.neededBy.map((n) => `\`${n}\``).join(', ')}`);
      return `- \`${a.axiom}\` (\`${a.id}\`) — ${a.ratified ? 'ratified' : 'PENDING'}${extras.length ? ` (${extras.join('; ')})` : ''}`;
    });
    const assumptionsSection = assumptionLines.length
      ? `## Under-Model Assumptions (review ALL — ratified included)

${assumptionLines.join('\n')}

Statements are in \`Tablet/Assumptions.lean\` / \`Tablet/Assumptions.tex\` between the per-assumption markers. Pending entries carry citations, hunt results, and class flags in \`ASSUMPTIONS_REVIEW.md\`; the ratified set is disclosed in \`APPROVED_AXIOMS.json\` / \`tcb_manifest.json\`.

`
      : '';
    const trustedBaseTask = `1. **Under-model assumptions** — axioms about ${pvLang} semantics that the extracted model omits: the full set (ratified and pending) is listed below. For each: is it a genuine language/compiler guarantee (never a program-maintained invariant)? Does the cited corpus text actually support it? Does the Lean statement say exactly what the NL claim says? A \`domain\`-class entry is BINDING — ratifying it fixes the validity hook's extension and thereby decides goal polarity; read the review file's preamble before approving one.
2. **TCB disclosures** (\`tcb_manifest.json\`) — the extraction provenance, opaque boundary primitives, and assumption set: the complete list of what any finished theorem in this run rests on beyond the Lean kernel and Mathlib.`;
    const pvSkip = `Do not spend review effort on: reading extraction-model node bodies (provenance-trusted), unfinished proofs (\`sorry\` is expected at this phase and is machine-gated later), or worker-authored support statements (machine-checked for correspondence; not part of the trusted base).`;
    const pvStructure = `## Structure

- \`Tablet/<name>.lean\` / \`Tablet/<name>.tex\` — node pairs (kinds above)
- \`GOAL.md\`, \`CORRESPONDENCE.md\` — the verification intent and the model-to-source correspondence policy
- \`tcb_manifest.json\`, \`APPROVED_AXIOMS.json\` — the disclosed trusted base
- \`PROPOSED_ASSUMPTIONS.json\`, \`ASSUMPTIONS_REVIEW.md\` — pending assumptions awaiting ratification (when present)${paperFiles.length ? '\n- `paper/` — auxiliary source documents' : ''}`;
    if (pvPinned) {
      readme = `# Verification Tablet Snapshot (program-verification run)

This package is for external expert review of this run's TRUSTED BASE — the parts machine checking cannot establish on its own.

## What this run is

The project verifies ${pvLang} code against pinned formal goals. \`Tablet/\` holds Lean/\`.tex\` node pairs of three kinds:

- **Extraction-model nodes** — machine-extracted from the ${pvLang} source by ${pvStack} under a pinned toolchain. Their trust is PROVENANCE — the extractor-toolchain pins and source-file sha256 digests recorded in \`tcb_manifest.json\` \`extraction_provenance\`, whose \`not_recorded\` list names the pins this run does not carry — not expert reading; the kernel reopens them only if the pinned extraction changes.
- **Goal & spec nodes** — the verification goals listed below plus the spec-layer definitions they quantify over, seeded byte-pinned and hash-guarded: the run cannot change their meaning without reopening human review.
- **Proof/support nodes** — worker-authored statements with NL proofs supporting the goals.

## Expert Task

Machine checking already gates statement correspondence, NL-proof soundness, and the axiom closure of finished proofs. Your judgment is the trust anchor for exactly three things:

${trustedBaseTask}
3. **Goal fidelity** — the pinned goals below against the verification intent (\`GOAL.md\`, \`CORRESPONDENCE.md\`). The run cannot alter them; it also cannot repair a mis-stated goal. Worth one careful pass if not already done.

${pvSkip}

${assumptionsSection}## Pinned Verification Goals
...
`;
      ...
      readme = `# Verification Tablet Snapshot (program-verification run, prose goals)

This package is for external expert review of this run's TRUSTED BASE and its goal statements.

## What this run is

The project verifies ${pvLang} code against goals the run derived from the prose intent in \`GOAL.md\`. \`Tablet/\` holds Lean/\`.tex\` node pairs of three kinds:

- **Extraction-model nodes** — machine-extracted from the ${pvLang} source by ${pvStack} under a pinned toolchain. Their trust is PROVENANCE — the extractor-toolchain pins and source-file sha256 digests recorded in \`tcb_manifest.json\` \`extraction_provenance\`, whose \`not_recorded\` list names the pins this run does not carry — not expert reading.
- **Goal & spec nodes** — worker-authored formalizations of the \`GOAL.md\` intent. Unlike a pinned-goal run, THESE ARE IN SCOPE for your review: their meaning is not seeded, so a human must vouch for it.
- **Proof/support nodes** — worker-authored statements with NL proofs supporting the goals.

## Expert Task

1. **Goal statement fidelity** — for each goal/spec node: does the \`.tex\` statement genuinely capture the \`GOAL.md\` intent, and does the Lean statement genuinely correspond to the \`.tex\`? Do not trust names or prose labels; verify meaning from the Lean and the definitions it references.
${trustedBaseTask.replace('1. **Under-model assumptions**', '2. **Under-model assumptions**').replace('2. **TCB disclosures**', '3. **TCB disclosures**')}

${pvSkip}

${assumptionsSection}## Goals
...
`;
  ...
  if (fs.existsSync(tabletDir)) {
    ...
    const srcExt = `.${backendNodeExtForRepo(repoPath)}`;
...
//     (~7.8 MB raw / ~0.5 MB gz; ~99% of `state` on the designs run).
//
...
const ATTENTION_STATES = {
  checker_disagreement_halt: {
    id: 'checker_disagreement_halt',
    tier: 'fault',
    rank: 100,
    icon: '⛔',           // no-entry
    label: 'HALTED — CHECKER DISAGREEMENT',
    summary: 'Two independent checkers disagreed. Soundness is in question and the supervisor will not dispatch new bursts.',
  },
  system_feedback_halt: {
    id: 'system_feedback_halt',
    tier: 'fault',
    rank: 90,
    icon: '⚠',           // warning
    label: 'HALTED — SYSTEM FEEDBACK',
    summary: 'An agent emitted system feedback the kernel was configured to halt on. The supervisor will not dispatch new bursts.',
  },
  malformed_halt_marker: {
    id: 'malformed_halt_marker',
    tier: 'fault',
    rank: 110,
    icon: '⛔',
    label: 'HALTED — MALFORMED HALT MARKER',
    summary: 'A halt marker is on disk but could not be parsed, so the halt reason is unknown. Treat as the most severe until read by hand.',
  },
  need_input: {
    id: 'need_input',
    tier: 'failure',
...
function haltAttention(haltState) {
  ...
  return markers.map(entry => {
    ...
    return {
      ...
      inspect: [`${(entry && entry.marker_path) || '<runtime>/halt-marker.json'} (diagnostics + clear instructions; delete the file to resume)`],
    };
...
function haltStateForRuntimeRoot(runtimeRoot) {
  ...
  state.attention = haltAttention(state);
  return state;
}

function kernelHaltState(projectInfo) {
  return haltStateForRuntimeRoot(runtimeRootForProject(projectInfo));
}
...
function runPauseScript(projectInfo, args, timeoutMs) {
  const runtimeRoot = runtimeRootForProject(projectInfo);
  if (!runtimeRoot) throw new Error('no runtime root for project');
  return execFileSync('bash', [PAUSE_SCRIPT, ...args, runtimeRoot, projectInfo.repoPath], {
...
function pauseAttention(projectInfo, status) {
  ...
  if (status.state === 'down' && !projectPhaseIsComplete(projectInfo)) {
    return {
...
app.post(`${BASE}/api/pause/config`, express.json(), (req, res) => {
  try { res.json(handlePauseConfig(resolveRepoPath(defaultProjectSlug()), req.body)); }
  catch (e) { res.status(400).json({ error: e.message }); }
});
app.post(`${BASE}/:project/api/pause/config`, express.json(), (req, res) => {
  try { res.json(handlePauseConfig(resolveRepoPath(projectFromRequest(req)), req.body)); }
  catch (e) { res.status(400).json({ error: e.message }); }
});

function handlePauseAction(projectInfo, action) {
  ...
  const out = runPauseScript(projectInfo, [action], action === 'resume' ? 45000 : 15000);
  invalidatePauseStatus(projectInfo);
  return { ok: true, output: out, ...pauseStateResponse(projectInfo) };
}
for (const action of ['arm', 'disarm', 'resume']) {
  app.post(`${BASE}/api/pause/${action}`, express.json(), (_req, res) => {
    try { res.json(handlePauseAction(resolveRepoPath(defaultProjectSlug()), action)); }
    catch (e) { res.status(500).json({ error: e.message, output: String(e.stderr || '') }); }
  });
  app.post(`${BASE}/:project/api/pause/${action}`, express.json(), (req, res) => {
    try { res.json(handlePauseAction(resolveRepoPath(projectFromRequest(req)), action)); }
    catch (e) { res.status(500).json({ error: e.message, output: String(e.stderr || '') }); }
  });
}
...
function gruntsStateForRuntimeRoot(runtimeRoot, opts) {
  ...
  const closureMap = asObject(state.sidecar_closures);
  let recentClosures;
  if (Object.keys(closureMap).length) {
    recentClosures = Object.entries(closureMap).map(([node, meta]) => {
      ...
      return {
        ...
        attempt_id: m.attempt_id ? String(m.attempt_id) : null,
        wall_ms: Number.isFinite(Number(m.wall_ms)) ? Number(m.wall_ms) : null,
      };
    });
...
// All event-log records in global index order, read from the per-cycle files.
function readEventLogRecords(projectInfo) {
  const files = eventLogCycleFiles(eventLogDirForProject(projectInfo));
  const out = [];
  for (const f of files) out.push(...readJsonlSync(f));
  return out;
}
...
function buildUsageRollup(projectInfo) {
  ...
  const events = readEventLogRecords(projectInfo);
...
const PROGRESS_DISK_CACHE_VERSION = 9;
...
function progressForCommit(repoPath, sha) {
  ...
  // `substantiveness_waived` predicates (model.rs): a node is waived on BOTH
  // lanes iff it claims a challenge target — i.e. `challenge_claims[node]` is
  // non-empty. Both kernel predicates are keyed on `challenge_claims`
  // identically, so a single set covers corr and subst. Empty / absent for
  // non-challenge runs (the field is `skip_serializing_if = is_empty`), so the
  // set is empty there and every callsite is a no-op — the non-challenge path
  // is byte-identical. This is the corr/subst analogue of the soundness
  // "or waived" treatment below (which waives on own-Lean-closed).
  const challengeClaims = state.challenge_claims || {};
  function laneWaived(n) {
    const claims = challengeClaims[n];
    return Array.isArray(claims) ? claims.length > 0 : false;
  }
  ...
  // the fail series structurally zero (dec2flt: real fails at cycles
  ...
  for (const node of presentNodes) {
    ...
    let leanText = '';
    try {
      leanText = execSync(`git -C ${JSON.stringify(repoPath)} show ${sha}:Tablet/${node}.${nodeExt}`,
        { encoding: 'utf8', stdio: ['ignore', 'pipe', 'ignore'], maxBuffer: 4 * 1024 * 1024 });
    } catch {
      hasSorry[node] = false;
      continue;
...
function attachSoundVerifierFailCounts(projectInfo, checkpoints) {
  ...
  // Stream-parse the event log line-by-line. Concatenate the per-cycle files
  // in global index order so the cursor walk below sees one continuous stream.
  let content = '';
...
function startServer() {
  ...
  return app.listen(PORT, (err) => {
    if (err) {
      console.error(`[viewer] listen failed on port ${PORT}: ${err.code || err.message}`);
      process.exit(1);
    }
    writeStatic();
