  ...
  if (pvTablet) {
    ...
    const pvGoalSection = pvPinned
      ? pvTargets.map((t) => {
          ...
          return `- \`${name}\`${res ? ` (${res})` : ''}${informal ? ` — ${informal}` : ''}`;
        }).join('\n')
...
- **Extraction-model nodes** — machine-extracted from the ${pvLang} source by ${pvStack} under a pinned toolchain. Their trust is PROVENANCE (the byte-pin recorded in \`tcb_manifest.json\`), not expert reading; the kernel reopens them only if the pinned extraction changes.
...
- **Extraction-model nodes** — machine-extracted from the ${pvLang} source by ${pvStack} under a pinned toolchain. Their trust is PROVENANCE (the byte-pin recorded in \`tcb_manifest.json\`), not expert reading.
