...
def pytest_configure(config: pytest.Config) -> None:
    ...
    keep working).
    """
    # Isabelle/HOL checker (B2a) opt-in live marker: spawns a REAL
    # `isabelle server` and is skip-by-default so it never runs in the
    # live-Lean window. Run explicitly with `-m isabelle_live` in a
    # throwaway scratch dir.
    config.addinivalue_line(
        "markers",
        "isabelle_live: opt-in test that spawns a real isabelle server "
        "(skip-by-default; never run during a live Lean run).",
    )
