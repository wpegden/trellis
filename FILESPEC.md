...
- A node file must not author any declaration whose **final name
  component is reserved-shaped** — i.e. it starts with `_`, or is
  `eq_`/`match_`/`proof_`/`omega_` followed only by digits/`_` (the shapes
  Lean reserves for compiler-generated internals such as `_sunfold`,
  `_proof_N`, equation lemmas, and matchers). This holds across every
  authoring surface: bare, `protected`/`private`, namespaced
  (`namespace NodeName … theorem eq_1 … end`), `where`/`let rec`-bound, and
  `axiom`. The local-closure check transparent-walks *generated* names of
  this shape (so it never records them as cross-node dependency keys); an
  *authored* declaration of the same shape would therefore be hidden as if
  generated and could cross a node boundary untracked. Such a name is
  rejected at the node's own acceptance — factor the auxiliary into its own
  registered node, or rename it to a non-reserved shape. (Generated
  members of a `def`/`structure`/`inductive`/`class` principal — field
  accessors, constructors, projections — are part of the principal and are
  unaffected; only *authored declaration commands* are scanned.)
- A node file must not define a **macro, syntax, or elaborator** —
  `macro`, `macro_rules`, `elab`, `elab_rules`, `syntax`,
  `declare_syntax_cat`, or `binder_predicate`. Such a command can
  synthesize a top-level declaration whose name never appears literally in
  source (e.g. a macro emitting `theorem NodeName.eq_1`), so it would
  bypass the local-closure/authored-name invariant above — an unregistered,
  unreviewed declaration could cross a node boundary untracked. The node is
  rejected at its own acceptance. Term-level operator notation
  (`notation`, `infix`/`infixl`/`infixr`, `prefix`, `postfix`) remains
  allowed: it cannot declare a top-level constant.
...
- **an authored declaration whose final name component is reserved-shaped
  (leading `_`, or `eq_`/`match_`/`proof_`/`omega_` + digits) is rejected**
  (the universal text-scan backstop is in `validate_lean_node_shape`,
  `kernel/src/filespec.rs`; the authoritative all-surfaces gate is the
  Lean-parse owner-file scan in `scripts/lean_local_closure.lean`, which runs
  on every node the local-closure probe runs on). Nodes accepted before this
  rule landed are not retroactively re-scanned.
