...
- `trellis.config.json` / `trellis.policy.json` — the default run configuration
  ...
  `CONFIG_TEMPLATE=...`.
- `trellis.isabelle.config.json` / `trellis.isabelle.policy.json` — ready-made
  templates for an Isabelle/HOL run (`workflow.default_target: "isabelle_hol"`);
  pass the config via `CONFIG_TEMPLATE=...` (see INSTALLATION.md §2g).
...
Point setup at it with:
    ...
    ./scripts/setup_repo.sh --loogle on|off <repo_path> examples/connectivity_threshold_gnp.tex

(`--loogle on|off` is required — see INSTALLATION.md §2f for what it gates.)
