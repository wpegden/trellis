...
#
# Idempotent by content sha: re-running with a source that produces exactly the
# bytes already stored at paper/refs/<id>.tex succeeds, leaves the file
# untouched, and only repairs the config entry if it is missing (the crash
# window between the file write and the config append, or a config regenerated
# from its template). An existing file with ANY other content is a hard error —
# reference papers are immutable (REFERENCE_PAPERS.md) — as is an existing
# config entry whose source_id disagrees.
#
...
# failure) passes the transcoded file through unchanged. Rendered into scratch
# first: what WOULD be written is what an existing file is compared against.
WOULD_WRITE="$SCRATCH/$REF_ID.normalized.tex"
if python3 "$SCRIPT_DIR/normalize_paper_envs.py" "$UTF8_TEX" "$WOULD_WRITE" 2>/dev/null; then
  :
else
  cp "$UTF8_TEX" "$WOULD_WRITE"
fi

# 3. The idempotency gate. Identical bytes ⇒ this exact registration already
# happened (or crashed between the file write and the config append): keep the
# file, and only the config entry below may still need work. Anything else at
# this path is a different paper under the same id — immutable, hard stop.
IDEMPOTENT=0
if [[ -e "$OUT" ]]; then
  if cmp -s "$WOULD_WRITE" "$OUT"; then
    IDEMPOTENT=1
    echo "  $REL_OUT already registered with identical content; leaving it untouched"
  else
    echo "ERROR: $OUT already exists with different content; reference papers are immutable — use a new id" >&2
    exit 1
  fi
fi

if [[ "$IDEMPOTENT" -eq 0 ]]; then
  # Duplicate-id config check BEFORE the file is written, so an abort leaves
  # no orphan paper/refs/<id>.tex behind. (An entry for an id whose file is
  # absent is anomalous — the file always lands first — so it is refused
...
# 4. Ensure the config entry (python; no jq dependency). Atomic rewrite:
# tmp + rename in the same directory (the kernel add-targets discipline),
# so a crash mid-write cannot clobber the operator's config. On the
# idempotent path a matching entry is a no-op and a mismatched one is an
# error; on the fresh path the duplicate-id check is repeated as defence
# in depth.
python3 - "$CONFIG" "$REF_ID" "$REL_OUT" "$SOURCE_ID" "$IDEMPOTENT" <<'PY'
import json
import os
import sys

config_path, ref_id, rel_out, source_id, idempotent_flag = sys.argv[1:6]
idempotent = idempotent_flag == "1"
data = json.loads(open(config_path, encoding="utf-8").read())
if not isinstance(data, dict):
    raise SystemExit(f"{config_path} is not a JSON object")
...
if not isinstance(entries, list):
    raise SystemExit("config.workflow.reference_papers is not an array")
for entry in entries:
    if not isinstance(entry, dict) or entry.get("id") != ref_id:
        continue
    if (
        idempotent
        and entry.get("tex_path") == rel_out
        and entry.get("source_id") == source_id
    ):
        print(f"  workflow.reference_papers entry '{ref_id}' already present; config unchanged")
        raise SystemExit(0)
    if idempotent:
        raise SystemExit(
            f"config already registers reference paper id `{ref_id}` with a different "
            f"entry (tex_path={entry.get('tex_path')!r}, source_id={entry.get('source_id')!r}); "
            f"reference registrations are immutable — use a new id"
        )
    raise SystemExit(f"config already registers reference paper id `{ref_id}`")
...
print(f"  Appended workflow.reference_papers entry '{ref_id}' to {config_path}")
PY

# 5. Commit BOTH files with explicit pathspecs.
if git -C "$REPO" rev-parse --is-inside-work-tree >/dev/null 2>&1; then
  git -C "$REPO" add -- "$REL_OUT" "trellis.config.json"
  # A re-registration that reproduces byte-identical content (setup_repo.sh
  # resuming an interrupted setup: the config was regenerated from the
  # template and this entry re-appended, landing back on what HEAD already
  # records) leaves nothing to commit. Say so instead of failing the run —
  # and keep every other commit failure fatal.
  if [ -z "$(git -C "$REPO" status --porcelain -- "$REL_OUT" "trellis.config.json")" ]; then
    echo "  Nothing to commit: $REL_OUT + trellis.config.json already match HEAD"
  else
