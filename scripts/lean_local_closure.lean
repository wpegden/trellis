...
* `Ctor.inj` / `Ctor.injEq` are recognized by checking that their immediate
  parent is an environment-recognized constructor.
* `Lean.isAuxRecursor` recognizes the eliminator family
  (`.casesOn` / `.recOn` / `.brecOn` / `.below`) at the environment level,
  rather than relying on the downstream Rust name-prefix list.
...
run needs it, and only with a provenance-based check.
