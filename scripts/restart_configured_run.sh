...
usage() {
  cat <<'EOF'
Usage:
  scripts/restart_configured_run.sh [--no-run] [--no-current] [--no-reset]
...
Flags:
  ...
  --no-reset          Resume an already-prepared/existing run WITHOUT wiping it.
                      Skips the npm CLI update, `setup_repo.sh --reset`,
                      `rm -rf <runtime_root>`, and `trellis.sh init` — keeping
                      the prepared repo + runtime state (protocol_state.json)
                      intact. Use this to launch a revision run prepared by
                      setup_revision_repo.sh, or to resume any existing run.
...
# Namespace the tmux-sweep glob by the RUN SLUG (runtime root name with a
# trailing `-runtime` stripped, e.g. `connectivity-isa-runtime` ->
# `connectivity-isa`), NOT `runtime_root.parent.name`. Every run lives under
# `~/math/`, so the parent name is the literal `math` for every run; sweeping
# `trellis-math-*` would kill ANOTHER run's burst sessions on the shared
# `tmux -L trellis` server. Mirrors bridge.py `_runtime_session_namespace`.
runtime_namespace_source = (
    runtime_root.name.removesuffix("-runtime") or runtime_root.name
)
...
plan: restart_configured_run.sh dry-run
  ...
  reset_repo=$RESET_REPO$([[ "$RESET_REPO" -eq 0 ]] && echo " (--no-reset: keep prepared repo + runtime state, skip npm update / setup_repo --reset / rm -rf runtime / trellis.sh init)")
...
# --no-reset resumes an already-prepared/existing run: skip the npm CLI
# update, the `setup_repo.sh --reset` repo recreation, the `rm -rf` of the
# runtime root, and `trellis.sh init`. This preserves a revision seed
# (RevisionStating state from setup_revision_repo.sh) or any in-progress run.
if [[ "$RESET_REPO" -eq 1 ]]; then
  ...
  echo "  --no-reset: keeping prepared repo $REPO_PATH and runtime root $RUNTIME_ROOT intact"
fi
...
# Soundness fingerprints run strict: the citation discipline (every proof
# \noderef in the import closure) has been enforced corpus-wide, and the
# permissive kernel default exists only for legacy corpora. Overridable
# per-invocation via the environment.
TRELLIS_SOUNDNESS_FINGERPRINT_MODE="${TRELLIS_SOUNDNESS_FINGERPRINT_MODE:-v2_strict}"
SUPERVISOR_CHECKER_ENV="TRELLIS_CHECKER_SOCKET='$CHECKER_SOCKET_PATH' TRELLIS_SOUNDNESS_FINGERPRINT_MODE='$TRELLIS_SOUNDNESS_FINGERPRINT_MODE'"
