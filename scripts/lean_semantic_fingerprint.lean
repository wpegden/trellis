...
its root: a bare `env.find? <nodeName>` fast path for top-level (math)
nodes, else a scan of the node's OWN module `Tablet.<node>` for the unique
...
namespace (`dec2flt_full_integer.Spec.dec2flt_total` in module
`Tablet.dec2flt_total`), so the previous bare-stem seed
...
/-- FILESPEC name-parity test (shared rule with `lean_local_closure.lean`):
some trailing component run of `name`, joined with `_`, equals the node
stem (`dec2flt_full_integer.Spec.FiniteDecimal.value` matches stem
`FiniteDecimal_value` via its last two components). -/
...
/-- Generated/internal-detail guard for root resolution (a reduced form of
`lean_local_closure.lean::isTabletGeneratedArtifact`; ambiguity below is a
fail-loud `ERR`, so the guard only needs to filter the common generated
members out of the candidate scan). -/
private def isGeneratedArtifact (env : Environment) (name : Name) : Bool :=
...
/-- Outcome of resolving a node's seed declaration. -/
private inductive RootResolution where
  | resolved (name : Name)
  | missing
  | ambiguous (candidates : Array Name)

/-- Resolve the seed declaration for `nodeName`: bare fast path, else the
unique stem-matching non-generated declaration in the node's own module
`Tablet.<nodeName>`. Mirrors `lean_local_closure.lean::resolveRoot`. -/
private def resolveRoot (env : Environment) (nodeName : String) : RootResolution :=
  ...
  if (env.find? bare).isSome then
    ...
    let ownModule := nameFromString s!"Tablet.{nodeName}"
    let candidates : Array Name :=
      env.constants.fold (init := #[]) fun acc name _ =>
        if declMatchesStem name nodeName
            ...
            && !isGeneratedArtifact env name then
          acc.push name
        else
          acc
...
def main (args : List String) : IO UInt32 := do
  ...
  for nodeName in nodeNames do
    try
      ...
      | .missing =>
          IO.println s!"ERR\t{nodeName}\tno declaration in module Tablet.{nodeName} matches the node stem (seed resolution failed)"
      | .ambiguous candidates =>
          let names := String.intercalate ", " (candidates.toList.map toString)
          IO.println s!"ERR\t{nodeName}\tambiguous seed resolution: multiple stem-matching declarations in module Tablet.{nodeName}: {names}"
      | .resolved declName =>
