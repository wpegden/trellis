...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        help=(
            "Explicit alias mapping, repeatable. For papers whose \\newtheorem "
            "declarations carry no usable title (e.g. \\newtheorem{thm}{}[section]), "
            "auto-detection finds nothing; supply --map thm=theorem instead. "
            "Explicit mappings override auto-detected ones."
        ),
    )
    ...
    for spec in args.map:
        ...
        if not sep or not alias or not canonical:
            parser.error(f"--map expects ALIAS=CANONICAL, got {spec!r}")
        if canonical not in CANONICAL_ENVS:
            parser.error(
                f"--map target {canonical!r} is not a kernel-canonical env "
                f"({', '.join(sorted(CANONICAL_ENVS))})"
            )
        if alias in aliases and aliases[alias] != canonical:
            print(
                f"note: --map overrides detected {alias!r} -> {aliases[alias]!r} "
                f"with {canonical!r}",
                file=sys.stderr,
            )
