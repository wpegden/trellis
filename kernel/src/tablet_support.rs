...
pub fn sync_tablet_render_support_from_repo(
    ...
    let raw = match driver {
        ...
        // identical to the historical hardcoded-Lean construction.
        crate::backend::CheckerDriver::Lean(_) => {
            ...
            run_repo_command_json_with_stdin(
                repo_path,
        ...
        // `isabelle_sync_session` handler derives the session dir from its
        ...
        crate::backend::CheckerDriver::IsabelleHol(_) => run_repo_command_json(
            repo_path,
            driver.sync_tablet_support_op(),
            &[repo_path.display().to_string()],
        )?,
    ...
    serde_json::from_value(raw)
        .map_err(|err| format!("parse {} output failed: {err}", driver.sync_tablet_support_op()))
}
...
/// Olean-staleness fix (end-to-end): an olean is current with its source
/// CONTENT iff it exists AND the provenance sidecar the checker writes beside
/// it (`<node>.olean.srcclosure`, holding the source-closure hash the olean
/// was built from) equals the current `lean_closure_cache_key` for the node.
///
...
/// the sidecar against the recomputed key closes that gap. Fail-closed: a
/// missing/unreadable sidecar, an uncomputable key, or a mismatch all return
/// `false`, forcing a real re-materialize (which refreshes both olean and
/// sidecar) rather than serving a possibly-stale artefact.
...
fn all_oleans_content_current(repo_path: &Path, nodes: &BTreeSet<NodeId>) -> bool {
    nodes.iter().all(|node| {
        let cleaned = node.trim();
        if cleaned.is_empty() {
            true
        } else {
            olean_content_current(repo_path, cleaned)
        }
    })
}
...
fn materialize_tablet_oleans(
    ...
    if let Some(ref k) = cache_key {
        ...
        if let Some((stored_key, stored_value)) = cache.get(&lookup_key) {
            if stored_key == k && all_oleans_content_current(repo_path, requested_nodes) {
    ...
    if let Some(ref k) = cache_key {
        ...
        {
            if all_oleans_content_current(repo_path, requested_nodes) {
...
mod tests {
    ...
    fn write_node_olean(repo: &Path, node: &str) {
        ...
        // Olean-staleness fix: the real checker materialize records a
        // provenance sidecar holding the source-closure hash the olean was
        // built from; the cache-hit guard (`all_oleans_content_current`)
        // requires it. Mirror that here so a present olean is also
        // content-current for the source state at write time.
        if let Some(key) = crate::cache_key::lean_closure_cache_key(repo, node) {
            let sidecar = repo
                .join(".lake/build/lib/lean/Tablet")
                .join(format!("{node}.olean.srcclosure"));
            write_test_file(&sidecar, &format!("{key}\n"));
        }
    }
...
"#,
    ...
    fn materialize_oleans_cache_falls_back_when_olean_content_stale() {
        ...
        let sidecar = repo.join(".lake/build/lib/lean/Tablet/A.olean.srcclosure");
