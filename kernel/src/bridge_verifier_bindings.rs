...
fn phase_name(phase: crate::Phase) -> &'static str {
    match phase {
        ...
        crate::Phase::RevisionStating => "revision_stating",
...
mod tests {
    ...
    fn sample_request(kind: RequestKind) -> WrapperRequest {
        WrapperRequest {
            id: 1,
            kind,
            work_kind: crate::model::WorkerWorkKind::Standard,
            cycle: 1,
            ...
            configured_challenge_targets: BTreeMap::new(),
            current_challenge_claims: BTreeMap::new(),
            challenge_targets_configured: false,
            node_role: BTreeMap::new(),
            is_pv: false,
            trust_base_required_v1: false,
            pv_live_polarity: BTreeMap::new(),
            trust_seam_repair_context: None,
            ...
            rejected_deviations: BTreeMap::new(),
            current_deviation_files: BTreeMap::new(),
            node_deviation_claims: BTreeMap::new(),
            configured_reference_papers: BTreeMap::new(),
            ...
            kernel_scheduled_sound_review_nodes: BTreeSet::new(),
            sidecar_queue: Vec::new(),
            sidecar_window_open: false,
            sidecar_advertise_queue_fields: false,
            process_memory_active: false,
            ...
            protected_reapproval_corr_fingerprint_pairs: BTreeMap::new(),
            ...
            pending_worker_audit_request: None,
            pending_memory_challenges: Vec::new(),
            pending_node_retirement: None,
            latest_node_retirement_decline: None,
            audit_request_admissible: false,
            consumed_global_repair_grant: false,
            targeted_next_active_nodes: BTreeSet::new(),
            theorem_restructure_next_active_nodes: BTreeSet::new(),
            ...
            latest_worker_under_model_disproof: String::new(),
            latest_worker_under_model_route_opinion: String::new(),
            latest_worker_under_model_reasoning: String::new(),
            assumption_authoring: None,
            pending_assumption_draft: None,
            pending_authoring_candidate: String::new(),
            ...
            formalization_complete_view: false,
            cleanup_repair_candidates: BTreeSet::new(),
            ...
            // Advisory view fields this fixture has no opinion about come from
            // Default, which also keeps the telemetry-reachability scan in
            // model.rs honest: naming them here would register this fixture as
            // a reader of a map it never reads.
            ..Default::default()
        }
    }
    ...
    fn easy_close_worker_skipped_when_must_close_false() {
        ...
        // default-pending-task Worker requests where `must_close_active=false`.
        let tmp = tempdir().expect("tempdir");
        let config_path = tmp.path().join("trellis.config.json");
        fs::write(
            &config_path,
