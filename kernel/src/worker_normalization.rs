...
/// pair exists on disk so the extracted model can import `RustValidSliceU8`,
/// but it is deliberately not an ordinary tablet node.  Filtering at this
/// boundary is defense in depth for the pre-validation immutability check in
/// `runtime_cli`: a pre-existing support carrier can never be laundered into
/// `new_nodes` merely because the post-worker disk scanner sees its stem.
...
/// Uncovered-target orphan window: open iff some configured paper target
/// has no PRE-burst present node claiming it. While open, worker bursts
/// may create new live orphan nodes — the DAG may be built in layers,
/// support-first, before the covering statement lands — so the same-burst
/// orphan-deletion clauses of `tablet_deletion_contract_errors` are
/// waived. Once every configured target is covered the window closes and
/// the b67ccbf same-burst contract applies exactly as before.
///
...
///
/// Challenge targets are deliberately excluded: the window is a paper-
/// target (stating/revival) construction affordance.
fn orphan_construction_window_open(normalization: &WorkerNormalizationInput) -> bool {
    normalization.configured_targets.iter().any(|target| {
