use crate::model::{
    ...
    NodeSetUpdates, PvRole, RefPaperId, TargetClaimUpdates, TargetId, Update, WorkerOutcome,
...
pub struct WorkerNormalizationInput {
    ...
    /// PRE-burst PV sketch set. `None` means a non-PV/math acceptance;
    /// `Some(empty)` is meaningful and is therefore serialized: it says the
    /// PV burst was dispatched after every current proof had real NL prose.
    /// The option keeps the added carrier absent from math-mode bytes.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub current_sketch_proof_nodes: Option<BTreeSet<NodeId>>,
...
pub struct WorkerAcceptanceInput {
    ...
    #[serde(default)]
    pub conditional_theorem_proposal: Option<crate::trust_base::ConditionalTheoremProposal>,
    #[serde(default)]
    pub conditional_theorem_withdrawals: BTreeSet<crate::model::ChallengeTargetId>,
    #[serde(default)]
    pub node_deviation_claims: BTreeMap<NodeId, BTreeSet<DeviationId>>,
    /// Reference-paper claims (node -> full replacement set), the
    /// `node_deviation_claims` twin.
    #[serde(default)]
    pub node_reference_grounds: BTreeMap<NodeId, BTreeSet<RefPaperId>>,
    #[serde(default)]
    ...
    /// Configured reference-paper ids at the moment the worker burst was
    /// issued. Every id named in `node_reference_grounds` must be in this
    /// set — the kernel rejects unknown ids deterministically
    /// (fail-closed) instead of silently pruning them.
    #[serde(default)]
    pub configured_reference_papers: BTreeSet<RefPaperId>,
    ...
    /// True for either Cleanup validation envelope. Production sets this from
    /// the request validation kind; the normalizer also recognizes Cleanup
    /// plan steps so direct callers cannot bypass the transition policy.
    #[serde(default)]
    pub cleanup_transition_preflight_required: bool,
    /// Whether the engine's baseline satisfied `formalization_valid()` when
    /// this worker was dispatched.
    #[serde(default)]
    pub cleanup_baseline_formalization_valid: bool,
    /// Cleanup's formalization-regression clause is active only in the actual
    /// Cleanup phase; the proof-phase cleanup validation pass may legitimately
    /// leave formalization work for the next stage.
    #[serde(default)]
    pub cleanup_enforce_formalization_validity: bool,
    ...
    /// Stage 7 (plan doc 32, N3): true exactly on trust-required PV worker
    /// requests — the runs whose deviation lane is the seam-repair lane.
    /// Every `deviation_requests` entry must then carry a valid
    /// `seam_repair` declaration (and must not anywhere else).
    #[serde(default)]
    pub trust_required_pv: bool,
    /// Stage 7 (N3): the request-carried operands of the seam-repair
    /// declaration check (`WrapperRequest.trust_seam_repair_context`).
    #[serde(default)]
    pub seam_repair_context: Option<crate::model::TrustSeamRepairContext>,
}
...
pub struct HydrateWorkerResponseInput {
    ...
    #[serde(default)]
    pub current_node_deviation_claims: BTreeMap<NodeId, BTreeSet<DeviationId>>,
    /// Reference-paper registry (id -> spec), for claimed_reference_shas
    /// hydration. Empty for legacy payloads / registry-free runs.
    #[serde(default)]
    pub configured_reference_papers: BTreeMap<RefPaperId, crate::model::ReferencePaperSpec>,
    #[serde(default)]
    pub current_node_reference_grounds: BTreeMap<NodeId, BTreeSet<RefPaperId>>,
    pub approved_paper_fingerprints: BTreeMap<TargetId, Fingerprint>,
    ...
    /// GAP B (W10): path to the prose GOAL file, present ONLY for PV
    /// prose-goal hydration (gated at gate-prep on `is_pv`). Feeds
    /// the paper fingerprint's `goal_prose_sha256` referent axis; `None`
    /// keeps math / mode-B fingerprints byte-identical.
    #[serde(default)]
    pub goal_prose_path: Option<PathBuf>,
...
pub fn normalize_worker_response(
    ...
    normalize_worker_response_excluding(input, &BTreeSet::new())
}

/// Normalize a worker response while keeping kernel-owned support carriers
/// out of the live theorem namespace.
///
/// Most callers have no reserved carriers and use `normalize_worker_response`.
/// Trust-v1 acceptance passes the seed-owned `Assumptions` carrier here: that
/// pair exists on disk as the authoring surface for reviewed under-model
/// assumptions, but it is deliberately not an ordinary tablet node. Filtering
/// at this boundary is defense in depth for the pre-validation immutability
/// check in `runtime_cli`: a pre-existing support carrier can never be
/// laundered into `new_nodes` merely because the post-worker disk scanner sees
/// its stem.
pub fn normalize_worker_response_excluding(
    input: &WorkerNormalizationInput,
    excluded_present_nodes: &BTreeSet<NodeId>,
) -> Result<WorkerNormalizationOutput, String> {
    let mut present_nodes = present_nodes_from_repo(&input.repo_path)?;
    present_nodes.retain(|node| !excluded_present_nodes.contains(node));
    ...
    // Reserved support carriers are intentionally absent from the live node
    // graph, but they are still registered kernel-owned occupants of the
    // Tablet source namespace.  Admit their source paths to the layout check
    // without allowing them into snapshots or structural update maps.
    let layout_registered_nodes: BTreeSet<NodeId> = present_nodes
        .union(excluded_present_nodes)
        .cloned()
        .collect();
    contract_errors.extend(tablet_lean_layout_errors(
        &input.repo_path,
        &layout_registered_nodes,
    ));
    contract_errors.extend(dormant_dual_location_errors(&input.repo_path, &present_nodes));
...
fn validation_step_kind_name(step: &WorkerValidationExecutionPlanStep) -> &'static str {
    match step {
        ...
        WorkerValidationExecutionPlanStep::FinalCleanupCorrRepair { .. } => {
            "final_cleanup_corr_repair"
        }
    }
}
...
/// True when this plan is a `Phase::Cleanup` worker envelope of any
/// shape. The correspondence-repair step counts: it is a cleanup burst,
/// so the whole `final_cleanup_contract_errors` contract (no difficulty
/// updates, no target-claim / coverage / challenge-claim / node-kind /
/// proof-ness changes, outcome ∈ {valid, invalid}) applies to it
/// unchanged. A repair edits one `.tex` statement block and nothing
/// else, so none of those bans can bind — which is the point of running
/// them.
...
/// The nodes an extraction burst is allowed to classify, if this plan is an
/// extraction: the newly present nodes (in the normalized snapshot, absent
/// from `current_present_nodes`) of a burst that created EXACTLY the number
/// of helpers its dispatched kind is entitled to — one per DISPATCHED PARENT
/// for `ExtractHelper`, exactly ONE for `ExtractShared` whatever its parent
/// count, since a shared extraction folds one block spanning every declared
/// parent into a single helper.
///
/// Node kind and proof-ness are DERIVED, not asserted — `node_kinds_from_repo`
/// classifies from the `.tex` environment — so the diff of a burst that
/// creates a node necessarily contains `Set` entries for it. The carve-out
/// admits that derived truth for the burst's helpers; it does not admit a
/// worker assertion, and it does not relax the ban for any node that already
/// existed.
///
/// The exemption is keyed to the DISPATCHED PARENT SET — a singleton for a
/// single-task dispatch (`target_node = Some(parent)`), the batch's target
/// nodes for a batch dispatch (`target_node = None`, parents carried in
/// `authorized_nodes`) — together with the per-kind creation entitlement
/// above. For `ExtractHelper` that pairing is exactly the set
/// `extract_helper_gate_errors` derives for G1, which demands one fresh
/// `{.lean, .tex}` pair per dispatched parent. Keying on a count the
/// dispatch fixes in advance, rather than on "however many nodes appeared",
/// is the whole protective content of this helper: a burst that creates a
/// node it was NOT dispatched to create has a created-set size that no
/// longer matches its entitlement, so the exemption disappears and the
/// general proof-node/kind bans reject it. Do NOT re-widen this to "any
/// number of new nodes" — that would let an extraction burst smuggle in
/// arbitrary node births, which is the proof-formalization phase's job and
/// clears a different gate set.
///
/// A mismatched created set (too few, too many, or an extraction with no
/// dispatched parent) yields an empty set and the general bans apply
/// unchanged — G1 is what rejects that shape, with a better message.
fn final_cleanup_extract_helper_created_nodes(
    input: &WorkerAcceptanceInput,
    normalized: &WorkerNormalizationOutput,
) -> BTreeSet<NodeId> {
    let dispatch = input.validation_execution_plan.iter().find_map(|step| match step {
        WorkerValidationExecutionPlanStep::FinalCleanupPreserving {
            task_kind: Some(kind),
            target_node,
            authorized_nodes,
            ...
            kind,
            CleanupTaskKind::ExtractHelper { .. } | CleanupTaskKind::ExtractShared { .. }
        ) =>
        {
            let parents = match target_node {
                // Single dispatch: scope is exactly `{target_node}`, so a
                // reviewer-supplied `authorized_nodes` cannot widen it. Mirrors
                // `extract_helper_parents` in `runtime_cli_observations`.
                Some(target) => BTreeSet::from([target.clone()]),
                None => authorized_nodes.clone(),
            };
            // How many fresh nodes the dispatched kind is entitled to create.
            // `ExtractHelper` lifts a block out of each parent separately, so
            // it owes one helper per dispatched parent. `ExtractShared` folds
            // ONE block shared across every declared parent into ONE helper,
            // so its entitlement is exactly one however many parents were
            // dispatched — deriving it from the parent count would demand a
            // helper per parent and reject every well-formed shared
            // extraction.
            let expected_creations = match kind {
                CleanupTaskKind::ExtractShared { .. } => 1,
                _ => parents.len(),
            };
            Some((parents, expected_creations))
        }
        _ => None,
    });
    let Some((parents, expected_creations)) = dispatch else {
        return BTreeSet::new();
    };
    if parents.is_empty() || expected_creations == 0 {
        return BTreeSet::new();
    }
    let added: BTreeSet<NodeId> = normalized
        .snapshot
        .present_nodes
        .difference(&input.normalization.current_present_nodes)
        .cloned()
        .collect();
    if added.len() != expected_creations {
        return BTreeSet::new();
    }
    added
}
...
fn final_cleanup_contract_errors(
    ...
    let removed_nodes: BTreeSet<NodeId> = input
        ...
        .cloned()
        .collect();
    let extract_helper_created = final_cleanup_extract_helper_created_nodes(input, normalized);
    let mut errors = Vec::new();
    if matches!(
        input.payload_outcome,
        WorkerOutcome::Stuck
    ...
    let illegal_target_claim_updates: Vec<_> = normalized
        ...
        .filter_map(|(node, update)| match update {
            Update::Same => None,
            // `normalize_target_claim_updates` forces an entry for every new
            // node rather than dropping it when it matches the node's current
            // (absent) claims, because `worker_contract_errors` demands the
            // worker state one explicitly. An extraction's own fresh helper
            // therefore arrives here as `Set({})` however the burst behaved,
            // and banning it outright leaves no satisfiable response: omitting
            // the key fails the explicit-report rule and supplying it fails
            // this one. Admit the empty forced value for the burst's own new
            // helpers. A NON-empty set is still a real claim change and stays
            // banned, so cleanup remains unable to move a paper target.
            Update::Set(targets)
                if targets.is_empty() && extract_helper_created.contains(node) =>
            {
                None
            }
    ...
    let illegal_node_kind_updates: Vec<_> = normalized
        ...
        .filter_map(|(node, update)| match update {
            Update::Same => None,
            // An extraction's newly created nodes (one per dispatched
            // parent) arrive with a derived kind; every other node's kind
            // stays frozen.
            Update::Set(_) if extract_helper_created.contains(node) => None,
            Update::Set(_) => Some(node.clone()),
        })
        .collect();
    ...
    let illegal_proof_updates: Vec<_> = normalized
        ...
        .filter_map(|(node, update)| match update {
            ...
            // An extracted helper is a lemma with a proof, so the burst
            // that creates it necessarily sets its proof-ness. Only
            // `Set(true)`, and only for the burst's own new helpers (the
            // count its dispatched kind entitles it to): an extraction may
            // not clear any node's proof-ness.
            Update::Set(true) if extract_helper_created.contains(node) => None,
            Update::Set(_) => Some(node.clone()),
        })
        .collect();
    if !illegal_proof_updates.is_empty() {
    ...
    let illegal_challenge_claim_updates: Vec<_> = normalized
        ...
        .filter_map(|(node, update)| match update {
            Update::Same => None,
            // Same forced-entry shape as the paper-target claims above:
            // `normalize_challenge_claim_updates` also takes the new-node set
            // as its `force_nodes`, and `challenge_claim_contract_errors`
            // demands an explicit entry per new node once challenge targets
            // are configured. Without this carve-out an extraction on a
            // challenge-configured run faces the same unsatisfiable pair the
            // paper-target rules produced. Empty forced values only, and only
            // for the burst's own helpers.
            Update::Set(targets)
                if targets.is_empty() && extract_helper_created.contains(node) =>
            {
                None
...
/// Layered-construction orphan window: open iff some configured paper target
/// has no PRE-burst present node claiming it, or — on a PV input — some
/// configured paper/challenge target has no present claiming node with a
/// non-SKETCH NL proof. While open, worker bursts may create new live orphan
/// nodes — the DAG may be built in layers, support-first, before the covering
/// statement / real proof lands — so the same-burst orphan-deletion clauses
/// of `tablet_deletion_contract_errors` are waived. Once every configured
/// target is covered and, in PV, carries a real NL proof, the window closes
/// and the b67ccbf same-burst contract applies exactly as before.
///
/// DESIGN DECISION — the window is gated on the PRE-burst state (the
/// `input.normalization.current_*` maps), i.e. the window the worker was
/// dispatched under, NOT the post-response normalized snapshot. A burst
/// that itself covers the final uncovered target AND leaves unrelated
/// orphans is therefore still accepted: its orphans become
/// pre-existing-tolerated afterwards and are sweepable by later bursts.
/// Judging the burst by a window it closed itself would punish exactly
/// the covering statement the window exists to enable.
///
/// Math-mode behavior deliberately remains coverage-only. The PV-only signal
/// is `current_sketch_proof_nodes: Some(..)`; `Some(empty)` is affirmative and
/// meaningful, while `None` preserves legacy/math behavior byte-for-byte.
fn orphan_construction_window_open(normalization: &WorkerNormalizationInput) -> bool {
    let uncovered_paper_target = normalization.configured_targets.iter().any(|target| {
        !normalization.current_present_nodes.iter().any(|node| {
            normalization
    ...
    if uncovered_paper_target {
        return true;
    }
    let Some(sketch_proof_nodes) = normalization.current_sketch_proof_nodes.as_ref() else {
        return false;
    };
    let has_real_paper_proof = |target: &TargetId| {
        normalization.current_present_nodes.iter().any(|node| {
            !sketch_proof_nodes.contains(node)
                && normalization.current_proof_nodes.contains(node)
                && normalization
                    .current_target_claims
                    .get(node)
                    .is_some_and(|targets| targets.contains(target))
    ...
    let has_real_challenge_proof = |target: &ChallengeTargetId| {
        normalization.current_present_nodes.iter().any(|node| {
            !sketch_proof_nodes.contains(node)
                && normalization.current_proof_nodes.contains(node)
                && normalization
    ...
    normalization
        .configured_targets
        .iter()
        .any(|target| !has_real_paper_proof(target))
        || normalization
            .configured_challenge_targets
            .iter()
            .filter(|(_, spec)| spec.kind == ChallengeTargetKind::Theorem)
            .map(|(target, _)| target)
            .any(|target| !has_real_challenge_proof(target))
}
...
fn tablet_deletion_contract_errors(
    ...
    // involved). In the dec2flt example's cycle 280, the flip left seven pre-existing
    ...
    // Uncovered-target orphan window (see
    // `orphan_construction_window_open` for the semantics and the
    // pre-burst gating decision). While open, the waiver is NARROW:
    // clause 1 keeps its "only orphans may be deleted" half — removed
    // nodes must lie in the same-burst orphan set, which includes
    // pre-existing orphans, so orphan sweeping stays legal — and waives
    // only the must-delete-own-new-orphans half; clause 2 (NEW live
    // orphans) is waived for NON-CLEANUP bursts only (a cleanup burst
    // may never leave new orphans, window or not). The `deleted_nodes ==
    // removed_nodes` exactness check above and the cleanup-worker
    // deletion contract (`cleanup_contract_errors`) stay in force
    // regardless of the window.
    let window_open = orphan_construction_window_open(&input.normalization);

    if !cleanup_worker {
        let same_burst_orphans =
            same_burst_orphan_nodes_before_deletion(input, normalized, &removed_nodes);
        // Window open: no deletions are REQUIRED (layered construction
        // may leave its own orphans live); deletion LEGALITY is
        // unchanged — supported nodes may never be deleted.
        let required: BTreeSet<NodeId> = if window_open {
            BTreeSet::new()
        } else {
            same_burst_orphans
...
/// True when the first nonblank line of the TeX proof environment is the
/// exact `SKETCH:` marker.
...
/// Contract check for `WorkerResponse.node_reference_grounds`: every
/// claimed reference-paper id must be in the configured registry
/// (`configured_reference_papers`). Fail-closed rule: unknown ids are a
/// deterministic worker rejection naming this rule, never a silent
/// prune — mirrors `node_deviation_claim_unknown_id_errors`.
fn node_reference_ground_contract_errors(input: &WorkerAcceptanceInput) -> Vec<String> {
    let mut errors = Vec::new();
    for (node, claims) in &input.node_reference_grounds {
        for id in claims {
            if input.configured_reference_papers.contains(id) {
                continue;
            }
            errors.push(format!(
                "node_reference_grounds.{node} claims reference paper `{id}` but no such id is in the configured reference-paper registry (rule: every node_reference_grounds id must name a configured reference paper). Remove the claim or fix the id (typo?); only the operator can register reference papers."
            ));
        }
    }
    errors
...
/// The seam-repair declaration contract. On a
/// trust-required PV run every deviation request must carry a declaration
/// that passes the kernel's exact check (the request-carried context makes
/// acceptance and authorization run the same operands); anywhere else a
/// declaration is an illegal stray (the runtime checker already strips
/// unadvertised strays on non-trust requests, so reaching this error means
/// a non-CLI path authored one).
fn deviation_seam_repair_contract_errors(input: &WorkerAcceptanceInput) -> Vec<String> {
    let mut errors = Vec::new();
    let context = input.seam_repair_context.clone().unwrap_or_default();
    for (id, request) in &input.deviation_requests {
        if request.path.trim().is_empty() {
            continue;
        }
        match &request.seam_repair {
            None => {
                if input.trust_required_pv {
                    errors.push(format!(
                        "deviation `{}` lacks a structured seam-repair declaration",
                        id.as_str()
                    ));
                }
            }
            Some(declaration) => {
                if !input.trust_required_pv {
                    errors.push(format!(
                        "deviation `{}` carries a seam_repair declaration outside a \
                         trust-required PV run",
                        id.as_str()
                    ));
                    continue;
                }
                if let Err(reason) =
                    crate::model::validate_seam_repair_declaration_in_context(
                        &context,
                        declaration,
                    )
                {
                    errors.push(format!(
                        "deviation `{}` seam-repair declaration: {reason}",
                        id.as_str()
                    ));
                }
            }
    ...
    // Deleting a deviation an authorized
    // adaptation-ledger row is keyed to is refused too — the row records an
    // adaptation already applied to the pinned crate and the `.tex` is its
    // justification, so the deletion would orphan the audit trail.  Revising
    // the `.tex` stays legal: a re-verified edit is a fresh authorization
    // and writes the next `@N` row.
    if input.trust_required_pv {
        for id in &input.deviation_deletions {
            if context.ledger_bound_deviations.contains(id.as_str()) {
                errors.push(format!(
                    "deviation `{}` is named by an authorized adaptation-ledger row and cannot \
                     be deleted; the row is the audit record of an adaptation already applied \
                     to the pinned crate — revise the deviation instead",
                    id.as_str()
                ));
            }
        }
...
pub fn accept_worker_response(
    ...
    accept_worker_response_excluding(input, &BTreeSet::new())
}

#[cfg(test)]
mod seam_repair_contract_tests {
    use super::*;

    fn declaration() -> crate::model::SeamRepairDeclaration {
        crate::model::SeamRepairDeclaration {
            ...
            paths: vec![crate::trust_base::AdaptationPathDelta {
                path: "model/Definitions.lean".into(),
                before_sha256: None,
                after_sha256: crate::trust_base::raw_sha256(b"after"),
            }],
            citation: crate::trust_base::AdaptationCitation::LanguageGuarantee {
                statement: "every slice length is ≤ isize::MAX".into(),
            },
            affected_targets: std::collections::BTreeSet::from([
                crate::model::ChallengeTargetId::from("goal:correct"),
            ]),
        }
    }

    fn context() -> crate::model::TrustSeamRepairContext {
        crate::model::TrustSeamRepairContext {
            configured_challenge_target_ids: std::collections::BTreeSet::from([
                "goal:correct".to_string(),
            ]),
            seed_adaptation_paths: std::collections::BTreeSet::new(),
            ledger_bound_deviations: std::collections::BTreeSet::new(),
        }
    }

    fn input_with(
        trust_required_pv: bool,
        seam_repair: Option<crate::model::SeamRepairDeclaration>,
    ) -> WorkerAcceptanceInput {
        WorkerAcceptanceInput {
            trust_required_pv,
            seam_repair_context: Some(context()),
            deviation_requests: BTreeMap::from([(
                DeviationId::from("dev-a"),
                DeviationRequest {
                    path: "reference/dev-a.tex".into(),
                    summary: "seam repair".into(),
    ...
    /// Stage 7 (N3): a trust-required PV deviation request MUST carry a
    /// valid declaration; a declaration anywhere else is an illegal stray.
    #[test]
    fn deviation_seam_repair_contract_gates_by_trust_and_shape() {
        // Missing on a trust run.
        let errors = deviation_seam_repair_contract_errors(&input_with(true, None));
        assert_eq!(errors.len(), 1);
        assert!(errors[0].contains("seam-repair declaration"), "{errors:?}");
        // Stray outside a trust run.
        let errors =
            deviation_seam_repair_contract_errors(&input_with(false, Some(declaration())));
        assert_eq!(errors.len(), 1);
        assert!(errors[0].contains("outside a trust-required"), "{errors:?}");
        // Valid on a trust run.
        let errors =
            deviation_seam_repair_contract_errors(&input_with(true, Some(declaration())));
        assert!(errors.is_empty(), "{errors:?}");
        // Repository build provenance is not declaration shape: a
        // crate-touching path with the legacy `unaffected` value is valid.
        let mut crate_touch = declaration();
        crate_touch.paths[0].path = "crate/src/lib.rs".into();
        let errors = deviation_seam_repair_contract_errors(&input_with(
            true,
            Some(crate_touch),
        ));
        assert!(errors.is_empty(), "{errors:?}");
        // Unregistered affected target refuses.
        let mut bad = declaration();
        bad.affected_targets =
            std::collections::BTreeSet::from([crate::model::ChallengeTargetId::from(
                "goal:missing",
            )]);
        let errors = deviation_seam_repair_contract_errors(&input_with(true, Some(bad)));
        assert_eq!(errors.len(), 1);
        assert!(errors[0].contains("unknown challenge target"), "{errors:?}");
    }

    /// Stage 9 (N5 consolidation): a deviation an authorized
    /// adaptation-ledger row is keyed to cannot be DELETED — the row is the
    /// audit record of an adaptation already applied to the pinned crate
    /// and the `.tex` is its justification.  Revising it stays legal: a
    /// re-verified edit is a fresh authorization and writes the next `@N`
    /// row.
    #[test]
    fn ledger_bound_deviation_rejects_deletion_but_accepts_revision() {
        let mut bound_context = context();
        bound_context
            .ledger_bound_deviations
            .insert("dev-a".to_string());
        let mut deletion = input_with(true, None);
        deletion.deviation_requests.clear();
        deletion.seam_repair_context = Some(bound_context.clone());
        deletion
            .deviation_deletions
            .insert(DeviationId::from("dev-a"));
        let errors = deviation_seam_repair_contract_errors(&deletion);
        assert_eq!(errors.len(), 1, "{errors:?}");
        assert!(
            errors[0].contains("adaptation-ledger row") && errors[0].contains("cannot be \
             deleted"),
            "{errors:?}"
        );
        // Re-registration of the SAME row-bound deviation is untouched.
        let mut revision = input_with(true, Some(declaration()));
        revision.seam_repair_context = Some(bound_context);
        assert!(
            deviation_seam_repair_contract_errors(&revision).is_empty(),
            "a row-bound deviation may still be revised"
        );
    }
}

/// Acceptance variant used when on-disk kernel support carriers must remain
/// outside the worker-authored theorem namespace.  Keeping this as an
/// explicit parameter (rather than a name-global rule) preserves ordinary
/// non-PV nodes named `Assumptions` and the legacy assumption-authoring lane.
pub fn accept_worker_response_excluding(
    input: &WorkerAcceptanceInput,
    excluded_present_nodes: &BTreeSet<NodeId>,
) -> Result<WorkerAcceptanceOutput, String> {
    let normalized =
        normalize_worker_response_excluding(&input.normalization, excluded_present_nodes)?;
    ...
    contract_errors.extend(deviation_seam_repair_contract_errors(input));
    ...
    contract_errors.extend(node_reference_ground_contract_errors(input));
    ...
    // One shared state-aware transition preflight. The checker has proved
    // post-validity when every preserving contract and validation step passed;
    // the engine recomputes the same predicate from the applied state. A
    // pre-existing invalid baseline is not charged to this burst.
    let cleanup_transition_preflight = if input.cleanup_transition_preflight_required
        || cleanup_worker
    {
        let configured_challenge: BTreeSet<ChallengeTargetId> = input
            .normalization
            ...
            .cloned()
            .collect();
        let baseline_roots = roots_from_claim_maps(
            &input.normalization.configured_targets,
            &input.normalization.current_target_claims,
            &configured_challenge,
            &input.normalization.current_challenge_claims,
        ...
        let baseline_live_orphans = orphan_nodes_for_parts(
            &input.normalization.current_present_nodes,
            &baseline_roots,
            &input.normalization.current_deps,
        );
        let post_live_orphans = post_response_orphan_nodes(input, &normalized);
        let post_formalization_valid = contract_errors.is_empty() && validation_errors.is_empty();
        Some(
            crate::worker_transition::evaluate_cleanup_transition_preflight(
                crate::worker_transition::CleanupTransitionPreflightInput {
                    enforce_formalization_validity: input.cleanup_enforce_formalization_validity,
                    baseline_formalization_valid: input.cleanup_baseline_formalization_valid,
                    post_formalization_valid,
    ...
    if input.payload_outcome == WorkerOutcome::Valid {
        if let Some(preflight) = cleanup_transition_preflight.as_ref() {
            contract_errors.extend(preflight.rejection_reasons.clone());
        }
    }
    ...
    let (final_outcome, ok, errors) = match input.payload_outcome {
        ...
        WorkerOutcome::UncertifiableUnderCertificateContext => (
            ...
            false,
            vec![
                "uncertifiable_under_certificate_context is legal only on the specialized \
                 witness-evaluation response-only contract"
                    .to_string(),
            ],
        ),
    };
    ...
    Ok(WorkerAcceptanceOutput {
        response: WorkerResponse {
            ...
            conditional_theorem_proposal: input.conditional_theorem_proposal.clone(),
            conditional_theorem_withdrawals: input.conditional_theorem_withdrawals.clone(),
            node_deviation_claims: if final_outcome == WorkerOutcome::Valid {
                input.node_deviation_claims.clone()
            } else {
                BTreeMap::new()
            },
            node_reference_grounds: if final_outcome == WorkerOutcome::Valid {
                input.node_reference_grounds.clone()
            } else {
                BTreeMap::new()
            },
...
pub fn prepare_worker_gate_observations(
    ...
    // R4 Slice 2: the declaration-lock baseline READS the node-source file. The
    // path is the tablet target's (`.lean` on the all-Lean live run ⇒ the read
    // is byte-identical to the historical hardcoded `Tablet/<node>.lean`; `.thy`
    // for Isabelle/HOL). The previously hardcoded `.lean` path silently weakened
    // the Tier-1 lock to vacuous for Isabelle (`Tablet/<node>.lean` is absent ⇒
    // no baseline captured ⇒ the declaration-signature check never fires).
    let source_target = tablet_target_for_repo(&input.repo_path);

    if let Some(active_node) = input.active_node.as_ref() {
        let source_path = node_source_path(&input.repo_path, active_node, source_target);
        if source_path.exists() {
            let lean_content = read_text(&source_path);
            if input.observation_plan.capture_imports_before {
                // `extract_imports` is the Lean import-list parser; for a `.thy`
                // it returns [] (conservative no-drift). Import-drift detection
                // for Isabelle is a follow-up (likely corr-lane-owned).
    ...
    if input.observation_plan.capture_baseline_declaration_hashes {
        for node in &input.current_present_nodes {
            let source_path = node_source_path(&input.repo_path, node, source_target);
            if source_path.exists() {
...
/// Lean (`Tablet/<node>.lean`), `.thy` for Isabelle/HOL. Reads the
...
/// the extension is `"lean"`, making the path byte-identical to the historical
/// hardcoded `Tablet/<node>.lean`.
///
...
/// Walk `<repo>/Tablet/` recursively for node-source files and reject any that
/// are neither `Tablet/Preamble.<ext>`, the kernel-managed `Tablet/Axioms.lean`,
/// nor `Tablet/<X>.<ext>` for some `X` in `present_nodes`. The extension `<ext>`
/// is the tablet target's node-source extension (`"lean"` on the all-Lean live
/// run ⇒ byte-identical to the historical `.lean`-only walk; `"thy"` for
/// Isabelle/HOL). Subdirectory-nested node-source files are always rejected:
/// they fall outside the tablet protocol's compilation surface and would
/// otherwise fail later at build time with opaque cascading "object file ...
/// does not exist" errors across every importing node.
///
...
fn tablet_lean_layout_errors(repo_path: &Path, present_nodes: &BTreeSet<NodeId>) -> Vec<String> {
    ...
    // R4: the node-source extension is the tablet target's (`"lean"` on the
    // all-Lean live run ⇒ byte-identical to the historical `Some("lean")`
    // filter; `"thy"` for an Isabelle/HOL tablet). The walk rejects only stray
    // node-source files; `.tex` / other extensions are skipped for both targets.
    let ext = crate::backend::descriptor_for(tablet_target_for_repo(repo_path)).node_source_ext;
    ...
    offending
        .into_iter()
        .map(|path| {
            // The leading "All Lean source" wording is kept verbatim so the
            // Lean-path message is byte-identical (pinned by
            // `normalize_worker_response_rejects_non_node_lean_files_under_tablet`);
            // only the path extensions are parameterized on the target's `ext`.
            format!(
                "{path}: not a registered tablet node. All Lean source under Tablet/ must live in a registered tablet node (Tablet/<NodeName>.{ext}) or in Tablet/Preamble.{ext}. Move shared declarations into Preamble.{ext} or factor them into a tablet node."
            )
        })
        .collect()
}

/// PV dormant store: reject a worker result that leaves a node present in BOTH
/// `Tablet/` and `Dormant/`. A `Decide` pair's two sides carry DIFFERENT node
/// names (`<Primary>` vs `<Primary>__Refutation`), so a correctly-flipped pair
/// never collides on a stem — a same-stem file in both directories only arises
/// in the broken state (e.g. a stale seed stub left behind), where a later
/// polarity flip's `rename` could silently clobber the accepted `Tablet/` copy
/// (see `dormant_store::move_if_present`'s overwrite guard, the last line of
/// defense). Rejecting at acceptance keeps the one-side invariant enforced at
/// the same chokepoint as the other layout errors, before the state commits.
fn dormant_dual_location_errors(repo_path: &Path, present_nodes: &BTreeSet<NodeId>) -> Vec<String> {
    if !crate::dormant_store::dormant_dir(repo_path).exists() {
        return Vec::new();
    }
    present_nodes
        .iter()
        .filter(|node| crate::dormant_store::node_in_dormant(repo_path, node))
        .map(|node| {
            format!(
                "Tablet/{stem}.lean and Dormant/{stem}.lean: node `{stem}` exists in both \
                 Tablet/ and Dormant/; a Decide side must live in exactly one location — flip \
                 polarity first (audit set_live_polarity on the active pair) or have the \
                 operator reconcile the dormant copy; refusing the result.",
                stem = node.as_str()
            )
        })
        .collect()
...
fn declaration_hash_for_gate(
    ...
    // Route on the REPO's backend, not a pinned Lean. The caller already
    // resolves `source_target` to read the right node file, then handed the
    // `.thy` content to a Lean-pinned splitter that requires the `-- BODY`
    // marker — which does not exist in Isabelle. The first proof-formalization
    // worker dispatch on an Isabelle run therefore failed gate-prep with
    // `filespec_split: no \`-- BODY\` marker line found` before any burst
    // launched, five times in a row, and the run halted on the
    // transport-failure budget. The proof-phase observation flags
    // (`capture_expected_active_hash`, `capture_baseline_declaration_hashes`)
    // are why 140 stating-phase bursts never reached this line.
    crate::backend::source_model_for(tablet_target_for_repo(repo_path))
...
fn challenge_claim_contract_errors(
    ...
    // W5 / D2 (exactly-one): a WorkerAuthored prose GOAL target — or its
    // kernel-derived refutation twin — takes EXACTLY ONE statement node.
    // Checked over the POST-update claims restricted to post-update
    // present nodes, so a claim may MOVE in one delta (old claimant
    // deleted or unclaimed, new claimant added) but never duplicate.
    // Stated against explicit `challenge_claims` only; the paper-side
    // prefix auto-claim never touches these maps.
    let mut authored_claimants: BTreeMap<&ChallengeTargetId, Vec<&NodeId>> = BTreeMap::new();
    for (node, targets) in next_challenge_claims {
        if !present_nodes.contains(node) {
            continue;
        }
        for target in targets {
            if crate::model::is_authored_family_challenge_target(
                &input.configured_challenge_targets,
                target,
            ) {
                authored_claimants.entry(target).or_default().push(node);
            }
        }
    }
    for (target, mut nodes) in authored_claimants {
        if nodes.len() > 1 {
            nodes.sort();
            errors.push(format!(
                "prose goal target `{}` is already covered by `{}`; a prose GOAL target \
                 takes exactly one statement node — revise `{}` instead of adding a second \
                 (claimants: {:?})",
                target.as_str(),
                nodes[0].as_str(),
                nodes[0].as_str(),
...
/// W4.4: the declared theorem name from a statement-only FILESPEC slice —
/// the token after the `theorem` keyword on the first declaration line.
/// `None` when the slice does not open with a theorem declaration (the
/// caller falls back to the node stem; the binding-time re-check is
/// authoritative either way).
pub fn declared_theorem_name(slice: &str) -> Option<String> {
    let line = slice
        .lines()
        .map(str::trim)
...
fn challenge_conformance_errors(
    ...
    for (node, claims) in next_challenge_claims {
        ...
        for target in claims {
            ...
            // W4.4: an UNBOUND WorkerAuthored spec has no pinned bytes yet —
            // the unconditional byte compare would make the claim a
            // permanent, unsatisfiable reject (`"" != actual`). For this
            // case ONLY, replace the byte/name-parity checks with the
            // authoring-time negability lint: the statement must have a
            // shape the kernel can negate FAIL-LOUD at binding (mode B's
            // config-time F1/F2 errors, surfaced at authoring instead).
            // Once bound (`lean` non-empty) the ordinary checks below
            // freeze the statement like any pinned target.
            if spec.statement_provenance == crate::model::StatementProvenance::WorkerAuthored
                && spec.lean.trim().is_empty()
            {
                let content =
                    match crate::filespec_split::read_node_file(repo_path, node.as_str()) {
                        Ok((content, _hash)) => content,
                        Err(err) => {
                            errors.push(format!(
                                "prose goal statement rule failed: node `{node}` claims \
                                 authored goal target `{target}` but its tablet file could \
                                 not be read: {err}"
                            ));
                            continue;
                        }
                    };
                let actual = match crate::filespec_split::prescribed_region(&content, false) {
                    Ok(region) => region,
                    Err(err) => {
                        errors.push(format!(
                            "prose goal statement rule failed: node `{node}` claims \
                             authored goal target `{target}` but its FILESPEC statement \
                             slice could not be computed: {err}"
                        ));
                        continue;
                    }
                };
                let declared_name = declared_theorem_name(&actual)
                    .unwrap_or_else(|| node.as_str().to_string());
                let candidate = ChallengeTargetSpec {
                ...
                if let Some(reason) =
                    crate::model::refutation_unsupported_reason(&candidate)
                {
                    errors.push(format!(
                        "prose goal statement rule failed for node `{node}` claiming \
                         authored goal target `{target}`: the statement cannot be \
                         refuted, so a decide pair cannot bind it: {reason}"
                    ));
                }
                continue;
            }
...
mod tests {
    ...
    /// **Byte-identical for Lean** — the resolver's failure lattice. Every
    /// degenerate config (absent file, malformed JSON, missing/empty
    /// `workflow`/`default_target`, unrecognized value) MUST resolve to
    /// `Lean`: live Lean runs carry accepted closure records whose policy
    /// routing (`installed_record_policy_verdict`, checker-driver and
    /// node-source selection) keys on this function, so a Lean repo must
    /// never be misrouted. Only an explicit `"isabelle_hol"` selects the
    /// Isabelle backend.
    #[test]
    fn tablet_target_for_repo_resolves_degenerate_configs_to_lean() {
        use crate::backend::BackendId;
        let dir = tempdir().unwrap();
        let repo = dir.path();
        let config = repo.join("trellis.config.json");

        // Absent config (the common fixture / smoke-repo case).
        assert_eq!(tablet_target_for_repo(repo), BackendId::Lean);
        ...
        // No `workflow` object.
        write(&config, r#"{"other": 1}"#);
        assert_eq!(tablet_target_for_repo(repo), BackendId::Lean);

        // `workflow` present, no `default_target`.
        write(&config, r#"{"workflow": {}}"#);
        assert_eq!(tablet_target_for_repo(repo), BackendId::Lean);

        // Empty / whitespace-only `default_target`.
        write(&config, r#"{"workflow": {"default_target": "  "}}"#);
        assert_eq!(tablet_target_for_repo(repo), BackendId::Lean);

        // Unrecognized value.
        write(&config, r#"{"workflow": {"default_target": "coq"}}"#);
        assert_eq!(tablet_target_for_repo(repo), BackendId::Lean);

        // Explicit lean stays lean; explicit isabelle_hol (and only that)
        // selects the Isabelle backend.
        write(&config, r#"{"workflow": {"default_target": "lean"}}"#);
        assert_eq!(tablet_target_for_repo(repo), BackendId::Lean);
        write(&config, r#"{"workflow": {"default_target": "isabelle_hol"}}"#);
        assert_eq!(tablet_target_for_repo(repo), BackendId::IsabelleHol);
    }
...
";
    ...
    /// An `ExtractHelper` plan step, in either dispatch shape: a single
    /// dispatch names its parent in `target_node`; a batch passes
    /// `target_node = None` and the kernel-derived parent set in
    /// `authorized_nodes` (the FinalCleanup batch arm of
    /// `current_worker_validation_execution_plan`).
    fn final_cleanup_extract_helper_step(
        target: Option<&str>,
        authorized: &[&str],
    ) -> WorkerValidationExecutionPlanStep {
        WorkerValidationExecutionPlanStep::FinalCleanupPreserving {
            task_kind: Some(CleanupTaskKind::ExtractHelper {
                ordinal: if target.is_some() { 1 } else { 0 },
                hint: String::new(),
            }),
            target_node: target.map(NodeId::from),
            task_obligations: BTreeMap::new(),
            authorized_nodes: node_set(authorized),
            protected_statement_node_set: BTreeSet::new(),
        }
    }

    /// Drive `final_cleanup_contract_errors` over an extraction burst that
    /// created `helpers`: each helper arrives with a DERIVED kind and
    /// proof-ness, exactly as `normalize_worker_response` would emit for a
    /// node born in this burst.
    fn final_cleanup_extract_helper_contract_errors(
        step: WorkerValidationExecutionPlanStep,
        current_present_nodes: &[&str],
        helpers: &[&str],
    ) -> Vec<String> {
        let mut next_present_nodes = node_set(current_present_nodes);
        next_present_nodes.extend(node_set(helpers));
        let input = WorkerAcceptanceInput {
            payload_outcome: WorkerOutcome::Valid,
            normalization: WorkerNormalizationInput {
                current_present_nodes: node_set(current_present_nodes),
            ...
            validation_execution_plan: vec![step],
            ..WorkerAcceptanceInput::default()
        };
        let normalized = WorkerNormalizationOutput {
            snapshot: WorkingSnapshot {
                present_nodes: next_present_nodes,
                ..WorkingSnapshot::default()
            },
            proof_node_updates: helpers
                .iter()
                .map(|helper| (NodeId::from(*helper), Update::Set(true)))
                .collect(),
            node_kind_updates: helpers
                .iter()
                .map(|helper| (NodeId::from(*helper), Update::Set(NodeKind::Proof)))
                .collect(),
            // `normalize_target_claim_updates` forces an entry for every new
            // node (its `force_nodes` argument is the new-node set), so a
            // helper always reaches the contract checks as `Set({})` rather
            // than being dropped. Mirror that here: a harness that omits it
            // tests a response shape the normalizer cannot produce.
            target_claim_updates: helpers
                .iter()
                .map(|helper| (NodeId::from(*helper), Update::Set(BTreeSet::new())))
                .collect(),
            ..WorkerNormalizationOutput::default()
        };
        final_cleanup_contract_errors(&input, &normalized)
    ...
    #[test]
    fn final_cleanup_single_extract_helper_accepts_its_one_created_node() {
        let errors = final_cleanup_extract_helper_contract_errors(
            final_cleanup_extract_helper_step(Some("Parent1"), &[]),
            &["Preamble", "Parent1"],
            &["Helper1"],
        );
        ...
        assert!(
            errors.is_empty(),
            "a single-parent extraction's one helper is exempt, got {:?}",
            errors
        );
    }

    #[test]
    fn final_cleanup_batch_extract_helper_accepts_one_created_node_per_parent() {
        // Regression: the exemption used to admit exactly ONE created node,
        // so a k-parent batch producing the k helpers G1 demands was
        // rejected for "changing proof-node classification" — no batched
        // extraction could ever be accepted.
        let errors = final_cleanup_extract_helper_contract_errors(
            final_cleanup_extract_helper_step(None, &["Parent1", "Parent2", "Parent3"]),
            &["Preamble", "Parent1", "Parent2", "Parent3"],
            &["Helper1", "Helper2", "Helper3"],
        );

        assert!(
            errors.is_empty(),
            "k parents producing k helpers is the shape G1 demands, got {:?}",
            errors
        );
    }

    #[test]
    fn final_cleanup_batch_extract_helper_rejects_an_undispatched_extra_node() {
        // The exemption is keyed to the dispatched parent set, so a burst
        // that also births a node it was not dispatched to create loses it.
        let errors = final_cleanup_extract_helper_contract_errors(
            final_cleanup_extract_helper_step(None, &["Parent1", "Parent2"]),
            &["Preamble", "Parent1", "Parent2"],
            &["Helper1", "Helper2", "Smuggled"],
        );
        ...
        assert!(
            errors
                .iter()
                .any(|err| err.contains("final cleanup may not change proof-node classification")),
            "an extra undispatched node must still be rejected, got {:?}",
            errors
        );
    }

    #[test]
    fn final_cleanup_single_extract_helper_rejects_a_second_created_node() {
        // Single-parent dispatch is unchanged: its scope is exactly
        // `{target_node}`, so one helper is exempt and a second is not.
        let errors = final_cleanup_extract_helper_contract_errors(
            final_cleanup_extract_helper_step(Some("Parent1"), &[]),
            &["Preamble", "Parent1"],
            &["Helper1", "Helper2"],
        ...
        assert!(
            errors
                .iter()
                .any(|err| err.contains("final cleanup may not change proof-node classification")),
            "a single dispatch may create exactly one helper, got {:?}",
            errors
        );
    }

    fn final_cleanup_extract_shared_step(
        target: &str,
        co_parents: &[&str],
    ) -> WorkerValidationExecutionPlanStep {
        WorkerValidationExecutionPlanStep::FinalCleanupPreserving {
            task_kind: Some(CleanupTaskKind::ExtractShared {
                co_parents: node_set(co_parents),
                ordinal: 1,
                hint: String::new(),
            }),
            target_node: Some(NodeId::from(target)),
    ...
    #[test]
    fn final_cleanup_extract_shared_accepts_one_helper_for_the_whole_parent_set() {
        // Regression: `final_cleanup_extract_helper_created_nodes` matched
        // only `ExtractHelper`, so an `ExtractShared` dispatch fell through to
        // an empty exemption set and its one new helper tripped both the
        // node-kind and proof-node bans — no shared extraction could ever be
        // accepted, which is every dispatch of the kind. Note the entitlement
        // runs OPPOSITE to the batch case above: `ExtractShared` folds one
        // block spanning all parents into ONE helper, so deriving the expected
        // count from the parent count would demand one helper per parent and
        // reject the well-formed shape.
        let errors = final_cleanup_extract_helper_contract_errors(
            final_cleanup_extract_shared_step("Parent1", &["Parent2", "Parent3"]),
            &["Preamble", "Parent1", "Parent2", "Parent3"],
            &["SharedHelper"],
        );
        assert!(
            errors.is_empty(),
            "a shared extraction's single helper is exempt however many \
             parents were dispatched, got {:?}",
            errors
        );

        // The protective content survives: the entitlement is exactly one, so
        // a burst smuggling in a second node loses the exemption.
        let errors = final_cleanup_extract_helper_contract_errors(
            final_cleanup_extract_shared_step("Parent1", &["Parent2", "Parent3"]),
            &["Preamble", "Parent1", "Parent2", "Parent3"],
            &["SharedHelper", "Extra"],
        ...
        assert!(
            errors
                .iter()
                .any(|err| err.contains("final cleanup may not change proof-node classification")),
            "a shared extraction may create exactly one helper, got {:?}",
            errors
        );
    }

    #[test]
    fn final_cleanup_lintfix_still_rejects_any_created_node() {
        // The exemption belongs to the extraction kinds alone: no other
        // envelope creates nodes.
        let errors = final_cleanup_extract_helper_contract_errors(
            WorkerValidationExecutionPlanStep::FinalCleanupPreserving {
                task_kind: Some(CleanupTaskKind::LintFix {
                    warning_text: "unused variable `x`".to_string(),
                }),
                target_node: Some(NodeId::from("Parent1")),
                task_obligations: BTreeMap::new(),
                authorized_nodes: BTreeSet::new(),
                protected_statement_node_set: BTreeSet::new(),
            },
        ...
        assert!(
            errors
                .iter()
                .any(|err| err.contains("final cleanup may not change proof-node classification")),
            "only an extraction may create a node, got {:?}",
            errors
        );
    }
    ...
    /// R4 Slice 1: with an `isabelle_hol` target the layout walk filters on the
    /// `.thy` node-source extension, so a stray `Tablet/Stray.thy` is rejected
    /// while a registered `Tablet/A.thy` + `Tablet/Preamble.thy` are accepted.
    /// A `Tablet/Leftover.lean` is NOT a node-source file under Isabelle and is
    /// skipped (only the target's extension is walked). The rejection prose
    /// carries the `.thy` extension.
    #[test]
    fn tablet_lean_layout_errors_rejects_unregistered_top_level_thy() {
        let tmp = tempdir().unwrap();
        let repo = tmp.path().join("repo");
        fs::create_dir_all(repo.join("Tablet")).unwrap();
        write(
        ...
        write(&repo.join("Tablet/Preamble.thy"), "");
        write(&repo.join("Tablet/A.thy"), "");
        // `Stray` is on disk but not in the registered node set passed in.
        write(&repo.join("Tablet/Stray.thy"), "");
        // A `.lean` is not a node-source file under the Isabelle target ⇒ skipped.
        write(&repo.join("Tablet/Leftover.lean"), "");
        ...
        assert_eq!(errors.len(), 1, "got {:?}", errors);
        assert!(errors[0].starts_with("Tablet/Stray.thy: not a registered tablet node."));
        assert!(
            errors[0].contains("Tablet/<NodeName>.thy") && errors[0].contains("Preamble.thy"),
            "prose must carry the .thy extension, got {:?}",
            errors[0]
        );
    }

    #[test]
    fn dormant_dual_location_errors_rejects_same_stem_in_both_directories() {
        let tmp = tempdir().unwrap();
        let repo = tmp.path().join("repo");
        fs::create_dir_all(repo.join("Tablet")).unwrap();
        write(&repo.join("Tablet/Preamble.lean"), "");
        // `X` present in Tablet/ AND lingering in Dormant/ — the broken state.
        write(&repo.join("Tablet/X.lean"), "accepted proof");
        write(&repo.join("Dormant/X.lean"), "stale seed stub");

        let errors = dormant_dual_location_errors(
            &repo,
            &BTreeSet::from([NodeId::from("X"), NodeId::from("Preamble")]),
        );
        assert_eq!(errors.len(), 1, "got {:?}", errors);
        assert!(
            errors[0].contains("Tablet/X.lean") && errors[0].contains("Dormant/X.lean"),
            "error must name both paths: {:?}",
            errors[0]
        );
        assert!(
            errors[0].contains("exists in both Tablet/ and Dormant/")
                && errors[0].contains("a Decide side must live in exactly one location")
                && errors[0].contains("flip polarity first (audit set_live_polarity on the active pair)")
                && errors[0].contains("reconcile the dormant copy")
                && errors[0].contains("refusing the result"),
            "error must carry the directive phrasing: {:?}",
            errors[0]
        );
    }

    #[test]
    fn dormant_dual_location_errors_accepts_correctly_flipped_pair() {
        // A correctly-flipped Decide pair: primary live in Tablet/, refutation
        // dormant in Dormant/ — DIFFERENT stems, no collision, no error.
        let tmp = tempdir().unwrap();
        let repo = tmp.path().join("repo");
        fs::create_dir_all(repo.join("Tablet")).unwrap();
        write(&repo.join("Tablet/Preamble.lean"), "");
        write(&repo.join("Tablet/Correct.lean"), "live primary");
        write(&repo.join("Dormant/Correct__Refutation.lean"), "dormant side");

        let errors = dormant_dual_location_errors(
            &repo,
            &BTreeSet::from([NodeId::from("Correct"), NodeId::from("Preamble")]),
        );
        assert!(errors.is_empty(), "got {:?}", errors);

        // No Dormant/ directory at all (ordinary non-PV run): no error.
        let repo2 = tmp.path().join("repo2");
        fs::create_dir_all(repo2.join("Tablet")).unwrap();
        write(&repo2.join("Tablet/A.lean"), "");
        let errors =
            dormant_dual_location_errors(&repo2, &BTreeSet::from([NodeId::from("A")]));
        assert!(errors.is_empty(), "got {:?}", errors);
    }

    #[test]
    fn normalize_worker_response_rejects_dual_location_node_as_contract_error() {
        // The hook point: a worker result that creates `Tablet/X.lean` while
        // `Dormant/X.lean` exists must surface the dual-location rejection in
        // `contract_errors` (the POST-response snapshot: present_nodes is
        // re-derived from the on-disk Tablet/ the worker just wrote).
        let tmp = tempdir().unwrap();
        let repo = tmp.path().join("repo");
        fs::create_dir_all(repo.join("Tablet")).unwrap();
        write(&repo.join("Tablet/Preamble.lean"), "");
        write(&repo.join("Tablet/Preamble.tex"), "");
        write(&repo.join("Tablet/X.lean"), "import Tablet.Preamble\n");
        write(&repo.join("Tablet/X.tex"), "\\begin{definition}X\\end{definition}\n");
        write(&repo.join("Dormant/X.lean"), "stale seed stub");
        ...
        .unwrap();
        let dual_errors: Vec<&String> = normalized
            .contract_errors
            .iter()
            .filter(|e| e.contains("exists in both Tablet/ and Dormant/"))
            .collect();
        assert_eq!(
            dual_errors.len(),
            1,
            "expected exactly one dual-location rejection, got {:?}",
            normalized.contract_errors
        );
        assert!(
            dual_errors[0].contains("Dormant/X.lean")
                && dual_errors[0].contains("refusing the result"),
            "rejection message incomplete: {:?}",
            dual_errors[0]
        );
    }
    ...
    #[test]
    fn uncovered_target_window_tolerates_new_live_orphans() {
        // Uncovered-target orphan window: `t.b` is configured but no
        // pre-burst node claims it, so the window is open and the same
        // burst that the closed-window twin
        // (`accept_worker_response_rejects_live_orphan_after_valid_delta`)
        // rejects — a Valid delta stranding B — is accepted: the DAG may
        // be built in layers, support-first.
        let tmp = tempdir().unwrap();
        let repo = tmp.path().join("repo");
        write_supported_orphan_fixture(&repo, false, true);
        let mut normalization = rooted_ab_normalization(repo);
        ...
        assert!(
            output.final_outcome == WorkerOutcome::Valid && output.errors.is_empty(),
            "window-open burst leaving a new orphan must be accepted, got {:?} with errors {:?}",
            output.final_outcome,
            output.errors
        );
    }
    ...
    #[test]
    fn pv_sketch_window_opens_then_closes_for_real_nl_proof_at_acceptance() {
        fn pv_normalization(repo: PathBuf, sketch: bool) -> WorkerNormalizationInput {
            let mut normalization = rooted_ab_normalization(repo);
            normalization.configured_targets.clear();
            normalization.current_target_claims.clear();
            let target = ChallengeTargetId::from("goal:a");
            normalization.configured_challenge_targets.insert(
                target.clone(),
                ChallengeTargetSpec {
                    kind: ChallengeTargetKind::Theorem,
                    name: "A".to_string(),
                    lean: "theorem A : True := by".to_string(),
                    ..ChallengeTargetSpec::default()
                },
            );
            normalization.current_challenge_claims.insert(
                NodeId::from("A"),
                BTreeSet::from([target]),
            );
            normalization.current_sketch_proof_nodes = Some(if sketch {
                BTreeSet::from([NodeId::from("A")])
            } else {
                BTreeSet::new()
            });
        ...
        let tmp = tempdir().unwrap();
        let sketch_repo = tmp.path().join("sketch");
        write_supported_orphan_fixture(&sketch_repo, false, true);
        write(
            &sketch_repo.join("Tablet/A.lean"),
            "import Tablet.Preamble\n\n-- [TABLET NODE: A]\ntheorem A : True := by\n-- BODY\n  trivial\n",
        );
        let sketch_output = accept_worker_response(&WorkerAcceptanceInput {
            request_id: 47,
            cycle: 4,
            payload_outcome: WorkerOutcome::Valid,
            normalization: pv_normalization(sketch_repo, true),
            ..WorkerAcceptanceInput::default()
        })
        .unwrap();
        assert!(
            sketch_output.final_outcome == WorkerOutcome::Valid
                && sketch_output.errors.is_empty(),
            "a covered PV target with only a SKETCH: proof must keep the PRE-burst support-first window open; got {:?}",
            sketch_output.errors
        );

        let real_repo = tmp.path().join("real");
        write_supported_orphan_fixture(&real_repo, false, true);
        write(
            &real_repo.join("Tablet/A.lean"),
            "import Tablet.Preamble\n\n-- [TABLET NODE: A]\ntheorem A : True := by\n-- BODY\n  trivial\n",
        );
        let real_output = accept_worker_response(&WorkerAcceptanceInput {
            request_id: 48,
            cycle: 4,
            payload_outcome: WorkerOutcome::Valid,
            normalization: pv_normalization(real_repo, false),
            ..WorkerAcceptanceInput::default()
        })
        .unwrap();
        assert_eq!(real_output.final_outcome, WorkerOutcome::Invalid);
        assert!(
            real_output
                .errors
                .iter()
                .any(|error| error.contains("NEW live orphan nodes") && error.contains("B")),
            "once every configured PV target has a real NL proof the historical same-burst orphan contract must apply: {:?}",
            real_output.errors
        );
    }

    #[test]
    fn uncovered_target_window_keeps_deletion_legality_but_not_requirement() {
        // The window waiver is NARROW: deletion LEGALITY (only orphans
        // may be deleted) binds even while the window is open — a
        // window-open burst deleting the supported node B is rejected
        // with the clause-1 string (required half empty: nothing is
        // demanded, but B is still illegal to delete)...
        let tmp = tempdir().unwrap();
        let repo = tmp.path().join("repo");
        write_supported_orphan_fixture(&repo, true, false);
        let mut normalization = rooted_ab_normalization(repo);
        ...
        assert_eq!(
            output.errors,
            vec![
                "non-cleanup deleted_nodes must cover the orphans this burst created and contain only orphans; deleted [NodeId(\"B\")], required (same-burst minus pre-existing) [], pre-existing (tolerated) []"
                    .to_string()
            ]
        );

        // ...while orphan SWEEPING stays legal window-open: deleting the
        // pre-existing orphan B (nothing imports it, it claims nothing)
        // is accepted even though no deletion was required.
        let repo2 = tmp.path().join("repo2");
        write_supported_orphan_fixture(&repo2, false, false);
        let mut normalization = rooted_ab_normalization(repo2);
        normalization
            .configured_targets
            .insert(TargetId::from("t.b"));
        normalization.current_deps.insert(
        ...
        assert!(
            output.final_outcome == WorkerOutcome::Valid && output.errors.is_empty(),
            "window-open orphan sweeping must stay legal, got {:?} with errors {:?}",
            output.final_outcome,
            output.errors
        );
    }
    ...
    #[test]
    fn burst_covering_last_target_while_leaving_orphans_is_accepted() {
        // Pre-burst window gating decision: the window the worker was
        // DISPATCHED under governs. A burst that itself covers the final
        // uncovered target (A picks up the `t.b` claim) AND strands B is
        // still accepted; B becomes a pre-existing-tolerated orphan for
        // later bursts (sweepable), per the design note on
        // `orphan_construction_window_open`.
        let tmp = tempdir().unwrap();
        let repo = tmp.path().join("repo");
        write_supported_orphan_fixture(&repo, false, true);
        // New node C lands this burst and claims `t.b`.
        write(
        ...
        normalization
            .target_claim_updates
            .insert(NodeId::from("C"), BTreeSet::from([TargetId::from("t.b")]));
        ...
        assert!(
            output.final_outcome == WorkerOutcome::Valid && output.errors.is_empty(),
            "burst closing the window itself must not be judged by the closed window, got {:?} with errors {:?}",
            output.final_outcome,
            output.errors
        );
        // The covering claim actually landed (the window is closed for the
        // NEXT burst, which the post-close rejection twin below pins).
        assert_eq!(
            output.response.snapshot.coverage.get(&TargetId::from("t.b")),
            Some(&BTreeSet::from([NodeId::from("C")]))
        );
    }

    #[test]
    fn window_open_cleanup_burst_still_rejected_for_new_live_orphans() {
        // GAP-2 pin: the clause-2 waiver is non-cleanup-only. A cleanup-
        // validation burst that leaves a NEW live orphan is rejected with
        // master's exact string even while `t.b` is configured-but-
        // uncovered (window open).
        let tmp = tempdir().unwrap();
        let repo = tmp.path().join("repo");
        write_supported_orphan_fixture(&repo, false, true);
        let mut normalization = rooted_ab_normalization(repo);
        ...
        assert_eq!(output.final_outcome, WorkerOutcome::Invalid);
        assert!(
            output.errors.contains(
                &"valid worker response leaves NEW live orphan nodes: [NodeId(\"B\")]".to_string()
            ),
            "cleanup bursts must keep the clause-2 rejection window-open; got {:?}",
            output.errors
        );
    }

    #[test]
    fn closed_window_orphan_rejections_are_byte_identical_to_master() {
        // Invariance pin: when EVERY configured target is covered the
        // window predicate is false and both clauses produce byte-
        // identical error strings to pre-window master (b67ccbf).
        let tmp = tempdir().unwrap();
        let repo = tmp.path().join("repo");
        write_supported_orphan_fixture(&repo, false, true);

        // Clause (2): NEW live orphan after a valid delta.
        let output = accept_worker_response(&WorkerAcceptanceInput {
            request_id: 43,
            cycle: 4,
            payload_outcome: WorkerOutcome::Valid,
            normalization: rooted_ab_normalization(repo.clone()),
            ..WorkerAcceptanceInput::default()
        })
        .unwrap();
        assert_eq!(output.final_outcome, WorkerOutcome::Invalid);
        assert_eq!(
            output.errors,
            vec![
                "non-cleanup deleted_nodes must cover the orphans this burst created and contain only orphans; deleted [], required (same-burst minus pre-existing) [NodeId(\"B\")], pre-existing (tolerated) []"
                    .to_string(),
                "valid worker response leaves NEW live orphan nodes: [NodeId(\"B\")]".to_string()
            ]
        );

        // Clause (1): supported-node deletion.
        let repo2 = tmp.path().join("repo2");
        write_supported_orphan_fixture(&repo2, true, false);
        let output = accept_worker_response(&WorkerAcceptanceInput {
            request_id: 44,
            cycle: 4,
            payload_outcome: WorkerOutcome::Valid,
            deleted_nodes: BTreeSet::from([NodeId::from("B")]),
            normalization: rooted_ab_normalization(repo2),
            ..WorkerAcceptanceInput::default()
        })
        .unwrap();
        assert_eq!(output.final_outcome, WorkerOutcome::Invalid);
    ...
    #[test]
    fn accept_worker_response_rejects_unknown_reference_paper_claim() {
        // Fail-closed: a node_reference_grounds id outside the configured
        // registry is a deterministic worker rejection naming the rule,
        ...
        let claimed_node = NodeId::from("A");
        let base_normalization = WorkerNormalizationInput {
            repo_path: repo,
            configured_targets: BTreeSet::from([TargetId::from("t.a")]),
            current_present_nodes: BTreeSet::from([
                NodeId::from("Preamble"),
        ...
        // Unknown id: deterministic rejection naming the rule; the
        // Invalid outcome zeroes the response's claims.
        let output = accept_worker_response(&WorkerAcceptanceInput {
            request_id: 1,
            cycle: 2,
            payload_outcome: WorkerOutcome::Valid,
            node_reference_grounds: BTreeMap::from([(
                claimed_node.clone(),
                BTreeSet::from([RefPaperId::from("typo2020")]),
            )]),
            configured_reference_papers: BTreeSet::from([RefPaperId::from("smith2020")]),
            normalization: base_normalization.clone(),
            ..WorkerAcceptanceInput::default()
        })
        .unwrap();
        assert_eq!(output.final_outcome, WorkerOutcome::Invalid);
        assert!(
            output.errors.iter().any(|err| {
                err.contains("node_reference_grounds.A")
                    && err.contains("typo2020")
                    && err.contains("configured reference paper")
            }),
            "expected the unknown-id rule to be named; got {:?}",
            output.errors
        );
        assert!(output.response.node_reference_grounds.is_empty());

        // Known id: accepted, and the claims ride the response.
        let output = accept_worker_response(&WorkerAcceptanceInput {
            request_id: 1,
            cycle: 2,
            payload_outcome: WorkerOutcome::Valid,
        ...
        assert!(
            output.final_outcome == WorkerOutcome::Valid && output.errors.is_empty(),
            "expected Valid; got {:?} with {:?}",
            output.final_outcome,
            output.errors
        );
        assert_eq!(
            output.response.node_reference_grounds,
            BTreeMap::from([(claimed_node, BTreeSet::from([RefPaperId::from("smith2020")]))])
        );
    }
    ...
    /// R4 Slice 2: with an `isabelle_hol` target the declaration-lock baseline
    /// READS `Tablet/<node>.thy` (via `node_source_path`), NOT the absent
    /// `Tablet/<node>.lean`. The historical hardcoded `.lean` read found no file
    /// for an Isabelle tablet, so `expected_active_hash` /
    /// `baseline_declaration_hashes` stayed empty and the Tier-1 lock was
    /// vacuous. This asserts the hashes are now NON-empty (the `.thy` is found
    /// and hashed), so the declaration-signature lock fires for Isabelle. NO
    /// `.lean` file exists — only `.thy` — so a non-empty hash proves the read
    /// moved to the node-source path.
    #[test]
    fn prepare_worker_gate_observations_isabelle_captures_thy_baselines() {
        let tmp = tempdir().unwrap();
        let repo = tmp.path().join("repo");
        fs::create_dir_all(repo.join("Tablet")).unwrap();
        write(
        ...
        // Isabelle Preamble + node theories (NO `.lean` anywhere). The body
        // carries a declaration-head line the (cfg(test)) text hasher locks
        // onto, so the captured baseline hash is non-empty — the point being it
        // is captured AT ALL now that the read targets `Tablet/<node>.thy`.
        write(
            &repo.join("Tablet/Preamble.thy"),
            "theorem Preamble := imports Main\n",
        );
        write(&repo.join("Tablet/Preamble.tex"), "");
        write(
        ...
        // Adversarial: the absent-`.lean` path must NOT be what gets read. Prove
        // it by leaving no `.lean` at all — the only way a hash is captured is
        // the `.thy` read.
        assert!(!repo.join("Tablet/A.lean").exists());
        ...
        // The `.thy` was snapshotted and the lock baseline captured — non-vacuous.
        assert!(output.before_snapshot.contains_key("A.thy"));
        assert!(
            !output.expected_active_hash.is_empty(),
            "Isabelle declaration lock must capture a non-empty active hash from the .thy"
        );
        assert!(output.baseline_declaration_hashes.contains_key("A"));
        assert!(
            !output.baseline_declaration_hashes["A"].is_empty(),
            "the A baseline hash must be non-empty (read from Tablet/A.thy)"
        );
...
";
    ...
    /// W4.4: a claim on an UNBOUND WorkerAuthored spec (empty `lean`) skips
    /// the byte/name-parity compare — which would otherwise be a permanent
    /// unsatisfiable reject — and instead runs the authoring-time
    /// negability lint (mode B's F1/F2 config-time errors, surfaced at
    /// authoring). Once bound, the ordinary byte pin freezes the statement.
    #[test]
    fn authored_claim_skips_byte_conformance_until_bound() {
        let authored_spec = |lean: &str| ChallengeTargetSpec {
            kind: ChallengeTargetKind::Theorem,
            resolution: crate::model::ChallengeResolution::Decide,
            statement_provenance: crate::model::StatementProvenance::WorkerAuthored,
            lean: lean.into(),
            name: if lean.is_empty() { String::new() } else { "GoalStmt".into() },
            ..ChallengeTargetSpec::default()
        };
        let goal_file = "\
import Tablet.Preamble
...
theorem GoalStmt (x : Nat) : x + 0 = x := by
-- BODY
  sorry
";
        // Unbound: a worker-authored statement with a negatable shape is a
        // clean accept — no byte compare, no name parity against the empty
        // spec.
        let tmp = tempdir().unwrap();
        let repo = challenge_repo(&tmp, "GoalStmt", goal_file);
        let input = WorkerNormalizationInput {
            repo_path: repo.clone(),
            configured_challenge_targets: BTreeMap::from([(
                ChallengeTargetId::from("goal:f"),
                authored_spec(""),
            )]),
            current_present_nodes: BTreeSet::from([NodeId::from("Preamble")]),
            challenge_claim_updates: BTreeMap::from([(
                NodeId::from("GoalStmt"),
                BTreeSet::from([ChallengeTargetId::from("goal:f")]),
            )]),
            target_claim_updates: BTreeMap::from([(NodeId::from("GoalStmt"), BTreeSet::new())]),
            ..WorkerNormalizationInput::default()
        };
        let normalized = normalize_worker_response(&input).unwrap();
        assert!(
            normalized.contract_errors.is_empty(),
            "an unbound authored claim with a negatable statement accepts; got {:?}",
            normalized.contract_errors
        );

        // Unbound + unnegatable shape (implicit binder): rejected by the
        // authoring-time lint, by name.
        let bad_file = goal_file.replace("(x : Nat)", "{x : Nat}");
        let tmp = tempdir().unwrap();
        let repo = challenge_repo(&tmp, "GoalStmt", &bad_file);
        let input = WorkerNormalizationInput {
            repo_path: repo,
            configured_challenge_targets: BTreeMap::from([(
                ChallengeTargetId::from("goal:f"),
        ...
        assert!(
            normalized
                .contract_errors
                .iter()
                .any(|error| error.contains("cannot be refuted")),
            "an unnegatable authored statement is rejected at authoring time; got {:?}",
            normalized.contract_errors
        );

        // Bound (spec.lean non-empty): an edited statement is byte-
        // conformance rejected exactly like a pinned target.
        let edited = goal_file.replace("x + 0 = x", "x + 1 = x + 1");
        let tmp = tempdir().unwrap();
        let repo = challenge_repo(&tmp, "GoalStmt", &edited);
        let input = WorkerNormalizationInput {
            repo_path: repo,
            configured_challenge_targets: BTreeMap::from([(
                ChallengeTargetId::from("goal:f"),
                authored_spec("theorem GoalStmt (x : Nat) : x + 0 = x := by"),
            )]),
            current_present_nodes: BTreeSet::from([NodeId::from("Preamble")]),
            challenge_claim_updates: BTreeMap::from([(
                NodeId::from("GoalStmt"),
        ...
        assert!(
            normalized
                .contract_errors
                .iter()
                .any(|error| error.contains("challenge byte-conformance rule failed")),
            "a bound statement edit is conformance-rejected; got {:?}",
            normalized.contract_errors
        );
    }

    #[test]
    fn second_claimant_on_authored_target_is_rejected() {
        let goal_file = |name: &str| {
            format!(
                "import Tablet.Preamble\n\n-- [TABLET NODE: {name}]\ntheorem {name} : 1 \
                 \u{2264} 2 := by\n-- BODY\n  sorry\n"
            )
        };
        let authored_registry = || {
            BTreeMap::from([
                (
                    ChallengeTargetId::from("goal:f"),
                    ChallengeTargetSpec {
                ...
                (
                    ChallengeTargetId::from("goal:f__refutation"),
                    ChallengeTargetSpec {
                        kind: ChallengeTargetKind::Theorem,
        ...
        let exactly_one_error = |errors: &[String]| {
            errors
                .iter()
                .any(|error| error.contains("takes exactly one statement node"))
        };

        // Two claimants on the authored primary: rejected by name.
        let tmp = tempdir().unwrap();
        let repo = challenge_repo(&tmp, "GoalA", &goal_file("GoalA"));
        write(&repo.join("Tablet/GoalB.lean"), &goal_file("GoalB"));
        write(
            &repo.join("Tablet/GoalB.tex"),
            "\\begin{theorem}stmt\\end{theorem}\n\\begin{proof}TODO\\end{proof}\n",
        );
        let input = WorkerNormalizationInput {
            repo_path: repo.clone(),
            configured_challenge_targets: authored_registry(),
            current_present_nodes: BTreeSet::from([NodeId::from("Preamble")]),
            challenge_claim_updates: BTreeMap::from([
                (
                    NodeId::from("GoalA"),
            ...
            target_claim_updates: BTreeMap::from([
                (NodeId::from("GoalA"), BTreeSet::new()),
                (NodeId::from("GoalB"), BTreeSet::new()),
            ]),
            ..WorkerNormalizationInput::default()
        };
        let normalized = normalize_worker_response(&input).unwrap();
        assert!(
            exactly_one_error(&normalized.contract_errors),
            "second claimant must be rejected by the exactly-one rule; got {:?}",
            normalized.contract_errors
        );

        // Moving the claim in ONE delta is legal: the standing claimant
        // unclaims while the replacement claims.
        let input = WorkerNormalizationInput {
            repo_path: repo.clone(),
            configured_challenge_targets: authored_registry(),
            current_present_nodes: BTreeSet::from([
        ...
        assert!(
            !exactly_one_error(&normalized.contract_errors),
            "a delete-then-claim move in one delta must not trip the exactly-one \
             rule; got {:?}",
            normalized.contract_errors
        );

        // Twin variant: two claimants on the kernel-derived refutation twin
        // of an authored primary are rejected too.
        let input = WorkerNormalizationInput {
            repo_path: repo.clone(),
            configured_challenge_targets: authored_registry(),
            current_present_nodes: BTreeSet::from([NodeId::from("Preamble")]),
            challenge_claim_updates: BTreeMap::from([
                (
                    NodeId::from("GoalA"),
                    BTreeSet::from([ChallengeTargetId::from("goal:f__refutation")]),
                ),
                (
                    NodeId::from("GoalB"),
                    BTreeSet::from([ChallengeTargetId::from("goal:f__refutation")]),
        ...
        assert!(
            exactly_one_error(&normalized.contract_errors),
            "the refutation twin is a singleton too; got {:?}",
            normalized.contract_errors
        );

        // Seed-pinned control: two claimants on a pinned target do NOT trip
        // the exactly-one rule (other checks may still complain — name
        // parity etc. — but never this one).
        let input = WorkerNormalizationInput {
            repo_path: repo,
            configured_challenge_targets: BTreeMap::from([(
                ChallengeTargetId::from("challenge:pinned"),
            ...
            challenge_claim_updates: BTreeMap::from([
                (
                    NodeId::from("GoalA"),
                    BTreeSet::from([ChallengeTargetId::from("challenge:pinned")]),
                ),
                (
                    NodeId::from("GoalB"),
                    BTreeSet::from([ChallengeTargetId::from("challenge:pinned")]),
        ...
        assert!(
            !exactly_one_error(&normalized.contract_errors),
            "seed-pinned targets keep >=1-claimant freedom; got {:?}",
            normalized.contract_errors
        );
    }
...
";
    ...
    /// A manifest-derived `Def`-kind `ChallengeTargetSpec` uses the same
    /// whole-declaration byte pin, so it rejects a worker that weakens the
    /// predicate body. A `Spec` predicate
    ...
    #[test]
    fn pv_manifest_def_byte_pin_rejects_weakened_spec_body() {
        let tmp = tempdir().unwrap();
        // A pinned predicate definition: a real postcondition over the model output.
