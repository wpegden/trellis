use crate::model::{
    Blocker, BlockerKind, BlockerObject, NodeId, Phase, PvGoalMode, PvRole, RefPaperId,
    RequestKind, RetryOutcomeKind, TargetId, TaskMode, WorkerProfile, WorkerValidationKind,
    WrapperRequest,
};
...
fn pv_worker_prompt_fragments(request: &WrapperRequest) -> Vec<&'static str> {
    ...
    if pv_spec_mode(request) && has_decide_target(request) {
        fragments.push(if request.trust_base_required_v1 {
            "pv/worker/proof_formalization/71_target_false_trust_v1.md"
        } else {
            "pv/worker/proof_formalization/70_target_false_under_model_spec_mode.md"
        });
    }
    fragments
}
...
fn worker_scenario_prompt_fragments(request: &WrapperRequest) -> Vec<&'static str> {
    ...
    if request.trust_base_required_v1
        && (!request.trust_conditional_theorem_candidates.is_empty()
            || !request.trust_source_validation_guidance.is_empty())
    {
        fragments.push("pv/worker/common/06_trust_conditional_candidates.md");
    }
            ...
            if !request.is_pv && has_blocker_kind(request, BlockerKind::PaperFaithfulness) {
                ...
                if request.substantiveness_blocker_adjudicable {
                    ...
                    if request.phase.is_theorem_stating_like() {
                        // Theorem/revision phase: `request_allowed_reset_blockers`
                        // offers reset ids, so keep the reset-capable framing.
                        "review/common/05_after_failed_substantiveness.md"
                    } else {
                        // Proof phase: `request_allowed_reset_blockers` returns
                        // empty (reset is theorem-stating-only), so a reset bullet
                        // would contradict `allowed_reset_ids=[]`. Frame around
                        // worker `.tex` repair instead.
...
pub fn worker_contract_payload(request: &WrapperRequest) -> Value {
    ...
    // Required-v1 trust work is projected exclusively from the verified seed
    // definition closure.  Keep it separate from the general challenge
    // registry so workers can distinguish the auxiliary conditional theorem
    // from the unrestricted target it must never replace or weaken.
    if request.trust_base_required_v1
        && (!request.trust_conditional_theorem_candidates.is_empty()
            || !request.trust_source_validation_guidance.is_empty())
    {
        ...
        {
            summary.insert(
                "trust_conditional_theorem_candidates".to_owned(),
                json!(request.trust_conditional_theorem_candidates),
            );
            summary.insert(
                "trust_source_validation_guidance".to_owned(),
                json!(request.trust_source_validation_guidance),
            );
            summary.insert(
                "trust_seed_authority".to_owned(),
                json!({
                    "source": "verified_seed_definition_closure",
                    "configuration_may_override": false,
                    "candidate_statement_policy": "exact_bytes_at_named_node",
                    "unrestricted_target_policy": "candidate_is_auxiliary_and_does_not_replace_or_weaken_the_unrestricted_target",
                }),
            );
        }
    }
    ...
    if continue_allowed {
        ...
        if request.sidecar_advertise_queue_fields {
            ...
            prompt_schema_example.insert(
                ...
                json!(["queued node names to drop; required for any queued node you route via next_active in this response"]),
            );
        }
        ...
        json!({
            ...
            "dispatch_semantics": {
                ...
                "cleanup_batch_tasks": format!(
                    "Optional<[task_index]>; dispatch multiple Pending LintFix tasks to a single worker burst, up to {} distinct unprotected nodes (the kernel derives the exact multi-node scope). LintFix-only — never batch Substitution tasks (dispatch those one per burst via cleanup_next_task). Mutually exclusive with cleanup_next_task. Whole-burst atomic rollback: any one node's rejection rolls back the entire batch, so batch only high-confidence independent lint-fixes; a rejected batch leaves its tasks Pending for serial re-dispatch.",
                ...
                "authorized_nodes": "Worker edit scope. For Substitution, include all importers; the target is implicit and deletable. For LintFix, single-node — or the kernel-derived set of distinct target nodes when cleanup_batch_tasks is used.",
            }
        })
    } else {
        Value::Null
...
fn initial_planner_contract_payload(
    ...
    // Role fragment per source kind, wired explicitly like the other pv
    // stuck_audit selections (there is no overlay mechanism): math runs read
    // the manuscript, PV prose runs read the goal file, PV spec-mode runs
    // read the pinned challenge statements. The coverage variant has one PV
    // role file for both prose and spec mode (spec-mode routing keeps the
    // selection total; the trigger is live on PV mode-A runs, whose paper
    // targets are seeded from the verification targets, and structurally
    // dead on challenge-only mode-B configs).
    let (initial_planner_role, initial_source_of_truth) = if pv_spec_mode(request) {
        (
            if coverage {
                "pv/stuck_audit/01c_coverage_planner_role.md"
            } else {
                "pv/stuck_audit/01c_initial_planner_role_spec_mode.md"
            },
            pv_stuck_audit_source_of_truth(request),
        )
    } else if request.is_pv {
...
pub fn stuck_math_audit_contract_payload(request: &WrapperRequest) -> Value {
    ...
    if under_model_audit {
        ...
        obj.insert(
            ...
            if request.trust_base_required_v1 {
                json!("must be empty in trust protocol v1; neither this audit nor any worker may mint a condition or assumption")
            } else {
    ...
    if under_model_audit {
        prompt_fragments.push(if request.trust_base_required_v1 {
            ...
            "pv/stuck_audit/07_under_model_adjudication_spec_mode.md"
        });
    }
...
mod tests {
    ...
    fn stuck_math_audit_contract_surfaces_effective_decide_live_polarity() {
        ...
        assert!(contract["prompt_fragments"]
            ...
            .expect("prompt fragments")
            .iter()
            .any(|f| f == "pv/stuck_audit/06_decide_polarity_flip_spec_mode.md"));
    }
    ...
    #[test]
    fn initial_planner_contract_selects_pv_spec_mode_role_fragment() {
        let mut request = initial_planner_request(
            true,
            PvGoalMode::Spec,
            crate::model::InitialPlanningSource::PvChallengeSpecs {
                specs: std::collections::BTreeMap::from([(
                    crate::model::ChallengeTargetId::from("goal:g"),
        ...
        assert!(
            frags.contains(
                &"pv/stuck_audit/01c_initial_planner_role_spec_mode.md".to_string()
            ),
            "PV spec-mode run must select the spec-mode initial-planner role; got {frags:?}"
        );
    }

    // ===== Coverage re-planning contract selection =====

    fn coverage_planner_request(is_pv: bool, goal_mode: PvGoalMode) -> WrapperRequest {
        let mut request = initial_planner_request(
            is_pv,
            goal_mode,
            crate::model::InitialPlanningSource::PaperManuscript {
    ...
    fn coverage_planner_contract_selects_coverage_role_fragment_math() {
        let mut request = coverage_planner_request(false, PvGoalMode::Spec);
    ...
    #[test]
    fn coverage_planner_contract_selects_pv_role_fragment_both_modes() {
        // One PV coverage role file for both prose (reachable on PV mode-A
        // runs, whose configured paper targets are seeded from the
        // verification targets) and spec mode (routing kept total).
        for goal_mode in [PvGoalMode::Prose, PvGoalMode::Spec] {
            let mut request = coverage_planner_request(true, goal_mode);
            let frags = initial_planner_fragments(&mut request);
            assert!(
                frags.contains(&"pv/stuck_audit/01c_coverage_planner_role.md".to_string()),
                "PV coverage carrier ({goal_mode:?}) must select the PV coverage role; got {frags:?}"
            );
        }
    }
    ...
    fn coverage_replanning_false_contract_is_byte_identical_to_cycle_one() {
        ...
        let mut request = initial_planner_request(false, PvGoalMode::Spec, source.clone());
        let mut explicit = initial_planner_request(false, PvGoalMode::Spec, source);
        {
            let ctx = explicit
                .stuck_math_audit
                .initial_planning
    ...
    fn all_audit_scenario_requests(is_pv: bool) -> Vec<(&'static str, WrapperRequest)> {
        let goal_mode = if is_pv { PvGoalMode::Spec } else { PvGoalMode::default() };
    ...
    fn substantiveness_blocker_adjudicable_selects_failed_fragment_theorem_phase() {
        // Theorem/revision phase: `request_allowed_reset_blockers` offers reset
        // ids, so the reset-capable fragment is correct.
        let request = substantiveness_fail_request(Phase::TheoremStating);
        assert_eq!(
            review_primary_scenario_prompt_fragment(&request),
            "review/common/05_after_failed_substantiveness.md",
            "theorem-phase substantiveness Fail keeps the reset-capable framing"
        );
    }

    #[test]
    fn substantiveness_blocker_adjudicable_selects_proof_phase_fragment() {
        // Proof phase: `request_allowed_reset_blockers` returns empty, so a
        // reset bullet would contradict `allowed_reset_ids=[]` (the 2026-07-04
        // req-4937 halt). Must select the proof-phase repair framing instead.
        let request = substantiveness_fail_request(Phase::ProofFormalization);
        assert_eq!(
            review_primary_scenario_prompt_fragment(&request),
            "review/common/05_proof_phase_substantiveness.md",
            "proof-phase substantiveness Fail must not render a reset bullet"
        );
    }
    ...
    fn pv_worker_theorem_stating_cycle1_selects_pv_fragments() {
        ...
        let mut request = WrapperRequest {
            ...
            configured_challenge_targets: BTreeMap::from([(
                crate::ChallengeTargetId::from("ct"),
                crate::ChallengeTargetSpec::default(),
            )]),
            node_role: BTreeMap::from([
                (spec.clone(), PvRole::Spec),
                (model.clone(), PvRole::ExtractionModel),
            ]),
            worker_context: crate::model::WorkerContext {
                worker_profile: WorkerProfile::Theorem,
                validation_kind: WorkerValidationKind::TheoremGlobal,
        ...
        assert!(request.is_pv, "is_pv must be true on the cycle-1 PV request");
        populate_request_prompt_contracts(&mut request, None);
        let frags = contract_fragments(&request.worker_contract);

        for want in [
            // Mode B (default): every GOAL.md-referencing PV worker fragment is
            // the `_spec_mode` variant (no GOAL.md; pinned targets + crate).
            "pv/common/TRELLIS_FORMALIZATION_SCHEME_spec_mode.md",
            "pv/common/18_source_of_truth_spec_mode.md",
            "pv/worker/common/00_intro.md",
            "pv/worker/theorem_stating/05_frontier_work_spec_mode.md",
            // Mode B (default): the pinned-statement "prove only" first-request
            // fragment, not the mode-A scaffold-disposal one.
            "pv/worker/theorem_stating/12_first_request_decomposition_spec_mode.md",
            // Mode B: the pinned-Spec import-and-prove variant, not authoring.
            "pv/worker/theorem_stating/10_spec_proving.md",
            "pv/worker/theorem_stating/10_mode_guidance.md",
            // The DAG-size sizing (15) is now a single GOAL.md-free file shared
            // across both modes.
            "pv/worker/theorem_stating/15_initial_dag_size.md",
            "pv/worker/theorem_stating/20_common_failure_modes_spec_mode.md",
            "pv/canonical/SUBSTANTIVENESS_spec_mode.md",
            ...
            assert!(
                frags.iter().any(|f| f == want),
                "PV worker must carry {want}; got {frags:?}"
            );
        }
        // Mode B carries ZERO GOAL.md references (the defining property).
        assert!(
            !contract_text_mentions_goal_md(&request.worker_contract),
            "mode-B PV worker prompt must have no GOAL.md reference"
        );
        for forbidden in [
            "worker/common/18_reference_paper.md",
            "worker/common/19_paper_focus_fragments.md",
            ...
            "worker/theorem_stating/12_first_request_dag_decomposition.md",
            "worker/theorem_stating/18_challenge_targets.md",
            "worker/theorem_stating/15_initial_dag_size.md",
            "worker/theorem_stating/20_common_failure_modes.md",
            // Mode B must NOT carry any mode-A (GOAL.md-bearing) PV variant.
            // (15_initial_dag_size.md is now the shared file, carried in BOTH
            // modes — it is intentionally absent from this forbidden list.)
            ...
            // The challenge-target framing (18) is no longer selected in mode B:
            // the pinned + pre-seeded + auto-claimed reality is conveyed by 10/12.
            "pv/worker/theorem_stating/18_targets.md",
            "pv/worker/theorem_stating/18_targets_spec_mode.md",
            "pv/worker/theorem_stating/15_initial_dag_size_spec_mode.md",
            "pv/worker/theorem_stating/12_first_request_decomposition.md",
        ] {
            assert!(
                !frags.iter().any(|f| f == forbidden),
                "PV worker must NOT carry {forbidden}; got {frags:?}"
            );
        }

        // Mode A (prose goals): the targets + first-request fragments flip to
        // the spec-AUTHORING / scaffold-disposal variants.
        let mut mode_a = request.clone();
        mode_a.pv_goal_mode = PvGoalMode::Prose;
        populate_request_prompt_contracts(&mut mode_a, None);
        let mode_a_frags = contract_fragments(&mode_a.worker_contract);
        for want in [
            // Mode A keeps every GOAL.md-bearing PV variant byte-for-byte.
            "pv/common/TRELLIS_FORMALIZATION_SCHEME.md",
            "pv/common/18_source_of_truth.md",
            "pv/worker/theorem_stating/05_frontier_work.md",
            "pv/worker/theorem_stating/15_initial_dag_size.md",
            ...
            assert!(
                mode_a_frags.iter().any(|f| f == want),
                "PV mode-A worker carries {want}; got {mode_a_frags:?}"
            );
        }
        for forbidden in [
            "pv/common/TRELLIS_FORMALIZATION_SCHEME_spec_mode.md",
            ...
            assert!(
                !mode_a_frags.iter().any(|f| f == forbidden),
                "PV mode-A worker must NOT carry {forbidden}; got {mode_a_frags:?}"
            );
        }
        // Mode A preserves its GOAL.md references (we did not break mode A).
        assert!(
            contract_text_mentions_goal_md(&mode_a.worker_contract),
            "mode-A PV worker prompt must still reference GOAL.md"
        );
    }
    ...
    fn pv_reviewer_selects_pv_fragments() {
        ...
        for want in [
            // Mode B (default): GOAL.md-referencing reviewer fragments are the
            // `_spec_mode` variants. 33b additionally fixes the false
            // "worker-authored goal theorems" framing for the pinned-goal regime.
            "pv/review/27_source_of_truth_spec_mode.md",
            "pv/review/33b_theorem_target_orientation_spec_mode.md",
            "pv/review/25_verifier_reasoning.md",
            "pv/review/05_after_clean_verification.md",
            "pv/review/31_need_input_spec_mode.md",
            "pv/review/33_routing_hints_spec_mode.md",
            ...
            assert!(
                frags.iter().any(|f| f == want),
                "PV reviewer must carry {want}; got {frags:?}"
            );
        }
        // Mode-B reviewer prompt carries zero GOAL.md references.
        assert!(
            !contract_text_mentions_goal_md(&request.review_contract),
            "mode-B PV reviewer prompt must have no GOAL.md reference"
        );
        for forbidden in [
            "review/common/27_reference_paper.md",
            ...
            "review/common/05_after_clean_verification.md",
            "review/common/31_need_input.md",
            // C1 + 30a + routing-hints PV forks.
            "shared/91_structured_request_pointer.md",
            "review/common/30a_blocker_actions.md",
            "review/common/33_routing_hints.md",
            // Mode B must NOT carry the mode-A (GOAL.md-bearing) PV variants.
            "pv/review/27_source_of_truth.md",
            ...
            assert!(
                !frags.iter().any(|f| f == forbidden),
                "PV reviewer must NOT carry {forbidden}; got {frags:?}"
            );
        }
        for want in [
            "pv/shared/91_structured_request_pointer.md",
        ...
        assert!(
            !frags.iter().any(|f| f.contains("_paper_faithfulness.md")),
            "PV reviewer must carry no 05_after_*_paper_faithfulness fragment; got {frags:?}"
        );

        // Mode A (prose goals): every GOAL.md-bearing reviewer variant is
        // preserved byte-for-byte and the prompt still references GOAL.md.
        let mut mode_a = request.clone();
        mode_a.pv_goal_mode = PvGoalMode::Prose;
        populate_request_prompt_contracts(&mut mode_a, None);
        let mode_a_frags = contract_fragments(&mode_a.review_contract);
        for want in [
            "pv/review/27_source_of_truth.md",
            "pv/review/33b_theorem_target_orientation.md",
            "pv/review/31_need_input.md",
            ...
            assert!(
                mode_a_frags.iter().any(|f| f == want),
                "PV mode-A reviewer carries {want}; got {mode_a_frags:?}"
            );
        }
        assert!(
            contract_text_mentions_goal_md(&mode_a.review_contract),
            "mode-A PV reviewer prompt must still reference GOAL.md"
        );
    }
    ...
    fn pv_shared_review_audit_forks_are_selected() {
        // Reviewer in ProofFormalization with a challenge target configured:
        // exercises 36_authorized_nodes + 33d_challenge_targets.
        let mut review = WrapperRequest {
            kind: crate::model::RequestKind::Review,
            phase: Phase::ProofFormalization,
            is_pv: true,
            configured_challenge_targets: BTreeMap::from([(
                crate::ChallengeTargetId::from("montgomery_reduce"),
                crate::ChallengeTargetSpec::default(),
            )]),
            node_role: BTreeMap::from([(NodeId::from("Correctness"), PvRole::Correctness)]),
            ..WrapperRequest::default()
        };
        populate_request_prompt_contracts(&mut review, None);
        let rfrags = contract_fragments(&review.review_contract);
        for want in [
            "pv/review/36_authorized_nodes.md",
            "pv/review/33d_challenge_targets.md",
            "pv/review/30a_blocker_actions.md",
            // Mode B (default): routing-hints is the `_spec_mode` variant.
            "pv/review/33_routing_hints_spec_mode.md",
            "pv/shared/91_structured_request_pointer.md",
        ] {
            assert!(
                rfrags.iter().any(|f| f == want),
                "PV proof reviewer must carry {want}; got {rfrags:?}"
            );
        }
        for forbidden in [
            "review/common/36_authorized_nodes.md",
            ...
            "review/common/33_routing_hints.md",
            "shared/91_structured_request_pointer.md",
            // Mode B must NOT carry the GOAL.md-bearing routing-hints variant.
            "pv/review/33_routing_hints.md",
        ] {
            assert!(
                !rfrags.iter().any(|f| f == forbidden),
                "PV proof reviewer must NOT carry {forbidden}; got {rfrags:?}"
            );
        }
        assert!(
            !contract_text_mentions_goal_md(&review.review_contract),
            "mode-B PV proof reviewer prompt must have no GOAL.md reference"
        );

        // Math proof reviewer keeps the math originals (zero-diff guard).
        let mut math_review = WrapperRequest {
            kind: crate::model::RequestKind::Review,
            phase: Phase::ProofFormalization,
            configured_challenge_targets: BTreeMap::from([(
                crate::ChallengeTargetId::from("t"),
                crate::ChallengeTargetSpec::default(),
            )]),
            ..WrapperRequest::default()
        };
        populate_request_prompt_contracts(&mut math_review, None);
        let mfrags = contract_fragments(&math_review.review_contract);
        for want in [
            "review/common/36_authorized_nodes.md",
            "review/common/33d_challenge_targets.md",
            "review/common/30a_blocker_actions.md",
            ...
            assert!(
                mfrags.iter().any(|f| f == want),
                "math proof reviewer must carry {want}; got {mfrags:?}"
            );
        }

        // Correspondence verifier: PV scratchpad fork (no host lake env).
        let spec = NodeId::from("Spec");
        let mut corr = WrapperRequest {
            kind: crate::model::RequestKind::Corr,
            phase: Phase::TheoremStating,
        ...
        populate_request_prompt_contracts(&mut corr, None);
        let cfrags = contract_fragments(&corr.corr_contract);
        assert!(
            cfrags.iter().any(|f| f == "pv/verifier/correspondence/07_scratchpad.md"),
            "PV corr must carry the PV scratchpad fork; got {cfrags:?}"
        );
        assert!(
            !cfrags.iter().any(|f| f == "verifier/correspondence/07_scratchpad.md"),
            "PV corr must NOT carry the host-lake scratchpad; got {cfrags:?}"
        );

        // Stuck-math-audit: PV 03/04/05 forks.
        ...
        for want in [
            // Mode B (default): GOAL.md-referencing stuck-audit fragments are
            // the `_spec_mode` variants (history/scratchpad/output + role +
            // source-of-truth).
            "pv/stuck_audit/03_history_access_spec_mode.md",
            "pv/stuck_audit/04_scratchpad_spec_mode.md",
            "pv/stuck_audit/05_output_contract_spec_mode.md",
            ...
            assert!(
                afrags.iter().any(|f| f == want),
                "PV stuck-audit must carry {want}; got {afrags:?}"
            );
        }
        for forbidden in [
            "stuck_math_audit/common/03_history_access.md",
            ...
            "stuck_math_audit/common/05_output_contract.md",
            "shared/91_structured_request_pointer.md",
            // Mode B must NOT carry the GOAL.md-bearing PV variants.
            "pv/stuck_audit/03_history_access.md",
            "pv/stuck_audit/04_scratchpad.md",
            "pv/stuck_audit/05_output_contract.md",
            "pv/stuck_audit/01_role.md",
            ...
            assert!(
                ...
                "PV stuck-audit must NOT carry {forbidden}; got {afrags:?}"
            );
        }
        assert!(
            !contract_text_mentions_goal_md(&audit.stuck_math_audit_contract),
            "mode-B PV stuck-audit prompt must have no GOAL.md reference"
        );

        // Mode A (prose goals): the GOAL.md-bearing stuck-audit variants stay.
        let mut audit_a = audit.clone();
        audit_a.pv_goal_mode = PvGoalMode::Prose;
        populate_request_prompt_contracts(&mut audit_a, None);
        let audit_a_frags = contract_fragments(&audit_a.stuck_math_audit_contract);
        for want in [
            "pv/stuck_audit/03_history_access.md",
            "pv/stuck_audit/04_scratchpad.md",
            "pv/stuck_audit/05_output_contract.md",
            ...
            assert!(
                audit_a_frags.iter().any(|f| f == want),
                "PV mode-A stuck-audit carries {want}; got {audit_a_frags:?}"
            );
        }
        assert!(
            contract_text_mentions_goal_md(&audit_a.stuck_math_audit_contract),
            "mode-A PV stuck-audit prompt must still reference GOAL.md"
        );
    }
    ...
    fn pv_soundness_selects_pv_floor_and_def() {
        ...
        // Mode B (default): the GOAL.md-referencing pv_floor overlay + soundness
        // def are the `_spec_mode` variants.
        assert!(
            frags.iter().any(|f| f == "pv/verifier/soundness/05_pv_floor_spec_mode.md"),
            "PV Sound must carry the lean-mode pv_floor overlay; got {frags:?}"
        );
        assert!(
            frags.iter().any(|f| f == "pv/canonical/SOUNDNESS_spec_mode.md"),
            "PV Sound must carry the lean-mode PV soundness def; got {frags:?}"
        );
        for forbidden in [
            "canonical/SOUNDNESS.md",
            ...
            assert!(
                !frags.iter().any(|f| f == forbidden),
                "PV mode-B Sound must NOT carry {forbidden}; got {frags:?}"
            );
        }
        assert!(
            !contract_text_mentions_goal_md(&request.sound_contract),
            "mode-B PV Sound prompt must have no GOAL.md reference"
        );

        // Mode A (prose goals): the GOAL.md-bearing PV soundness fragments stay.
        let mut mode_a = request.clone();
        mode_a.pv_goal_mode = PvGoalMode::Prose;
        populate_request_prompt_contracts(&mut mode_a, None);
        let mode_a_frags = contract_fragments(&mode_a.sound_contract);
        assert!(mode_a_frags.iter().any(|f| f == "pv/verifier/soundness/05_pv_floor.md"));
        assert!(mode_a_frags.iter().any(|f| f == "pv/canonical/SOUNDNESS.md"));
        assert!(
            contract_text_mentions_goal_md(&mode_a.sound_contract),
            "mode-A PV Sound prompt must still reference GOAL.md"
        );

        // Math counterpart unchanged.
        let mut math = WrapperRequest {
            kind: crate::model::RequestKind::Sound,
            phase: Phase::TheoremStating,
            ..WrapperRequest::default()
        ...
        assert!(math_frags.iter().any(|f| f == "canonical/SOUNDNESS.md"));
        assert!(!math_frags.iter().any(|f| f == "pv/verifier/soundness/05_pv_floor.md"));
    }
    ...
    fn pv_selected_fragments_exist_on_disk() {
        ...
        let pv_paths = [
            ...
            "pv/verifier/correspondence/10_model_correspondence.md",
            "pv/verifier/substantiveness/05_fresh_node_frontier.md",
    ...
    #[test]
    fn required_v1_worker_receives_exact_seed_candidate_and_source_guidance() {
        let node = NodeId::from("ConditionalTarget");
        let statement = "theorem ConditionalTarget : True := by".to_owned();
        let candidate = crate::model::TrustConditionalTheoremCandidate {
            candidate_id: crate::model::ChallengeTargetId::from("conditional:target"),
            candidate_definition_sha256: crate::trust_base::raw_sha256(b"candidate"),
            profile_id: "bounded-profile".to_owned(),
            profile_definition_sha256: crate::trust_base::raw_sha256(b"profile"),
            target_id: "target".to_owned(),
            node_id: node.clone(),
            statement_utf8: statement.clone(),
            conditional_statement_sha256: crate::trust_base::tagged_hash(
                crate::trust_base::DomainTag::ConditionalizationSchema,
                statement.as_bytes(),
            ),
            active_statement_sha256: crate::trust_base::raw_sha256(statement.as_bytes()),
        };
        let guidance = crate::model::TrustSourceValidationGuidance {
            contract_definition_sha256: crate::trust_base::raw_sha256(b"contract"),
            target_id: "target".to_owned(),
            validation_method: "exact_rust_execution_v1".to_owned(),
            public_entry_point: "decode".to_owned(),
            concretization_schema_sha256: crate::trust_base::raw_sha256(b"schema"),
            concretization_schema_utf8:
                "closed RLE descriptor: [(run_length:u32, byte:u8)]; total length < 4096"
                    .to_owned(),
        };
        let request = WrapperRequest {
            kind: crate::model::RequestKind::Worker,
            ...
            trust_conditional_theorem_candidates: BTreeMap::from([(
                node.clone(),
                candidate,
            )]),
            trust_source_validation_guidance: BTreeMap::from([(
                "target".to_owned(),
                guidance,
            )]),
            ..WrapperRequest::default()
        ...
        let payload = worker_contract_payload(&request);
        let fragments = payload["prompt_fragments"].as_array().unwrap();
        assert!(fragments.iter().any(|fragment| {
            fragment == "pv/worker/common/06_trust_conditional_candidates.md"
        }));
        assert_eq!(
            payload["request_summary"]["trust_conditional_theorem_candidates"]
                ["ConditionalTarget"]["statement_utf8"],
            statement
        );
        assert_eq!(
            payload["request_summary"]["trust_source_validation_guidance"]["target"]
                ["public_entry_point"],
            "decode"
        );
        assert!(payload["request_summary"]["trust_source_validation_guidance"]["target"]
            ["concretization_schema_utf8"]
            .as_str()
            .unwrap()
            .contains("closed RLE descriptor"));
        assert_eq!(
            payload["request_summary"]["trust_seed_authority"]
                ["configuration_may_override"],
            false
        );
    }
}
