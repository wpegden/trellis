...
pub fn execute_approved_json(
    ...
    let mut value = serde_json::json!({
        ...
        "journal_predecessor_sha256": request.journal_predecessor_sha256,
    ...
    validate_execution_receipt(
        ...
        request.evidence,
        request.purpose,
        request.journal_predecessor_sha256,
    )?;
...
pub fn validate_execution_receipt(
    ...
    let predecessor: Sha256Digest =
        string_field(value, "journal_predecessor_sha256")?.parse()?;
    if predecessor != expected_predecessor {
        return Err(TrustError::new(
            "execution_receipt_predecessor_mismatch",
            "execution receipt belongs to another journal state",
        ));
    }
...
mod tests {
    ...
    fn approved_receipt_recomputes_captured_output() {
        ...
        let predecessor: Sha256Digest = "33".repeat(32).parse().unwrap();
