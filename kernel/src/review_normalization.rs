use crate::model::{
    AuditRequest, AuditRequestReasonKind, Blocker, BlockerKind, BlockerObject,
    GlobalRepairRequest, NodeDifficulty, NodeId, PaperFocusRange, PaperGrounding, Phase, RequestKind,
    ResetChoice, ResponseStatus, ReviewDecisionKind, ReviewResponse, StuckMathAuditReviewReport,
    TaskDismissal, TaskMode, Update, WorkerContextMode, WorkerWorkStyleHint, WrapperRequest,
...
pub struct RawReviewPayload {
    ...
    /// Correspondence-repair dispatch: the present node whose `.tex`
    /// statement block the next worker burst may rewrite. Legal only in
    /// Phase::Cleanup + Continue; see `ReviewResponse::cleanup_repair_node`.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub cleanup_repair_node: Option<String>,
...
fn diagnose_review_illegality(request: &WrapperRequest, review: &ReviewResponse) -> String {
    ...
    // Advisory diagnostics only, but they must agree with
    // `cleanup_v2_review_fields_rejection_reason`: the latch demands Done
    // only when Done is reachable. With outstanding work — which a
    // Cleanup state may now carry, see `ProtocolState::formalization_valid`
    // — Done is independently illegal, so telling the reviewer "decision
    // must be done" would name the one decision every other gate refuses.
    if request.phase == Phase::Cleanup && request.cleanup_force_done_view {
        if request.formalization_complete_view && review.decision != ReviewDecisionKind::Done {
            return "the cleanup consecutive-invalid-worker threshold has fired (cleanup_force_done is set on this request) and no work is outstanding; decision must be done — Continue would just queue another worker burst that is also expected to fail".into();
        }
        if review.decision == ReviewDecisionKind::Continue
            && (review.cleanup_next_task.is_some()
                || review
                    .cleanup_batch_tasks
                    .as_ref()
                    .is_some_and(|batch| !batch.is_empty()))
        {
            return "the cleanup consecutive-invalid-worker threshold has fired (cleanup_force_done is set on this request); no further cleanup task may be dispatched. Clear the outstanding correspondence work with cleanup_repair_node, or dispatch nothing and let the verifier run".into();
        }
...
mod tests {
    ...
    fn normalize_review_response_accepts_global_repair_step_a_in_restructure_mode() {
        // Regression (live run, cycle 757, 2026-06-21): a StuckMathAudit
    ...
    fn normalize_review_response_carries_cleanup_request_reaudit_on_done() {
        ...
        // (below CLEANUP_AUDIT_MAX_ROUNDS).
        let request = WrapperRequest {
            kind: RequestKind::Review,
            id: 1,
            cycle: 1,
    ...
    /// Sibling of the `validate_trellis_reviewer_result_data` allowlist:
    /// `RawReviewPayload` is the second place `cleanup_repair_node` can
    /// be dropped between the LLM's JSON and the kernel's response.
    #[test]
    fn normalize_review_response_carries_cleanup_repair_node() {
        let raw: RawReviewPayload = serde_json::from_value(serde_json::json!({
            "decision": "continue",
            "next_mode": "cleanup",
            "reason": "repair the helper statement",
            "allow_new_obligations": true,
            "must_close_active": false,
            "reset": "none",
            "cleanup_repair_node": "  Helper  "
        }))
        .expect("deserialize raw review payload");
        assert_eq!(raw.cleanup_repair_node.as_deref(), Some("  Helper  "));

        let normalized = normalize_review_response(&ReviewNormalizationInput {
            request: WrapperRequest {
                kind: RequestKind::Review,
                phase: Phase::Cleanup,
                cleanup_repair_candidates: BTreeSet::from([NodeId::from("Helper")]),
                allowed_decisions: BTreeSet::from([ReviewDecisionKind::Continue]),
                allowed_next_modes: BTreeSet::from([TaskMode::Cleanup]),
                allowed_resets: BTreeSet::from([ResetChoice::None]),
                ..WrapperRequest::default()
        ...
        .expect("normalize review");
        assert_eq!(
            normalized.response.cleanup_repair_node,
            Some(NodeId::from("Helper")),
            "the node id must survive normalization, trimmed"
        );

        // A blank string is "no repair", not a node named by whitespace.
        let blank: RawReviewPayload = serde_json::from_value(serde_json::json!({
            "decision": "continue",
            "next_mode": "cleanup",
        ...
        .expect("deserialize raw review payload");
        let normalized_blank = normalize_review_response(&ReviewNormalizationInput {
            request: WrapperRequest {
                kind: RequestKind::Review,
                phase: Phase::Cleanup,
                allowed_decisions: BTreeSet::from([ReviewDecisionKind::Continue]),
        ...
        .expect("normalize review");
        assert_eq!(normalized_blank.response.cleanup_repair_node, None);
    }
    ...
    fn cleanup_batch_request(
        ...
        // Every task target is present, as on a live request — the
        // presence legality clause rejects dispatching a task whose
        // target a substitution deleted.
        let current_present_nodes = tasks.iter().map(|t| t.target_node.clone()).collect();
        WrapperRequest {
            kind: RequestKind::Review,
            id: 1,
            cycle: 1,
    ...
    #[cfg(test)]
    fn extract_helper_batch_task(node: &str) -> crate::model::CleanupAuditTask {
        crate::model::CleanupAuditTask {
            target_node: node.into(),
            rationale: "oversized".into(),
            confidence: crate::model::CleanupTaskConfidence::High,
            kind: crate::model::CleanupTaskKind::ExtractHelper {
                ordinal: 1,
                hint: "the second case split".into(),
            },
            status: crate::model::CleanupTaskStatus::Pending,
            audit_origin_round: 1,
            swept_parents: BTreeSet::new(),
    ...
    #[cfg(test)]
    fn dead_code_elim_task(node: &str) -> crate::model::CleanupAuditTask {
        crate::model::CleanupAuditTask {
            target_node: node.into(),
            rationale: "suspect dead proof context".into(),
            confidence: crate::model::CleanupTaskConfidence::Medium,
            kind: crate::model::CleanupTaskKind::DeadCodeElim {
    ...
    #[cfg(test)]
    fn extract_shared_task() -> crate::model::CleanupAuditTask {
        crate::model::CleanupAuditTask {
            target_node: "A".into(),
            rationale: "shared region".into(),
            confidence: crate::model::CleanupTaskConfidence::High,
            kind: crate::model::CleanupTaskKind::ExtractShared {
                co_parents: BTreeSet::from(["B".into(), "C".into()]),
                ordinal: 1,
                hint: "region".into(),
            },
            status: crate::model::CleanupTaskStatus::Pending,
    ...
    /// A kind-homogeneous ExtractHelper batch on distinct present parents
    /// is legal — batching is per-kind, not LintFix-only.
    #[test]
    fn review_response_legal_accepts_homogeneous_extract_helper_batch() {
        let request = cleanup_batch_request(
            vec![
                extract_helper_batch_task("A"),
                extract_helper_batch_task("B"),
            ],
            &[],
        );
        let response = batch_response(vec![0, 1]);
        assert!(
            request.review_response_legal(&response),
            "a homogeneous ExtractHelper batch must be legal: {:?}",
            request.review_response_rejection_reasons(&response)
        );
    }

    #[test]
    fn review_response_legal_rejects_dead_code_elim_batch() {
        let request = cleanup_batch_request(
            vec![dead_code_elim_task("A"), dead_code_elim_task("B")],
            &[],
        );
        let msg = request
            .cleanup_v2_review_fields_rejection_reason(&batch_response(vec![0, 1]))
            .expect("dead-code sweeps are single-dispatch only");
        assert!(msg.contains("dead_code_elim") && msg.contains("cleanup_next_task"));
    }

    #[test]
    fn review_response_legal_rejects_extract_shared_batch() {
        let request = cleanup_batch_request(vec![extract_shared_task()], &[]);
        let msg = request
            .cleanup_v2_review_fields_rejection_reason(&batch_response(vec![0]))
            .expect("extract_shared is single-dispatch only");
        assert_eq!(
            msg,
            "cleanup_batch_tasks index 0 is an extract_shared task; extract_shared is dispatched via cleanup_next_task only"
        );
    }

    /// A mixed-kind batch is rejected: a burst has exactly one acceptance
    /// envelope, so batched tasks must share one kind.
    #[test]
    fn review_response_legal_rejects_mixed_kind_batch() {
        use crate::model::CleanupTaskStatus::Pending;
        let request = cleanup_batch_request(
            vec![lintfix_task("A", Pending), extract_helper_batch_task("B")],
            &[],
        );
        let msg = request
            .cleanup_v2_review_fields_rejection_reason(&batch_response(vec![0, 1]))
            .expect("a mixed-kind batch must be rejected");
        assert!(
            msg.contains("mixes task kinds"),
            "the error must name the homogeneity precondition: {msg}"
        );
    }
    ...
    /// A task whose target node is no longer present (deleted by an
    /// accepted substitution) is undispatchable, via both dispatch modes.
    #[test]
    fn review_response_legal_rejects_dispatch_of_absent_target_task() {
        use crate::model::CleanupTaskStatus::Pending;
        let mut request = cleanup_batch_request(
            vec![lintfix_task("A", Pending), lintfix_task("Gone", Pending)],
            &[],
        );
        request
            .current_present_nodes
            .remove(&crate::model::NodeId::from("Gone"));

        assert!(
            !request.review_response_legal(&batch_response(vec![0, 1])),
            "a batch naming an absent-target task must be rejected"
        );
        let scalar = crate::model::ReviewResponse {
            decision: ReviewDecisionKind::Continue,
            next_mode: TaskMode::Cleanup,
            cleanup_next_task: Some(1),
            ..crate::model::ReviewResponse::default()
        };
        assert!(
            !request.review_response_legal(&scalar),
            "a scalar dispatch of an absent-target task must be rejected"
        );
    }
    ...
    fn cleanup_batch_rejection_reasons_are_specific() {
        ...
        assert!(
            msg.contains("no acceptance envelope exists for more than one substitution"),
            "the error must state the capability limit, not a policy: {msg}"
        );
