...
/// trailing whitespace ignored) ends with `:=`, `by`, or `where`. The
/// body delimiter is `:= by` (tactic mode), `:=` followed by a term
/// that may sit below the marker (term mode), a multi-line `:= by`
/// split where `by` is on its own line, or `where` for a data
/// declaration (`structure`/`inductive`/`class`, whose fields or
/// constructors are the body) and structure-instance syntax. All four
/// end with `:=`, `by`, or `where` on the line immediately above the
/// marker. Empty content above the marker is rejected too — placing
/// the marker at column 0 of line 1 makes the file structurally
/// broken.
fn validate_body_marker_position(
    ...
    if trimmed.is_empty() {
        return Err(
            "filespec_split: line immediately above `-- BODY` marker is empty; the marker must sit directly after the principal declaration's body delimiter (line ending with `:=`, `by`, or `where`)".to_string(),
        );
    }
    let ends_with_assign = trimmed.ends_with(":=");
    // For "ends with `where`": the body delimiter for a data
    // declaration (`structure`/`inductive`/`class`) and for
    // structure-instance syntax is the `where` keyword. Accept the bare
    // word `where` on its own line or a `where` preceded by a
    // non-identifier character (so identifiers like `elsewhere` or
    // qualified `Foo.where` are not mistaken for the keyword).
    let ends_with_where = if trimmed == "where" {
        true
    } else if let Some(stripped) = trimmed.strip_suffix("where") {
    ...
    if !(ends_with_assign || ends_with_by || ends_with_where) {
        return Err(format!(
            "filespec_split: line immediately above `-- BODY` marker must end with `:=`, `by`, or `where`; got {:?}",
            trimmed,
        ));
    }
    Ok(())
...
/// Extract the challenge-conformance comparison region from a tablet
/// node file. The region starts after the `-- [TABLET NODE: <Name>]`
/// marker line and runs to the `-- BODY` marker line; with
/// `include_body` it additionally extends through end of file, with
/// the `-- BODY` marker line itself excluded (the marker is
/// kernel-owned FILESPEC scaffolding, never part of a prescription).
/// Surrounding blank lines are trimmed; interior bytes are returned
/// verbatim, line-joined with `\n`, no trailing newline. Used by the
/// challenge byte-conformance check at worker acceptance, which
/// compares the result against the imported prescription with no
/// further normalization.
pub fn prescribed_region(content: &str, include_body: bool) -> Result<String, String> {
    let split_record = split(content, "")?;
    let tablet_marker_start = find_tablet_node_marker_line(content)?;
    if tablet_marker_start >= split_record.body_marker_start_byte {
        return Err(format!(
            "filespec_split: `-- [TABLET NODE: ...]` marker line (byte {tablet}) must precede `-- BODY` marker line (byte {body})",
    ...
    let marker_line_end = content[tablet_marker_start..]
        .find('\n')
        .map(|offset| tablet_marker_start + offset + 1)
        .unwrap_or(content.len());
    let mut region =
        content[marker_line_end..split_record.body_marker_start_byte].to_string();
    if include_body {
        region.push_str(&content[split_record.body_marker_end_byte..]);
    }
    Ok(trim_surrounding_blank_lines(&region))
}

/// Extract the **namespace context** of a node file: the ordered
/// `namespace …` / `end …` lines in the file PREAMBLE (the region
/// strictly above the `-- [TABLET NODE: <Name>]` marker), joined by
/// `\n`, trimmed.
///
/// These are exactly the lines that scope the principal declaration and
/// therefore decide its fully-qualified name (an Aeneas model decl under
/// `namespace ntt_montgomery` is the trusted `ntt_montgomery.FIELD_MODULUS`;
/// strip the line and it silently becomes top-level `FIELD_MODULUS`). For a
/// challenge target this is pinned as part of the node identity.
///
/// Everything else in the preamble stays free: `import` lines (the per-node
/// DAG needs them), file-level `open …` lines (an `open` cannot rename a
/// declaration — it only brings names into scope, so a worker may add/remove
/// an `open` freely), `set_option`, and comments are all excluded here. Thus
/// adding an `open` or an `import` does NOT change the namespace context, but
/// stripping or altering a `namespace`/`end` line DOES.
///
/// Only the preamble is scanned: a `namespace`/`end` that appears below the
/// marker is part of the declaration body (already covered by the byte-pin),
/// not the scoping context, so it is ignored here.
pub fn namespace_context(content: &str) -> Result<String, String> {
    let tablet_marker_start = find_tablet_node_marker_line(content)?;
    let preamble = &content[..tablet_marker_start];
    Ok(namespace_context_from_preamble(preamble))
}

/// Same namespace/end extraction as [`namespace_context`], but applied to a
/// raw preamble string that has NO tablet marker (e.g. a challenge target's
/// pinned `namespace_context`, which may be configured as a full preamble like
/// `"open Aeneas …\nnamespace ntt_montgomery"`).
///
/// Keeping only the `namespace …`/`end …` lines on BOTH the extracted-node side
/// and the pinned-spec side is what makes the byte-compare symmetric: the pin
/// declares the namespace scope, and any `open`/`import`/`set_option` lines a
/// configurer included in the pinned value are normalised away (opens are free).
/// Without this, a pin carrying a full preamble can never byte-equal the
/// namespace-only value the node extractor produces, so the target is
/// permanently unsatisfiable.
pub fn namespace_context_from_preamble(preamble: &str) -> String {
    let ctx_lines: Vec<&str> = preamble
        .lines()
...
/// W9: the node preamble's `import` modules and `open` namespaces, in file
/// order — "the bound node's actual import block" a statement binding
/// captures so the deferred certificate context can be derived from it.
/// Only the preamble (above the tablet marker) is scanned, mirroring
/// [`namespace_context`]; an `open A B C` line contributes each listed
/// namespace separately.
pub fn import_open_context(content: &str) -> Result<(Vec<String>, Vec<String>), String> {
    let tablet_marker_start = find_tablet_node_marker_line(content)?;
    let preamble = &content[..tablet_marker_start];
    ...
    for line in preamble.lines() {
        ...
        } else if let Some(namespaces) = line.strip_prefix("open ") {
            opens.extend(namespaces.split_whitespace().map(str::to_owned));
        }
    }
    Ok((imports, opens))
}

/// Drop leading and trailing blank lines (and the trailing newline);
/// interior lines are preserved verbatim.
fn trim_surrounding_blank_lines(region: &str) -> String {
...
/// Remove Lean comments while preserving strings and the tablet-node marker.
///
/// Comments and docstrings carry no declaration semantics, but previously
/// changing one below the tablet marker changed the protected hash. The
/// scanner understands nested `/- ... -/` comments and line comments, and
/// leaves quoted strings byte-visible so comment delimiters inside them are
/// not misclassified. The tablet-node marker is itself a line comment but is
/// deliberately retained: it binds the protected region to its declared node.
fn strip_non_marker_comments(decl: &str) -> String {
    ...
    while index < chars.len() {
        ...
        if chars[index] == '-' && chars.get(index + 1) == Some(&'-') {
            ...
            if comment.trim().starts_with(TABLET_NODE_MARKER_PREFIX) {
...
/// Normalise the statement region for hashing: remove non-semantic comments
/// and collapse whitespace. Namespace qualification remains byte-visible.
/// The old global string replacement (`Real.` → empty, etc.) could also alter
/// that text inside a larger identifier (`MyReal.Number` → `MyNumber`) and
/// therefore admitted genuine collisions between different declarations.
fn normalize_declaration(decl: &str) -> String {
    strip_non_marker_comments(decl)
        .split_whitespace()
        .collect::<Vec<_>>()
        .join(" ")
}
...
";
    ...
    #[test]
    fn declaration_hash_strict_keeps_namespace_qualification_visible() {
        let v1 = "\
import Tablet.Preamble
-- [TABLET NODE: Foo]
...
";
    ...
    #[test]
    fn declaration_hash_strict_ignores_comments_without_identifier_collisions() {
        let base = "\
import Tablet.Preamble
-- [TABLET NODE: Foo]
theorem Foo : MyReal.Number = MyReal.Number := by
-- BODY
  rfl
";
        let comment_only_edit = "\
import Tablet.Preamble
-- [TABLET NODE: Foo]
/-- Documentation that does not alter the declaration. -/
theorem Foo : MyReal.Number = MyReal.Number := by -- explanatory comment
-- BODY
  rfl
";
        let formerly_colliding_identifier = "\
import Tablet.Preamble
-- [TABLET NODE: Foo]
theorem Foo : MyNumber = MyNumber := by
-- BODY
  rfl
";
        let repo = std::path::Path::new("/dev/null");
        let base_hash = declaration_hash_strict(repo, base, "Foo").unwrap();
        assert_eq!(
            base_hash,
            declaration_hash_strict(repo, comment_only_edit, "Foo").unwrap(),
            "comments and docstrings below the tablet marker are non-semantic"
        );
        assert_ne!(
            base_hash,
            declaration_hash_strict(repo, formerly_colliding_identifier, "Foo").unwrap(),
            "namespace text embedded inside an identifier must remain hash-visible"
        );
    }
...
";
        let err = validate_filespec(content, "Foo").unwrap_err();
        assert!(err.contains("empty") || err.contains("end with `:=`, `by`, or `where`"));
    }
...
";
        let err = validate_filespec(content, "Foo").unwrap_err();
        assert!(err.contains("end with `:=`, `by`, or `where`"));
    }

    #[test]
    fn validate_filespec_accepts_where_delimiter_structure() {
        let content = "\
import Tablet.Preamble
...
";
    ...
    #[test]
    fn validate_filespec_accepts_where_delimiter_inductive() {
        let content = "\
import Tablet.Preamble
...
";
    ...
    #[test]
    fn validate_filespec_accepts_instance_structure_where() {
        let content = "\
import Tablet.Preamble

-- [TABLET NODE: Foo]
instance Foo : Group T where
-- BODY
  mul := sorry
";
        assert!(validate_filespec(content, "Foo").is_ok());
    ...
    #[test]
    fn validate_filespec_rejects_identifier_ending_in_where() {
        // `elsewhere` ends with the substring `where` but is an
        // identifier, not the Lean `where` keyword.
        let content = "\
import Tablet.Preamble

-- [TABLET NODE: Foo]
def Foo : Nat := elsewhere
-- BODY
  42
";
        let err = validate_filespec(content, "Foo").unwrap_err();
    ...
    #[test]
    fn prescribed_region_include_body_covers_where_fields() {
        // Byte-pinning over a data declaration's fields uses the
        // include_body path, which must capture everything below `-- BODY`.
        let content = "\
import Tablet.Preamble
...
-- BODY
  ...
  y : Nat
";
        let region = prescribed_region(content, true).expect("region");
        assert!(region.contains("structure Foo where"));
        assert!(region.contains("x : Nat"));
        assert!(region.contains("y : Nat"));
        // Without the body, only the head is pinned.
        let head_only = prescribed_region(content, false).expect("region");
        assert!(head_only.contains("structure Foo where"));
        assert!(!head_only.contains("x : Nat"));
    }
