...
pub fn validate_trellis_stuck_math_audit_result_data(data: &Value) -> ArtifactValidationOutput {
    ...
    if !under_model_ruling.is_empty() {
        ...
        // that actually complied (verified on the dec2flt run: audits 610
...
/// Audit-ordered node retirement: validate + re-emit the audit's
/// `node_retirement_request` sub-object. Shape: `{nodes: [non-empty
/// string, ...] (non-empty), reason: non-empty string}`. Absent or null
/// → None (no request; byte-compatible with existing payloads).
    ...
    fn gap_validator_accepts_critic_accept_with_tasks() {
        let result = validate_trellis_stuck_math_audit_result_data(&json!({
            ...
            "tasks": [{
                ...
                "body": "Implement the accepted route and arrive Lean-closed."
            }]
        }));
        assert!(result.ok, "unexpected errors: {:?}", result.errors);
        let data = result.data.expect("validated payload");
