...
pub struct SidecarRuntimeConfig {
    ...
    /// above any real run's eligible population (~108 on `perfect`) so
    ...
    /// `DEFAULT_AUTO_DISPATCH_MAX_ATTEMPTS` (5), not off: measured over
    /// a whole run, attempts #1–5 produced every closure the arm ever
    /// made (41 in 523 tries) while attempts #6–18 produced 0 in 297 —
    /// 36% of total spend for zero yield. The cap keeps 100% of the
    /// historical successes and deletes the futility loop, so it is on
    ...
    ///
    /// Measured on `perfect`: the grunt model's lifetime close rate by
...
/// see that field's doc for the measurement behind 5.
pub const DEFAULT_AUTO_DISPATCH_MAX_ATTEMPTS: u32 = 5;
...
pub fn build_candidates_export(
    ...
    let kernel_queue = {
        ...
        let mut by_node: std::collections::BTreeMap<&NodeId, SidecarQueueExportRow> =
            kernel_queue.iter().map(|row| (&row.node, row.clone())).collect();
        ranked
            .iter()
...
// `auto_dispatch.max_attempts` (default 5, 0 = unlimited), counted per
...
/// Per-node prior-attempt counts from the daemon's `attempted.json`.
///
...
/// `attempted.json` — the same rows `read_attempt_counts` counts, kept
/// individually so the attempt cap can count per CONTENT.
///
...
pub fn collect_auto_dispatch_candidates(
    ...
    let spent_this_cycle: std::collections::BTreeSet<&NodeId> = state
        ...
        .map(|row| &row.node)
        .collect();
    // Attempt ceiling (default 5, 0 = unlimited): stop refilling a node
    ...
    let candidates = sidecar_candidate_nodes(state)
        ...
        .filter(|c| {
            max_nl_proof_chars == 0
                || c.proof_length.map_or(true, |n| n <= max_nl_proof_chars)
        })
        .collect();
...
mod tests {
    ...
    fn config_auto_dispatch_defaults_on_and_takes_an_off_switch() {
        ...
        assert!(load_sidecar_runtime_config(&path)
            .expect("parse ok")
            .expect("block present")
...
pub fn declared_names_permissive(content: &str) -> std::collections::BTreeSet<String> {
    const KEYWORDS: &[&str] = &[
        "theorem", "lemma", "def", "abbrev", "example", "structure", "inductive", "class",
        "instance", "axiom", "opaque", "initialize", "builtin_initialize",
    ];
    ...
    const MODIFIERS: &[&str] = &[
        "private", "protected", "scoped", "local", "noncomputable", "unsafe", "partial",
        "nonrec", "meta",
    ];
    ...
    for raw in stripped.lines() {
        ...
        loop {
            ...
            if !matches!(head, "set_option" | "open" | "universe" | "attribute" | "variable") {
                break;
            }
            match line.find(" in ") {
        ...
        // principal's OWN namespace, which `isOwnNodeAux` sanctions and
        // the closure walk covers (it walks value as well as type, so
...
fn is_plausible_decl_name(token: &str) -> bool {
    ...
    (first.is_alphabetic() || first == '_' || first == '\u{00AB}')
        && !token.starts_with("--")
}
...
mod declared_names_delta_tests {
    ...
    fn a_body_cannot_introduce_a_top_level_declaration() {
        ...
        for body in attacks {
            ...
            assert_ne!(got, base, "body slipped a declaration past the delta: {body:?}");
        }
    }

    /// The other half: legitimate bodies must not move the set. Validated
    /// against all 626 marker-bearing nodes of the live `perfect` tablet
