...
/// One-shot backfill from the per-cycle event log. Idempotent — does
/// nothing if `burst-history.jsonl` already exists. Best-effort — silently
/// returns on any I/O or parse failure so a malformed event log can't
/// block startup.
///
/// `event_log_dir` is `<repo>/.trellis-history/event-log/`; this chains
/// readers over its `cycle-NNNNNN.jsonl` files in lexical (== global index)
/// order. For each `wrapper_response` event it reconstructs an approximate
/// `request_id` / `cycle` / response-kind row from the event payload (the
/// event log stores the full response object alongside the request id and
/// cycle, but does NOT store the dispatch-time request fields like
/// `active_node` / `mode` — we capture those from the response body when
/// present, otherwise leave them null).
pub fn backfill_if_missing(repo_path: &Path, event_log_dir: &Path) {
    let history = history_path(repo_path);
    if history.exists() {
        return;
    }
    let Ok(cycle_files) = crate::runtime::event_log_cycle_files(event_log_dir) else {
        return;
    };
    ...
    // Chain readers over the per-cycle files in order so the backfilled
    // ledger preserves global event order.
    let lines = cycle_files.iter().flat_map(|path| {
        std::fs::File::open(path)
            .map(|file| BufReader::with_capacity(1 << 20, file).lines())
            .into_iter()
            .flatten()
    });
...
mod tests {
    ...
    fn backfill_skips_if_history_exists() {
        ...
        let event_log_dir = tmpdir.path().join("event-log");
        std::fs::create_dir_all(&event_log_dir).unwrap();
        // Fake per-cycle event log containing one wrapper_response.
        let event = json!({
            "cycle": 5,
        ...
        std::fs::write(
            event_log_dir.join("cycle-000005.jsonl"),
            format!("{}\n", serde_json::to_string(&event).unwrap()),
        )
        .unwrap();
        backfill_if_missing(repo, &event_log_dir);
    ...
    #[test]
    fn backfill_chains_per_cycle_files_in_order() {
        let tmpdir = tempfile::tempdir().unwrap();
        let repo = tmpdir.path();
        std::fs::create_dir_all(repo.join(".trellis").join("logs")).unwrap();
        let event_log_dir = tmpdir.path().join("event-log");
        std::fs::create_dir_all(&event_log_dir).unwrap();
        let make = |cycle: u32, request_id: u32| {
            let event = json!({
                "cycle": cycle,
                "phase": "ProofFormalization",
                "ts_ms": 1_700_000_000_000_i64,
        ...
        // Two cycle files; backfill must read them in lexical (== cycle) order.
        std::fs::write(event_log_dir.join("cycle-000002.jsonl"), make(2, 20)).unwrap();
        std::fs::write(event_log_dir.join("cycle-000010.jsonl"), make(10, 30)).unwrap();
        backfill_if_missing(repo, &event_log_dir);
        let contents = std::fs::read_to_string(history_path(repo)).unwrap();
        ...
        assert_eq!(rows[0]["cycle"], json!(2));
        assert_eq!(rows[1]["cycle"], json!(10));
    }
}
