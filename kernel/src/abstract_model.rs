...
pub enum SpecAction {
    ...
    /// escalation. Under the GapResearch design this no longer routes to a
    /// blind HumanGate; it dispatches the GapResearch Planner on the shared
    /// StuckMathAudit lane (`gap_research` carrier set, fresh context).
    /// Mirror of kernel `apply_stuck_math_audit_response` Valid +
    /// need_input_audit Some + `confirm_need_input = true` arm (engine.rs).
    AcceptStuckMathAuditDispatchHumanGate,
    /// GapResearch Planner response that sets `route_needs_human`: routes to
    /// a real HumanGate (NeedInput). One of the two human paths. Mirror of
    /// `apply_gap_research_response`.
    AcceptGapPlannerHumanRequired,
    ...
    /// Parallel-closure sidecar boundary apply. Mirror of the kernel's
    /// `apply_sidecar_closure` (engine.rs) / the spec's
    /// `ApplySidecarClosure(n)` environment action (SupervisorCore.tla).
    ApplySidecarClosure,
    /// Parallel-closure sidecar boundary expiry of a SPENT queue
    /// generation. Mirror of the kernel's
    /// `apply_sidecar_attempt_outcomes` (engine.rs) / the spec's
    /// `ExpireSidecarQueueEntry(n)` environment action
    /// (SupervisorCore.tla).
    ExpireSidecarQueueEntry,
    /// Parallel-closure sidecar boundary top-up of an EMPTY grunt queue
    /// while grunts sit idle. Mirror of the kernel's
    /// `apply_sidecar_queue_auto_add` (engine.rs) / the spec's
    /// `AutoDispatchSidecarQueue` environment action
    /// (SupervisorCore.tla).
