...
use crate::model::{
    ...
    RevisionTargetDeltaKind, Stage, SubstantivenessStatus, TargetId,
};
...
pub fn import_revision_project(
    ...
    // A plain snapshot (bare ProtocolState or old envelope) decodes to itself;
    // a `trellis-shared-state/1` envelope is expanded before the `state` read.
    let parsed = crate::shared_state_codec::decode_shared_state(parsed).map_err(|err| {
        format!(
            "failed to decode full state {}: {err}",
    ...
    // Revision mode is a fresh rewind domain. The prior run's verifier state
    // is intentionally inherited below, but its LastClean mirrors and commit
    // pointer describe pre-revision bytes and must never become selectable in
    // this run. A later clean revision checkpoint establishes a new anchor.
    state.invalidate_last_clean_anchor();
...
mod tests {
    ...
    #[test]
    fn import_invalidates_prior_run_last_clean_anchor() {
        let fx = build_fixture(old_body(), new_body());
        let raw = fs::read_to_string(&fx.full_state).expect("read fixture state");
        let mut prior: ProtocolState = serde_json::from_str(&raw).expect("parse fixture state");
        prior.last_clean_live = prior.live.clone();
        prior.last_clean_node_kinds = prior.node_kinds.clone();
        prior.last_clean_verifier_mirror_ready = true;
        prior.last_clean_local_closure_mirror_ready = true;
        prior.last_clean_commit = Some("old-revision-clean-commit".into());
        prior.has_ever_been_clean = true;
        prior.cycles_since_clean = 7;
        write(&fx.full_state, &serde_json::to_string(&prior).unwrap());

        let result = import(&fx);

        assert!(!result.state.has_ever_been_clean);
        assert_eq!(result.state.cycles_since_clean, 0);
        assert!(!result.state.last_clean_verifier_mirror_ready);
        assert!(!result.state.last_clean_local_closure_mirror_ready);
        assert_eq!(result.state.last_clean_commit, None);
        assert_eq!(result.state.last_clean_live, WorkingSnapshot::default());
        assert!(result.state.last_clean_node_kinds.is_empty());
    }
    ...
    fn import_rebaselines_substantiveness_to_pass_for_every_present_node() {
        ...
        for node in &result.state.live.present_nodes {
            assert_eq!(
                ...
                Some(&SubstantivenessStatus::Pass),
    ...
    #[test]
    fn import_accepts_shared_state_encoded_envelope() {
        let fx = build_fixture(old_body(), new_body());
        // Minimal well-formed `trellis-shared-state/1`: the whole state lives
        // in `$pool` and `state` is a single reference to it. The encoder
        // would not pool a value seen once, but the decoder must accept it.
        let bare: Value = serde_json::from_str(&fs::read_to_string(&fx.full_state).unwrap()).unwrap();
    ...
    #[test]
    fn import_rejects_corrupt_shared_state_envelope() {
        let fx = build_fixture(old_body(), new_body());
        let envelope = serde_json::json!({
            "$format": "trellis-shared-state/1",
            "$strings": {},
            "$pool": {},
            "state": { "$r": "0123456789abcdef" },
        });
        write(&fx.full_state, &serde_json::to_string(&envelope).unwrap());
        let err = import_revision_project(
            &fx.config,
        ...
        .unwrap_err();
        assert!(err.contains("absent from $pool"), "got: {err}");
    }
