...
//! - **Approved** (human ratifies at the `AssumptionReview` gate): the kernel
//!   first writes the disclosure sink, then flips the trust switch —
...
//!   Now a worker can `exact`/`apply` `C` and the closure gate admits it.
...
//! only (mirrors `dormant_store` / `tablet_root`). The soundness wall is
//! unchanged: an assumption can be USED only after it is in
//! `APPROVED_AXIOMS.json`, and it is written there only by `project_approved`,
//! which the engine emits only from the human-gate APPROVE path.
//!
...
fn statement_hook_guards_var(statement: &str, var: &str) -> bool {
    ...
    for token in &tokens {
        ...
        prev_was_hook = token.starts_with("RustValid") && token.split('.').count() == 1
            // A hook APPLICATION head, not a hook-derived axiom NAME like
            // `RustValidSliceU8_len_isize_bound` (those contain `_` after the
            // hook stem only when they name axioms; the seeded/minted hook
            // symbols themselves are single CamelCase identifiers).
            && !token.contains('_');
    }
    false
}
        ...
        if !statement_hook_guards_var(lean_statement, &binder.var) {
            errors.push(format!(
                "quantified variable `{}` has over-approximating container type `{}` but no \
                 ...
                 that type's hook (if none exists, a carrier must be minted first)",
...
/// (`import Tablet.Assumptions`), holding the setup Rust-validity hook plus
/// worker-staged under-model `axiom` blocks. `project_approved` does not
/// rewrite Lean; it admits trust via the registry sinks.
...
///
/// Design (2026-07-02, operator decision on the dec2flt run): an assumption
...
/// Project an approved assumption `id` into the trust sinks atomically-ish
/// (each sink is written in turn; a failure short-circuits with `Err` so the
/// runtime halts rather than half-projecting). On success the record is REMOVED
...
/// then admits the axiom through `APPROVED_AXIOMS.json`; until then the closure
/// gate rejects any proof that reaches it. The approved-axioms write is the
/// trust switch, so it intentionally happens last.
...
/// idempotent (no duplicate). This is the ONLY write that makes `C` USABLE.
...
/// Extraction pins an auditor needs that the `pv_tablet` config does NOT
/// carry today. Each is dropped from `not_recorded` iff the config's
/// `extraction_toolchain` object gains a key of that name — so a future
/// config that pins them is reflected automatically, and until then the
/// manifest discloses their absence instead of implying them.
const CANDIDATE_UNRECORDED_TOOLCHAIN_PINS: [&str; 2] = ["rustc", "target_triple"];
...
/// `not_recorded` list for pins the run's config does not carry (rustc
/// version/commit, target triple, and — when the corpus manifest is absent —
/// the docs-corpus revision).
///
...
pub fn record_extraction_provenance(
    ...
    // Dedupe + sort the source pairs (a whole-crate extraction stamps the same
    // pair on every model node); drop fully-empty pairs.
    let digests: std::collections::BTreeSet<&ExtractionSourceDigest> = source_digests
        .iter()
        .filter(|d| !(d.source_file.trim().is_empty() && d.source_sha256.trim().is_empty()))
        .collect();
    ...
    let mut not_recorded: Vec<String> = CANDIDATE_UNRECORDED_TOOLCHAIN_PINS
        .iter()
        .filter(|pin| {
            !extraction_toolchain
                .as_object()
                .is_some_and(|obj| obj.contains_key(**pin))
        })
    ...
    let docs_corpus: Option<serde_json::Value> = fs::read_to_string(&corpus_path)
        .ok()
    ...
    if docs_corpus.is_none() {
        not_recorded.push("docs_corpus_revision".to_string());
    }
...
/// The seeded `Tablet/Assumptions.lean` scaffold the extractor emits (and that
/// setup falls back to). It `import Tablet.Preamble` so staged axioms can
/// reference model types. No cycle: every NODE imports `Tablet.Assumptions`
/// (prepended at materialize time) and `Tablet.Preamble`, but Preamble never
/// imports Assumptions. Kept in sync with
/// `scripts/extract_pv_model.py::_empty_assumptions_lean`.
...
mod tests {
    ...
    #[test]
    fn assumptions_scaffold_seeds_rust_validity_hook() {
        let scaffold = empty_assumptions_lean_scaffold();
        assert!(scaffold.contains(&format!(
            "axiom {RUST_VALIDITY_HOOK_NAME} : {RUST_VALIDITY_HOOK_TYPE}"
        )));
        assert!(!scaffold.contains("opaque RustValidSliceU8"));
    }
    ...
    fn extraction_provenance_flags_unrecorded_pins_honestly() {
        let repo = tmp();
        // The real config shape: toolchain pins charon/aeneas/lean only, and
        // the repo has no docs-corpus manifest ⇒ all three absent pins are
    ...
    fn extraction_provenance_embeds_docs_corpus_and_drops_recorded_pins() {
        ...
        .unwrap();
        // A config that DOES pin rustc drops it from not_recorded.
        let toolchain = serde_json::json!({"charon": "c", "rustc": "nightly-2026-06-01"});
        record_extraction_provenance(&repo, &toolchain, &[], &[]).unwrap();
        let tcb: serde_json::Value =
            serde_json::from_str(&fs::read_to_string(tcb_manifest_path(&repo)).unwrap()).unwrap();
        let prov = &tcb["extraction_provenance"];
        assert_eq!(prov["not_recorded"], serde_json::json!(["target_triple"]));
        assert_eq!(
