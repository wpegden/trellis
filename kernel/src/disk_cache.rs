...
mod tests {
    ...
    fn cache_dir_for_namespace_returns_none_when_env_unset() {
        // These tests used to save/restore by hand under no lock at all,
        // which is what let them tear a concurrent read in an unrelated
        // module. `EnvScope` takes the one crate-wide env lock.
        let mut env = crate::EnvScope::lock();
        env.unset(KERNEL_CACHE_ROOT_ENV);
    ...
    fn cache_dir_for_namespace_resolves_under_kernel_cache_subdir() {
        let tmp = tempdir().unwrap();
        let _env = crate::EnvScope::with_kernel_cache_root(tmp.path());
    ...
    fn cache_lookup_dirs_returns_writable_then_readonly() {
        // Both vars are process-global; `EnvScope` records each one's
        // entry value on first touch and restores both on drop, so the
        // rewrites below can't leak past the end of the test.
        let tmp_w = tempdir().unwrap();
        let tmp_r = tempdir().unwrap();
        let mut env = crate::EnvScope::lock();
        env.set(KERNEL_CACHE_ROOT_ENV, tmp_w.path());
        env.set(KERNEL_CACHE_READONLY_ROOT_ENV, tmp_r.path());
        ...
        env.unset(KERNEL_CACHE_READONLY_ROOT_ENV);
