...
///
/// This source-shape helper is only for cache/materialization bookkeeping. It
/// is not authoritative for certificate dependency roots; those come from
/// replay-attested `ModuleData.imports`.
fn extract_tablet_imports(lean_content: &str) -> BTreeSet<String> {
    strip_lean_comments(lean_content)
        .lines()
        .flat_map(|line| {
            ...
            if !tokens[..import_index]
                .iter()
                .all(|token| matches!(*token, "public" | "private" | "meta"))
            {
                return Vec::new().into_iter();
            }
...
/// Source-derived direct `Tablet.*` imports for non-certificate bookkeeping.
/// Certificate issuance must use replay-attested `ModuleData.imports` instead.
pub fn direct_tablet_imports(repo_path: &Path, node_name: &str) -> Option<BTreeSet<String>> {
    let content =
        fs::read_to_string(repo_path.join("Tablet").join(format!("{node_name}.lean"))).ok()?;
    let mut imports = extract_tablet_imports(&content);
    imports.remove(PREAMBLE_NAME);
    imports.remove(node_name);
    Some(imports)
}

/// Remove Lean `--` comments and nested `/- ... -/` comments while
/// preserving newlines.  Import commands are then parsed as whitespace-
/// delimited module identifiers, covering several modules per command and
/// Unicode/dotted identifiers without accepting commented-out imports.
fn strip_lean_comments(content: &str) -> String {
...
///   - `cache_v=2` version tag. Replay validity is now an explicit olean-hash
///     attestation in the existing provenance sidecar, so deploying replay no
///     longer makes byte-current pre-deploy oleans source-stale.
...
/// complete artifact-bundle shas in for the same reason — which is why the server-side sidecar
...
/// the whole key (unlike the server-side None-on-missing). Every consuming
/// cache now has a full source+replay consistency guard, so an absent olean is
/// already a forced miss there. Keeping the key total preserves construction
/// in pre-build windows while still changing it the moment an olean appears
...
pub fn lean_closure_cache_key_with_oleans(repo_path: &Path, node_name: &str) -> Option<String> {
    ...
    let mut blob = String::new();
    blob.push_str("olean_closure_v=2\n");
    blob.push_str(&format!("source={source_key}\n"));
    for member in &closure {
        let exported = olean_dir.join(format!("{member}.olean"));
        let server = PathBuf::from(format!("{}.server", exported.display()));
        let private = PathBuf::from(format!("{}.private", exported.display()));
        for (level, path, include) in [
            ("exported", &exported, true),
            ("server", &server, server.exists()),
            ("private", &private, private.exists()),
        ] {
            if !include {
                continue;
            }
            match fs::read(path) {
                Ok(bytes) => blob.push_str(&format!(
                    "artifact:{member}:{level}:{}={}\n",
                    bytes.len(),
                    hash_bytes(&bytes)
                )),
                Err(_) => blob.push_str(&format!("artifact:{member}:{level}=<absent>\n")),
            }
        }
        if private.exists() && !server.exists() {
            blob.push_str(&format!(
                "artifact:{member}=<unsupported-private-without-server>\n"
            ));
        }
    }
    Some(hash_text(&blob))
...
mod tests {
    ...
    #[test]
    fn import_walker_covers_comments_multiple_dotted_and_unicode_modules() {
        let content = r#"
/- outer comment
   /- import Tablet.Hidden -/
-/

-- import Tablet.AlsoHidden
import A.B.C Tablet.Dep Tablet.Δ -- several imports on one line
import Tablet.Other
public meta import all Tablet.PublicModule
"#;
        assert_eq!(
            extract_tablet_imports(content),
            [
                "Dep".to_string(),
                "Other".to_string(),
        ...
        seed_minimal_repo(&repo);
        write(
            &repo.join("Tablet/Root.lean"),
            "import Tablet.Left Tablet.Right\n",
        );
        write(&repo.join("Tablet/Left.lean"), "import Tablet.Δ\n");
        write(&repo.join("Tablet/Right.lean"), "-- leaf\n");
        write(&repo.join("Tablet/Δ.lean"), "-- transitive-only leaf\n");
        let mut closure = BTreeSet::new();
        recursive_imports(&repo, "Root", &mut closure);
        assert_eq!(
            closure,
            ["Left", "Right", "Root", "Δ"]
                .into_iter()
    ...
    fn olean_key_changes_when_olean_content_changes() {
        ...
        let server = PathBuf::from(format!("{}.server", olean.display()));
        let private = PathBuf::from(format!("{}.private", olean.display()));
        write(&server, "server-v1");
        write(&private, "private-v1");
        let split = lean_closure_cache_key_with_oleans(&repo, "A").unwrap();
        write(&private, "private-v2");
        let split_private_changed = lean_closure_cache_key_with_oleans(&repo, "A").unwrap();
        assert_ne!(
            split, split_private_changed,
            "private part must change the key"
        );
        write(&server, "server-v2");
        let split_server_changed = lean_closure_cache_key_with_oleans(&repo, "A").unwrap();
        assert_ne!(
            split_private_changed, split_server_changed,
            "server part must change the key"
        );

        // Deterministic for unchanged inputs.
        let split_server_changed_again = lean_closure_cache_key_with_oleans(&repo, "A").unwrap();
        assert_eq!(split_server_changed, split_server_changed_again);
