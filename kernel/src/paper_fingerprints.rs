...
struct PaperTargetFingerprint {
    ...
    /// GAP B (W10): the sha256 of the CURRENT prose goal file's text —
    /// the referent axis. Populated only for PV prose-mode observations
    /// (`goal_prose_path` passed); empty (and off the wire) for math /
    /// mode-B, keeping those fingerprints byte-identical. A mid-run goal
    /// edit shifts this axis, so every faithfulness verdict drops to
    /// Unknown derived-ly with no bespoke invalidation code.
    #[serde(default, skip_serializing_if = "String::is_empty")]
    goal_prose_sha256: Fingerprint,
}
...
pub fn observe_paper_faithfulness_fingerprints(
    ...
    // GAP B (W10): hash the CURRENT prose referent once per observation.
    // A missing/unreadable file hashes as empty text — distinct from any
    // real content, so verdicts recorded against real bytes fail closed.
    let goal_prose_sha256 = goal_prose_path
        .map(|path| hash_text(&read_text(path)))
        .unwrap_or_default();
    ...
    configured_targets
        ...
        .map(|target| {
            ...
            let fingerprint = PaperTargetFingerprint {
                ...
                goal_prose_sha256: goal_prose_sha256.clone(),
            };
            (
                target.clone(),
...
mod tests {
    ...
    /// GAP B (W10): with a goal-prose path passed (PV prose mode), a goal
    /// file edit flips every target's paper fingerprint (⇒ all
    /// faithfulness verdicts drop to Unknown derived-ly). Without the path
    /// (math / mode-B), the same edit changes nothing and the fingerprint
    /// stays byte-identical to the pre-feature shape.
    #[test]
    fn goal_prose_edit_flips_fingerprint_only_when_goal_axis_active() {
        let tmp = tempfile::tempdir().expect("tempdir");
        let tablet = tmp.path().join("Tablet");
        fs::create_dir_all(&tablet).expect("tablet dir");
        fs::write(tablet.join("Preamble.lean"), "").expect("preamble lean");
        fs::write(tablet.join("Preamble.tex"), "").expect("preamble tex");
        fs::write(
        ...
        fs::write(&goal, "verify f is correct\n").expect("goal");

        let configured_targets: BTreeSet<TargetId> = set(&["f"]);
        let target_claims: BTreeMap<NodeId, BTreeSet<TargetId>> =
            BTreeMap::from([(NodeId::from("Cover"), set(&["f"]))]);
        let present_nodes: BTreeSet<NodeId> = set(&["Preamble", "Cover"]);
        let l_def: BTreeMap<NodeId, BTreeSet<NodeId>> =
            BTreeMap::from([(NodeId::from("Cover"), BTreeSet::new())]);

        let observe = |goal_path: Option<&std::path::Path>| {
        ...
        let with_goal_before = observe(Some(&goal));
        let without_goal_before = observe(None);
        fs::write(&goal, "verify f is correct, AND g\n").expect("edit goal");
        let with_goal_after = observe(Some(&goal));
        let without_goal_after = observe(None);

        assert!(
            !with_goal_before.get("f").unwrap().is_empty(),
            "prerequisite: non-empty fingerprint"
        );
        assert_ne!(
            with_goal_before.get("f"),
            with_goal_after.get("f"),
            "a goal-prose edit must flip the fingerprint when the axis is active"
        );
        assert_eq!(
            without_goal_before.get("f"),
            without_goal_after.get("f"),
            "without the goal axis the edit must change nothing"
        );
        assert_eq!(
            without_goal_before.get("f"),
            without_goal_after.get("f"),
        );
        // Axis inactive ⇒ wire shape has no goal field at all (math /
        // mode-B fingerprints stay byte-identical to the pre-feature form).
        assert!(
            !without_goal_before.get("f").unwrap().contains("goal_prose_sha256"),
            "inactive axis must stay off the fingerprint wire"
        );
    }
