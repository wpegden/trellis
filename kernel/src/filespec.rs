...
fn parse_decl_line(trimmed: &str) -> Option<(String, String)> {
    ...
    // `partial` is part of the command syntax rather than the declaration
    // name. Recognize the authored principal so it reaches closure probing,
    // where the source-level partiality check rejects it explicitly.
    if tokens[0] == "partial" && tokens.len() >= 3 && tokens[1] == "def" {
        return Some((tokens[1].to_string(), tokens[2].to_string()));
    }
    if [
        "theorem",
        "lemma",
...
/// Worker-authoring policy command keywords. The keyword set mirrors the
/// Lean command syntax kinds in
/// `scripts/lean_local_closure.lean::policyCommandKinds`
/// (verified against v4.30.0-rc1; still exercised green under the
/// current pin v4.33.0 on 2026-08-27 by the `--scan-only` cases in
/// `kernel/tests/local_closure_smoke.rs` — `scan_only_plain_macro_rejected`,
/// `scan_only_plain_elab_rejected`, `scan_only_macro_forge_rejected_at_owner`,
/// `scan_only_legit_notation_no_false_positive` and siblings, which drive
/// the real Lean parse against a v4.33.0-built fixture).
/// `#eval`/`#eval!` and notation/mixfix commands are intentionally absent.
/// Locally scoped macro/syntax/elaborator definitions are filtered by
/// `macro_defining_commands` below.
...
const LOCAL_POLICY_DEFINITION_KEYWORDS: &[&str] = &[
    "macro",
    "macro_rules",
    "elab",
    "elab_rules",
...
pub fn reserved_shaped_authored_components(lean_content: &str, node_name: &str) -> Vec<String> {
    ...
    for raw in lean_content.lines() {
        ...
        let decl_id =
            decl_id.trim_end_matches(|c: char| c == ':' || c == '(' || c == '{' || c == '[');
        ...
        if is_reserved_shaped_component(final_component)
            && !offenders.iter().any(|o| o == final_component)
        {
...
/// Text backstop for the worker-authoring policy. Returns forbidden command
/// keywords in first-seen order. Local macro/syntax/elaborator definitions
/// and `#eval`/`#eval!` are allowed.
///
...
pub fn macro_defining_commands(lean_content: &str) -> Vec<String> {
    ...
    for raw in lean_content.lines() {
        ...
        let after_attrs = strip_leading_attribute_groups(line);
        let locally_scoped = after_attrs
            .split_whitespace()
            .next()
            .is_some_and(|token| token == "local");
        let line = strip_leading_decl_modifiers(line);
        let head = line.split_whitespace().next().unwrap_or("");
        if locally_scoped && LOCAL_POLICY_DEFINITION_KEYWORDS.contains(&head) {
            continue;
        }
...
/// In the dec2flt example, request 952 (2026-07-04) rejected an
...
/// Resolve the exact fully-qualified Lean name of a node's registered
/// principal declaration from its authoritative source.
///
/// This is registration data, not an environment heuristic: the unique
/// FILESPEC principal head is combined with the namespace stack recorded in
/// the source preamble.  Certificate issuance passes this exact name to the
/// Lean probe and the probe requires the environment to contain that name in
/// `Tablet.<node>`; it never guesses by excluding generated-looking names.
pub fn exact_principal_name(lean_content: &str, node_name: &str) -> Result<String, String> {
    let matching: Vec<DeclarationHead> = declaration_heads(lean_content)
        .into_iter()
        .filter(|head| decl_name_matches_node(&head.name, node_name))
        .collect();
    let local_name = match matching.as_slice() {
        [] => {
            return Err(format!(
                "node `{node_name}` has no FILESPEC principal declaration to register"
            ))
        }
        [only] => only
            .name
        ...
        many => {
            return Err(format!(
                "node `{node_name}` has multiple FILESPEC principal declarations at lines {:?}",
                many.iter().map(|head| head.line).collect::<Vec<_>>()
            ))
        }
    };
    if local_name.is_empty() {
        return Err(format!(
            "node `{node_name}` has an empty FILESPEC principal declaration name"
        ));
    }

    // Register the namespace active at the declaration's exact source line.
    // FILESPEC permits the namespace command below the node marker, so looking
    // only at the pre-marker context silently registered a namespaced theorem
    // as a top-level name. Track namespace/section nesting up to the matched
    // declaration instead.
    let principal_line = matching[0].line;
    let mut command_stack: Vec<Option<String>> = Vec::new();
    let mut block_comment_depth = 0_u32;
    for raw_line in lean_content
        .lines()
        .take(principal_line.saturating_sub(1) as usize)
    {
        let uncommented = strip_lean_comments_line(raw_line, &mut block_comment_depth);
        let line = uncommented.trim();
        if let Some(rest) = line.strip_prefix("namespace ") {
            let namespace = rest.trim();
            if namespace.is_empty() || namespace.split_whitespace().count() != 1 {
                return Err(format!(
                    "node `{node_name}` has an unsupported namespace command before its principal: `{line}`"
                ));
            }
            command_stack.push(Some(namespace.to_string()));
        } else if line == "section" || line.starts_with("section ") {
            command_stack.push(None);
        } else if line == "end" || line.starts_with("end ") {
            if command_stack.pop().is_none() {
                return Err(format!(
                    "node `{node_name}` has an unmatched `end` before its registered principal"
                ));
            }
        }
    }
    let namespaces: Vec<String> = command_stack.into_iter().flatten().collect();

    if let Some(rooted) = local_name.strip_prefix("_root_.") {
        return Ok(rooted.to_string());
    }
    if namespaces.is_empty() {
        Ok(local_name)
    } else {
        Ok(format!("{}.{}", namespaces.join("."), local_name))
    }
}

fn strip_lean_comments_line(line: &str, block_depth: &mut u32) -> String {
    let bytes = line.as_bytes();
    let mut index = 0;
...
pub fn validate_lean_node_shape(lean_content: &str, node_name: &str) -> Vec<String> {
    ...
    // Early text backstop for the worker-authoring policy. The Lean parser's
    // `--scan-only` operation is the layout-complete gate.
    for keyword in macro_defining_commands(lean_content) {
        errors.push(format!(
            "FILESPEC authoring policy forbids `{keyword}` in a Tablet node: macro/syntax/elaborator definitions must be `local`; `declare_syntax_cat`, `run_cmd`, `run_elab`, `run_meta`, `by_elab`, `initialize`, and `builtin_initialize` are forbidden; `#eval` and `#eval!` are allowed."
        ));
    }
...
mod tests {
    use super::{
        declaration_heads, exact_principal_name, is_definition_kind, is_proof_bearing_kind,
        is_reserved_shaped_component, macro_defining_commands, parse_top_level_envs,
        reserved_shaped_authored_components, tex_statement_environment,
        validate_declaration_kind_shape, validate_lean_node_shape, validate_tex_format,
    };
    ...
    fn structure_inductive_class_are_definition_kinds_instance_is_proof_bearing() {
        for kind in [
            ...
            assert!(
                !is_proof_bearing_kind(kind),
                "{kind} should not be proof-bearing"
            );
        }
    ...
    fn dotted_principal_declaration_matches_its_sanitized_node_stem() {
        // In the dec2flt example, request 952: `Tablet/FiniteDecimal_value.lean`
    ...
    #[test]
    fn exact_principal_registration_includes_source_namespace_context() {
        let content = "namespace crate\nnamespace Model\n-- [TABLET NODE: FiniteDecimal_value]\n\nnoncomputable def FiniteDecimal.value : Nat :=\n-- BODY\n  0\n";
        assert_eq!(
            exact_principal_name(content, "FiniteDecimal_value").unwrap(),
            "crate.Model.FiniteDecimal.value"
        );

        let top_level = "-- [TABLET NODE: Foo]\n\ntheorem Foo : True := by\n-- BODY\n  trivial\n";
        assert_eq!(exact_principal_name(top_level, "Foo").unwrap(), "Foo");

        let below_marker = "-- [TABLET NODE: Foo]\nimport Tablet.Preamble\nnamespace crate\n\ntheorem Foo : True := by trivial\nend crate\n";
        assert_eq!(
            exact_principal_name(below_marker, "Foo").unwrap(),
            "crate.Foo"
        );
    }

    #[test]
    fn exact_principal_registration_rejects_ambiguity() {
        let content = "-- [TABLET NODE: Foo]\n\ntheorem Foo : True := by trivial\nlemma Foo : True := by trivial\n";
        let error = exact_principal_name(content, "Foo").unwrap_err();
        assert!(error.contains("multiple FILESPEC principal"), "{error}");
    }
    ...
    fn reserved_shaped_component_matches_lean_isinternaldetail_string_cases() {
        ...
        for s in [
            "_helper", "_hidden", "_sunfold", "eq_1", "match_12", "proof_3", "omega_0", "eq_",
            "eq__",
        ] {
            assert!(
                is_reserved_shaped_component(s),
                "{s} should be reserved-shaped"
            );
        }
        // Not reserved.
        for s in [
            "helper", "realAux", "eq", "equiv", "eqOf", "matcher", "proof", "omega", "eq_def",
            "eq_1a", "Foo",
        ] {
            assert!(
                !is_reserved_shaped_component(s),
                "{s} should NOT be reserved-shaped"
            );
        }
    ...
    fn backstop_no_false_positives() {
        ...
        assert!(
            reserved_shaped_authored_components("theorem Owner : True := trivial\n", "Owner")
                .is_empty()
        );
    ...
    fn parse_decl_line_strips_inline_attributes() {
        ...
        assert_eq!(
            parse_decl_line("@[global_simps, irreducible] def FIELD_MODULUS : Std.I32 := 3329#i32"),
        ...
        // Preserve the historical shape scanner's narrow treatment of
        // modifiers: protected auxiliaries are handled by the Lean source
        // scanner, not counted as additional principals here.
        assert_eq!(
            parse_decl_line("@[simp] protected theorem Foo : True := trivial"),
            None
        );
        assert_eq!(
            parse_decl_line("partial def Loop (n : Nat) : Nat := Loop n"),
            Some(("def".to_string(), "Loop".to_string()))
        );
    ...
    #[test]
    fn authoring_policy_backstop_allows_local_definitions_and_eval() {
        for src in [
            "local macro \"op\" : term => `(1)\n",
            "local macro_rules | `(1) => `(2)\n",
            "local elab \"op\" : term => return default\n",
            "local elab_rules : term | `(1) => `(2)\n",
            "local syntax \"op\" : term\n",
            "local binder_predicate x \" > \" y => `($x > $y)\n",
            "#eval (1 + 1 : Nat)\n",
            "#eval! (1 + 1 : Nat)\n",
        ] {
            assert!(
                macro_defining_commands(src).is_empty(),
                "allowed policy construct was flagged: {src:?}"
            );
        }
    }
    ...
    fn validate_lean_node_shape_rejects_macro_command() {
        ...
        assert!(
            errors
                .iter()
                .any(|e| e.contains("authoring policy") && e.contains("`macro`")),
