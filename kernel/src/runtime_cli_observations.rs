...
pub fn parse_local_closure_response(
    node: &str,
    raw: serde_json::Value,
) -> Result<LocalClosureProbeOutput, String> {
    ...
    let probe = LocalClosureProbeOutput {
        ...
        strict_theorem_deps,
        strict_definition_deps,
        // Classified below by the Rust trust-boundary validator.  The Lean
        // checker reports every strict definition through the ordinary field
        // because it does not own campaign authority.
...
pub fn validate_probe_present_nodes(
...
/// Trust-v1 extension of the C-K/C-N dependency validator.  An authenticated
/// seed support carrier may be absent from `present_nodes`, but only when the
/// collector reached it as a strict definition.  Move that entry into a
/// distinct record category so downstream node-lifecycle checks are not
/// weakened and the dependency remains explicit.
pub(crate) fn validate_probe_present_nodes_with_seed_support(
    probe: &mut LocalClosureProbeOutput,
    present_nodes: &BTreeSet<NodeId>,
    node_kinds: &BTreeMap<NodeId, crate::NodeKind>,
    seed_support_definition_nodes: &BTreeSet<NodeId>,
) {
    let support_keys: Vec<NodeId> = probe
        .strict_definition_deps
        .keys()
        .filter(|node| seed_support_definition_nodes.contains(*node))
        .cloned()
        .collect();
    for node in support_keys {
        let semantic_hash = probe
            .strict_definition_deps
            .remove(&node)
            .expect("support key came from strict_definition_deps");
        if let Some(previous) = probe
            .seed_support_definition_deps
            .insert(node.clone(), semantic_hash.clone())
        {
            if previous != semantic_hash {
                probe.errors.push(format!(
                    "seed support definition dependency `{}` carries conflicting semantic hashes",
                    node.as_str()
                ));
    ...
    let invalid_support: Vec<String> = probe
        .seed_support_definition_deps
        .keys()
        .filter_map(|node| {
            if !seed_support_definition_nodes.contains(node) {
                Some(format!("{} (not authenticated)", node.as_str()))
            } else if present_nodes.contains(node) {
                Some(format!("{} (also an ordinary present node)", node.as_str()))
            } else {
                None
            }
        })
    ...
    if !invalid_support.is_empty() {
        probe.errors.push(format!(
            "local-closure probe contains invalid seed support definition dependencies: {invalid_support:?}"
        ));
        probe.status = "internal_error".to_string();
    }
...
/// One-shot, LOGGED load-time repair of a `Decide` pair's LIVE side whose
/// structural registration was lost by the pre-fix `apply_decide_polarity_flip`
/// (audit round 2, B2).
///
/// `apply_decide_polarity_flip` used to promote a node into
/// `live.present_nodes` without writing `node_kinds` or `challenge_claims`.
/// Nothing backfills that after the fact: `normalize_challenge_claims` is
/// retain-only, and the fixed registration code only runs on a FUTURE flip. A
/// pair stranded by a PAST flip therefore stays claim-less forever, and the
/// consequence is not merely wasted verifier cycles —
/// `apply_local_closure_probe_results` records the structured
/// `pv_refutation_findings` entry ("contract decided false, witness attached")
/// only when the closing node CLAIMS the refutation target. A stranded
/// refutation that closes cleanly therefore produces a run report with the
/// verdict silently missing: a defect in the campaign's deliverable.
///
/// Why a load-time migration and not a normalize-time repair: doing this
/// silently inside `normalize_live_structural_state` on every call would mask
/// FUTURE occurrences of the registration bug and defeat the `validate()`
/// backstop that now fails loud on a present Decide-pair node with the wrong
/// `node_kinds` value. A load-time migration heals legacy state once, loudly,
/// and leaves `validate()` free to reject anything new. This is the same
/// posture as `migrate_soundness_fingerprint_schema`.
///
/// Why this is re-derivation rather than guessing: for the live side of a
/// configured `Decide` pair, both the node kind and the claimed target are
/// deterministic functions of `configured_challenge_targets` — the kernel
/// SYNTHESISED the refutation statement from that same spec
/// (`refutation_statement`), and the mode-B config seed and the (fixed) flip
/// both derive them exactly this way. Same footing as the existing
/// `apply_pv_target_autoseed` for `target_claims`.
///
/// MATH-MODE NEUTRALITY: the whole body is inside a loop over
/// `configured_challenge_targets` filtered by `is_decide_primary`
/// (`kind == Theorem && resolution == Decide`). A non-PV run has an empty
/// registry, so the function returns `Ok(false)` without touching a single
/// field. There is no shared code path.
///
/// OPERATOR NOTE (logged): installing a challenge claim also re-arms
/// `challenge_conformance_errors`' byte-pin of the node's declaration at the
/// next worker acceptance. That is the intended contract for a Decide
/// refutation — it must prove exactly the kernel-synthesised `¬T` — but if the
/// stranded node's declaration drifted while unclaimed, the next worker result
/// will be rejected with a contract error naming the mismatch.
pub(crate) fn migrate_stranded_decide_registration(
    state: &mut crate::ProtocolState,
    repo_path: &Path,
) -> Result<bool, String> {
    // Never rewrite structural registration while ANY request is in flight.
    //
    // For a Worker burst the original reason stands: disk is mid-mutation and
    // the acceptance path is about to install a wholesale observation anyway
    // (mirrors `migrate_soundness_fingerprint_schema`).
    //
    // The guard is not Worker-only, because `validate()` requires the persisted
    // in-flight request to equal `expected_request` recomputed from the state
    // ("in-flight request payload does not match derived state"), and that
    // projection reads the very facts this migration writes: a challenge claim
    // drives the Substantiveness/NodeCorr waivers, and `node_kinds` /
    // `open_nodes` drive the verify frontiers and blockers. Repairing
    // underneath a request that was derived BEFORE the repair therefore
    // desynchronises the pair and fails the next validate() — which, since
    // round-2 follow-up 1 moved this migration ahead of the trust-required
    // load-time `validate()`, would be a hard load failure on an otherwise
    // healthy run. A mid-transaction state is not the moment to re-derive
    // registration; the repair is idempotent and lands at the next idle load.
    if state.in_flight_request.is_some() {
        return Ok(false);
    }
    let primaries: Vec<trellis_kernel::ChallengeTargetId> = state
    ...
    let mut kind_repaired: BTreeSet<NodeId> = BTreeSet::new();
    for primary in &primaries {
        ...
        let live_target = state.live_target_of_decide_pair(primary);
        let Some(spec) = state.configured_challenge_targets.get(&live_target) else {
            // Not repairable by re-derivation — there is no spec to derive
            // from. `validate()` rejects this state loudly (F2); say so here
            // so the operator sees the cause before the halt message. This
            // ordering is real, not aspirational: since round-2 follow-up 1
            // the migration runs inside `SupervisorRuntime::load` BEFORE
            // `reconcile_trust_journal_projection`'s `validate()`, so on a
            // trust-required run this line reaches stderr ahead of the halt.
            eprintln!(
                "trellis: decide-registration migration: node {node} is the live \
                 side of Decide pair {primary} but its challenge target \
                 {live_target} is NOT in the configured registry; cannot \
                 re-derive its registration"
            );
            continue;
        };
        ...
        if state.node_kinds.get(&node) != Some(&expected_kind) {
            let before = state.node_kinds.get(&node).copied();
            state.node_kinds.insert(node.clone(), expected_kind);
            kind_repaired.insert(node.clone());
            repairs.push(format!(
                "node_kinds[{node}]: {before:?} -> {expected_kind:?} \
                 (from challenge target {live_target}, kind {:?})",
                spec.kind
            ));
        }
        // Claim backfill, guarded on the target being UNCLAIMED. If some node
        // already claims this target, the registry is not "stranded" and a
        // second claimant would be a new, unreviewed fact — leave it to
        // `validate()` / the verifier lanes rather than inventing coverage.
        let existing_claimant = state
            .challenge_claims
            .iter()
            .find(|(_, targets)| targets.contains(&live_target))
            .map(|(claimant, _)| claimant.clone());
        match existing_claimant {
            // Some OTHER node already claims the live side's target. Do not
            // add a second claimant (a challenge claim is EXCLUSIVE — see
            // `challenge_conformance_errors`), but do not pass over it in
            // silence either: a Decide pair whose live target is claimed by a
            // node that is not the live side is a shape the operator has to
            // see. Same logging posture as the unconfigured-target branch
            // above (round-2 follow-up 4).
            Some(claimant) if claimant != node => {
                eprintln!(
                    "trellis: decide-registration migration: NOT backfilling \
                     challenge_claims[{node}] — challenge target {live_target} \
                     (the live side of Decide pair {primary}) is already claimed \
                     by {claimant}; a challenge claim is exclusive, so the \
                     recorded claim is left as-is for validate()/the verifier \
                     lanes to judge"
                );
            }
            // Already claimed by this very node: nothing stranded, nothing to say.
            Some(_) => {}
            None => {
                if !state.challenge_claims.contains_key(&node) {
                    state
                        .challenge_claims
                        .insert(node.clone(), BTreeSet::from([live_target.clone()]));
                    repairs.push(format!(
                        "challenge_claims[{node}]: (absent) -> {{{live_target}}} — this \
                         re-arms the Substantiveness/NodeCorr waivers AND the \
                         challenge_conformance_errors byte-pin of {node}'s declaration"
                    ));
                }
            }
        }
    ...
    // A node that just became proof-bearing has no closure-tier standing and
    // its openness was never observed under the correct kind. Re-derive
    // openness from the node's actual source file — authoritative, and the
    // same fact `open_nodes_from_repo` would produce at the next observation.
    // Without it, a repaired sorry-bodied node is a present proof_node outside
    // `open_nodes`, which (a) reads soundness-clean via the synthetic
    // `VerifierPass` in `current_sound_assessment` and (b) trips `validate()`'s
    // closure coverage invariant outright once the closure tier is active.
    for node in &kind_repaired {
        let one = BTreeSet::from([node.clone()]);
        if trellis_kernel::open_nodes_from_repo(repo_path, &one).contains(node) {
            if state.live.open_nodes.insert(node.clone()) {
                repairs.push(format!(
                    "live.open_nodes: + {node} (re-derived from {node}'s source \
                     file, which still carries an open marker)"
                ));
            }
        }
    }
    // `proof_nodes` is derived from `node_kinds` — recompute it (and the rest
    // of the structural projections) rather than hand-patching.
    state.normalize_all_structural_state();
    state.ensure_node_metadata();
    eprintln!(
        "==============================================================\n\
         trellis: DECIDE-REGISTRATION MIGRATION (audit round 2, B2)\n\
         Repaired {} stranded registration fact(s) on the live side of a\n\
         configured Decide pair. These were lost by a polarity flip that ran\n\
         before the registration fix; nothing else would ever backfill them,\n\
         and an unclaimed refutation that closes records NO refutation finding\n\
         in the run report.\n{}\n\
         ==============================================================",
        repairs.len(),
        repairs
...
pub(crate) fn extract_tex_proof_block(tex_content: &str) -> String {
...
fn parse_print_axioms_output(output: &str) -> Option<Vec<String>> {
    ...
    // FORGERY GATE (2026-07-31 audit, demonstrated). This parser reads the
    // STDOUT of `import Tablet.<Node>` + `#print axioms <Node>`, and the
    // import runs any `initialize` block the module carries — including one
    // an untrusted grunt wrote into the proof body. That block can print
    // whatever it likes BEFORE Lean's real answer, and the old code
    // short-circuited on the first "does not depend on any axioms" it saw,
    // so the forgery won and the real verdict was never reached. The audit's
    // reproduction emitted both lines and this function returned `[]`.
    //
    // Anchoring on the node name does not help — the forged line carries it
    // too. What a forgery cannot do is REMOVE Lean's own line, so the tell
    // is that two verdicts appear where a clean run has exactly one.
    //
    // `None` is the caller's fail-closed signal (`command_ok` false ->
    // `axioms_valid` false), so an ambiguous audit rejects the attempt
    // rather than believing either line.
    let clean_marker = "does not depend on any axioms";
    let deps_marker = "depends on axioms:";
    // "does not depend on any axioms" does not contain "depends on axioms:",
    // so these counts are independent.
    let verdicts = normalized.matches(clean_marker).count()
        + normalized.matches(deps_marker).count();
    if verdicts > 1 {
        return None;
    }
    if normalized.contains(clean_marker) {
        return Some(Vec::new());
    }
...
pub(crate) fn proof_worker_delta_step_result_with_under_model_assumptions(
        ...
        && !active_is_definition_kind
    {
        ...
        validate_probe_present_nodes_with_seed_support(
            &mut local,
            &augmented_present_nodes,
            &augmented_node_kinds,
...
mod tests {
    ...
    // ---------------------------------------------------------------
    // Audit round 2, B2 — stranded Decide-registration load migration.
    // ---------------------------------------------------------------

    /// A configured `Decide` pair whose live polarity is `Disprove`, i.e. the
    /// `<Name>__Refutation` node is the live side.
    fn stranded_decide_state() -> crate::ProtocolState {
        ...
        state.configured_challenge_targets.insert(
            ...
            trellis_kernel::ChallengeTargetSpec {
                ...
                name: "Correct".into(),
                lean: "theorem Correct : True :=".into(),
        ...
        state.configured_challenge_targets.insert(
            ...
            trellis_kernel::ChallengeTargetSpec {
                ...
                lean: "theorem Correct__Refutation : ¬ True :=".into(),
                ..trellis_kernel::ChallengeTargetSpec::default()
            },
        );
        state
        ...
        state.deps.insert(node, BTreeSet::new());
        state
    }

    /// The campaign shape (`dec2flt_total__Refutation`,
    /// `parse_number_faithful__Refutation`): a past polarity flip left the live
    /// refutation present and proof-bearing but with NO `challenge_claims`
    /// entry, and nothing backfills it — `normalize_challenge_claims` is
    /// retain-only. Without the claim, `apply_local_closure_probe_results`
    /// records no `pv_refutation_findings` entry when the node closes, so the
    /// run report silently omits the "contract decided false" verdict.
    #[test]
    fn decide_registration_migration_backfills_stranded_challenge_claim() {
        let dir = tempdir().unwrap();
        let repo = dir.path();
        write_node(
            repo,
            ...
            "-- [TABLET NODE: Correct__Refutation]\ntheorem Correct__Refutation : ¬ True := by\n  simp\n",
        );
        let node = NodeId::from("Correct__Refutation");
        let refutation = trellis_kernel::refutation_target_id(
            &trellis_kernel::ChallengeTargetId::from("goal:correct"),
        );
        let mut state = stranded_decide_state();
        state.node_kinds.insert(node.clone(), NodeKind::Proof);
        state.proof_nodes.insert(node.clone());
        assert!(
            !state.challenge_claims.contains_key(&node),
            "fixture precondition: the claim is stranded"
        );

        let changed =
            migrate_stranded_decide_registration(&mut state, repo).expect("migration runs");

        assert!(changed, "a stranded claim must be repaired");
        assert_eq!(
            state.challenge_claims.get(&node),
            Some(&BTreeSet::from([refutation])),
            "the live refutation must claim its own configured target"
        );
        // Idempotent: a second load repairs nothing and reports no change.
        assert!(
            !migrate_stranded_decide_registration(&mut state, repo).expect("second run"),
            "the migration must be a one-shot repair, not a per-load rewrite"
        );
    }

    /// The other half of the stranded shape: the flip wrote no `node_kinds`
    /// entry at all, so `effective_node_kinds` materialised the `Definition`
    /// default and `proof_nodes_from_kind_map` dropped the theorem for good.
    /// The migration re-derives the kind from the configured spec, and
    /// re-derives openness from the node's own source file so a repaired
    /// `sorry`-bodied node does not read soundness-clean through
    /// `current_sound_assessment`'s synthetic `VerifierPass`.
    #[test]
    fn decide_registration_migration_repairs_wrong_node_kind_and_reopens() {
        let dir = tempdir().unwrap();
        let repo = dir.path();
        write_node(
            repo,
        ...
        let node = NodeId::from("Correct__Refutation");
        let mut state = stranded_decide_state();
        // The incident value: the materialised `Definition` default.
        ...
        assert!(changed);
        assert_eq!(state.node_kinds.get(&node), Some(&NodeKind::Proof));
        assert!(
            state.proof_nodes.contains(&node),
            "the repaired kind must flow through to proof_nodes"
        );
        assert!(
            state.live.open_nodes.contains(&node),
            "the repaired node's body is `sorry`; openness must be re-derived"
        );
        assert!(state.needs_sound(&node), "and it must be sound-eligible");
        state
            .validate()
            .expect("the repaired state must satisfy the decide-pair invariant");
    }

    /// Audit round 2, follow-up 4 — when the live side's challenge target is
    /// already claimed by a DIFFERENT node the migration must not invent a
    /// second claimant (a challenge claim is exclusive), and must say so rather
    /// than skipping in silence. The state is left byte-identical.
    #[test]
    fn decide_registration_migration_skips_and_reports_a_foreign_claimant() {
        let dir = tempdir().unwrap();
        let repo = dir.path();
        write_node(
            repo,
        ...
        let mut state = stranded_decide_state();
        // The kind is already right, so the claim is the only candidate repair.
        state.node_kinds.insert(node.clone(), NodeKind::Proof);
        state.proof_nodes.insert(node.clone());
        // Some other present node already claims the live target.
        let squatter = NodeId::from("Squatter");
        state.live.present_nodes.insert(squatter.clone());
        state
            .challenge_claims
            .insert(squatter, BTreeSet::from([refutation]));
        let before = serde_json::to_string(&state).unwrap();

        assert!(
            !migrate_stranded_decide_registration(&mut state, repo).expect("migration runs"),
            "a foreign claimant is not a strand the migration may repair"
        );
        assert_eq!(
            before,
            serde_json::to_string(&state).unwrap(),
            "the recorded exclusive claim must be left exactly as it was"
        );
        assert!(
            !state.challenge_claims.contains_key(&node),
            "no second claimant may be invented"
        );
    }

    /// MATH-MODE NEUTRALITY. A state with no configured challenge targets — an
    /// ordinary math run — must come out of the migration byte-identical, with
    /// `changed = false` so nothing is even persisted.
    #[test]
    fn decide_registration_migration_is_inert_for_math_mode() {
        let dir = tempdir().unwrap();
        let repo = dir.path();
        write_node(
            repo,
        ...
        state.proof_nodes.insert(node.clone());
        state.node_kinds.insert(node.clone(), NodeKind::Proof);
        // Deliberately claim-less and NOT in `open_nodes` — the exact two
        // shapes the migration repairs for a Decide pair. A math run must be
        // left alone anyway: there is no configured pair to derive from.
        let before = serde_json::to_string(&state).unwrap();

        let changed =
            migrate_stranded_decide_registration(&mut state, repo).expect("migration runs");

        assert!(!changed, "math mode must report no change");
        assert_eq!(
            before,
            serde_json::to_string(&state).unwrap(),
            "math-mode state must be byte-identical across the migration"
        );
    }

    /// A non-Decide PV run (challenge targets configured, but `Prove`-only
    /// resolution) is equally untouched: `is_decide_primary` requires
    /// `resolution == Decide`.
    #[test]
    fn decide_registration_migration_is_inert_for_non_decide_challenge_targets() {
        let dir = tempdir().unwrap();
        let repo = dir.path();
        write_node(
            repo,
        ...
        assert!(!migrate_stranded_decide_registration(&mut state, repo).expect("migration runs"));
        assert_eq!(before, serde_json::to_string(&state).unwrap());
    }

    /// Round-2 follow-up 1, safety half: the guard is not Worker-only. Any
    /// in-flight request defers the repair, because `validate()` pins the
    /// persisted request against `expected_request` recomputed from the state,
    /// and that projection reads exactly the facts this migration writes
    /// (claims drive waivers; kinds/openness drive the frontiers). Repairing
    /// under a Review request would desync the pair and — now that the
    /// migration runs ahead of the trust-required load-time `validate()` —
    /// convert a healthy resume into a hard load failure.
    #[test]
    fn decide_registration_migration_defers_under_any_inflight_request() {
        let dir = tempdir().unwrap();
        let repo = dir.path();
        write_node(
            repo,
        ...
        state
            .node_kinds
            .insert(NodeId::from("Correct__Refutation"), NodeKind::Definition);
        state.in_flight_request = Some(trellis_kernel::WrapperRequest {
            id: 1,
            kind: crate::RequestKind::Review,
            ..trellis_kernel::WrapperRequest::default()
        });
        let before = serde_json::to_string(&state).unwrap();
    ...
    /// The migration must not rewrite structural registration under a live
    /// worker burst (disk is mid-mutation; acceptance installs a wholesale
    /// observation anyway). Same guard as the fingerprint migrations.
    #[test]
    fn decide_registration_migration_defers_under_inflight_worker() {
        let dir = tempdir().unwrap();
        let repo = dir.path();
        write_node(
            repo,
...
"#,
    ...
    #[test]
    fn validate_probe_routes_authenticated_seed_support_definition_only() {
        let envelope = serde_json::json!({
            "status": "ok",
            "kernel_axioms": [],
            "boundary_theorems": [],
            "strict_theorem_deps": [],
            "strict_definition_deps": [
                {"name": "Tablet.Assumptions", "semantic_hash": "support-semantic"},
            ],
            "errors": [],
        });
        let mut probe = parse_local_closure_response("Active", envelope).expect("parse envelope");
        let present_nodes = BTreeSet::from([NodeId::from("Active")]);
        let node_kinds = BTreeMap::from([(NodeId::from("Active"), crate::NodeKind::Proof)]);
        let support = BTreeSet::from([NodeId::from("Assumptions")]);
        ...
        assert_eq!(probe.status, "ok", "errors: {:?}", probe.errors);
        assert!(!probe.strict_definition_deps.contains_key("Assumptions"));
        assert_eq!(
            probe.seed_support_definition_deps.get("Assumptions"),
            Some(&"support-semantic".to_owned())
        );
    }

    #[test]
    fn validate_probe_rejects_support_name_without_authenticated_support_context() {
        let envelope = serde_json::json!({
            "status": "ok",
            "kernel_axioms": [],
            "boundary_theorems": [],
        ...
        validate_probe_present_nodes(
            &mut probe,
            &BTreeSet::from([NodeId::from("Active")]),
            &BTreeMap::from([(NodeId::from("Active"), crate::NodeKind::Proof)]),
        );
        assert_eq!(probe.status, "internal_error");
        assert!(probe.errors.iter().any(|error| {
            error.contains("strict_definition_deps=Assumptions")
                && error.contains("Patch C-K")
        }));
    }

    #[test]
    fn validate_probe_does_not_admit_seed_support_as_theorem_dependency() {
        let envelope = serde_json::json!({
            "status": "ok",
            "kernel_axioms": [],
            "boundary_theorems": [
                {"name": "Tablet.Assumptions", "statement_hash": "wrong-category"},
            ],
            "strict_theorem_deps": [],
            "strict_definition_deps": [],
            "errors": [],
        ...
        validate_probe_present_nodes_with_seed_support(
            ...
            &BTreeSet::from([NodeId::from("Active")]),
            &BTreeMap::from([(NodeId::from("Active"), crate::NodeKind::Proof)]),
            &BTreeSet::from([NodeId::from("Assumptions")]),
        );
        assert_eq!(probe.status, "internal_error");
        assert!(probe.errors.iter().any(|error| {
            error.contains("boundary_theorems=Assumptions") && error.contains("Patch C-K")
        }));
    }
