...
use trellis_kernel::{
    ...
    LocalClosureProbeOutput, NodeId, NodeKind, SoundFingerprintParts, TargetId,
...
///    (= reviewer `authorized_nodes ∩ revision_context.editable_nodes`);
...
pub(crate) fn revision_statement_edit_scope_step_result(
    ...
    if !out_of_scope.is_empty() {
        ...
        errors.push(format!(
            "Out-of-scope revision edits to existing nodes: {}. In RevisionStating a modified existing node must be in the reviewer-authorized editable set (authorized_nodes ∩ editable_nodes): {}.",
            out_of_scope.join(", "),
