...
/// design-gap CLASS out of the fail-loudly halt: recurrences of that
/// exact fingerprint then log-and-continue instead of freezing the run.
/// Absent / unparseable file ⇒ empty set ⇒ every emission still halts
/// (fail-closed default — the ack policy is a pure superset of today's
/// behavior).
pub const SYSTEM_FEEDBACK_ACK_STORE_FILENAME: &str = "system_feedback_acks.json";

/// Filename of the append-only log recording every acknowledged (and
/// therefore halt-suppressed) `system_feedback` emission. Acknowledged
/// gaps are still a signal worth auditing — they are logged here with
/// full text + fingerprint + cycle + request_id so a human retains a
/// durable record, they just no longer freeze the run.
pub const SYSTEM_FEEDBACK_DESIGN_GAP_LOG_FILENAME: &str = "system_feedback_design_gap_log.jsonl";
...
/// Runtime-root path of the acknowledged-gap audit log (JSONL).
pub fn system_feedback_design_gap_log_path() -> Option<PathBuf> {
    let raw = std::env::var(trellis_kernel::disk_cache::KERNEL_CACHE_ROOT_ENV).ok()?;
    let trimmed = raw.trim();
    if trimmed.is_empty() {
        return None;
    }
    Some(PathBuf::from(trimmed).join(SYSTEM_FEEDBACK_DESIGN_GAP_LOG_FILENAME))
}
...
/// Append one JSONL record for an acknowledged (halt-suppressed)
/// `system_feedback` emission. Best-effort: a failed write is logged to
/// stderr but never propagated (the design-gap log is a forensic layer,
/// not an authoritative one).
#[allow(clippy::too_many_arguments)]
fn append_system_feedback_design_gap_log(
    active_node: &str,
    ...
    let Some(path) = system_feedback_design_gap_log_path() else {
        return;
    };
    let ts = std::time::SystemTime::now()
        .duration_since(std::time::UNIX_EPOCH)
    ...
    let record = serde_json::json!({
        "schema_version": 1,
        "kind": "system_feedback_acknowledged",
        "unix_ts": ts,
    ...
    {
        eprintln!(
            "trellis: failed to append system_feedback design-gap log at {}: {err}",
            path.display()
        );
    }
}
...
pub fn write_system_feedback_halt_marker(
    ...
    // CONSERVATIVE ack policy — this is the ONLY branch that diverges from
    // the historical fail-loudly behavior, and it can only fire when an
    // operator has explicitly acknowledged THIS fingerprint. With the
    // default (no ack store on disk / empty store), `is_acknowledged`
    // always returns false and control falls through to the byte-for-byte
    // original halt path below: a pure superset.
    let ack_store = load_system_feedback_ack_store();
    if ack_store.is_acknowledged(&fingerprint) {
        append_system_feedback_design_gap_log(
            active_node,
            active_coarse_node,
            cycle,
            request_id,
        ...
        eprintln!(
            "trellis: system_feedback fingerprint {fingerprint} is ACKNOWLEDGED — \
             recording to design-gap log and CONTINUING (no halt). \
...
"#,
    ...
    fn system_feedback_unacked_writes_marker_with_fingerprint() {
        ...
        // No design-gap log for an unacked (halting) emission.
        assert!(!tmp
            .path()
            .join(SYSTEM_FEEDBACK_DESIGN_GAP_LOG_FILENAME)
            .exists());
    }

    /// Acked path: a fingerprint present in the store suppresses the halt
    /// marker, appends the full feedback to the design-gap log, and the
    /// function returns without freezing the run.
    #[test]
    fn system_feedback_acked_suppresses_marker_and_logs_gap() {
        ...
        let log = tmp.path().join(SYSTEM_FEEDBACK_DESIGN_GAP_LOG_FILENAME);
        assert!(log.exists(), "acked feedback MUST be recorded in the design-gap log");
        let line = std::fs::read_to_string(&log).unwrap();
        let rec: serde_json::Value = serde_json::from_str(line.trim()).unwrap();
        assert_eq!(rec["kind"], "system_feedback_acknowledged");
