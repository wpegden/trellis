...
pub struct RawCorrPayload {
    ...
    #[serde(default)]
    pub rust_witness_artifact:
        Option<crate::trust_base::RustWitnessCorrespondenceLaneVerdict>,
    #[serde(default)]
    pub conditional_theorem:
        Option<crate::trust_base::ConditionalCorrespondenceVerdict>,
    pub overall: String,
    pub summary: String,
...
pub struct CorrNormalizationInput {
    ...
    #[serde(default)]
    pub rust_witness_artifact_correspondence:
        Option<crate::trust_base::RustWitnessCorrespondenceRequest>,
    #[serde(default)]
    pub conditional_theorem_correspondence:
        Option<crate::trust_base::ConditionalCorrespondenceRequest>,
...
/// Each verdict carries an explicit typed outcome and an optional `comment`
/// (required for Fail and FalseAsStated, validated by
...
/// `issues[]`, with explicit typed per-node outcomes.
#[derive(Clone, Debug, Default, PartialEq, Eq, Serialize, Deserialize)]
#[serde(default)]
...
pub fn normalize_corr_response(
    ...
    if input.verify_nodes.is_empty()
        ...
        && input.rust_witness_artifact_correspondence.is_none()
        && input.conditional_theorem_correspondence.is_none()
    {
    ...
    let artifact_verdict = if let Some(request) = &input.rust_witness_artifact_correspondence {
        let mut agreed = None;
        for lane in &input.verify_lanes {
            let candidate = members
                .get(lane)
                .and_then(|member| member.payload.as_ref())
                .and_then(|payload| payload.rust_witness_artifact.as_ref())
                .ok_or_else(|| format!("correspondence lane {lane} omitted its Rust witness verdict"))?;
            if candidate.request_sha256 != request.request_sha256
                || candidate.reviewed_digests != request.reviewed_digests
                || candidate.reason.trim().is_empty()
            {
                return Err(format!(
                    "correspondence lane {lane} did not echo the artifact request binding"
                ));
            }
            if agreed.as_ref().is_some_and(|prior| prior != candidate) {
                return Err("correspondence lanes disagreed on the Rust witness verdict".into());
            }
            agreed = Some(candidate.clone());
        }
        Some(
            crate::trust_base::close_rust_witness_correspondence_verdict(
                &agreed.ok_or_else(|| "artifact correspondence has no configured lane".to_owned())?,
            )
    ...
    let conditional_verdict = if let Some(request) = &input.conditional_theorem_correspondence {
        let mut agreed = None;
        for lane in &input.verify_lanes {
            let candidate = members
                .get(lane)
                .and_then(|member| member.payload.as_ref())
                .and_then(|payload| payload.conditional_theorem.as_ref())
                .ok_or_else(|| format!("correspondence lane {lane} omitted its conditional theorem verdict"))?;
            if candidate.request_sha256 != request.request_sha256
                || candidate.proposal_sha256 != request.proposal.proposal_sha256
                || candidate.findings.trim().is_empty()
            {
                return Err(format!(
                    "correspondence lane {lane} did not echo the conditional request binding"
                ));
            }
            if agreed.as_ref().is_some_and(|prior| prior != candidate) {
                return Err("correspondence lanes disagreed on the conditional theorem verdict".into());
            }
            agreed = Some(candidate.clone());
        }
        Some(agreed.ok_or_else(|| "conditional correspondence has no configured lane".to_owned())?)
    } else {
        None
    };
    let mut node_lane_updates: CorrNodeLaneUpdates = BTreeMap::new();
    let mut target_lane_updates: CorrTargetLaneUpdates = BTreeMap::new();
    // Corr reviewer evidence is keyed per node, then per lane (mirroring the
    // sound lane). A single corr lane verdict spans every node in
    // `verify_nodes`, so each lane contributes one per-node evidence entry
    // per verified node, with `issues[]` filtered to that node. This lets a
    // Review request attach only the active node's corr findings rather than
    // a multi-node aggregate.
    let mut reviewer_evidence: BTreeMap<NodeId, BTreeMap<LaneId, CorrReviewerLaneEvidence>> =
        BTreeMap::new();
    ...
    for lane in &input.verify_lanes {
        ...
        // Split the lane's aggregate evidence into one per-node entry per
        // verified node. Each entry carries only its own node's `issues[]`
        // (none if that node passed) plus the lane-level decision/summary/
        // comments, and is self-labeled with `node` so downstream bucketing
        // and node-scoped Review presentation can never cross wires.
        let issues_by_node: BTreeMap<NodeId, Vec<VerifierIssue>> = {
            let mut map: BTreeMap<NodeId, Vec<VerifierIssue>> = BTreeMap::new();
            for issue in &synthesized_issues {
                map.entry(issue.node.clone())
                    .or_default()
                    .push(issue.clone());
            }
            map
        };
        // Emit an entry for every verified node, plus any node that picked up
        // a synthesized issue but is not itself in `verify_nodes` (e.g. the
        // `Preamble` rollup), so no finding is silently dropped.
        let mut evidence_nodes: BTreeSet<NodeId> =
            input.verify_nodes.iter().cloned().collect();
        evidence_nodes.extend(issues_by_node.keys().cloned());
                ...
                .insert(
                    ...
                    CorrReviewerLaneEvidence {
                        ...
                        correspondence: CorrReviewerPhaseEvidence {
                            ...
                            issues: issues_by_node.get(node).cloned().unwrap_or_default(),
                        },
    ...
    Ok(CorrNormalizationOutput {
        response: CorrResponse {
            ...
            target_lane_updates,
            reviewer_evidence,
            rust_witness_artifact_correspondence: artifact_verdict,
            conditional_theorem_correspondence: conditional_verdict,
        },
    })
}
        ...
        if is_per_node_scenario {
            ...
            for verdict in &payload.substantiveness.verdicts {
                ...
                let status = match verdict.verdict.trim() {
                    ...
                    "FalseAsStated" => SubstantivenessStatus::FalseAsStated,
...
mod tests {
    ...
    #[test]
    fn artifact_corr_requires_identical_request_bound_lane_verdicts() {
        let digest = crate::trust_base::raw_sha256(b"artifact-corr-binding");
        ...
        let request = crate::trust_base::RustWitnessCorrespondenceRequest {
            schema: crate::trust_base::RUST_WITNESS_CORRESPONDENCE_REQUEST_SCHEMA.into(),
            target_id: crate::model::ChallengeTargetId::from("goal:generic"),
            goal_target_prose_utf8: "generic goal target".into(),
            ...
            reviewed_digests: reviewed.clone(),
            request_sha256: digest,
        };
        let lane_verdict = crate::trust_base::RustWitnessCorrespondenceLaneVerdict {
            ...
            decision: crate::trust_base::RustWitnessCorrespondenceDecision::Pass,
            reason: "independent generic correspondence judgment".into(),
        };
        let member = |lane: &str, verdict| CorrLaneMemberInput {
            lane_id: lane.into(),
            ok: true,
            payload: Some(RawCorrPayload {
                rust_witness_artifact: Some(verdict),
                overall: "APPROVE".into(),
                summary: "artifact correspondence reviewed".into(),
                ..RawCorrPayload::default()
            }),
            error: String::new(),
        };
        let input = CorrNormalizationInput {
            request_id: 4,
            cycle: 2,
            verify_lanes: BTreeSet::from(["v1".into(), "v2".into()]),
            verify_nodes: BTreeSet::new(),
            verify_targets: BTreeSet::new(),
            rust_witness_artifact_correspondence: Some(request),
            conditional_theorem_correspondence: None,
            members: vec![
                member("v1", lane_verdict.clone()),
                member("v2", lane_verdict.clone()),
            ],
        };
        let output = normalize_corr_response(&input).unwrap();
        assert!(output
            .response
        ...
        mismatched.members[1]
            ...
            .as_mut()
            .unwrap()
            .request_sha256 = crate::trust_base::raw_sha256(b"mismatch");
        assert!(normalize_corr_response(&mismatched).is_err());
    }
    ...
    fn normalize_corr_response_silent_node_defaults_to_fail() {
        ...
        // Evidence is now keyed per node, then per lane. Each verified node
        // gets its own entry carrying only its own synthesized issue(s):
        // alpha passed (no issue), beta and gamma each surface a single
        // default-Fail "did not emit a comment" issue self-labeled with the
        // owning node.
        let evidence = &output.response.reviewer_evidence;
        assert_eq!(
        ...
        assert!(evidence[&NodeId::from("alpha")]["v1"]
            .correspondence
            .issues
            .is_empty());
        for node in [NodeId::from("beta"), NodeId::from("gamma")] {
            let lane_ev = &evidence[&node]["v1"];
            assert_eq!(lane_ev.node, node);
            assert_eq!(lane_ev.correspondence.issues.len(), 1);
            assert_eq!(lane_ev.correspondence.issues[0].node, node);
        }
    }
    ...
    #[test]
    fn normalize_paper_response_preserves_false_as_stated() {
        let output = normalize_paper_response(&PaperNormalizationInput {
            request_id: 12,
            cycle: 6,
            verify_lanes: BTreeSet::from(["v1".to_string()]),
            verify_targets: BTreeSet::new(),
            verify_nodes: BTreeSet::from([NodeId::from("GoalStmt")]),
            verify_deviations: BTreeSet::new(),
            members: vec![PaperLaneMemberInput {
                lane_id: "v1".to_string(),
                ok: true,
                payload: Some(RawPaperLanePayload {
                    substantiveness: RawSubstantivenessPhasePayload {
                        ...
                        verdicts: vec![RawSubstantivenessVerdict {
                            node: "GoalStmt".to_string(),
                            verdict: "FalseAsStated".to_string(),
                            comment: "pinned-model counterexample".to_string(),
                        }],
                    },
                    overall: "APPROVE".to_string(),
                    summary: "false intended obligation".to_string(),
                    ..RawPaperLanePayload::default()
                }),
                ..PaperLaneMemberInput::default()
            }],
        ...
        assert_eq!(
            serde_json::to_value(&output.response.node_lane_updates["v1"]["GoalStmt"])
                .unwrap(),
            serde_json::json!({"Set": "FalseAsStated"})
        );
        assert!(output
            .response
            .node_reviewer_evidence
            .contains_key("GoalStmt"));
    }
    ...
    fn normalize_corr_response_maps_preamble_item_failures_to_preamble() {
        ...
        // The synthesized issue list (now under the per-node Preamble
        // evidence entry) should reflect the rolled-up Preamble entry, not
        // the raw `Preamble[1]` sub-item key.
        let issues = &output.response.reviewer_evidence[&NodeId::from(PREAMBLE_NAME)]["v1"]
            .correspondence
            .issues;
