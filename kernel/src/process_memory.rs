...
/// True iff `process-memory/` holds at least one ACTIVE entry.
///
/// The advertisement predicate for `memory_challenges` (spec §5): the
/// challenge channel is meaningful exactly when there is settled memory to
/// challenge. Mirrors the bridge's `process_memory_block`, which renders ""
/// (and so drops out of the prompt) under the same condition, so a run
/// without active memory keeps byte-identical prompts and contracts.
/// Short-circuits on the first active entry.
pub fn has_active_entries(repo_path: &Path) -> bool {
    ...
    for cone in cones.flatten() {
        ...
        let Ok(files) = fs::read_dir(&cone_path) else {
            continue;
        };
        for file in files.flatten() {
            ...
            if parse_entry(&text).is_some_and(|(fm, _)| fm.status == "active") {
                return true;
            }
        }
    }
...
mod tests {
    ...
    #[test]
    fn has_active_entries_tracks_the_advertisement_predicate() {
        // The `memory_challenges` advertisement gate: false on a repo with
        // no `process-memory/` at all (pre-migration runs render
        // byte-identically), true once an active entry exists, false again
        // once every entry is retired — mirroring the bridge's
        // `process_memory_block`, which renders "" under the same
        // conditions.
        let dir = temp_repo();
        let repo = dir.path();
        assert!(
            !has_active_entries(repo),
            "no process-memory/ directory ⇒ nothing to challenge"
        );

        apply_file_ops(repo, &[add_op("pm-0001-route-x", "ConeA")]).unwrap();
        assert!(has_active_entries(repo), "one active entry ⇒ advertise");

        apply_file_ops(
            repo,
            &[ProcessMemoryFileOp::Retire {
                entry_id: "pm-0001-route-x".to_string(),
                reason: "superseded by the paper".to_string(),
                cycle: 9,
                request_id: 42,
            }],
        )
        ...
        assert!(
            !has_active_entries(repo),
            "all entries retired ⇒ back to no advertisement"
        );
    }
