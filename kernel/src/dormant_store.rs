...
pub fn validate_configured_decide_layout(
    ...
    for primary in state.configured_challenge_targets.keys() {
        ...
        // Prose audit repair (finding 1): an UNBOUND WorkerAuthored pair has
        // no materialized pair nodes to validate — its skeleton `name` is
        // empty until `StatementBound` patches it, so the resolved node names
        // below would be the degenerate `""`/`"__Refutation"`. The claimant
        // statement node the worker authors pre-binding is an ordinary
        // Tablet node policed by ordinary acceptance (the W4.4 negability
        // lint + FILESPEC), not a pair layout. The exemption is EXACTLY the
        // unbound window: the moment the binding latches,
        // `pv_statement_unbound` is false and the pair is validated below
        // like any seed-pinned pair (`StatementBound` seeds the dormant twin
        // in the same step, before this gate runs on the post-binding state).
...
pub fn recover_interrupted_configured_decide_flips(
    ...
    for primary in state.configured_challenge_targets.keys() {
        ...
        // Prose audit repair (finding 1): no flip can ever have been applied
        // to an UNBOUND WorkerAuthored pair (`statement_unbound` bounces
        // every flip pre-binding), so there is no interrupted flip to
        // recover — and the unbound skeleton's node names are degenerate
        // (`""`/`"__Refutation"`). Skip, mirroring the layout validator.
        if state.pv_statement_unbound(primary) {
            continue;
        }
...
/// into `Dormant/`, so a later polarity flip has a file to promote into
...
///
/// Callers: mode-B config init (`runtime_cli` seed), the prose
/// `StatementBound` step (via `ProtocolCommand::SeedDormantRefutationFile`,
/// executed BEFORE the post-event Decide layout gate — audit finding 1), and
/// the CLI's post-binding self-heal sweep.
///
...
pub fn seed_dormant_refutation_file(
    repo_path: &Path,
    ...
    let dormant_dir = dormant_dir(repo_path);
    ...
    let dormant_src = render_dormant_refutation_source(
        &primary_src,
        &primary_lean_path,
...
/// Replace a dormant authored refutation after the live primary's effective
/// Lean context changed. Unlike the idempotent initial seeder, this
/// deliberately resets the twin's proof body to `sorry`: any old proof was
/// elaborated under the stale import/open authority and cannot be promoted as
/// verification of the refreshed declaration. The primary's current FREE
/// region and the kernel-derived statement/gloss are the only inputs.
pub fn refresh_dormant_refutation_file(
    repo_path: &Path,
    primary_node_stem: &str,
    refutation_node_name: &str,
    refutation_lean: &str,
    ...
    let primary_lean_path = repo_path
        .join("Tablet")
        .join(format!("{primary_node_stem}.lean"));
    let primary_src = fs::read_to_string(&primary_lean_path).map_err(|err| {
        format!(
            "PV dormant refresh: failed to read live primary {}: {err}",
    ...
    fs::create_dir_all(&dormant_dir)
        .map_err(|err| format!("failed to create {}: {err}", dormant_dir.display()))?;
    fs::write(&dormant_lean, dormant_src)
        .map_err(|err| format!("failed to refresh {}: {err}", dormant_lean.display()))?;
    fs::write(&dormant_tex, tex)
        .map_err(|err| format!("failed to refresh {}: {err}", dormant_tex.display()))?;
    Ok(())
}

fn render_dormant_refutation_source(
    primary_src: &str,
    primary_lean_path: &Path,
    refutation_node_name: &str,
    refutation_lean: &str,
