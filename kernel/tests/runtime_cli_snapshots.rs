...
#[test]
fn snapshot_validate_trellis_stuck_math_audit_result_minimal_assumptions_lane() {
    snap_case!(
        "validate_trellis_stuck_math_audit_result",
        "minimal_assumptions_lane"
    );
}
...
// ============================================================================
// add_paper_targets — error wire shapes. The action is fs-touching (runtime
// root + config), so the SUCCESS shape is pinned by
// tests/add_targets_mode.rs::cli_action_end_to_end against a real runtime
// root; these snapshots pin the deterministic error wire format. Error
// responses exit(1), hence the error-tolerant runner.
// ============================================================================

fn run_runtime_cli_expect_error(request_json: &Value) -> Value {
    let exe = env!("CARGO_BIN_EXE_trellis_runtime_cli");
    let mut child = Command::new(exe)
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
    ...
    assert!(
        !output.status.success(),
        "expected an error exit, got success:\n{}",
        String::from_utf8_lossy(&output.stdout)
    );
    serde_json::from_slice(&output.stdout).unwrap_or_else(|err| {
        panic!(
...
#[test]
fn snapshot_add_paper_targets_missing_config() {
    let request = load_fixture("add_paper_targets", "missing_config");
    let response = run_runtime_cli_expect_error(&request);
    insta::with_settings!({
        snapshot_path => "snapshots/runtime_cli_snapshots",
        prepend_module_to_snapshot => false,
    }, {
        insta::assert_json_snapshot!("add_paper_targets__missing_config", response);
    });
}

#[test]
fn snapshot_add_paper_targets_config_missing_repo_path() {
    let manifest = std::fs::canonicalize(env!("CARGO_MANIFEST_DIR")).expect("canonicalize");
    let config = manifest.join("tests/fixtures/add_targets/bad_config.json");
    let request = serde_json::json!({
        "action": "add_paper_targets",
        "root": "/nonexistent/trellis-add-targets/root",
        "config_path": config,
    });
    let response = run_runtime_cli_expect_error(&request);
    let manifest_pattern = escape_regex(&manifest.to_string_lossy());
    insta::with_settings!({
        snapshot_path => "snapshots/runtime_cli_snapshots",
        prepend_module_to_snapshot => false,
        filters => vec![(manifest_pattern.as_str(), "[KERNEL]")],
    }, {
        insta::assert_json_snapshot!("add_paper_targets__config_missing_repo_path", response);
    });
}
