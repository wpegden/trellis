...
use trellis_kernel::{
    CorrStatus, GateKind, LocalClosureRecord, NodeId, NodeKind, PendingTask, Phase, ProtocolState,
    RefPaperId, ReferencePaperSpec, RequestKind, RuntimeMetadata, RuntimePaths, Stage,
    SupervisorRuntime, TargetId, WorkerResponse,
};
...
fn clean_fixture() -> ProtocolState {
    ...
    for proof in state.proof_nodes.clone() {
        ...
        record.closure_version = "reference-papers-fixture-v1".to_string();
