...
use trellis_kernel::{
    apply_event, CorrStatus, LocalClosureRecord, NodeCertificateEvidence, NodeDifficulty, NodeId,
    NodeKind, ObservedDeclarationUse, Phase, ProtocolCommand, ProtocolEvent, ProtocolState,
    SidecarClosurePayload, Stage, TargetId, TransitionError,
};
...
fn set_exact_uses(record: &mut LocalClosureRecord, uses: &[(&str, &str)]) {
    record.node_certificate_evidence = Some(NodeCertificateEvidence {
        exact_declaration_uses: uses
            .iter()
            .map(|(owner, kind)| ObservedDeclarationUse {
                owner: node(owner),
                reached_declaration: format!("{owner}.generatedMember"),
...
fn apply_success_mutates_exactly_the_closure_surface() {
    ...
    // Keep this checkpoint non-clean so the separate LastClean recapture
    // behavior does not widen this closure-surface assertion.
    let other = node("Other");
    state.live.present_nodes.insert(other.clone());
    state.live.open_nodes.insert(other.clone());
    state.node_kinds.insert(other.clone(), NodeKind::Proof);
    state.proof_nodes.insert(other.clone());
    state.node_difficulty.insert(other.clone(), NodeDifficulty::Hard);
    state.easy_attempts.insert(other, 0);
    let outcome =
...
#[test]
fn apply_reasserts_an_exact_same_cycle_lane_drift_generation() {
    let mut state = eligible_state();
    state.cycle = 177;
    state
        .live
    ...
    let prune = state.sidecar_queue_prune_log.last().unwrap();
    assert_eq!((prune.entry_seq, prune.reason.as_str()), (7, "lane_drift"));

    // The check has settled by apply time. The SidecarClosure event itself is
    // proof that a completed closure is being ingested; the engine rebuilds
    // only generation 7 from its deterministic prune row, passes the normal
    // membership gate, and consumes it atomically with the close.
    state
        .live
        .corr_current_fingerprints
        .insert(node("Rung"), "c1".to_string());
    let applied = apply_event(state, sidecar_event("Rung"))
        .expect("the completed lane-drifted generation must survive");
    assert!(!applied.state.live.open_nodes.contains(&node("Rung")));
    assert!(applied.state.sidecar_queue.is_empty());
    assert_eq!(
        applied.state.sidecar_queue_prune_log.last().unwrap().reason,
        "lane_drift",
        "the forensic prune remains visible even though its closure recovered"
    );
}
...
/// Regression (live run, cycles 310/311) — a grunt closure whose
...
fn state_with_dep_bearing_neighbour() -> ProtocolState {
    let mut state = eligible_state();
    for (dep, kind) in [
        ("SharedHelper", NodeKind::Proof),
        ("SharedDef", NodeKind::Definition),
        ("K33Graph", NodeKind::Definition),
        ("ChromaticNumberIso", NodeKind::Proof),
        ("Helper", NodeKind::Proof),
        ("ThmDep", NodeKind::Proof),
        ("DefDep", NodeKind::Definition),
    ] {
        let dep = node(dep);
        state.live.present_nodes.insert(dep.clone());
        // Dependency fixtures are deliberately not closure owners under test.
        state.live.open_nodes.insert(dep.clone());
        state.node_kinds.insert(dep.clone(), kind);
        state
            .node_difficulty
            .insert(dep.clone(), NodeDifficulty::Hard);
        state.easy_attempts.insert(dep.clone(), 0);
        if kind == NodeKind::Proof {
            state.proof_nodes.insert(dep);
        }
    }
    ...
    set_exact_uses(
        &mut neighbour,
        &[("SharedHelper", "theorem"), ("SharedDef", "definition")],
    );
...
fn apply_keeps_strict_dep_consumers_in_sync() {
    ...
    set_exact_uses(
        &mut payload.record,
        &[("K33Graph", "definition"), ("SharedHelper", "theorem")],
    );
    let outcome = apply_event(state, ProtocolEvent::SidecarClosure { payload })
    ...
    assert_eq!(
        ...
        "an exact owner shared across records lands in the strict index"
    );
}

/// Lean no longer assigns lifecycle semantics to the legacy boundary/strict
/// buckets. Exact owner evidence is preserved by sidecar application and all
/// exact owners share the strict invalidation index.
#[test]
fn apply_indexes_exact_owners_without_legacy_boundary_split() {
    let state = state_with_dep_bearing_neighbour();
    let mut payload = payload_for("Rung");
    payload
        .record
    ...
    set_exact_uses(
        &mut payload.record,
        &[("ChromaticNumberIso", "theorem"), ("SharedHelper", "theorem")],
    );
    let outcome = apply_event(state, ProtocolEvent::SidecarClosure { payload })
        .expect("a closure whose record carries exact owners must apply");
    let n = node("Rung");
    assert!(outcome.state.boundary_statement_consumers.is_empty());
    assert_eq!(
        outcome
            .state
            .strict_dep_consumers
    ...
    assert_eq!(
        outcome
            ...
            .get(&node("SharedHelper")),
        Some(&[node("AlreadyClosed"), n].into_iter().collect()),
        "the exact owner joins the existing consumer set"
    );
    assert_eq!(
        outcome.state.local_closure_records[&node("Rung")]
            .node_certificate_evidence
            .as_ref()
            .expect("sidecar must retain exact-use evidence")
...
#[test]
fn stating_phase_with_uncovered_targets_applies_by_default() {
    let n = node("Rung");
    let target = TargetId::from("t");
    let mut state = eligible_state();
    state.phase = Phase::TheoremStating;
    state.configured_targets.insert(target.clone());
    state.live.coverage.insert(target.clone(), Default::default());
    state
        .live
        .paper_current_fingerprints
        .insert(target.clone(), String::new());
    state.committed.coverage.insert(target.clone(), Default::default());
    state
        .committed
        .paper_current_fingerprints
        .insert(target.clone(), String::new());
    ...
    let outcome = apply_event(state, sidecar_event("Rung"))
        .expect("the default stating window must not wait for complete target coverage");
    assert!(!outcome.state.live.open_nodes.contains(&n));
}
...
fn outcome_batch_expires_each_matching_generation() {
    ...
    for (name, seq) in [("Other", 8u64), ("Third", 9)] {
        ...
        state
            ...
            .insert(node(name), trellis_kernel::SubstantivenessStatus::Pass);
        state
            .live
            .substantiveness_current_fingerprints
