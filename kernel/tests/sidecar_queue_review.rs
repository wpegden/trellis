...
use trellis_kernel::{
    ...
    PaperFocusRange, PaperGrounding, ProtocolState, RequestKind, ResponseStatus,
    ReviewDecisionKind, ReviewResponse,
...
#[test]
fn add_window_open_accepts_in_uncovered_theorem_stating() {
    // The stating window is open by default even while the orphan
    // construction window reports uncovered targets.
    let mut state = proof_review_state(9);
    ...
    response.sidecar_queue_add = vec![nid("Rung")];
    response.paper_focus_ranges = vec![PaperFocusRange {
        start_line: 1,
        end_line: 1,
        reason: "uncovered target under review".to_string(),
        doc: None,
    }];
    response.paper_grounding = PaperGrounding {
        consulted_cited_ranges: true,
        basis_summary: "consulted the target statement before routing".to_string(),
    };
    assert!(
        state.review_response_legal(&response),
        "uncovered stating queue add must be legal; reasons = {:?}",
        state.review_response_rejection_reasons(&response)
    );
    let outcome = apply_review(state, response);
...
/// The kernel lane holds nearly every eligible node, so almost every
/// reviewer add names a node already resident there. That add is LEGAL
/// (the reviewer cannot see the kernel lane, so "already queued" would
/// reject on a fact it was never shown) and it PROMOTES the entry:
/// same generation, moved to the back of the queue, lane flipped.
#[test]
fn add_of_a_kernel_lane_node_is_legal_and_promotes_it_keeping_the_generation() {
    let mut state = proof_review_state(9);
    state.sidecar_queue.push(trellis_kernel::SidecarQueueEntry {
        node: nid("Rung"),
        entry_seq: 4,
        queued_at_cycle: 5,
        origin: trellis_kernel::SidecarQueueOrigin::Kernel,
    });
    state.sidecar_queue.push(trellis_kernel::SidecarQueueEntry {
        node: nid("Lift"),
        entry_seq: 5,
        queued_at_cycle: 5,
        origin: trellis_kernel::SidecarQueueOrigin::Reviewer,
    });
    state.sidecar_queue_seq = 5;
    let mut response = continue_response(&state, Some("Act"));
    response.sidecar_queue_add = vec![nid("Rung")];
    assert!(
        state.review_response_legal(&response),
    ...
    let outcome = apply_review(state, response);
    let rows: Vec<(&str, u64, trellis_kernel::SidecarQueueOrigin)> = outcome
        .state
        .sidecar_queue
        .iter()
        .map(|entry| (entry.node.as_str(), entry.entry_seq, entry.origin))
        .collect();
    assert_eq!(
        rows,
        vec![
            ("Lift", 5, trellis_kernel::SidecarQueueOrigin::Reviewer),
            ("Rung", 4, trellis_kernel::SidecarQueueOrigin::Reviewer),
        ],
        "promotion keeps the generation and appends in submission order"
    );
    assert_eq!(
        outcome.state.sidecar_queue_seq, 5,
        "a promotion mints nothing: the in-flight attempt on generation 4 stays valid"
    );
}
...
fn remove_queued_node_is_legal_and_preserves_survivor_order() {
    ...
    // PF Continue requires next_active, so this response removes Lift
    // and routes at Act while naming Act's removal explicitly too.
    let mut response = continue_response(&state, Some("Act"));
...
/// The mirror of the add clause: a KERNEL-lane entry is not the
/// reviewer's to retire (the boundary refill would put it straight
/// back), and the reviewer was never shown it, so naming one reads
/// exactly like naming a node that is not queued.
#[test]
fn remove_of_a_kernel_lane_node_rejects() {
    let mut state = proof_review_state(9);
    state.sidecar_queue.push(trellis_kernel::SidecarQueueEntry {
        node: nid("Rung"),
        entry_seq: 4,
...
// ── Routing dequeues (owner rule, Q7) ───────────────────────────────────

#[test]
fn next_active_on_queued_node_is_legal_and_dequeues() {
    let mut state = proof_review_state(9);
    assert_setup_routes(&state, "Rung");
    for (i, name) in ["Rung", "Lift"].iter().enumerate() {
        state.sidecar_queue.push(trellis_kernel::SidecarQueueEntry {
            node: nid(name),
            entry_seq: (i + 1) as u64,
            queued_at_cycle: 5,
    ...
    assert!(
        state.review_response_legal(&response),
        "routing the worker at a queued node must be legal; reasons = {:?}",
        state.review_response_rejection_reasons(&response)
    );
    let outcome = apply_review(state, response);
    ...
    assert_eq!(
        outcome
            ...
            .collect::<Vec<_>>(),
        vec!["Lift"],
        "the routed node's entry is gone; the rest of the queue is untouched"
    );
    assert_eq!(
        outcome.state.sidecar_queue_seq, 2,
        "an implicit removal never touches the generation counter"
    );
    assert!(
        outcome.state.sidecar_queue_prune_log.is_empty(),
        "an implicit removal journals nothing, exactly like the explicit verb"
    );
}

#[test]
fn routed_node_leaves_the_export_and_the_auto_dispatch_candidates() {
    // The daemon's cancel-watch keys on the export: an in-flight attempt
    // whose `(node, entry_seq)` no longer appears there is cancelled
    // (`_cancel_removed`). And the routed node must not come straight
    // back through the idle-pool top-up at the same boundary.
    let mut state = proof_review_state(9);
    assert_setup_routes(&state, "Rung");
    state.sidecar_queue.push(trellis_kernel::SidecarQueueEntry {
        node: nid("Rung"),
    ...
    let response = continue_response(&state, Some("Rung"));
    let outcome = apply_review(state, response);
    let repo = std::path::Path::new("/nonexistent-repo");
    let export = trellis_kernel::sidecar::build_candidates_export(
        repo,
        &outcome.state,
        &trellis_kernel::SidecarRuntimeConfig::default(),
        &Default::default(),
    );
    assert!(
        export.queue.is_empty(),
        "the export the daemon polls no longer lists the routed entry; got {:?}",
        export.queue
    );
    // The kernel-queue refill reads the SAME eligibility predicate,
    // which excludes the active node — no cool-off of its own is needed.
    let candidates = trellis_kernel::sidecar::collect_auto_dispatch_candidates(
        repo,
        &outcome.state,
    ...
    assert!(
        !candidates
            .iter()
            .any(|candidate| candidate.node == nid("Rung")),
        "the routed node must not be an auto-dispatch candidate; got {:?}",
        candidates
            .iter()
            .map(|c| c.node.as_str())
            .collect::<Vec<_>>()
    ...
    assert!(
        !export
            .eligible_now
            .iter()
            .any(|row| row.node == nid("Rung")),
        "nor an advisory add-candidate"
    );
}
...
fn next_active_on_queued_node_with_remove_is_legal_routes_and_dequeues() {
    ...
    assert!(
        state.review_response_legal(&response),
        "naming the removal explicitly stays legal; reasons = {:?}",
        state.review_response_rejection_reasons(&response)
    );
    let outcome = apply_review(state, response);
    assert_eq!(outcome.state.active_node, Some(nid("Rung")), "routed");
    assert!(
        ...
        "dequeued once, idempotently"
    );
}

#[test]
fn next_active_on_added_node_rejects_as_incoherent() {
    // Queueing a node for a grunt and routing the primary worker at it
    // in the SAME response is self-contradictory: the add would be
    // undone by the routing that accompanies it. Same posture as the
    // add ∩ remove clause.
    let state = proof_review_state(9);
    assert_setup_routes(&state, "Rung");
    let mut response = continue_response(&state, Some("Rung"));
    ...
    assert_reject_with_reason(
        &state,
        &response,
        "sidecar_queue_add: Rung is also next_active in the same response",
    );
}
...
fn rejected_response_reissues_review_carrying_queue_reason() {
    ...
    assert!(
        outcome
            ...
            .iter()
            .any(|reason| reason.contains("Lift is not in the sidecar queue")),
    ...
    let reissued: Vec<&WrapperRequest> = outcome
        ...
        .filter_map(|c| match c {
            ProtocolCommand::IssueRequest { request } => Some(request.as_ref()),
            _ => None,
        })
        .collect();
    assert!(
...
fn review_payload_and_fragments_gate_on_advertise_flag() {
    ...
    let payload = trellis_kernel::request_contracts::review_contract_payload(&request, None);
    ...
    let payload = trellis_kernel::request_contracts::review_contract_payload(&advertised, None);
...
fn malformed_reissue_is_byte_identical_with_nonempty_queue() {
    ...
    let queue_section = |request: &WrapperRequest| -> String {
        ...
        advertised.sidecar_advertise_queue_fields = true;
        let payload =
            trellis_kernel::request_contracts::review_contract_payload(&advertised, None);
