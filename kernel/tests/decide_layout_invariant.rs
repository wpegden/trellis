...
fn cli_step_rejects_present_node_without_node_kind_or_deps() {
    ...
    seed_valid_prove_layout(&repo);
    let goal_bytes = b"Verify the synthetic decide target.\n";
    ...
    let paths = RuntimePaths::new(root.clone());
    let mut state = decide_pair_state_missing_deps();
    state.pv_goal_prose_sha256 = Some(
        trellis_kernel::trust_base::raw_sha256(goal_bytes).to_string(),
    );
    SupervisorRuntime::initialize_with_metadata(
        paths,
        state,
