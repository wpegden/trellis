...
fn local_closure_smoke_own_aux_body_cross_node_dep_still_recorded() {
    ...
    // body. The own-aux transparent walk must still WALK the aux's value,
    // so `Helper` is recorded as a boundary theorem — the filter must
    // never hide a genuine cross-node reference. The boundary cut at
    // `Helper` still applies: no `sorryAx` in `kernel_axioms`.
    ...
    assert!(
        boundaries
            .iter()
            .any(|n| n == "Helper" || n == "Tablet.Helper"),
        "cross-node boundary theorem `Helper` referenced inside the own aux's \
         body must still be recorded (not hidden by the transparent walk); \
         boundary_theorems: {boundaries:?}",
    );
    assert!(
        !all_dep_names(&env).iter().any(|n| n.contains("bridgeReachable")),
...
fn local_closure_smoke_own_aux_body_unapproved_axiom_still_surfaces() {
    // Fix A no-hiding guard (axiom side): `UsesOwnAuxAxiom`'s own `let rec
    // axReachable` aux references the project axiom `AxiomForgeEq1` inside
    // its body. The own-aux transparent walk must surface the axiom in
    // `kernel_axioms` in BOTH the primary and the axcheck collectors, so
    // the Rust approved-axiom policy still sees it — an own-aux filter
    // that dropped the aux body would let an unapproved axiom hide inside
    // a `let rec`.
    ...
    assert!(
        env.kernel_axioms.iter().any(|a| a == "AxiomForgeEq1"),
        "project axiom referenced inside the own aux's body must surface in \
         the PRIMARY collector's kernel_axioms; got {:?}",
        env.kernel_axioms,
    );
    let ax = env
        .axiomization_check
        ...
        .expect("axiomization_check present");
    assert!(
        ax.kernel_axioms.iter().any(|a| a == "AxiomForgeEq1"),
        "project axiom referenced inside the own aux's body must surface in \
         the AXCHECK collector's kernel_axioms too (Edit-3 mirror); got {:?}",
        ax.kernel_axioms,
    );
    assert!(
        !all_dep_names(&env).iter().any(|n| n.contains("axReachable")),
        "the own aux itself must not be a dep key; deps: {:?}",
        all_dep_names(&env),
    );
...
#[test]
fn local_closure_smoke_ind_pred_brec_on_transparent_with_invalidation_edge() {
    // FIX B regression: `UsesIndPredInduction` is proved by structural
    // recursion over the recursive inductive predicate `IndPredReach`, so
    // its elaborated proof term retains `IndPredReach.brecOn` /
    // `IndPredReach.below` (verified pre-fix: the probe recorded the dotted
    // boundary key `IndPredReach.brecOn` and a DUPLICATED
    // `strict_definition_deps[IndPredReach]` entry from `below` — the exact
    // live dec2flt failure shape). Post-fix both are transparent-walked and
    // the node probes clean.
    ...
    // THE CRITICAL ASSERTION (invalidation edge): the real authored
    // dependency — the inductive predicate `IndPredReach` — must be recorded
    // as a strict definition dep. The transparent walk must never hide the
    // dependency from invalidation.
    let strict_defs = boundary_names(&env.strict_definition_deps);
    assert!(
        strict_defs
            .iter()
            .any(|n| n == "IndPredReach" || n == "Tablet.IndPredReach"),
        "IndPredReach must be recorded as a strict definition dep \
         (the invalidation edge); got {strict_defs:?}",
    );
    // No dotted recursor-family key may leak into ANY dep array.
    let deps = all_dep_names(&env);
    assert!(
        !deps
            .iter()
            .any(|n| n.contains(".brecOn") || n.contains(".below")),
        "IndPredBelow-generated `.brecOn` / `.below` must be transparent-walked, \
         never recorded as dep keys; deps: {deps:?}",
    );
    assert!(
        axiom_subset_of_canonical_four(&env.kernel_axioms),
...
#[test]
fn local_closure_smoke_cross_module_forged_brec_on_stays_recorded() {
    // FIX B soundness (the module co-location clause): `NonRecPred` is a
    // NON-recursive Prop inductive, so Lean generates no `brecOn` for it and
    // `ForgeBrecOnAux.lean` can author `theorem NonRecPred.brecOn` in a
    // DIFFERENT module without collision. The forgery matches the suffix and
    // inductive-parent conjuncts of `isRecursorFamilyRealization`; ONLY the
    // co-location clause keeps it out of the generated classification. The
    // consumer's probe must RECORD the dotted `NonRecPred.brecOn` dep key so
    // the kernel's C-K guard rejects it fail-closed — mirrors the
    // `UsesOwnerToCtorIdx` fail-OPEN regression pin.
    ...
    assert!(
        deps.iter()
            .any(|n| n == "Tablet.NonRecPred.brecOn" || n == "NonRecPred.brecOn"),
        "cross-module forged NonRecPred.brecOn must remain a recorded dep key \
         (NOT transparent-walked as a generated artifact — the co-location \
         clause is what keeps this fail-closed); deps: {deps:?}; envelope: {env:?}",
    );
...
fn local_closure_smoke_own_module_forged_brec_on_surfaces_sorry_ax() {
    ...
    let (stdout, stderr, code) = run_probe("UsesOwnBrecOnForge").expect("run probe");
    assert_eq!(
        code,
        Some(0),
    ...
    assert!(
        !all_dep_names(&env)
            .iter()
            .any(|n| n.contains(".brecOn")),
        "the own-module forgery itself is classified generated and must not \
         be a dep key; deps: {:?}",
        all_dep_names(&env),
    );
    assert_axcheck_agreed(&env, "UsesOwnBrecOnForge");
}

#[test]
fn local_closure_smoke_brec_on_under_non_inductive_parent_stays_recorded() {
    // FIX B fail-closed (the inductive-parent clause): `DefParentBrecOn` is
    // a `def`, so the hand-authored, module-co-located
    // `theorem DefParentBrecOn.brecOn` fails the inductive-parent conjunct
    // of `isRecursorFamilyRealization`. The consumer's probe must keep the
    // dotted dep key recorded so the kernel's C-K guard rejects it.
    ...
    assert!(
        deps.iter()
            .any(|n| n == "Tablet.DefParentBrecOn.brecOn" || n == "DefParentBrecOn.brecOn"),
        "recursor-family suffix under a NON-inductive parent must remain a \
         recorded dep key (fail-closed); deps: {deps:?}; envelope: {env:?}",
    );
...
fn local_closure_smoke_ctor_size_of_spec_transparent_with_invalidation_edge() {
    ...
    let deps = all_dep_names(&env);
    assert!(
        !deps.iter().any(|n| n.contains("sizeOf_spec")),
        "the generated ctor sizeOf_spec theorem must be transparent-walked, \
         never recorded as a dep key; deps: {deps:?}",
    );
    assert!(
        axiom_subset_of_canonical_four(&env.kernel_axioms),
...
fn validate_probe_present_nodes_accepts_real_namespaced_dep_probe() {
    ...
    let node_kinds: BTreeMap<NodeId, NodeKind> = [
        ...
        (NodeId::from("NamespacedStrictThm"), NodeKind::Proof),
