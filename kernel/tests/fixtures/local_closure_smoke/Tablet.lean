...
import Tablet.UsesEnumDerivedDecEqOfNatCtorIdx
import Tablet.ForgedOfNatCtorIdx
import Tablet.UsesForgedOfNatCtorIdx
...
import Tablet.ByElab
import Tablet.LegitNotation
-- Source-policy/certificate separation fixtures.
import Tablet.PolicyEval
import Tablet.PolicyLocalSyntax
import Tablet.PolicyInitialize
...
-- DEEPLY-NAMESPACED-DEP ownership regression:
...
-- declaration-name gate. Two categories: (a) a `structure` declared
...
-- Generality regression: a custom deriving handler emits the unrelated root
-- name `CobaltMoth922` from the WeirdDeriveOwner module.
import Tablet.WeirdDeriveOwner
import Tablet.UsesWeirdDeriveOwner
-- Rename-invariance twin: same handler shape, different generated name.
import Tablet.RenamedDeriveOwner
import Tablet.UsesRenamedDeriveOwner
-- Certificate-v2 falsifier hunt: two artifacts serialize the same theorem
-- name with different proof dependencies. Both import orders must retain both
-- providers; merged const2ModIdx attribution is diagnostic only.
import Tablet.CollisionLeft
import Tablet.CollisionRight
import Tablet.CollisionConsumerForward
import Tablet.CollisionConsumerReverse
-- Certificate-boundary regression: the consumer's direct import is split
-- across lines and comments, so only ModuleData.imports is authoritative.
import Tablet.MultilineDep
import Tablet.MultilineConsumer
