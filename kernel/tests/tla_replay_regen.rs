...
fn normalize_fixture_state(mut state: AbstractState) -> AbstractState {
    ...
    if state.substantiveness_status.is_empty() {
            ...
            .chain(state.committed.present_nodes.iter())
        {
            ...
            state
                ...
                .entry(node.clone())
                .or_insert(trellis_kernel::SubstantivenessStatus::Pass);
    ...
    if let Some(request) = state.in_flight_request.clone() {
        state.in_flight_request = Some(Box::new(state.expected_request(request.id, request.kind)));
    }
    state
}
...
fn action_matches_event(action: &SpecAction, state: &AbstractState, event: &AbstractEvent) -> bool {
    match (action, event) {
        ...
        (SpecAction::ApplySidecarClosure, AbstractEvent::SidecarClosure { .. }) => true,
        (
            SpecAction::ExpireSidecarQueueEntry,
            AbstractEvent::SidecarAttemptOutcomes { .. },
        ) => true,
        (
            SpecAction::AutoDispatchSidecarQueue,
            AbstractEvent::SidecarQueueAutoAdd { .. },
        ) => true,
        (
        ...
        (
            SpecAction::AcceptGapPlannerHumanRequired,
            AbstractEvent::WrapperResponse {
                response: WrapperResponse::StuckMathAudit(audit),
            },
        ) => {
...
fn slim_event(event: &AbstractEvent) -> Value {
    ...
    let default_event: AbstractEvent = match event {
        ...
        AbstractEvent::FinalizeAuthorizedPackage { record } => {
            // Trust-only variant: keep the payload as-is (no default exists
            // for the finalization record; math fixtures never carry it).
            AbstractEvent::FinalizeAuthorizedPackage {
                record: record.clone(),
            }
        }
        ...
        AbstractEvent::SidecarClosure { .. } => AbstractEvent::SidecarClosure {
            payload: trellis_kernel::SidecarClosurePayload::default(),
        },
        AbstractEvent::SidecarAttemptOutcomes { .. } => AbstractEvent::SidecarAttemptOutcomes {
            payload: trellis_kernel::SidecarAttemptOutcomesPayload::default(),
        },
        AbstractEvent::SidecarQueueAutoAdd { .. } => AbstractEvent::SidecarQueueAutoAdd {
            payload: trellis_kernel::SidecarQueueAutoAddPayload::default(),
        },
        AbstractEvent::StatementBound { target, binding } => {
            // Prose-only variant (W4): keep the payload as-is — math
            // fixtures never carry it, and the binding bytes ARE the event.
            AbstractEvent::StatementBound {
                target: target.clone(),
                binding: binding.clone(),
            }
        }
        AbstractEvent::ConditionalActivation { payload } => {
            AbstractEvent::ConditionalActivation {
...
/// Step 7 parity map: the Rust generation record and event now implement the
/// abstract lifecycle introduced by the checked TLA model. The ordinary trace
/// fixture does not invent semantic verifier/human payloads, so this focused
/// mirror pins the stage/disposition vocabulary and the invariants beside the
/// regenerator that must preserve the activation event.
#[test]
fn conditional_protocol_tla_stages_and_invariants_are_mirrored_in_rust() {
    let spec = include_str!("../../spec/SupervisorProtocol.tla");
    for definition in [
        "ChallengeResolution",
        "ResolutionPolicy ==",
        ...
        assert!(
            spec.contains(definition),
            "SupervisorProtocol.tla lost step-4 definition `{definition}`"
        );
    }
    let stages = [
        trellis_kernel::trust_base::ConditionalStage::None,
        trellis_kernel::trust_base::ConditionalStage::Proposed,
        trellis_kernel::trust_base::ConditionalStage::CorrespondencePass,
        trellis_kernel::trust_base::ConditionalStage::HumanApproved,
        trellis_kernel::trust_base::ConditionalStage::Open,
        trellis_kernel::trust_base::ConditionalStage::Closed,
    ];
    let serialized: Vec<String> = stages
        .iter()
        .map(|stage| serde_json::to_value(stage).unwrap().as_str().unwrap().to_owned())
        .collect();
    assert_eq!(
        serialized,
        [
    ...
    let dispositions = [
        trellis_kernel::trust_base::ConditionalDisposition::Rejected,
        trellis_kernel::trust_base::ConditionalDisposition::Withdrawn,
        trellis_kernel::trust_base::ConditionalDisposition::Superseded,
    ];
    assert_eq!(
        dispositions
            .iter()
        ...
        vec![
            ...
            Value::String("withdrawn".into()),
            Value::String("superseded".into()),
        ]
    );

    let event = AbstractEvent::ConditionalActivation {
        payload: trellis_kernel::trust_base::ConditionalActivationPayload::default(),
    };
    let replayed: AbstractEvent =
        serde_json::from_value(serde_json::to_value(&event).unwrap()).unwrap();
    assert_eq!(replayed, event);
    assert_eq!(slim_event(&event)["event"], "conditional_activation");
}

#[test]
fn phase0_protocol_tla_stage_action_and_invariant_vocabulary_matches_rust() {
    use trellis_kernel::phase0::{Phase0Stage, MAX_PHASE0_HANDBACKS};

    let spec = include_str!("../../spec/SupervisorProtocol.tla");
    assert!(spec.contains("MaxPhase0Handbacks == 32"));
    assert_eq!(MAX_PHASE0_HANDBACKS, 32);
    let stage_definition = spec
        .lines()
        .find(|line| line.starts_with("Phase0StageValues =="))
        .expect("structured Phase0StageValues definition");
    let tla_stages: BTreeSet<_> = stage_definition
        .split('"')
        .skip(1)
        .step_by(2)
        .collect();
    let rust_stages: BTreeSet<_> = [
        (Phase0Stage::Worker, "Worker"),
        (Phase0Stage::Check, "Check"),
        (Phase0Stage::SeamAudit, "SeamAudit"),
        (Phase0Stage::CorrespondenceAudit, "CorrespondenceAudit"),
        (Phase0Stage::Sealed, "Sealed"),
        (Phase0Stage::Incomplete, "Incomplete"),
    ]
    .into_iter()
    .map(|(stage, tla)| {
        assert_eq!(stage.tla_name(), tla);
        tla
    })
    .collect();
    assert_eq!(tla_stages, rust_stages);

    let definitions: BTreeSet<_> = spec
        .lines()
        .filter_map(|line| line.strip_suffix(" =="))
        .filter(|name| !name.is_empty() && !name.contains(char::is_whitespace))
        .collect();
    for definition in [
        "Phase0InitialCheckerFailure", "Phase0WorkerRevision",
        "Phase0CheckerFailure", "Phase0CheckSuccess", "Phase0SeamAuditPass",
        "Phase0AuditFindings", "Phase0CorrespondenceAuditPassAndSeal",
        "Phase0ProductionExtractionAndBootstrap", "Phase0IncompleteStop",
        "Phase0GoalFrozen", "AdaptedTreeEqualsLedgerReplay",
        "NoUnloggedAdaptationBytes", "EveryEntryHasFailureAndAblation",
        "AuditPassBindsCurrentGeneration", "DiagnosticExtractionCannotSeedCampaign",
        "NoProductionExtractionFromUnsealedTree", "BootstrapConsumesSealedBundle",
        "SealedGenerationIsCleanAndComplete", "AdvanceGateBindsSealedGeneration",
        "Phase0BoundOnlyStops",
    ] {
        assert!(definitions.contains(definition), "missing Phase-0 TLA definition {definition}");
    }
    // This mirror had drifted: it still pinned 54 while the spec and
    // `prompt_contract_version()` had moved on, so the coupling it exists to
    // enforce was dead. Retargeted to the current version.
    assert_eq!(
        spec.matches("promptContractVersion |-> IF kind = \"none\" THEN 0 ELSE 64").count(),
        1
    );
    assert_eq!(spec.matches(r"promptContractVersion \in 0..64").count(), 1);
    assert_eq!(spec.matches("THEN 0 ELSE 64").count(), 2);
    assert!(!spec.contains("promptContractVersion |-> 61"));
}

#[test]
fn phase0_small_bound_replay_exercises_checker_revision_and_incomplete_stop() {
    let replay: Value = serde_json::from_str(include_str!(
        "fixtures/phase0_incomplete_replay.json"
    ))
    .unwrap();
    assert_eq!(replay["schema"], "trellis-phase0-tla-replay/v1");
    assert_eq!(replay["max_phase0_handbacks"], 3);
    ...
    for required in [
        ...
        assert!(actions.contains(&required));
    }
    let checker = steps
        .iter()
        .find(|step| step["action"] == "Phase0CheckerFailure")
        .unwrap();
    assert_eq!(checker["handbacks"], 0, "checker failures are not semantic handbacks");
    let terminal = steps.last().unwrap();
    assert_eq!(
        terminal["stage"],
        trellis_kernel::phase0::Phase0Stage::Incomplete.tla_name(),
    );
    assert_eq!(terminal["handbacks"], replay["max_phase0_handbacks"]);
}
