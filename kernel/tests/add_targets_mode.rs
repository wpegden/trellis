...
use trellis_kernel::{
    ...
    EventLogRecord, GateKind, HumanChoice, HumanGateResponse, NodeId, NodeKind, PendingTask,
    Phase, ProofEditMode, ProtocolState, RequestKind, ResponseStatus, RevisionActions,
    RevisionContext,
    RevisionKind, RevisionNodeAction, RevisionNodeDisposition, RevisionPlanningContext,
    RevisionTargetAction, RevisionTargetDeltaKind,
    RuntimeMetadata, RuntimePaths, Stage, StuckMathAuditResponse, SupervisorRuntime,
    TargetEditMode, TargetId, WorkerValidationExecutionPlanStep, AUDIT_REPORT_TEXT_MIN_CHARS,
};
...
fn full_flow_planner_statement_gate_reaches_proof_formalization() {
    ...
    state
        .substantiveness_approved_fingerprints
        .insert(w.clone(), "subst-w".into());
    state.substantiveness_status.insert(w.clone(), CorrStatus::Pass);
    state.commit_live();
