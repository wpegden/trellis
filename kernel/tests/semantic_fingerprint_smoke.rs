...
fn assert_fingerprint_changes_on_mutation(
    consumer: &str,
    dep_file: &str,
    from: &str,
    to: &str,
    ...
    let original = std::fs::read_to_string(&dep_path)
        .unwrap_or_else(|e| panic!("read {dep_file}: {e}"));
    assert!(
    ...
    assert!(build.success(), "lake build must succeed for mutated {dep_file}");
