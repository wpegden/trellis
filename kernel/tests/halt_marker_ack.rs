...
use trellis_kernel::runtime_cli_observations_halt::{
    ...
    halt_history_path, load_system_feedback_ack_store, system_feedback_ack_store_path,
    system_feedback_fingerprint, HaltMarkerAckOutcome, SystemFeedbackAckStore,
...
// ---- system_feedback fingerprint-ack store: persistence roundtrip ----
//
// Mirrors the checker-disagreement ack's on-disk persistence: the ack
// store is JSON under the runtime root and MUST survive a reload
// (serialize → disk → deserialize) with the acknowledged set intact.

#[test]
fn system_feedback_ack_store_survives_reload() {
    let dir = local_tempdir();
    let _guard = CacheRootGuard::install(dir.path());

    // Empty by default (no store on disk).
    ...
    let result = acknowledge_system_feedback_fingerprint(&fingerprint, "tracked in ISSUE-9", feedback)
        .expect("ack persists");
    assert!(result.newly_added);

    // The store file exists on disk under the runtime root.
    let store_path = system_feedback_ack_store_path().expect("store path resolves");
    assert!(store_path.exists(), "ack store must be persisted to disk");

    // Reload from a cold read: the acknowledged fingerprint is present.
    let reloaded = load_system_feedback_ack_store();
    assert!(reloaded.is_acknowledged(&fingerprint));
    let ack = reloaded.acknowledged.get(&fingerprint).expect("ack entry");
    assert_eq!(ack.reason, "tracked in ISSUE-9");
    assert_eq!(ack.sample_feedback, feedback);

    // Explicit serialize/deserialize roundtrip fidelity.
    let json = serde_json::to_string_pretty(&reloaded).unwrap();
    let parsed: SystemFeedbackAckStore = serde_json::from_str(&json).unwrap();
    assert_eq!(parsed, reloaded);

    // Idempotent re-ack: not newly added, still one entry.
    let again = acknowledge_system_feedback_fingerprint(&fingerprint, "again", feedback).unwrap();
    assert!(!again.newly_added);
    assert_eq!(again.total_acknowledged, 1);
}
