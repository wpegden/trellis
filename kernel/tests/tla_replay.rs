...
use trellis_kernel::{
    apply_abstract_event, AbstractCommand, AbstractEvent, AbstractState, HumanChoice,
    LocalClosureRecord, NodeId, NodeKind, RequestKind, ReviewDecisionKind, SpecAction, TargetId,
    TraceFixture, WorkerOutcome,
};
...
fn legacy_fresh_closure_record(node: &NodeId) -> LocalClosureRecord {
    LocalClosureRecord {
        node: node.clone(),
        closure_version: "tla-replay-v1".to_owned(),
        toolchain_hash: "tla-replay-toolchain".to_owned(),
        lean_executable_hash: "tla-replay-lean".to_owned(),
        lake_executable_hash: "tla-replay-lake".to_owned(),
        checker_script_hash: "tla-replay-checker".to_owned(),
        lake_manifest_hash: "tla-replay-manifest".to_owned(),
        preamble_hash: "tla-replay-preamble".to_owned(),
        approved_axioms_hash: "tla-replay-axioms".to_owned(),
        active_decl_hash: format!("tla-replay-decl-{node}"),
        active_statement_hash: format!("tla-replay-statement-{node}"),
        ..LocalClosureRecord::default()
    }
}

/// `tla_replay.json` predates the local-closure lifecycle and represents a
/// sorry-free declaration as already complete. Preserve that abstraction by
/// supplying fresh, dependency-free records for closed registered nodes when
/// (and only when) the fixture contains no closure state at all. Definition
/// owners now need these records just like proof owners; weakening production
/// eligibility to accommodate the legacy trace would test the wrong model.
fn backfill_legacy_closure_records(state: &mut AbstractState) {
    let closure_state_is_absent = state.local_closure_records.is_empty()
    ...
    if !closure_state_is_absent {
        return;
    }

    let live_nodes: Vec<NodeId> = state
        .live
        .present_nodes
        .iter()
        .filter(|node| !state.live.open_nodes.contains(*node))
        .filter(|node| {
            state.proof_nodes.contains(*node)
                || matches!(
                    state.node_kinds.get(*node),
                    Some(NodeKind::Proof | NodeKind::Definition)
                )
        })
        .cloned()
        .collect();
    for node in live_nodes {
        state
            ...
            .insert(node.clone(), legacy_fresh_closure_record(&node));
    }

    let committed_nodes: Vec<NodeId> = state
        .committed
        .present_nodes
        .iter()
        .filter(|node| !state.committed.open_nodes.contains(*node))
        .filter(|node| {
            state.committed_proof_nodes.contains(*node)
                || matches!(
...
fn normalize_fixture_state(mut state: AbstractState) -> AbstractState {
    ...
    backfill_legacy_closure_records(&mut state);
    ...
    commands
        ...
        .map(|command| match command {
            AbstractCommand::IssueRequest { request } => AbstractCommand::IssueRequest {
                request: Box::new(state.expected_request(request.id, request.kind)),
            },
            other => other,
        })
        .collect()
...
fn action_matches_event(action: &SpecAction, state: &AbstractState, event: &AbstractEvent) -> bool {
    match (action, event) {
        ...
        (
            ...
            // GapResearch Planner signalling `route_needs_human` while the
            // gap_research lane is active. Mirror of
            // `apply_gap_research_response`'s needs-human arm.
            audit.status == trellis_kernel::ResponseStatus::Ok
