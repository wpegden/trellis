...
// PV #1 keystone — mode B (Lean goals, the default). A `pv_tablet` with
// `verification_target_statements` seeds the goal STATEMENTS as byte-pinned
// challenge `Theorem`s alongside the model `def`s, leaves `configured_targets`
// EMPTY (so the faithfulness lane never arms — `paper_verify_targets` is empty),
// and roles each goal node by its declared role. `goal_mode` defaults to "lean".
#[test]
fn runtime_cli_init_pv_mode_b_seeds_goal_statements_as_challenge_theorems() {
    let tmp = project_tempdir();
    let workspace = tmp.path().join("workspace");
    let repo = workspace.join("repo");
    let root = workspace.join("runtime");
    ...
    fs::create_dir_all(repo.join("Tablet")).expect("tablet dir");
    seed_support_script(&repo);
    // Mode-B auto-claim reads each configured target's seeded node file at init
    // (the real flow writes them before init); seed minimal node files so it
    // finds them.
    for (node, body) in [
        ("fModel", "def fModel : Nat :=\n  0"),
        ("f_Correctness", "theorem f_Correctness : fModel = 0 := by rfl"),
    ] {
        fs::write(
            repo.join("Tablet").join(format!("{node}.lean")),
            format!("{body}\n"),
        )
        .expect("seed node file");
    }
    let config_dir = workspace.join("config");
    fs::create_dir_all(&config_dir).expect("config dir");
    let config_path = config_dir.join("trellis.config.json");
    fs::write(
        ...
        serde_json::json!({
            ...
            "pv_tablet": {
                ...
                // goal_mode omitted ⇒ defaults to "lean" (mode B).
                "verification_target_statements": [{
                    "id": "goal:f_Correctness",
                    "node": "f_Correctness",
                    "name": "f_Correctness",
                    "role": "correctness",
                    "lean": "theorem f_Correctness : fModel = 0 :=",
                    "namespace_context": ""
                }]
            }
        })
    ...
    let init_json: Value = serde_json::from_slice(&init_output.stdout).expect("parse init output");
    assert_eq!(init_json["status"], "ok", "pv mode-B init: {init_json}");

    // The goal statement is registered as a byte-pinned challenge THEOREM
    // (statement-only slice), forced to `theorem` kind.
    let goal = &init_json["state"]["configured_challenge_targets"]["goal:f_Correctness"];
    assert_eq!(goal["kind"], serde_json::json!("theorem"));
    assert_eq!(goal["name"], serde_json::json!("f_Correctness"));
    assert_eq!(
        goal["lean"],
        serde_json::json!("theorem f_Correctness : fModel = 0 :=")
    );
    // The model def is still seeded as a challenge `def` (both kinds coexist).
    assert_eq!(
        init_json["state"]["configured_challenge_targets"]["model:fModel"]["kind"],
        serde_json::json!("def")
    );
    // The seeded tablet node files are auto-claimed: each challenge target is
    // already covered by its matching node at init.
    assert_eq!(
        init_json["state"]["live"]["challenge_coverage"],
        serde_json::json!({
            "goal:f_Correctness": ["f_Correctness"],
            "model:fModel": ["fModel"]
        })
    );
    // KEYSTONE: `configured_targets` is EMPTY ⇒ `paper_verify_targets()` is
    // empty ⇒ no Paper-faithfulness dispatch (the mechanism, not a special
    // gate). The faithfulness lane is suppressed by the empty bucket.
    assert_eq!(
        init_json["state"]["configured_targets"],
        serde_json::json!([])
    );
    // No prose-goal prefixes recorded (the mode-A autoseed stays inert).
    assert!(
        init_json["state"]["pv_verification_target_prefixes"]
            .as_array()
            .map(|a| a.is_empty())
            .unwrap_or(true),
        "mode B records no verification-target prefixes"
    );
    // Roles: the goal node carries its declared role; the model node stays
    // ExtractionModel.
    assert_eq!(
        init_json["state"]["node_role"]["f_Correctness"],
        serde_json::json!("correctness")
    );
    assert_eq!(
        init_json["state"]["node_role"]["fModel"],
        serde_json::json!("extraction_model")
    );
    // Mode B is the default ⇒ `pv_goal_mode` stays off the wire
    // (skip_serializing_if = is_lean), so it serializes as absent.
    assert!(
        init_json["state"].get("pv_goal_mode").is_none(),
        "mode B (default Lean) keeps pv_goal_mode off the wire"
    );
    // The PV gate flag is on.
    assert_eq!(
        init_json["state"]["pv_tablet_configured"],
        serde_json::json!(true)
    );
}

// PV "prove OR disprove": a `Decide` goal statement auto-seeds the paired
// byte-pinned refutation target `<id>__refutation` (name `<name>__Refutation`)
// whose `lean` is the EXACT `¬T`, with its own empty coverage and a mirrored
// node role. The all-Prove wire stays byte-identical (no `resolution` field on
// a Prove statement).
#[test]
fn runtime_cli_init_pv_decide_autoseeds_refutation_pair() {
    let tmp = project_tempdir();
    let workspace = tmp.path().join("workspace");
    let repo = workspace.join("repo");
    let root = workspace.join("runtime");
        ...
        serde_json::json!({
            ...
            "pv_tablet": {
                ...
                "verification_target_statements": [{
                    ...
                    "name": "f_Correctness",
                    "role": "correctness",
                    "lean": "theorem f_Correctness (x : Nat) : f x = 0 :=",
                    "namespace_context": "",
                    "resolution": "decide"
                }]
            }
    ...
    let init_json: Value = serde_json::from_slice(&init_output.stdout).expect("parse init output");
    assert_eq!(init_json["status"], "ok", "pv decide init: {init_json}");

    let targets = &init_json["state"]["configured_challenge_targets"];
    // The primary carries `resolution: decide`.
    assert_eq!(
        targets["goal:f_Correctness"]["resolution"],
        serde_json::json!("decide")
    );
    // The paired refutation target is auto-seeded with the EXACT ¬T.
    let refutation = &targets["goal:f_Correctness__refutation"];
    assert_eq!(refutation["kind"], serde_json::json!("theorem"));
    assert_eq!(
        refutation["name"],
        serde_json::json!("f_Correctness__Refutation")
    );
    assert_eq!(
        refutation["lean"],
        serde_json::json!(
            "theorem f_Correctness__Refutation : ¬ (∀ (x : Nat), f x = 0) := by"
        )
    );
    // The refutation target is itself prove-only ⇒ `resolution` stays off the
    // wire (skip_serializing_if = is_prove).
    assert!(
        refutation.get("resolution").is_none(),
        "refutation target is prove-only ⇒ resolution off the wire"
    );
    // Both polarities carry empty challenge coverage.
    assert_eq!(
        init_json["state"]["live"]["challenge_coverage"]["goal:f_Correctness"],
        serde_json::json!([])
    );
    assert_eq!(
        init_json["state"]["live"]["challenge_coverage"]["goal:f_Correctness__refutation"],
        serde_json::json!([])
    );
    // The refutation NODE carries a mirrored role.
    assert_eq!(
        init_json["state"]["node_role"]["f_Correctness__Refutation"],
        serde_json::json!("correctness")
    );
}

// PV "prove OR disprove" F1/F2: a `Decide` primary whose shape the kernel cannot
// negate (universe-annotated name, or implicit/instance binders) is rejected
// FAIL-LOUD at config parse time rather than emitting a broken byte-pin.
#[test]
fn runtime_cli_init_pv_decide_rejects_unrefutable_shape() {
    fn init_decide(lean: &str, name: &str) -> Value {
        let tmp = project_tempdir();
        let workspace = tmp.path().join("workspace");
        let repo = workspace.join("repo");
        let root = workspace.join("runtime");
        ...
        .expect("write config");
        let out = run_runtime_cli(&serde_json::json!({
            "action": "init_from_config",
            "root": root,
            "config_path": config_path,
        }));
        serde_json::from_slice(&out.stdout).expect("parse init output")
    }

    // F2: implicit binder.
    let implicit = init_decide("theorem T {x : Nat} : f x = 0 :=", "T");
    assert_ne!(implicit["status"], "ok", "implicit-binder Decide must reject");
    assert!(
        implicit.to_string().contains("cannot be refuted"),
        "expected refutation reject: {implicit}"
    );
    // F1: universe-annotated name.
    let universe = init_decide("theorem T.{u} : f = 0 :=", "T.{u}");
    assert_ne!(universe["status"], "ok", "universe-name Decide must reject");
    assert!(
        universe.to_string().contains("universe annotation"),
        "expected universe reject: {universe}"
    );
    // Control: a fully-explicit shape is accepted.
    let ok = init_decide("theorem T (x : Nat) : f x = 0 :=", "T");
    assert_eq!(ok["status"], "ok", "explicit-binder Decide must init: {ok}");
}

// PV mode-B contract-predicate pins: `verification_target_definitions` seeds
// the Spec/precondition predicate `def`s as byte-pinned challenge `Def`s
// (whole-decl through EOF) alongside the model defs and goal statements, records
// each def node's PV role (`spec`/`precondition`), and leaves `configured_targets`
// EMPTY (no faithfulness lane). This is the soundness keystone: the pinned
// theorem's postcondition/precondition predicates are TRUSTED givens, not
// worker-authored, so a worker cannot weaken `Spec := True` / `Pre := False`.
#[test]
fn runtime_cli_init_pv_mode_b_seeds_target_definitions_as_challenge_defs() {
    let tmp = project_tempdir();
    let workspace = tmp.path().join("workspace");
    let repo = workspace.join("repo");
    let root = workspace.join("runtime");
    ...
    seed_support_script(&repo);
    // Mode-B auto-claim reads each configured target's seeded node file at init;
    // seed minimal node files so it finds them.
    for (node, body) in [
        ("montgomery_reduce", "def montgomery_reduce : Nat := 0"),
        ("montgomery_reduce_Spec", "def montgomery_reduce_Spec : Prop := True"),
        ("MontgomeryReduceInputPre", "def MontgomeryReduceInputPre : Prop := True"),
        ("montgomery_reduce_Correctness", "theorem montgomery_reduce_Correctness : True := trivial"),
    ] {
        fs::write(
            repo.join("Tablet").join(format!("{node}.lean")),
            format!("{body}\n"),
    ...
    fs::write(
        ...
        serde_json::json!({
            ...
            "pv_tablet": {
                ...
                "extraction_toolchain": {"charon": "v1", "aeneas": "v0", "lean": "4.30"},
                "extraction_models": [{
                    "id": "model:montgomery_reduce",
                    "node": "montgomery_reduce",
                    "name": "montgomery_reduce",
                    "lean": "def montgomery_reduce (value : Int) : Result Int :=\n  .ok value",
                    "namespace_context": "",
                    "provenance": {"source_sha256": "deadbeef"}
                }],
                // goal_mode omitted ⇒ defaults to "lean" (mode B).
                "verification_target_definitions": [
                    {
                        "id": "spec:montgomery_reduce",
                        "node": "montgomery_reduce_Spec",
                    ...
                    {
                        "id": "pre:montgomery_reduce",
                        "node": "MontgomeryReduceInputPre",
                        "name": "MontgomeryReduceInputPre",
                        "role": "precondition",
                        "lean": "def MontgomeryReduceInputPre (value : Int) : Prop :=\n  0 \u{2264} value",
                        "namespace_context": ""
                    }
                ],
                "verification_target_statements": [{
                    "id": "goal:montgomery_reduce_Correctness",
                    "node": "montgomery_reduce_Correctness",
                    "name": "montgomery_reduce_Correctness",
                    "role": "correctness",
                    "lean": "theorem montgomery_reduce_Correctness (value ret : Int) (h : MontgomeryReduceInputPre value) (he : montgomery_reduce value = .ok ret) : montgomery_reduce_Spec value ret :=",
                    "namespace_context": ""
                }]
            }
        })
    ...
    let registry = &init_json["state"]["configured_challenge_targets"];
    // The Spec predicate def is a byte-pinned challenge DEF (whole-decl,
    // include_body), forced to `def` kind — the predicate BODY is pinned.
    assert_eq!(registry["spec:montgomery_reduce"]["kind"], serde_json::json!("def"));
    assert_eq!(
        registry["spec:montgomery_reduce"]["lean"],
        serde_json::json!("def montgomery_reduce_Spec (value ret : Int) : Prop :=\n  ret = value % 7")
    );
    // The precondition predicate def is likewise a byte-pinned challenge DEF.
    assert_eq!(registry["pre:montgomery_reduce"]["kind"], serde_json::json!("def"));
    assert_eq!(
        registry["pre:montgomery_reduce"]["lean"],
        serde_json::json!("def MontgomeryReduceInputPre (value : Int) : Prop :=\n  0 \u{2264} value")
    );
    // The goal theorem (which REFERENCES the pinned predicates) rides as a
    // challenge Theorem (statement-only pin); the model def stays a `def`.
    assert_eq!(
        registry["goal:montgomery_reduce_Correctness"]["kind"],
        serde_json::json!("theorem")
    );
    assert_eq!(registry["model:montgomery_reduce"]["kind"], serde_json::json!("def"));
    // The seeded tablet node files are auto-claimed: each challenge target is
    // already covered by its matching pinned node at init.
    assert_eq!(
        init_json["state"]["live"]["challenge_coverage"],
        serde_json::json!({
            "goal:montgomery_reduce_Correctness": ["montgomery_reduce_Correctness"],
            "model:montgomery_reduce": ["montgomery_reduce"],
            "pre:montgomery_reduce": ["MontgomeryReduceInputPre"],
            "spec:montgomery_reduce": ["montgomery_reduce_Spec"]
        })
    );
    // KEYSTONE: `configured_targets` is still EMPTY ⇒ no Paper-faithfulness
    // dispatch (the new pinned defs do not arm the lane).
    assert_eq!(init_json["state"]["configured_targets"], serde_json::json!([]));
    // Roles: the Spec def node carries `spec`, the precond def node carries the
    // NEW `precondition` role, the goal node `correctness`, the model
    // `extraction_model`.
    let roles = &init_json["state"]["node_role"];
    assert_eq!(roles["montgomery_reduce_Spec"], serde_json::json!("spec"));
    assert_eq!(
        roles["MontgomeryReduceInputPre"],
        serde_json::json!("precondition")
    );
    assert_eq!(
        roles["montgomery_reduce_Correctness"],
        serde_json::json!("correctness")
    );
    assert_eq!(roles["montgomery_reduce"], serde_json::json!("extraction_model"));
    // Mode B is the default ⇒ `pv_goal_mode` stays off the wire.
    assert!(
        init_json["state"].get("pv_goal_mode").is_none(),
        "mode B (default Lean) keeps pv_goal_mode off the wire"
    );
    assert_eq!(init_json["state"]["pv_tablet_configured"], serde_json::json!(true));
}

// PV mode-B contract-predicate pins are a mode-B ("lean") construct: declaring
// `verification_target_definitions` under `goal_mode: "prose"` is rejected at
// parse (mode-A goals are worker-authored prose; there is no pinned-predicate
// contract to trust).
#[test]
fn runtime_cli_init_pv_prose_rejects_target_definitions() {
    let tmp = project_tempdir();
    let workspace = tmp.path().join("workspace");
    let repo = workspace.join("repo");
    let root = workspace.join("runtime");
    ...
    fs::write(
        ...
        serde_json::json!({
            ...
            "pv_tablet": {
                ...
                "verification_backend": "lean",
                "goal_mode": "prose",
                "verification_targets": ["montgomery_reduce"],
                "verification_target_definitions": [{
                    "id": "spec:montgomery_reduce",
                    "node": "montgomery_reduce_Spec",
                    "name": "montgomery_reduce_Spec",
    ...
    assert_ne!(
        init_json["status"], "ok",
        "prose mode with verification_target_definitions must be rejected: {init_json}"
    );
}

// PV #1 — mode A (prose goals). `goal_mode: "prose"` + the existing
...
// available), records the autoseed prefixes, and writes `pv_goal_mode: "prose"`
// onto the wire (the non-default mode IS serialized).
#[test]
fn runtime_cli_init_pv_mode_a_seeds_configured_targets_and_records_prose_mode() {
    let tmp = project_tempdir();
    let workspace = tmp.path().join("workspace");
    let repo = workspace.join("repo");
    let root = workspace.join("runtime");
    ...
    let init_json: Value = serde_json::from_slice(&init_output.stdout).expect("parse init output");
    assert_eq!(init_json["status"], "ok", "pv mode-A init: {init_json}");
    ...
    // The non-default mode is recorded on the wire.
    assert_eq!(
        init_json["state"]["pv_goal_mode"],
        serde_json::json!("prose")
    );
    // No goal-statement challenge theorems in mode A (only the model def).
    assert_eq!(
        init_json["state"]["configured_challenge_targets"]["model:fModel"]["kind"],
        serde_json::json!("def")
    );
    assert!(
        init_json["state"]["configured_challenge_targets"]
            ...
            .is_none(),
        "mode A seeds no goal-statement challenge targets"
    );
}

// PV #1 — config discipline: mode B ("lean") with the mode-A
// `verification_targets` set is a half-config that would silently re-arm the
// faithfulness lane; reject it at parse so the operator picks one carrier.
#[test]
fn runtime_cli_init_pv_mode_b_rejects_prose_verification_targets() {
    let tmp = project_tempdir();
    let workspace = tmp.path().join("workspace");
    let repo = workspace.join("repo");
    let root = workspace.join("runtime");
    ...
    assert_ne!(
        init_json["status"], "ok",
        "mode B with verification_targets must be rejected: {init_json}"
    );
}
