...
//!   * spool claim/defer/finalize mechanics (rename-claim, unbounded
//!     infrastructure retry, verdict append preserving unknown
//!     daemon fields);
...
use trellis_kernel::sidecar::{
    ...
    parse_attempt_record, preflight_claimed_attempt, publish_grunt_feedback,
    run_journal_recovery, sidecar_body_ban_scan, spool_dirs, splice_proof_body,
    supervisor_workspace_tablet_dir, sweep_orphaned_claims, write_apply_journal, DeferOutcome,
    JournalRecoveryOutcome, SidecarApplyJournal, SidecarPreflight, SidecarRuntimeConfig,
};
...
fn eligible_state() -> ProtocolState {
    ...
    state
        ...
        .insert(n.clone(), "c1".to_string());
    state.substantiveness_status.insert(n.clone(), trellis_kernel::SubstantivenessStatus::Pass);
    state
        .live
...
#[test]
fn drift_row_ts_phase_defaults_open_and_content_gates() {
    // Stating-like phase defaults open even with uncovered targets.
    let mut state = eligible_state();
    ...
    // Operators can restore the old after-coverage cost gate.
    let gated_cfg = SidecarRuntimeConfig {
        phases_stating_after_coverage: true,
        ..cfg()
    };
    assert_eq!(
        preflight_claimed_attempt(&state, &gated_cfg, &record, Some(NODE_FILE)),
        SidecarPreflight::Reject {
            reason: "ineligible".to_string()
        }
    );
    // Once covered, the restored gate opens; a concurrent statement edit
    // still dies on the content gate.
    state
        .live
        .coverage
    ...
    assert_eq!(
        preflight_claimed_attempt(&state, &gated_cfg, &record, Some(&edited)),
        SidecarPreflight::Reject {
            reason: "stale_content".to_string()
        }
    );
...
#[test]
fn preflight_recovers_the_exact_generation_pruned_by_lane_drift() {
    // Production cycle 177: a worker burst moved the statement-check
    // fingerprint while this already-queued generation's completed closure
    // was entering the spool. The event-tail prune ran first; by ingest the
    // statement check had settled back to Pass, but membership was gone and
    // the old gate returned `not_queued`.
    let mut state = eligible_state();
    state.cycle = 177;
    state
        .live
        .corr_current_fingerprints
        .insert(node("Rung"), "c-drifted".to_string());
    state.prune_sidecar_queue();
    assert!(state.sidecar_queue.is_empty());
    assert_eq!(state.sidecar_queue_prune_log.last().unwrap().reason, "lane_drift");

    // Re-verification restored Pass without restoring the pruned entry. The
    // claimed closure carries the exact generation that the prune row names.
    state
        .live
        .corr_current_fingerprints
        .insert(node("Rung"), "c1".to_string());
    let record = record_for_content(NODE_FILE, "  trivial\n");
    assert_eq!(
        preflight_claimed_attempt(&state, &cfg(), &record, Some(NODE_FILE)),
        SidecarPreflight::Proceed,
        "the lane's own same-cycle drift prune must not destroy completed work"
    );
}
...
fn ban_scan_rejects_every_forbidden_and_extra_token() {
    // All 19 kernel keywords (incl. sorryAx first) ...
...
/// Exec-command family (finding 1): the elaboration-time arbitrary-IO
/// primitives. `run_cmd` / `#eval` were already banned; `run_tac`
/// (a TACTIC, reachable directly inside a proof body), `run_elab`, and
/// `run_meta` are the residual holes closed here. Each was empirically
/// shown to execute IO under the tablet toolchain (v4.30.0-rc2 +
/// Mathlib), and re-confirmed on 2026-08-27 under the current pin
/// v4.33.0 by the marker-file probe described on
/// `backend::LEAN_FORBIDDEN_KEYWORDS`. A grunt body carrying any of
/// them is rejected by the preflight scan.
#[test]
fn ban_scan_rejects_exec_command_family_including_run_tac() {
    // `run_tac` in tactic position — the direct in-proof-body ACE vector.
    assert_eq!(
        sidecar_body_ban_scan("  run_tac do IO.println \"x\"\n  trivial\n").as_deref(),
        Some("run_tac")
    );
    // `run_elab` / `run_meta` as trailing command-slot forms.
    assert_eq!(
        sidecar_body_ban_scan("  trivial\nrun_elab do pure ()\n").as_deref(),
        Some("run_elab")
    );
    ...
    // Token boundaries hold: identifiers that merely PREFIX-match a
    // banned exec token (no legitimate Lean symbol does, but the scan
    // must not over-reject) are not caught.
    assert_eq!(sidecar_body_ban_scan("  exact run_tactical\n"), None);
    assert_eq!(sidecar_body_ban_scan("  exact run_elaborate\n"), None);
    assert_eq!(sidecar_body_ban_scan("  exact run_metadata'\n"), None);
}
...
#[test]
fn spool_infrastructure_retries_are_unbounded_and_preserve_unknown_fields() {
    let dir = tempdir();
    ...
    // A second infrastructure retry is still pending and still does not spend
    // the completed proof's generation.
    let claimed = claim_attempt(&spool, &re_pending).unwrap();
    let outcome = defer_attempt(&claimed, &spool, 8, "test").unwrap();
    assert_eq!(outcome, DeferOutcome::Deferred { deferrals: 2 });
    let re_pending = next_pending_attempt(&spool).expect("still pending");
    let value: serde_json::Value =
        serde_json::from_str(&std::fs::read_to_string(re_pending).unwrap()).unwrap();
    assert_eq!(value["deferrals"], serde_json::json!(2));
    assert_eq!(
        value["last_infrastructure_retry"]["context"],
        serde_json::json!("test")
    );
    assert!(std::fs::read_dir(&spool.rejected).unwrap().next().is_none());
}
...
#[test]
fn kernel_feedback_immediately_rewrites_an_existing_grunt_result() {
    let dir = tempdir();
    let gdir = dir.path().join("sidecar/grunts/1");
    std::fs::create_dir_all(&gdir).unwrap();
    let attempt_id = "sc-feedback-Rung";
    let result_path = gdir.join(format!("result-{attempt_id}.json"));
    let log_path = gdir.join(format!("attempt-{attempt_id}.log"));
    std::fs::write(
        &result_path,
        serde_json::json!({
            "status": "success",
            "detail": "closed",
            "attempt_id": "sc-feedback-Rung"
        })
        .to_string(),
    )
    .unwrap();

    let feedback_path = publish_grunt_feedback(
        dir.path(),
        &node("Rung"),
        attempt_id,
        "rejected",
    ...
    .unwrap();
    assert!(feedback_path.is_file(), "durable daemon handoff exists");
    let result: serde_json::Value =
        serde_json::from_str(&std::fs::read_to_string(result_path).unwrap()).unwrap();
    assert_eq!(result["status"], "rejected");
    assert_eq!(result["detail"], "transition refused");
    assert_eq!(result["grunt_outcome"]["status"], "success");
    assert_eq!(result["kernel_feedback"].as_array().unwrap().len(), 1);
    let feedback_id = result["kernel_feedback"][0]["feedback_id"]
        .as_str()
        .expect("feedback id");
    let log = std::fs::read_to_string(log_path).expect("feedback appended to grunt log");
    assert!(log.contains("kernel rejected: transition refused"));
    assert!(log.contains(feedback_id));
}

/// Orphaned `claimed/*` attempts are infrastructure recovery too: both fresh
/// and previously deferred closures return to pending without spending.
#[test]
fn journal_case_a4_orphaned_claims_never_spend_the_generation() {
    let dir = tempdir();
    let spool = spool_dirs(dir.path());
    ensure_spool_dirs(&spool).unwrap();
    std::fs::write(
    ...
    assert_eq!(
        outcomes["attempt-stale.json"],
        DeferOutcome::Deferred { deferrals: 2 }
    );
    assert_eq!(std::fs::read_dir(&spool.pending).unwrap().count(), 2);
...
/// Infrastructure retries never produce a spent-generation event, while a
/// mathematical failure still does.
#[test]
fn infrastructure_retry_does_not_spend_but_mathematical_failure_does() {
    use trellis_kernel::sidecar::defer_attempt;
    ...
    std::fs::write(&claimed, attempt_record_json("sha", "  trivial\n")).unwrap();
    // Repeated infrastructure retries stay in `pending/`; no outcome event is
    // created, so the exact queue generation survives.
    ...
    assert_eq!(second, DeferOutcome::Deferred { deferrals: 2 });

    // A mathematical failure is an actual attempt outcome and expires the
    // generation through the ordinary event path.
    let outcome = trellis_kernel::SidecarAttemptOutcome {
        node: node("Rung"),
        entry_seq: 7,
        attempt_id: "sc-math-failure".to_string(),
        status: "failed".to_string(),
        detail: "proof did not compile".to_string(),
