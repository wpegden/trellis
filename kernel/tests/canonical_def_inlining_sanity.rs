...
const PV_MODEL: &str = "pv/canonical/EXTRACTION_MODEL.md";
...
fn worker_theorem_global_sees_all_four_canonical_defs() {
    ...
    let fragments = fragments_of(&worker_contract_payload(&req, None));
...
fn reviewer_theorem_stating_sees_all_four_canonical_defs() {
    ...
    let fragments = fragments_of(&review_contract_payload(&req, None));
...
#[test]
fn reviewer_cleanup_sees_only_the_correspondence_def() {
    // A Cleanup state may carry an open correspondence blocker on a
    // non-protected node (`ProtocolState::formalization_valid`), and the
    // reviewer adjudicates it by dispatching the repair route — so it
    // gets the CORRESPONDENCE definition and nothing else. No other lane
    // can raise a blocker in Cleanup.
    ...
    assert_contains(&fragments, CORR, "reviewer/Cleanup");
...
fn verifier_paper_target_scenario_sees_only_faithfulness() {
    ...
    let fragments = fragments_of(&paper_contract_payload(&req, None));
...
fn verifier_soundness_sees_only_soundness() {
    ...
    let fragments = fragments_of(&soundness_contract_payload(&req, None));
...
fn no_duplicate_fragments_in_any_assembled_prompt() {
    ...
    let scenarios: Vec<(&str, Box<dyn Fn() -> Vec<String>>)> = vec![
        ...
        (
            ...
            Box::new(|| {
                fragments_of(&review_contract_payload(
                    &review_request(Phase::TheoremStating),
                    None,
                ))
            }),
        ),
        (
            ...
            Box::new(|| {
                fragments_of(&review_contract_payload(
                    &review_request(Phase::ProofFormalization),
                    None,
                ))
            }),
        ),
        (
            "reviewer/Cleanup",
            Box::new(|| {
                fragments_of(&review_contract_payload(&review_request(Phase::Cleanup), None))
            }),
        ),
        (
            ...
            Box::new(|| {
                ...
                req.paper_verify_targets = std::iter::once(TargetId::from("T1")).collect();
                fragments_of(&paper_contract_payload(&req, None))
            }),
        ),
        (
        ...
        (
            ...
            Box::new(|| {
                ...
                req.sound_verify_node = Some(node.clone());
                req.sound_verify_nodes = std::iter::once(node).collect();
                fragments_of(&soundness_contract_payload(&req, None))
            }),
        ),
    ];
...
// ===========================================================================
// Isabelle agent-enablement: the backend-selection hook (Option A)
// ===========================================================================
//
// `worker_contract_payload(request, repo_path)` resolves the tablet target via
// `tablet_target_for_repo(repo_path)` and branches the Lean-specific fragment
// slots. These tests assert:
//   * an `isabelle_hol` repo gets the `_isabelle` sibling fragments, and
//   * a Lean repo (and the `None` no-repo default) gets the UNCHANGED Lean
//     paths — the byte-identity guarantee for the live Lean run.

/// The Lean-specific worker fragment slots the hook branches, and their
/// Isabelle siblings. (15_loogle + 17_mathlib collapse to the single
/// 17_isabelle_library fragment under IsabelleHol.)
const LEAN_WORKER_SLOTS: &[&str] = &[
...
const ISABELLE_WORKER_SLOTS: &[&str] = &[
    "shared/20_read_files_isabelle.md",
    "worker/common/17_isabelle_library.md",
...
fn write_target_config(repo: &std::path::Path, target: &str) {
    std::fs::write(
        repo.join("trellis.config.json"),
        format!("{{\"workflow\": {{\"default_target\": \"{target}\"}}}}\n"),
    )
    .expect("write trellis.config.json");
}

fn proof_local_worker_request() -> WrapperRequest {
    worker_request(
        WorkerProfile::ProofHard,
        WorkerValidationKind::ProofLocal,
        Phase::ProofFormalization,
...
#[test]
fn worker_fragments_select_isabelle_siblings_for_isabelle_hol_repo() {
    let tmp = common::project_tempdir();
    let repo = tmp.path();
    write_target_config(repo, "isabelle_hol");

    let req = proof_local_worker_request();
    let fragments = fragments_of(&worker_contract_payload(&req, Some(repo)));

    for isa in ISABELLE_WORKER_SLOTS {
        assert_contains(&fragments, isa, "worker/IsabelleHol");
    }
    // The Lean-specific slots must NOT appear when the target is Isabelle.
    for lean in LEAN_WORKER_SLOTS {
        assert_not_contains(&fragments, lean, "worker/IsabelleHol");
    }
    assert_no_duplicates(&fragments, "worker/IsabelleHol");
}

#[test]
fn worker_fragments_stay_lean_for_lean_repo_and_no_repo() {
    // A repo explicitly configured for Lean, and the no-repo (`None`) default,
    // must both produce the verbatim Lean fragment paths — no Isabelle sibling
    // ever leaks onto the live Lean path.
    let tmp = common::project_tempdir();
    let repo = tmp.path();
    write_target_config(repo, "lean");

    let lean_repo_fragments =
        fragments_of(&worker_contract_payload(&proof_local_worker_request(), Some(repo)));
    let no_repo_fragments =
        fragments_of(&worker_contract_payload(&proof_local_worker_request(), None));

    // The two must be byte-identical (the default resolves to Lean).
    assert_eq!(
        lean_repo_fragments, no_repo_fragments,
        "explicit-Lean and no-repo worker fragment lists must match"
    );

    for (label, fragments) in [
        ("worker/Lean", &lean_repo_fragments),
        ("worker/None", &no_repo_fragments),
    ] {
        for lean in LEAN_WORKER_SLOTS {
            assert_contains(fragments, lean, label);
        }
        for isa in ISABELLE_WORKER_SLOTS {
            assert_not_contains(fragments, isa, label);
        }
    }
}

// --- Extraction-model canonical doc: one source of truth, all three PV roles ---
//
// `pv/canonical/EXTRACTION_MODEL.md` states the model-inviolate policy
// (byte-pinned identity = slice ∪ namespace context, the `open <crate>`
// idiom, and the reviewer's never-direct-an-edit obligation). It must reach
// the WORKER, REVIEWER, and VERIFIER (substantiveness/correspondence/soundness)
// on a PV request, and must be absent on an all-math request.

fn pv_worker_request(
    profile: WorkerProfile,
    validation: WorkerValidationKind,
    phase: Phase,
) -> WrapperRequest {
    let mut req = worker_request(profile, validation, phase);
    req.is_pv = true;
    req
}

#[test]
fn pv_worker_proof_local_sees_extraction_model_doc() {
    let req = pv_worker_request(
        WorkerProfile::ProofHard,
        WorkerValidationKind::ProofLocal,
        Phase::ProofFormalization,
    );
    let fragments = fragments_of(&worker_contract_payload(&req, None));
    assert_contains(&fragments, PV_MODEL, "pv-worker/ProofLocal");
    assert_no_duplicates(&fragments, "pv-worker/ProofLocal");
}

#[test]
fn pv_worker_theorem_global_sees_extraction_model_doc() {
    let req = pv_worker_request(
        WorkerProfile::Theorem,
        WorkerValidationKind::TheoremGlobal,
        Phase::TheoremStating,
    ...
    let fragments = fragments_of(&worker_contract_payload(&req, None));
    assert_contains(&fragments, PV_MODEL, "pv-worker/TheoremGlobal");
}

#[test]
fn math_worker_does_not_see_extraction_model_doc() {
    let req = worker_request(
        WorkerProfile::ProofHard,
        WorkerValidationKind::ProofLocal,
        Phase::ProofFormalization,
    ...
    let fragments = fragments_of(&worker_contract_payload(&req, None));
    assert_not_contains(&fragments, PV_MODEL, "math-worker/ProofLocal");
}

fn pv_review_request(phase: Phase) -> WrapperRequest {
    let mut req = review_request(phase);
    req.is_pv = true;
    req
}

#[test]
fn pv_reviewer_proof_formalization_sees_extraction_model_doc() {
    let req = pv_review_request(Phase::ProofFormalization);
    let fragments = fragments_of(&review_contract_payload(&req, None));
    assert_contains(&fragments, PV_MODEL, "pv-reviewer/ProofFormalization");
    assert_no_duplicates(&fragments, "pv-reviewer/ProofFormalization");
}

#[test]
fn pv_reviewer_theorem_stating_sees_extraction_model_doc() {
    let req = pv_review_request(Phase::TheoremStating);
    let fragments = fragments_of(&review_contract_payload(&req, None));
    assert_contains(&fragments, PV_MODEL, "pv-reviewer/TheoremStating");
}

#[test]
fn math_reviewer_does_not_see_extraction_model_doc() {
    let req = review_request(Phase::ProofFormalization);
    let fragments = fragments_of(&review_contract_payload(&req, None));
    assert_not_contains(&fragments, PV_MODEL, "math-reviewer/ProofFormalization");
}

#[test]
fn pv_verifier_substantiveness_sees_extraction_model_doc() {
    use trellis_kernel::model::NodeId;
    let mut req = WrapperRequest {
        kind: RequestKind::Paper,
        phase: Phase::TheoremStating,
    ...
    req.substantiveness_verify_nodes = std::iter::once(NodeId::from("N1")).collect();
    let fragments = fragments_of(&paper_contract_payload(&req, None));
    assert_contains(&fragments, PV_MODEL, "pv-verifier/substantiveness");
    assert_no_duplicates(&fragments, "pv-verifier/substantiveness");
}

#[test]
fn pv_verifier_correspondence_sees_extraction_model_doc() {
    use trellis_kernel::model::NodeId;
    let mut req = WrapperRequest {
        kind: RequestKind::Corr,
        phase: Phase::TheoremStating,
    ...
    req.verify_nodes = std::iter::once(NodeId::from("N1")).collect();
    let fragments = fragments_of(&correspondence_contract_payload(&req, None));
    assert_contains(&fragments, PV_MODEL, "pv-verifier/correspondence");
    assert_no_duplicates(&fragments, "pv-verifier/correspondence");
}

#[test]
fn pv_verifier_soundness_sees_extraction_model_doc() {
    use trellis_kernel::model::NodeId;
    let mut req = WrapperRequest {
        kind: RequestKind::Sound,
        phase: Phase::TheoremStating,
    ...
    req.sound_verify_nodes = std::iter::once(node).collect();
    let fragments = fragments_of(&soundness_contract_payload(&req, None));
    assert_contains(&fragments, PV_MODEL, "pv-verifier/soundness");
    assert_no_duplicates(&fragments, "pv-verifier/soundness");
}

#[test]
fn math_verifier_lanes_do_not_see_extraction_model_doc() {
    use trellis_kernel::model::NodeId;
    // substantiveness
    ...
    subst.substantiveness_verify_nodes = std::iter::once(NodeId::from("N1")).collect();
    assert_not_contains(
        &fragments_of(&paper_contract_payload(&subst, None)),
        PV_MODEL,
        "math-verifier/substantiveness",
    );
    // correspondence
    ...
    corr.verify_nodes = std::iter::once(NodeId::from("N1")).collect();
    assert_not_contains(
        &fragments_of(&correspondence_contract_payload(&corr, None)),
        PV_MODEL,
        "math-verifier/correspondence",
    );
    // soundness
    let mut sound = WrapperRequest {
        kind: RequestKind::Sound,
        phase: Phase::TheoremStating,
        ..WrapperRequest::default()
    };
    let node = NodeId::from("N1");
    sound.sound_verify_node = Some(node.clone());
    sound.sound_verify_nodes = std::iter::once(node).collect();
    assert_not_contains(
        &fragments_of(&soundness_contract_payload(&sound, None)),
        PV_MODEL,
        "math-verifier/soundness",
    );
}
