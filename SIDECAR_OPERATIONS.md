...
tries to close queued nodes with a cheap model, in isolated workspaces, without
touching the supervisor's own loop. README §10 covers what it is and how to
...
| `candidates.json` | **kernel → daemon**: the export the daemon reads; the daemon never writes it |
...
| `queue_empty` | the reviewer's queue is empty — the pool has nothing to do |
...
| `no_api_key` | the configured key file/variable produced nothing at request time |
...
| Daemon up, `SUSPENDED` | 3 consecutive transport failures | `ledger.jsonl` tail, `suspended_until_ms` | check the API key/endpoint; it retries after 30 min |
