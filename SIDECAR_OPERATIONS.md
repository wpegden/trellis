...
**`grunts` is not the number of workspaces on disk.** `status.json`'s `grunts`
is the CONFIGURED pool width (`daemon.grunts`); lowering it drops the extra
slots but has never removed their directories, and each one is a full Lean
checkout. One live run reported `grunts: 1` while `sidecar/grunts/` held
three at 16G + 12G + 9.9G — two of them orphaned by a 3 → 1 config change,
paid for again on every migrated resume. `status.json` now also carries
`workspaces` (the indices actually on disk) and `status` prints a line when
the two disagree. Deleting an orphaned `grunts/<K>/repo` by hand is safe: the
checkout is a cache, re-cloned by the next daemon's bootstrap.
...
* **not running + live attempt processes** — that is a drain window. The
  ...
  this reason.
* **not running + a `STOPPED:` / `DRAINED:` line** — the daemon took an exit
  path on purpose and said so. `status.json` carries a `stopped` block
  (reason, the run phase that caused it, what it did) and its `phase` reads
  `stopped` or `drained` rather than `running`. Every exit before this version
  left `phase: running` behind, so a stopped daemon and a crashed one were
  distinguishable only by `last_pass.status`. A `STOPPED: … phase_complete`
  line means the run is past formalization: do not restart it (§4).
...
### The third way out: the daemon stops itself at the end of formalization

There is nothing to operate here — it is listed with the sentinels because it
is the same shutdown, and because the state it leaves is what you will find.

The daemon reads the run's `phase` out of the export every pass. When that
phase is one the sidecar can never be given work in — `Cleanup` or `Complete`,
where every accepted node is Lean-closed by construction and the engine's
apply gate refuses a grunt closure — it takes **the `stop` path** (each
in-flight attempt cancelled as a process group, bookkept, its generation spent
and published), additionally **deletes every `grunts/<K>/repo` checkout**, and
exits. `last_pass.status` is `phase_complete` and `status.json` carries:

```json
"phase": "stopped",
"stopped": {"reason": "phase_complete", "run_phase": "Cleanup",
            "cancelled_attempts": 0, "workspaces_released": [0, 1, 2],
            "restartable": true}
```

Why each part is the way it is:

* **The daemon decides, not the kernel.** The export already carries the
  phase, so no kernel change and no rebuild is needed — and the kernel does
  not become a second writer under `sidecar/`, which is the ownership rule
  §6 is built around. A kernel-written sentinel would also collide with
  `clear_startup_sentinels`.
* **It cancels rather than drains.** In a terminal phase an attempt allowed
  to run on would spend its whole wall on a closure the apply gate rejects,
  unsupervised, with no next daemon to adopt it. This is the one case where
  `stop`'s cost — a spent generation — buys something, because the entries
  are moot in a phase whose window can never open.
* **An operator sentinel wins.** Both sentinels are checked first, so a
  `drain` for a redeploy at the very boundary that flips the phase keeps its
  children AND their workspaces.
* **A stale export never triggers it.** Same rule as the cancel-watch: never
  act on a frozen view. The export that carries the transition is written at
  that boundary, so the trigger is seen within one poll of the phase change.
  A daemon that finds a *stale* `Cleanup` export keeps polling; it winds down
  at the first fresh one.
* **It is an exit, not a disablement.** Nothing it writes stops a daemon from
  starting again over the same runtime root — no sentinel is left on disk —
  and the deleted checkouts are re-cloned by `startup()`'s bootstrap. That
  matters for `add-targets`, which revives a `Complete` run into
  `RevisionStating`: restart the daemon (§3) and the pool rebuilds itself.
  A daemon started while the run is *already* in a terminal phase skips the
  bootstrap entirely and winds down on its first pass, so it does not clone
  tens of GB in order to delete them.

The workspaces are the point of the sweep, not a side effect: the operator
rewind procedure deliberately does not quarantine `sidecar/`, so a migrated
resume copies them. It sweeps **every numeric
`grunts/<K>` directory**, not the configured pool width — see §1's note on
`grunts` vs `workspaces`.
...
| `phase_complete` | the run left the phases the sidecar works in; the daemon wound itself down and exited (§4) |
...
| `status` exit 1, `STOPPED: … phase_complete` | the run left formalization; the daemon wound itself down (§4) | the `stopped` block in `status.json` | nothing — restarting only repeats the wind-down. Start it again only if the run re-enters formalization |
