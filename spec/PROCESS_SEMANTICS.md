...
- **Parallel-closure sidecar (between cycles)**: at the inter-cycle
  ...
  strict no-op (byte-identical loop). Measured single-node gate cost
...
Valid worker acceptance is orphan-free:
...
- Valid responses that leave live orphans are rejected.
...
bursts must end orphan-free regardless of the window. Mirrored in
`SupervisorCore.tla` / `SupervisorProtocol.tla` as
`OrphanConstructionWindowOpen`.

Legacy `pending_task.orphan_cleanup_nodes` remains a persisted-state
carrier. New engine routing does not synthesize it.
