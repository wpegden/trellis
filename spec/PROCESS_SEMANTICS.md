...
- `substantiveness_status[node] ∈ {Unknown, Pass, FalseAsStated, Fail}`
  (per-node paper-lane verdict; prose mode uses `FalseAsStated` for a
  substantive intended obligation refuted under the pinned model; lane vote
  also admits `NotDoneYet` at the response level — see
  `SubstantivenessStatus` at model.rs —
...
- **Parallel-closure sidecar (between cycles)**: at the inter-cycle
  boundary (stage `Start`, no in-flight request) the runtime may ingest
  at most `sidecar.apply.max_applies_per_boundary` (default 1) spooled
  sidecar closures: journal recovery first (unconditional), then claim
  → gate sequence → `SidecarClosure` event (its apply emits
  `CommitCheckpoint`), then the `candidates.json` export — export
  always AFTER any same-boundary apply so the daemon sees post-apply
  eligibility. The gate sequence requires the node CURRENTLY in the
  reviewer-managed grunt queue with the attempt's exact entry
  generation (`not_queued` / `stale_generation` rejections), and the
  engine apply re-asserts membership and consumes the queue entry in
  the same transition. Absent `sidecar` config block ⇒ the hook is a
  strict no-op (byte-identical loop). The state/replay phase window is open
  throughout TheoremStating, RevisionStating, and ProofFormalization;
  `sidecar.phases.stating_after_coverage: true` narrows the runtime hook back
  to the original after-coverage stating window without entering replay
  state. Measured single-node gate cost
  on the live host (checker medians): compile ≈ 12.6 s (p90 20.4 s) +
  local-closure probe ≈ 5.5–6.7 s + axioms ≈ 2.3 s — comfortably
  inside the 300 s apply budget at an idle boundary; the corr
  no-drift recompute (gate 9) adds one fingerprint observation whose
  cost is workload-dependent (re-measured in the sidecar E2E rig
  rather than claimed constant). See CORE_SPEC_DEVIATIONS.md §29.
...
- **Cleanup unreachable-node maintenance**: before Cleanup dispatches its
  first request for a cycle, the kernel deletes the importer-free frontier of
  the graph outside the paper/challenge coverage closure and repeats to a
  fixpoint (§8).
...
- `Phase::Cleanup`: run the kernel-owned unreachable-node fixpoint before
  selecting or issuing the cycle's Audit, Review, or Worker request (§8).
...
- **`reset_blockers`** — "I want the panel to re-vote on this." Effect:
  ...
  node up and runs fresh. A reset clears nothing: `Unknown` is
  non-`Pass`, so `global_blockers()` keeps emitting the blocker and only
  a verifier pass retires it. **Scope restriction**: the allowed set
  (`request_allowed_reset_blockers` at model.rs) offers every failed
  blocker kind in theorem-stating review responses, `Substantiveness`
  blockers alone in proof-formalization, and nothing in cleanup.
  Further, only blockers whose current state is `Fail`
  (`current_failed_blockers` at model.rs) are eligible for reset;
  split-induced `Unknown` blockers cannot be reset — they must be
  resolved via task or override. Both offers are narrowed once more,
  to a `Fail` the reset can actually move: the status/fingerprint arm
  at the tail of `current_substantiveness_state`
  (`is_derived_substantiveness_fail` at model.rs). A node claiming a
  rejected deviation reads `Fail` from a short-circuit ahead of that
  arm and would survive its own reset, so — like the two
  empty-coverage filters below — it stays out of the offer and is
  repaired by the worker, while the `Deviation` blocker that produced
  it stays both resettable and taskable.

  The proof-formalization carve-out exists because substantiveness
  clause 2 is relational (it reads a node against its importers) while
  `SubstantivenessFingerprint` is node-local: a repair applied to an
  importer leaves the failed child's fingerprint untouched and off
  `substantiveness_verify_nodes()`, so the stale verdict has no other
  route back to the verifier.

  Practical consequence: in cleanup, and for every other blocker kind
  in proof-formalization, the partition reduces to
  `task ∪ override == request.blockers`. If the reviewer disagrees with
  a unanimous-Fail verdict there, the only recourse is a whole-state
  reset (`LastCommit`/`LastClean`) or to accept the Fail and issue the
  worker a task to address it.
...
- Proof-formalization global repair:
  - Step A (`global_repair_request != None`) is a non-worker Continue.
    ...
    scope escape hatch. An accepted Step A while a Step B grant is
    still pending supersedes that grant: the kernel drops it at the
    accept point and records it on the pending request
    (`superseded_grant`) for the adjudicating audit.
...
- **Kernel cleanup-start deletion**
  (`ProtocolState::cleanup_unreachable_deletion_plan` and
  `apply_cleanup_unreachable_deletion`): before a Cleanup cycle dispatches
  work, removes unreachable node pairs and every live/current state reference,
  commits the resulting state, and records the deletion rounds in the command,
  event log, protocol state, and checkpoint. This is kernel maintenance, not a
  FinalCleanup worker edit, so the worker validator's deletion ban remains
  unchanged.
...
- Picks `reset_blockers` → status goes back to Unknown; next verify
  re-runs the panel. Offered in theorem-stating and, for
  `Substantiveness` alone, in proof-formalization — see §4.4 for the
  scope restriction.
...
theorem-stating, and for a `Substantiveness` blocker in
proof-formalization, the reviewer's recourse for a unanimous-Fail is
`reset_blockers` — forcing a re-vote after (presumably) editing the
content. Everywhere else the reviewer must either accept the Fail
(assign the blocker to `task_blockers` so the worker addresses it),
leave it live for a later cycle to handle, or fall back to
`LastCommit`/`LastClean` whole-state resets.
...
- **`reset_blockers`** — durable status write (Unknown, clear
  ...
  blocker; no worker routing. Limited to the per-phase offer set and
  current state == Fail (§4.4).
...
## 8. Orphan deletion contract

An **orphan** is a node in `present_nodes` not reachable from any
configured target via the coverage/import closure.

Valid worker acceptance is same-burst responsible:

- `deleted_nodes` must exactly equal Tablet nodes removed on disk.
- Non-cleanup valid workers may remove exactly the same-burst orphan set:
  nodes that would be orphaned after the worker's import and claim edits,
  before applying those deletions.
- Valid responses that create new live orphans are rejected. Orphans already
  present in the dispatch baseline remain visible but do not reject unrelated
  work.
- Separate orphan-cleanup dispatch is retired; workers must reattach
  support or delete newly unsupported nodes in the same burst.

**Uncovered-target orphan window.** While some configured paper target
has an empty PRE-burst covering set (`orphan_construction_window_open`,
the window the worker was dispatched under), the must-delete-own-new-
orphans requirement and (for non-cleanup bursts) the leaves-NEW-orphans
rejection are waived: layered, support-first construction may leave live
orphans. Deletion legality is unchanged — removed nodes must still lie
in the same-burst orphan set (supported nodes may never be deleted). The
covering statement closes the window; a burst that itself covers the
final uncovered target while leaving unrelated orphans is still accepted
(they become pre-existing-tolerated, sweepable later). Cleanup-validation
bursts receive no construction-window waiver: their post-orphan set must be a
subset of their baseline-orphan set. Mirrored in
`SupervisorCore.tla` / `SupervisorProtocol.tla` as
`OrphanConstructionWindowOpen`.

Legacy `pending_task.orphan_cleanup_nodes` remains a persisted-state carrier.
New engine routing does not synthesize it. Absolute orphan inventory is an
operator-visible diagnostic rather than a per-burst acceptance requirement.

At the start of every Cleanup cycle, before any Audit, Review, or Worker request
is issued, the kernel resolves that absolute inventory itself. Roots are the
present nodes in paper `coverage` union challenge `challenge_coverage`; edges
are `deps[importer] -> imported`, and `Preamble` is excluded. The kernel removes
the importer-free frontier among unreachable nodes, recomputes reachability and
importers, and repeats to a fixpoint. A proposed deletion of `Preamble` or a
coverage root, an empty present coverage-root set, an empty surviving graph, a
lost coverage root, or an unreachable subgraph with no importer-free frontier
fails the transition loudly. The runtime validates every complete node pair
before unlinking its backend source (`.lean`/`.thy`) and `.tex` file. The pure
transition prunes current structural, verifier, closure, telemetry, scheduling,
and reviewer state, refreshes committed mirrors, and carries the identical
`CleanupUnreachableDeletionRecord` through the event command and checkpoint.
