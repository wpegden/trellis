...
would model the legality scope (the per-phase offer set at
`RequestAllowedResetBlockers`) as a structural constraint.
...
(Bug 2, live-run incident 2026-06-26) that live entirely below the spec's
...
2. **State-integrity faults route to LastCommit, never LastClean (round-2).**
   ...
   cycles back for that run). "clean" is a VERIFIER-LANE judgement, not
...
## 29. system_feedback halting is a runtime concern outside step semantics (opt-in knob)

**Kernel reality.** A non-empty `system_feedback` on an accepted response
is recorded to the unified `<runtime>/system_feedback_log.jsonl`, and —
only when the operator opts in via top-level `system_feedback_halt: true`
in `trellis.config.json` (env `TRELLIS_SYSTEM_FEEDBACK_HALT=1/0` takes
precedence in both directions; the shipped default is
log-and-continue) — a sticky `system_feedback_halt.json` marker is
written whose presence makes the supervisor `run` loop break and the
bridge refuse new dispatch. The marker CHECK is unconditional: a marker
already on disk (an operator-era halt) is honored regardless of the
current knob value. Response processing is never rolled back by the
halt — the response was accepted and persisted before the marker landed;
the halt only stops FUTURE dispatch. Per-fingerprint acknowledgments
(`ack_system_feedback`) log-and-continue in both modes.

**Disposition.** No spec impact. The halt/break sits entirely outside the
spec's step semantics (like the stop-after-checkpoint sentinel and the
checker-disagreement halt): the spec models accepted responses
identically whether or not the supervisor pauses afterwards, and neither
the marker, the knob, nor the log is spec state.

---

## 30. Parallel-closure sidecar: reviewer-managed grunt queue + atomic `ApplySidecarClosure` vs the runtime ingest pipeline

**Design (SupervisorCore.tla).** Two spec objects carry the sidecar:

- `sidecarQueue ⊆ Nodes` — the reviewer-managed grunt queue,
  abstracted as a SET (the spec's job is authority and lifecycle;
  order is daemon scheduling with no safety content). Lifecycle:
  `ReviewContinue` non-deterministically draws adds from the
  eligible-shaped open nodes (`SidecarQueueAddable`: statement lanes
  Pass) and removes from the queue, then drops the routed node:
  `sidecarQueue' = ((sidecarQueue \ rem) ∪ add) \ {newActive}` —
  routing the worker at a queued node dequeues it, with `newActive ∉
  add` the one shape left illegal (the routing would undo the add);
  `AcceptWorker` (the only other `openNodes`/`presentNodes` shrinker)
  conjoins the kernel tail-prune's spec image
  (`sidecarQueue' = sidecarQueue ∩ openNodes'`);
  `ApplySidecarClosure(n)` REQUIRES `n ∈ sidecarQueue` and consumes
  the entry in the same transition; `ExpireSidecarQueueEntry(n)`
  removes a queued entry at a boundary and touches nothing else.
  Invariant: `SidecarQueueSubsetOpen`. The queue-aware actions frame
  the sidecar vars explicitly in `Next`; every queue-blind action stays
  under the `UNCHANGED SidecarVars` blanket.
- `sidecarCloses` — applied-closure counter against the
  `MaxSidecarCloses` state-space bound (a TLC knob with no kernel
  counterpart).

`ApplySidecarClosure(n)` itself stays the environment-style atom:
enabled at `stage = "Start"` with nothing in flight, `n` open + queued,
subst+corr Pass, `n` not the active node / a pending-task carrier, and
the phase window open (ProofFormalization and `theorem_stating`
unconditionally; the latter also models RevisionStating, cf. §22). Effect:
the node closes, its
local-closure record installs, and its queue entry is consumed in the
SAME transition (the C-3 tier-coverage invariant), everything else
UNCHANGED.

**Kernel reality (`model.rs`, `sidecar.rs`, `bin/runtime_cli.rs`,
`engine.rs::apply_sidecar_closure`).** The queue is ordered kernel
state (`sidecar_queue` Vec — order is the reviewer's priority signal —
plus the monotone `sidecar_queue_seq` generation counter and a bounded
prune log), mutated ONLY by review-response apply
(`sidecar_queue_add` / `sidecar_queue_remove`, legality clauses with
named rejection reasons) and the deterministic `apply_event` tail
prune. The prune drops entries on the DURABLE conditions only
(deleted / closed / not_proof / sketch / statement-lane drift);
transient conditions (routed active, held target, pending-task
carrier, phase window) surface as export `blocked:<reason>` statuses
and never prune. Entries are ALSO removed by an event
(`ProtocolEvent::SidecarAttemptOutcomes`, next subsection) — still not
I/O inside `apply_event`: the spent-generation report arrives as an
event payload exactly as the closure record does, and the apply reads
nothing but that payload and the queue. The spec's set-abstraction and
the per-action prune-mirror conjunct are the projections of exactly
this: membership gates the apply, and any action that closes/deletes
nodes prunes in the same transition.

The atomic action refines into the runtime boundary hook: spool claim
(`pending/ → claimed/` by rename), queue-membership +
generation gates (`not_queued` / `stale_generation` — each grunt
attempt is keyed to one queue-entry generation, so a
removed-then-re-added entry can never be satisfied by a stale
attempt), content-addressed HARD pre-image gate (whole-file SHA-256 —
byte equality with the CURRENT tree, never commit identity), the
sidecar-specific ban scan, write-ahead journal, byte-frozen body
splice + two-repo worker mirror, FILESPEC + `declaration_hash_strict`
identity, full checker revalidation (compile/evaluate, env-
authoritative local-closure probe, approved axioms), a correspondence
no-drift postcondition, and only then `ProtocolEvent::SidecarClosure`
through the ordinary step machinery (sink-first durability; the event
carries the complete record so replay never touches a checker).
Kernel-only refinements with no spec counterpart: the generation
counter, defer-once-then-reject apply budgeting, and journal recovery
(match-HEAD invariant).

The state-only apply predicate is deliberately the permissive phase
envelope: configuration is not replay state. By default the runtime also
opens throughout TheoremStating/RevisionStating. An operator can restore
the original cost guard with
`sidecar.phases.stating_after_coverage: true`; the runtime hook/export then
wait until `~OrphanConstructionWindowOpen`, while replay remains independent
of that config. The original gate avoided spending grunt work while the
target DAG was incomplete and theorem-stating workers could still rewrite
any node; it was a speculative-work/discard-rate control, not a soundness
condition. Content/generation checks plus the unchanged substantiveness and
correspondence Pass requirements remain the safety boundary. The legacy
`TRELLIS_SIDECAR_STATING_IGNORE_COVERAGE` environment bypass is removed rather
than inverted: an environment-dependent engine predicate could reject replay
when the replay process has different environment. Reload refreshes the
state-derived `WrapperRequest.sidecar_window_open` bit on an old in-flight
Review before strict request validation; logged responses never author it.

The spec's "its local-closure record installs, everything else
UNCHANGED" carries one kernel-side obligation the spec cannot express,
because the object is unmodelled: the derived reverse indices
(`boundary_statement_consumers` / `strict_dep_consumers`, dispositioned
above as a kernel optimization) are a pure function of
`local_closure_records`, and `validate()` asserts that equality on
every transition. `apply_sidecar_closure` therefore rebuilds them via
`recompute_local_closure_reverse_indices` exactly as the primary accept
path does. This is refinement bookkeeping, not a design deviation —
"everything else UNCHANGED" holds of the modelled state, and the index
is a projection of the part that changed.

**Attempt outcomes: auto-prune of spent generations.** A queue entry is
the reviewer's "try this node once": the pool gets ONE attempt per
`entry_seq` generation, and the daemon's `attempted.json` row makes that
permanent — the manager skips the entry on every later pass. Without a
back-channel the spent entry stays queued forever, so the reviewer's
capacity view says "3 entries pending" when all three are dead work.
`ProtocolEvent::SidecarAttemptOutcomes` is that back-channel.

*Event shape.* `{ outcomes: [{ node, entry_seq, attempt_id, status,
detail, source }] }`, one batch per boundary, `source ∈ {daemon,
kernel_reject}`. The apply
(`engine.rs::apply_sidecar_attempt_outcomes`) is guarded exactly like
`apply_sidecar_closure` — stage `Start`, no in-flight request — refuses
a malformed payload (empty batch, empty node, a repeated generation),
and for each outcome removes the queue entry matching BOTH `node` and
`entry_seq`, recording a `attempt_spent:<status>` line in the same
bounded prune log the deterministic tail prune writes. It emits NO
commands: nothing on disk changed, so there is no `CommitCheckpoint`.

*Lane ownership.* The daemon publishes into a new spool lane
`outcomes/` by atomic rename out of its private `inflight/` — the exact
`pending/` protocol, with the same crash discipline
(`sweep_inflight_to_abandoned` on the daemon side,
`sweep_orphaned_outcome_claims` on the kernel side). The kernel alone
moves files out: claim by rename into `claimed_outcomes/`, terminal move
into `outcomes_consumed/` with a verdict block.

*The generation match is the SAME gate, not a new one.* `(node,
entry_seq)` is what `preflight_claimed_attempt` already uses to reject a
`stale_generation` attempt. A re-add mints `sidecar_queue_seq + 1`,
strictly greater, so a stale outcome can never expire a fresh
generation — it simply fails to match.

*Exempt statuses, and why each.* Pruned: `failed`, `budget_exhausted`,
`skipped_giant`, the threshold-burn `crashed` / `error`, a stop-sentinel
`cancelled`, and a terminal kernel rejection. NOT pruned: `success` (its
closure is in `pending/`; expiring the entry would make the closure
apply die `not_queued` and silently destroy completed grunt work);
`workspace_idle` (the runner returns before any bookkeeping — no attempt
happened); sub-threshold transport `error` (deliberately kept out of the
attempted-set, so the generation is still assignable); an ordinary
kernel `Deferred` (the attempt is going back to `pending/` and may still
land — but the D2 defer CEILING is terminal and does prune, below);
the rewind `cancelled` (the daemon wipes its attempted-set in the same
pass); and every transient block (`active_node`, `held_target`,
`pending_task`, `window_shut`, `unsplittable`), which never spends
anything. The invariant behind the list: an entry is expired only when
the daemon's own `attempted_contains` is permanently true for that exact
`(node, entry_seq)`. No judgment about whether a retry is warranted is
encoded anywhere — that stays the reviewer's.

*Kernel-side rejections.* The boundary apply gate rejects successful
attempts on gates the daemon cannot see (`stale_content`,
`evaluate_node`, `closure_probe`, `corr_drift`, `banned_token`,
`filespec`, `transition_error`), and terminally rejects one at the D2
defer ceiling (`apply_timeout` after the single permitted deferral). The daemon has already written its
`success` row, so the generation is spent with no daemon outcome to
publish; those rejections contribute `source: kernel_reject` outcomes
with `status: rejected:<gate>` into the same batch. Rejections stay
event-invisible in the settled-D8 sense (no attempt record enters the
event stream); what the event carries is the queue-lifecycle fact.

*Closure-awaiting-ingest gate, checked TWICE.* A node with a closure
still in `pending/` / `claimed/` never has its entry expired — the
closure apply re-asserts queue membership and would die `not_queued`,
destroying completed grunt work — and its outcome is RETURNED to the
lane rather than consumed. The check runs once as each record is
classified and again immediately before the event is stepped, because
the first reading is already stale: `publish_attempt` runs in the
attempt CHILD, so a closure reaches `pending/` before the child's
result file exists, and a stop-sentinel cancel or a crash-streak burn
can publish a spent-generation report for the very generation whose
proof is landing. Neither of the other risk-1 defenses covers that race
(no `success` was observed, and the closure is for that same
generation), so the late re-read is the whole defense.

*Post-rewind gate.* Each outcome carries the export `cycle` its
assignment was made from; `export_cycle > state.cycle` is dropped
unapplied. Three independent defenses cover a rewind: the daemon
publishes nothing from its rewind cancel, this gate drops anything that
slipped out earlier, and the daemon's blanket cancel + attempted-set
wipe re-opens every generation anyway.

*No-op on mismatch, and no auto-FILL.* An outcome that matches no entry
is a NO-OP with a logged note, deliberately not a refusal: failing to
remove is exactly the pre-feature behaviour, fail-loud would let one
stale record abort a whole boundary, and no-op-on-mismatch is what makes
the event idempotent (which the boundary hook relies on when a claim is
re-swept after a crash). The symmetric operation — automatically ADDING
an eligible node to the queue — is deliberately absent: queue membership
is reviewer authority, and only the removal of provably dead work is
mechanical.

Narrowing (not overturning) two prior records:
`SIDECAR_QUEUE_REDESIGN_PLAN.md` Q9 and `SIDECAR_IMPLEMENTATION_PLAN.md`
D8 still hold for rejections and discards as FORENSICS — those stay in
the spool, out of the event stream. What changes is the classification
of one fact: a SPENT GENERATION is queue-lifecycle information, and the
queue is kernel state, so the kernel must own it.

**Sound lane (owner decision 2026-07-23, supersedes settled D4).**
Sidecar eligibility does NOT consult the Sound lane at all — a
reviewer-pinned Fail and a verifier Fail are both eligible, and no
prune reason keys on a sound verdict. Principle: NL soundness exists
only to PREDICT that a Lean closure will be possible, so a landed
grunt proof supersedes any sound-lane verdict entirely — if the prose
was judged wrong but the Lean proof lands, the prediction was wrong
and the closure is pure win. The statement's meaning stays guarded by
the retained Substantiveness + Correspondence gates, and a closed
node's prose proof is treated as settled: it receives no further Sound
verification (closing retires the remaining soundness obligation).
The sound-lane tier annotation survives ONLY as advisory preference
information in the export's `eligible_now` feed — never a gate, never
daemon-side filtering. The SKETCH exclusion STAYS (different
rationale: a sketch is a placeholder mid-decomposition whose statement
is intended to be replaced; closing one would freeze a never-written
obligation).

**Superseded machinery.** The autonomous daemon scheduler from the
base branch (tier preference, `SystemRandom` pick, `(node, file_sha)`
tried-set with 7-day cool-off, plan-audit-A1 active-cone
deprioritization) is retired by this redesign: the reviewer's queue
order IS the schedule, and the daemon's only memory is the
one-attempt-per-generation dedupe (retry = reviewer re-add, minting a
new generation).

**Edge notes (all deliberate).**
- *Kernel-activation edge*: the kernel's own routing
  (`select_initial_proof_active_node` and friends) can make a QUEUED
  node the active node without any reviewer response. Waste-only, and
  blocked-not-pruned (where a reviewer `next_active` prunes): the
  entry turns `blocked:active_node` in the export, the manager never
  assigns it, and the apply gate re-asserts eligibility — nothing can
  mis-apply, and the entry becomes assignable again the moment the
  kernel routes away.
- *Add-then-route-away one-cycle asymmetry*: adds are evaluated
  against the CURRENT state, so one response may legally add node X
  while routing the worker somewhere that makes X transiently blocked
  the next cycle. Deliberate — the alternative (evaluating adds
  against the post-routing state) buys nothing: blocked entries just
  wait.
- *Routing dequeues, and binds `next_active` only (Q7)*: making a
  queued node a pending-task carrier or authorized-scope member is
  waste, not illegality — the transient-block classifier and the apply
  gates make a wrong apply impossible. The kernel restricts the
  dequeue further, to a Continue's `next_active`: AdvancePhase carries
  the field but the next phase rederives routing, and the spec's
  `ReviewAdvancePhase` leaves `sidecarQueue` unchanged, so the two
  agree. Auto-dispatch cannot re-queue the node it just routed —
  `sidecar_eligible` excludes the active node (spec: `adds ∩
  {activeNode} = {}` in `AutoDispatchSidecarQueue`), so the same-cycle
  spent-generation cool-off is not load-bearing here.
- *Cleanup*: entering Cleanup requires `formalization_complete`, i.e.
  every node closed — so the tail prune has provably emptied the queue
  by the time Cleanup routing (which is task-driven via `active_node`,
  not `next_active`) runs. Queue removals remain legal in every phase
  regardless (draining is never blocked on phase).
- *TheoremStating node reset*: a `theorem_stating_node` reset that
  reopens/rewrites statements leaves any affected entry first
  `blocked` and then drift-handled (statement-lane fingerprints move ⇒
  `lane_drift` prune, or `stale_content`/`stale_generation` rejection
  at apply) — safe with no special handling.

**Per-node closure provenance (`closure_provenance`).** The kernel
records, for every node whose closing transition it witnessed, WHO drove
it sorry-free (`worker` — the primary loop's accepted burst; `sidecar` —
a grunt, with the daemon attempt detail) and at which cycle. This
subsumes the earlier grunt-only `sidecar_closures` map, which had one
writer and no remover and therefore meant "was EVER grunt-closed": a
node a grunt closed, that then reopened, that a worker then closed
properly, stayed attributed to the grunt. The spec's `sidecarCloses`
counter is the abstraction of the `ClosedBy::Sidecar` half; the worker
half and the per-node keying are unmodelled, exactly like the reverse
indices dispositioned above.

The kernel-side obligation the spec cannot express is the same shape as
the reverse-index one: provenance must agree with `openNodes` over a
node's whole lifetime. It is discharged the way the grunt QUEUE already
discharges its own lifecycle — a deterministic tail prune
(`prune_closure_provenance`) that runs inside `apply_event` for EVERY
event immediately before `validate()`, keyed off
`live.present_nodes` / `live.open_nodes`, so every reopen and delete
path (worker statement edit, node deletion, cone clean /
theorem-stating baseline reset, `restore_committed`,
`apply_last_clean_reset`, revision import, add-targets revival) is
covered without per-path maintenance. `validate()` asserts the
resulting invariant (an entry exists ⇒ the node is present and not
open), so any future writer that skips the prune fails loudly rather
than misattributing. The converse is deliberately unasserted: closures
that predate the map, and nodes already sorry-free at import, have no
witnessed transition and carry no entry. No new event kind, no new
action, no TLA change — the map is audit-only and never read by a
dispatch decision.

Sidecar closures on challenge-claimed nodes are deliberately in scope
— owner decision 2026-07-22: a sidecar closure is part of the trellis
process, so closing a byte-pinned challenge / PV mode-B goal theorem
through it is a legitimate in-process result (pinned by
`sidecar_eligible_includes_challenge_claimed_nodes`).

**Kernel-queue refill: the pool's standing supply of ranked work.** The
reviewer queues to the pool depth it believes in, once per cycle, and
each entry is worth exactly one attempt. The pool drains that in
minutes and then idles until the next boundary — measured on the live
run at 5.1% utilisation over 13.2 h (50 attempts, 2.7 of 52.9 available
grunt-hours, twelve idle gaps over 20 minutes, one of 150), against a
169 s median attempt. `AutoDispatchSidecarQueue` (SupervisorCore.tla)
is the refill: at the same boundary as the close and the expiry, a
non-empty set of add-eligible, NOT-ALREADY-QUEUED nodes enters the
queue. Nothing else moves.

*The same queue, deliberately.* The entries the kernel mints are
`SidecarQueueEntry` values from the same monotone `sidecar_queue_seq`,
stamped with the current cycle, appended to the same ordered
`sidecar_queue`. The deterministic tail prune drops them on the same
durable conditions (`deleted` / `closed` / `not_proof` / `sketch` /
`lane_drift`), `SidecarAttemptOutcomes` expires their generations the
same way, `next_active` dequeues them the same way, and — the point —
`ApplySidecarClosure` reads the same membership, so a grunt result for
one of them applies through the UNCHANGED apply gate. The single
observable difference is that entries can appear without a review
response, which is exactly the spec change: `sidecarQueue` may grow at
a boundary as well as at a Reviewer turn.

*The lane tag has no spec counterpart.* Kernel-side, each entry carries
`origin: reviewer | kernel`. It is spec-invisible in the same way
`entry_seq` and `queued_at_cycle` are: every queue-aware spec action
reads and writes `sidecarQueue` origin-blind. The tag splits ONE state
queue into TWO export lists — `queue` (the reviewer's, the only one it
is shown and the only one it manages) and `kernel_queue` (the standing
ranked list) — plus a redacted `reviewer_candidates.json` that the
reviewer sandbox binds instead of `candidates.json`. The daemon drains
the reviewer lane first and falls back to the kernel lane. The
reviewer's add/remove legality is stated against reviewer-lane
membership only, and an add naming a kernel-lane node PROMOTES it
(same generation, moved to the back of the queue, lane flipped) rather
than rejecting as "already queued".

*What the spec drops.* `AutoDispatchSidecarQueue` picks ANY non-empty
unqueued subset of `SidecarQueueAddable`; the kernel picks the whole
ranked list, rank-ordering each boundary's mint batch and rotating
FIFO thereafter (rank: fewest prior grunt attempts, then
non-sketch before sketch — a SKETCH proof counts as longer than every
non-sketch proof — then shortest NL proof, then node id), bounded by
`kernel_queue_max`. Both the depth and the order are scheduling with no
safety content, and both read objects the spec does not have: the
daemon's attempt history and the tablet's `.tex` bodies. Dropping them
makes the spec action strictly more non-deterministic than the kernel —
the `ExpireSidecarQueueEntry` posture, and a safe over-approximation.

*The churn bound moved.* WAS `sidecarQueue = {}` with `sidecarQueue' =
adds`, pinned by `AutoDispatchOnlyIntoAnEmptyQueue` in
`SidecarCoreHarness`. That guard is exactly what capped utilisation: a
queue refilled to the idle pool's width empties again in one
attempt-length and cannot refill until the next boundary. It is
replaced by `adds \cap sidecarQueue = {}`, pinned by
`AutoDispatchNeverReAddsAQueuedNode` (a refill whose adds were already
queued would leave `sidecarQueue` unchanged, which is what the harness
history bit records; its counterfactual is one deleted conjunct). The
bound is what it always was — re-running the boundary hook adds nothing
the first pass did not — and it additionally means a boundary mints
generations only for what the lane does not already hold, so a resident
entry keeps its generation until that generation is SPENT.

*Kernel refinements with no spec counterpart.* (1) Prior-attempt counts
come from the daemon's `attempted.json` (all generations, unlike the
5-row-per-node window `status.json` mirrors); a present-but-unparseable
file skips the boundary rather than silently ranking every node at zero
attempts. (2) NL proof length is non-whitespace characters of the
`\begin{proof}` block, so a reflow cannot reorder the queue; an
unmeasurable proof sorts after every measured one. (3) A node whose
generation was declared spent in the CURRENT cycle is held back for the
rest of it — without that, the boundary that retires an entry re-mints
its node in the same breath, and the degenerate one-eligible-node case
re-attempts back to back forever. (4) `sidecar.auto_dispatch.enabled`
(default true) is the operator's off switch, config-edit not
binary-swap, like the `phases` toggles; `kernel_queue_max` (default
512) bounds the resident lane and `max_attempts` (default 5, 0 =
unlimited; legacy spelling `max_attempts_per_node`) is a per-node
attempt ceiling on the kernel lane's supply, counted per node content
so a repaired node regains its allowance. (5) The daemon heartbeat is NO
LONGER read: it was load-bearing only while the top-up was sized off
`idle - 1`, where a dead daemon's fossil `in_flight` could size a
dispatch off a pool that no longer exists. A refill to a fixed lane
depth reads nothing about the pool.

**Disposition: design intentional, kernel refinement** (the §24
"runtime/git concern" framing). The spec models the state-visible
atoms; everything below them is content-addressed plumbing whose
failure modes all collapse to "nothing happened" (worktree restored,
no event). SupervisorProtocol.tla parity is DEFERRED: the Protocol
module models the request/response wrapper choreography, which the
sidecar event never touches (it is boundary-only, request-free by
construction); adding the action there requires threading a new
variable through every explicit per-action frame (~110 actions) for no
new checked behavior. Queued as follow-up spec work; the Core BFS
carries the sidecar action + queue exhaustively, and the init-at-
queued/unqueued `SidecarCoreHarness` pins the membership gate's
counterfactual (`SidecarCloseOnlyAfterQueued`) and the expiry's blast
radius (`ExpireNeitherClosesNorInstalls` — give
`ExpireSidecarQueueEntry` an `openNodes'` conjunct by mistake and TLC
lands on it). That action is structurally immune to the
`QuiescentLiveEqualsCommitted` bug class the closure action hit: it
touches `sidecarQueue` and nothing else, and `sidecar_queue` is
deliberately unmirrored, so there is no live/committed asymmetry to get
wrong. The Rust test `outcome_touches_nothing_but_the_queue` stands in
for the deferred Protocol-module parity. Operational note
for enablement: the `sidecar` config block must be COMMITTED to the
run repo's `trellis.config.json` — a checkpoint `git reset --hard`
restores tracked config, so an uncommitted edit silently reverts at
the next reset.

## 31. Previous-lane findings: lane sets in the spec, node-keyed maps in the kernel

`previousSoundFindingLanes` / `previousCorrFindingLanes` /
`previousSubstantivenessFindingLanes` / `previousPaperFindingLanes` are
`\subseteq VerifierLanes` — a set of lane identifiers with no node
identity. The kernel's counterparts are
`BTreeMap<NodeId, BTreeMap<LaneId, ...>>` (the paper-faithfulness one
stays lane-keyed; see below). The scenario-fragment selection is
faithful at the spec's granularity: `SoundVerifierScenarioFragments`
appends `verifier/soundness/06_revisit_target.md` iff the set is
non-empty, exactly as `sound_scenario_prompt_fragments` does.

What the spec therefore cannot express is *which* node a stored
finding belongs to, and hence the "the request's verify set and the
stored findings' key set may be disjoint" hazard. The stored map
mirrors the most recent response for its lane, while the next verify
target is selected independently (backlog dispatch / reviewer request /
Unknown frontier), so a drain can hand node A's finding to node B's
verifier under revisit framing. The kernel closes this at request
construction — `request_previous_corr_lane_findings`,
`request_previous_sound_lane_findings`, and
`request_previous_substantiveness_lane_findings` intersect the stored
keys with the outgoing verify set, so an empty intersection also
renders the fresh-target fragment rather than a false revisit.

The paper-faithfulness lane keeps the spec's shape for a substantive
reason: `PaperReviewerLaneEvidence` carries no target identity at all
(`paper_faithfulness`, `overall`, `summary`, `comments`), so there is
no key to intersect against `paper_verify_targets`. Scoping that lane
requires first giving the evidence a target identity; until then the
lane-set abstraction and the kernel agree.

**Disposition: design intentional collapse.** Node-keying the four
finding variables would thread `Nodes`-indexed functions through every
verifier action's frame for a property (prompt framing) that has no
state-visible consequence — no verdict, status, or fingerprint reads
these fields. The kernel-side scoping is pinned by
`corr_request_scopes_previous_corr_lane_findings_to_verify_nodes`,
`sound_request_scopes_previous_sound_lane_findings_to_verify_nodes`,
and
`paper_request_scopes_previous_substantiveness_lane_findings_to_verify_nodes`.

## 32. Elaboration-cost records: telemetry with no refinement obligation

`ProtocolState.elaboration_cost_records: BTreeMap<NodeId, ElaborationCostRecord>`
has no counterpart in the intent spec, and deliberately never will.

The contrast with §21 is the point, because the two look superficially
alike and are opposite in kind. `local_closure_records` realizes a
modeled status (`localClosureStatus`), so §21 carries a refinement
obligation: every status flip must correspond to a records/unverified
mutation. Elaboration-cost records realize nothing. They record what one
elaboration of a node's Lean source cost — wall time, peak RSS, olean
size, and (once an offline measurement pass supplies it) heartbeats —
and no spec variable, invariant, or action depends on any of it.

**Disposition: below the abstraction line, permanently.** There is no
refinement obligation to discharge and no invariant to state. Nothing
gates, schedules, dispatches, reopens, or accepts on a cost value; the
map has no committed or last-clean mirror because a rollback has no
evidence to restore; and a missing record is an ordinary state that
readers cannot distinguish from a stale one (the single accessor,
`ProtocolState::fresh_elaboration_cost`, returns `Option` and collapses
both into `None`). Partial coverage is the permanent steady state, not a
transient to be closed.

This section exists so nobody later "restores parity" by adding a
coverage invariant or a status variable. Doing so is exactly what would
turn a missing measurement into an error. The kernel-side guard is the
source-scan test
`elaboration_cost_records_read_only_at_permitted_sites`.

The heartbeat field is populated asynchronously and stays below the same
line. The checker's background measurement build (a separate,
raised-cap, instrumented `lean` invocation — never the acceptance build,
whose behavior is byte-identical with the feature present or absent)
spools counts to disk; the runtime drains the spool onto
`WorkerResponse.late_heartbeats` at step time so the logged event, not
the spool, is what replay consumes; and the engine merges each count
into the node's existing record under the source-closure hash it was
actually measured under (`LateHeartbeatMeasurement`,
`drain_pending_heartbeat_measurements`). A dropped, lost, or stale
measurement is ordinary; no spec variable, invariant, or action sees any
of it.

## 33. Prose-mode decide pairs: flips no longer vacuous under `GoalMode = "prose"`

The spec's flip machinery (`AuditSoleFlipperProp` and the Decide-pair
arms) was written when Decide targets existed only in mode B
(`GoalMode = "lean"`); prose instances satisfied the flip invariants
vacuously. The W1–W6 prose-flip enablement registers worker-authored
decide pairs in prose mode: the pair binds mid-run at the D3 acceptance
point (`ProtocolEvent::StatementBound` — an event class the spec does not
model), after which the kernel's flip machinery operates on it exactly as
in mode B, PLUS one new non-halting refusal class `statement_unbound`
(flip/witness/give-up proposals on an authored pair whose statement has
not bound). The spec's `GoalMode ∈ {"lean","prose"}` assumption is
unchanged; what changed is that "prose" instances now exercise the flip
invariants non-vacuously and carry a binding event outside the spec's
event alphabet. The sole-flipper property itself is preserved (the
binding never mutates polarity; the audit lane remains the only flip
authority). The spec should gain the binding event and the
`statement_unbound` refusal at the next spec-maintenance opportunity.
...
2. **Design intentional collapse (#3, #7, #9, #12, #16, #22, #23, #25, #31):** the
