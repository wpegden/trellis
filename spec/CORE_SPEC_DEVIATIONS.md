...
- `sidecarQueue ⊆ Nodes` — the reviewer-managed grunt queue,
  ...
  Pass) and removes from the queue, under the owner's
  cancel-before-worker constraint
  `newActive ∉ (sidecarQueue ∪ add) \ rem` — routing the worker at a
  queued node requires removing it in the same response;
...
- *Kernel-activation edge*: the kernel's own routing
  ...
  blocked-not-pruned: the entry turns `blocked:active_node` in the
  export, the manager never assigns it, and the apply gate re-asserts
  eligibility — nothing can mis-apply.
- *Add-then-route-away one-cycle asymmetry*: adds are evaluated
  ...
  wait.
- *Cancel-before-worker binds `next_active` only (Q7)*: making a
  ...
  gates make a wrong apply impossible, and the reviewer fragment says
  to remove first.
