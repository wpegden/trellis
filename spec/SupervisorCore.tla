...
CONSTANTS
    ...
    \* Decide direction.
    MaxPolarityFlips,
    \* Parallel-closure sidecar: bound on how many sidecar closures a
    \* behavior may apply (state-space knob for the exhaustive BFS; the
    \* kernel has no such cap — its bound is the spool + eligibility).
    MaxSidecarCloses,
...
(* --- Parallel-closure sidecar (2 vars) ---------------------------------- *)
(* `sidecarCloses` counts applied sidecar closures (bounded by            *)
(* `MaxSidecarCloses`).  Kernel analog: the per-node provenance map      *)
(* `ProtocolState.closure_provenance` (audit-only, never dispatch-read). *)
(* `sidecarQueue` is the reviewer-managed grunt queue as a SET of nodes  *)
(* (Q8): the spec's job is authority (membership gates the apply) and    *)
(* lifecycle (reviewer add/remove, prune-on-close, dequeue-on-apply);    *)
(* order is daemon scheduling with no safety content.  Kernel analog:    *)
(* the ordered `ProtocolState.sidecar_queue` + `sidecar_queue_seq`       *)
(* generation counter + bounded prune log.                               *)
VARIABLES
    sidecarCloses,
    sidecarQueue
...
(* Parallel-closure sidecar cluster.  Queue-blind legacy actions hold it  *)
(* UNCHANGED via the single conjunct in `Next` (TLC's "variable not      *)
(* assigned" machine-checks completeness, the BackendVars precedent);    *)
(* the queue-aware actions (ReviewContinue mutates the queue,            *)
(* AcceptWorker carries the tail-prune image, ApplySidecarClosure        *)
(* consumes membership) frame the two vars explicitly in `Next`.         *)
SidecarVars == <<sidecarCloses, sidecarQueue>>
...
\* target has an empty covering set.  While open, valid worker bursts may
\* create live orphan nodes (layered, support-first construction); the
\* same-burst orphan contract binds only when every configured target is
\* covered.  Challenge targets are deliberately excluded.  Evaluated on
\* the UNPRIMED (pre-burst) variables inside the accept actions — the
\* window the worker was dispatched under — so a burst that itself covers
\* the final uncovered target while leaving unrelated orphans is still
\* accepted (kernel design note on `orphan_construction_window_open`).
OrphanConstructionWindowOpen ==
    \E t \in configuredTargets : coverage[t] = {}
...
AcceptedValidWorkerOrphanContract(deletedNodes, orphanedByDelta, livePresent, liveCoverage, liveChallengeCoverage) ==
    ...
    \* same-burst orphan set.  The window waiver is NARROW: deletion
    \* legality (`deletedNodes = orphanedByDelta`, the only-orphans-
    \* removable constraint — callers pass the post-delta orphan set
    \* intersected with `deletedNodes`) binds unconditionally; the window
    \* waives only the same-burst new-orphan obligation, and not in the
    \* cleanup phase. Responsibility is delta-based: pre-existing orphans
    \* remain visible but do not make an unrelated burst unacceptable.
    /\ deletedNodes \subseteq presentNodes
    /\ deletedNodes = orphanedByDelta
    /\ \/ (OrphanConstructionWindowOpen /\ phase # "cleanup")
       \/ CoreOrphanNodes(livePresent, liveCoverage, liveChallengeCoverage)
            \ CoreOrphanNodes(presentNodes, coverage, challengeCoverage) = {}
...
(* Sidecar queue redesign: the reviewer's add-candidates are the        *)
(* eligible-shaped open nodes (statement lanes Pass; the Sound lane is  *)
(* deliberately NOT consulted — owner decision 2026-07-23; the active-  *)
(* focus exclusions are folded into the newActive constraint below).    *)
SidecarQueueAddable ==
    {n \in openNodes :
        /\ correspondenceStatus[n] = "pass"
        /\ substantivenessStatus[n] = "pass"}
...
ReviewContinue ==
    ...
    /\ \E nextMode \in WorkerModes,
          ...
          queueAdd \in SUBSET SidecarQueueAddable,
          queueRemove \in SUBSET sidecarQueue :
        /\ taskCarriers \subseteq GlobalBlockers
        /\ \* Sidecar queue mutations (queue redesign): adds are new
           \* members, and routing the worker at a node DEQUEUES it —
           \* the same removal the reviewer's own verb performs, so
           \* naming it as well is idempotent.  An add the same
           \* response routes at is the one incoherence left: the
           \* routing would undo it.
           queueAdd \cap sidecarQueue = {}
        /\ newActive \notin queueAdd
        /\ sidecarQueue' =
               ((sidecarQueue \ queueRemove) \cup queueAdd) \ {newActive}
...
(* of-band operator action.  Two disjuncts:                              *)
(*                                                                        *)
(*   1. the historical in-place edit during theorem_stating — a          *)
(*      non-deterministic mutation of the configured-target set; the     *)
(*      reviewer/expert gate is then forced to re-approve;               *)
(*                                                                        *)
(*   2. add-targets mode (the offline `add_paper_targets` runtime-CLI    *)
(*      action, 2026-07): superset-only growth on a COMPLETED run.       *)
(*      CleanupDoneTerminal + GlobalBlockersExhaustive make in-place     *)
(*      growth at complete illegal (a new configured target carries a    *)
(*      non-pass faithfulness blocker, and complete requires an empty    *)
(*      blocker set), so the action atomically flips the phase back to   *)
(*      theorem_stating in the SAME step.  The kernel revives into       *)
(*      RevisionStating; no revision_stating phase exists in the specs,  *)
(*      and RevisionStating is theorem-stating-like for every gate the   *)
(*      specs model, so theorem_stating is the sanctioned abstraction.   *)
(*      Added targets re-enter the faithfulness lane fresh ("unknown" —  *)
(*      the kernel forces paper_status Unknown so a target that was      *)
(*      passed and later un-configured can never re-enter with a stale   *)
(*      pass).  approvedConfiguredTargets is UNCHANGED in both disjuncts *)
(*      (only the AdvancePhase freeze moves it), and both are mode-A     *)
(*      only (GoalMode # "lean"; add-targets additionally requires a     *)
(*      non-empty pre-add configured set).                               *)
(***************************************************************************)
EditConfiguredTargets ==
    \/ /\ stage = "Start"
       ...
       /\ UNCHANGED BackendVars
       /\ UNCHANGED PolarityVars
    \/ \* Add-targets mode: atomic complete -> theorem_stating revival with
       \* superset-only configured-target growth.  Preserves
       \* PhaseDormancyContract (the flip leaves "complete" in the same
       \* step; audit lanes / pending task / active seats stay or are reset
       \* to their dormant values) and resets the pending task, mirroring
       \* the kernel's Complete-revival recipe.
       /\ stage = "Start"
       /\ inFlightRequestKind = "none"
       /\ phase = "complete"
       /\ GoalMode # "lean"
       \* Mode-A guard: the run must already have configured targets.
       /\ configuredTargets # {}
       /\ \E added \in SUBSET (Targets \ configuredTargets) :
           /\ added # {}
           /\ configuredTargets' = configuredTargets \cup added
           \* Added targets re-enter the faithfulness lane fresh.
           /\ faithfulnessStatus' =
                  [t \in Targets |-> IF t \in added THEN "unknown"
                                     ELSE faithfulnessStatus[t]]
       /\ phase' = "theorem_stating"
       /\ hasPendingTask' = FALSE
       /\ pendingTaskKind' = "none"
       /\ pendingTaskCarriers' = {}
       /\ UNCHANGED workerMode
       /\ UNCHANGED approvedConfiguredTargets
       /\ UNCHANGED <<stage, cycle, activeNode, activeCoarseNode>>
       /\ UNCHANGED <<presentNodes, openNodes, coverage, approvedCoverage,
                      challengeCoverage>>
       /\ UNCHANGED CoarseDagVars
       /\ UNCHANGED <<correspondenceStatus, substantivenessStatus,
...
(* Also legal from "grant_available": a fresh accepted Step A supersedes *)
(* the pending grant (the kernel drops it and records it on the new      *)
(* request), modeled here by the step moving straight to                 *)
(* "request_pending" — the single-variable encoding cannot hold a grant  *)
(* and a request at once, which is exactly the kernel invariant.         *)
(***************************************************************************)
RequestGlobalRepairAudit ==
    /\ stage = "Reviewer"
    /\ inFlightRequestKind = "reviewer"
    /\ phase = "proof_formalization"
    /\ globalRepairStep \in {"none", "grant_available"}
...
(***************************************************************************)
(***************************************************************************)
(* Parallel-closure sidecar (environment-style action, like worker       *)
(* nondeterminism).  A fully validated out-of-band proof-body closure    *)
(* enters at the inter-cycle boundary: stage Start, nothing in flight,   *)
(* node open + subst/corr Pass + not the active node, phase window open  *)
(* (ProofFormalization and theorem_stating unconditionally; the latter   *)
(* also models RevisionStating, cf. deviations §22). Runtime config may  *)
(* restore the old stating-after-coverage restriction; config is not     *)
(* replay/spec state, so this action is the permissive state envelope.   *)
(* Effect: the node closes and its local-closure record installs in the  *)
(* SAME transition (C-3 tier coverage), everything else UNCHANGED.       *)
(* Kernel refinements below this atomic action (spool claim protocol,    *)
(* content-addressed pre-image gate, checker revalidation, write-ahead   *)
(* journal, worker-repo mirror) are recorded in CORE_SPEC_DEVIATIONS.md  *)
(* §29.                                                                  *)
(***************************************************************************)
ApplySidecarClosure(n) ==
    /\ sidecarCloses < MaxSidecarCloses
    /\ stage = "Start"
    /\ inFlightRequestKind = "none"
    /\ phase \in {"proof_formalization", "theorem_stating"}
    /\ n \in openNodes
    /\ n # activeNode
    /\ ~(hasPendingTask /\ n \in pendingTaskCarriers)
    /\ correspondenceStatus[n] = "pass"
    /\ substantivenessStatus[n] = "pass"
    \* Queue redesign: reviewer authority — the apply REQUIRES queue
    \* membership and CONSUMES the entry in the same transition.
    /\ n \in sidecarQueue
    /\ sidecarQueue' = sidecarQueue \ {n}
    /\ openNodes' = openNodes \ {n}
    /\ localClosureStatus' = [localClosureStatus EXCEPT ![n] = "verified"]
    /\ sidecarCloses' = sidecarCloses + 1
    /\ UNCHANGED PhaseStageVars
    /\ UNCHANGED <<presentNodes, coverage, approvedCoverage,
                   challengeCoverage, configuredTargets,
                   approvedConfiguredTargets>>
    /\ UNCHANGED CoarseDagVars
    /\ UNCHANGED LaneStatusVars
...
SidecarClose == \E n \in Nodes : ApplySidecarClosure(n)

(***************************************************************************)
(* Expiry of a SPENT grunt-queue generation (environment-style action,    *)
(* like ApplySidecarClosure).  The out-of-band pool gets ONE attempt per  *)
(* queue-entry generation; when that attempt is spent without closing     *)
(* the node, the entry is dead work and the kernel retires it at the      *)
(* next inter-cycle boundary.  The daemon is not a spec object, so        *)
(* "the environment may expire any queued entry at a boundary" is the     *)
(* right abstraction — and it is strictly MORE non-deterministic than     *)
(* the kernel (which expires only on a matching (node, entry_seq)         *)
(* report), hence a safe over-approximation.                             *)
(*                                                                        *)
(* Effect: the entry leaves the queue and NOTHING else moves — in         *)
(* particular the node stays open and its closure tier stays whatever it  *)
(* was.  `SidecarQueueSubsetOpen` is preserved trivially by a set shrink  *)
(* and TypeOK is unaffected.  The kernel refinement (the event-carried    *)
(* `(node, entry_seq)` generation match, the outcome spool lane, the      *)
(* post-rewind and closure-awaiting-ingest gates, the no-op-on-mismatch   *)
(* posture) is recorded in CORE_SPEC_DEVIATIONS.md §30.                   *)
(***************************************************************************)
ExpireSidecarQueueEntry(n) ==
    /\ stage = "Start"
    /\ inFlightRequestKind = "none"
    /\ n \in sidecarQueue
    /\ sidecarQueue' = sidecarQueue \ {n}
    ...
    /\ UNCHANGED PhaseStageVars
    \* The whole structure cluster, openNodes INCLUDED (the contrast with
    \* ApplySidecarClosure, which shrinks it): an expiry retires dead
    \* work, it never closes a node.
    /\ UNCHANGED StructureVars
    /\ UNCHANGED LocalClosureVars
    /\ UNCHANGED CoarseDagVars
    /\ UNCHANGED LaneStatusVars
...
SidecarExpire == \E n \in Nodes : ExpireSidecarQueueEntry(n)

(***************************************************************************)
(* Kernel-queue refill: the kernel tops the grunt queue up at the         *)
(* inter-cycle boundary so the pool always has ranked work in hand.  Same *)
(* boundary as the close and the expiry, and -- the point of the feature  *)
(* -- the SAME queue: the entries it adds are ordinary entries, so the    *)
(* only spec-visible change is that `sidecarQueue` can grow at a boundary *)
(* as well as at a reviewer turn.                                         *)
(*                                                                        *)
(* Abstraction: "the kernel may add any non-empty set of add-eligible     *)
(* nodes that are NOT ALREADY QUEUED, at a boundary".  The kernel's       *)
(* ranking (fewest prior grunt attempts, then non-sketch before sketch,   *)
(* then shortest NL proof, then node id), its lane tag, and its resident  *)
(* list depth are SCHEDULING, with no safety content -- and they read the *)
(* daemon's attempt history and the tablet's `.tex` bodies, neither of    *)
(* which is a spec object.  Dropping them makes this strictly MORE        *)
(* non-deterministic than the kernel (any unqueued subset, any size),     *)
(* hence a safe over-approximation, exactly like                          *)
(* `ExpireSidecarQueueEntry`.  The kernel refinement (the ranking, the    *)
(* attempt-count source, the length measure, the lane split, the config   *)
(* toggles) is recorded in CORE_SPEC_DEVIATIONS.md section 31.            *)
(*                                                                        *)
(* WAS `sidecarQueue = {}` with `sidecarQueue' = adds`.  That guard was   *)
(* the churn bound of the ORIGINAL feature, which topped an EMPTY queue   *)
(* up to the idle pool's width; it is also exactly what capped the pool   *)
(* at 5.1% utilisation, because a queue refilled to the pool's width      *)
(* empties again in one attempt-length and cannot refill until the next   *)
(* boundary, 20 to 150 minutes later.  The churn bound is now             *)
(* `adds \cap sidecarQueue = {}`: a boundary mints only for nodes the     *)
(* queue does not already hold, so a resident entry keeps its generation  *)
(* until that generation is SPENT and `ExpireSidecarQueueEntry` retires   *)
(* it.  Re-running the action at the same boundary (the human-gate poll   *)
(* loop re-enters the hook) therefore still adds nothing the first pass   *)
(* did not.                                                               *)
(***************************************************************************)
AutoDispatchSidecarQueue ==
    /\ stage = "Start"
    /\ inFlightRequestKind = "none"
    /\ \E adds \in SUBSET SidecarQueueAddable :
        /\ adds # {}
        /\ adds \cap sidecarQueue = {}
        /\ adds \cap {activeNode} = {}
        /\ ~(hasPendingTask /\ adds \cap pendingTaskCarriers # {})
        /\ sidecarQueue' = sidecarQueue \cup adds
    /\ UNCHANGED sidecarCloses
    /\ UNCHANGED PhaseStageVars
    \* Like the expiry and unlike the close: queueing work never closes a
    \* node, installs a record, or moves anything else.
    /\ UNCHANGED StructureVars
    /\ UNCHANGED LocalClosureVars
    /\ UNCHANGED CoarseDagVars
    /\ UNCHANGED LaneStatusVars
...
(* Every queue-blind pre-sidecar action holds both sidecar vars          *)
(* UNCHANGED via the single conjunct below (distribution over the        *)
(* disjunction is exactly per-action framing).  The two queue-aware      *)
(* actions moved OUT of the blanket: ReviewContinue assigns              *)
(* `sidecarQueue'` itself (reviewer add/remove), and AcceptWorker —      *)
(* the only other action that shrinks `openNodes`/`presentNodes` —      *)
(* conjoins the kernel tail-prune's spec image                           *)
(* (`sidecarQueue' = sidecarQueue \cap openNodes'`), so a queued node    *)
(* closed or deleted by the primary lane leaves the queue in the same    *)
(* transition.                                                           *)
LegacyNext ==
    \/ StartCycle
    \/ AcceptVerifier
...
QueueAwareNext ==
    \/ (AcceptWorker /\ sidecarQueue' = sidecarQueue \cap openNodes')
    \/ ReviewContinue

Next ==
    \/ (LegacyNext /\ UNCHANGED SidecarVars)
    \/ (QueueAwareNext /\ UNCHANGED sidecarCloses)
    \/ SidecarClose
    \/ SidecarExpire
    \/ AutoDispatchSidecarQueue
...
TypeOK ==
    /\ sidecarCloses \in 0..MaxSidecarCloses
    /\ sidecarQueue \subseteq Nodes
    /\ phase \in Phases
    /\ stage \in Stages
    /\ cycle \in 0..MaxCycle
...
(***************************************************************************)
(* CleanupBlockersAreAdmissible.  The cleanup phase's blocker contract.   *)
(*                                                                        *)
(* This used to read `phase = "cleanup" => GlobalBlockers = {}` — the      *)
(* design's "happy stop": nothing is ever outstanding in cleanup.  The    *)
(* kernel's per-burst cleanup acceptance predicate has weakened from      *)
(* `formalization_complete` to `formalization_valid`                      *)
(* (`kernel/src/model.rs`), which keeps every Lean-closure clause and     *)
(* admits exactly one class of open blocker: a CORRESPONDENCE blocker on  *)
(* a node OUTSIDE the ratified statement surface.  That is what lets a    *)
(* cleanup task create a helper node whose NL gloss has not been          *)
(* verified yet.                                                          *)
(*                                                                        *)
(* The weakening is a floor, not a redefinition of done.  Completion is   *)
(* unaffected: `ReviewDone` still requires `GlobalBlockers = {}`, as does *)
(* `ReviewAdvancePhase`, so nothing blocker-carrying reaches `complete`.  *)
(* `NoAdvancePhaseWithBlockers` and                                       *)
(* `CompleteRequiresChallengeCoverage` below are unchanged and still      *)
(* carry that weight.                                                     *)
(*                                                                        *)
(* Scope note: the abstract spec has no action that PRODUCES a cleanup    *)
(* state with an admissible blocker (helper extraction and the            *)
(* correspondence-repair route are not modelled).  So this invariant is   *)
(* currently weaker than the reachable state space requires.  That is     *)
(* deliberate — it stops the spec asserting a property the kernel no      *)
(* longer guarantees, without inventing actions whose fidelity has not    *)
(* been checked against the implementation.                               *)
(***************************************************************************)
CleanupProtectedStatementNodes ==
    UNION {coverage[t] : t \in Targets}
        \cup
    UNION {challengeCoverage[c] : c \in ChallengeTargets}

CleanupAdmissibleBlocker(b) ==
    /\ b \in presentNodes
    /\ b \notin CleanupProtectedStatementNodes
    /\ correspondenceStatus[b] # "pass"
    /\ soundnessStatus[b] = "pass"
    /\ deviationStatus[b] = "pass"
...
(***************************************************************************)
(* SidecarQueueSubsetOpen.  Queue redesign: every queued node is open.   *)
(* Kernel analog: the `validate()` queue invariant, guaranteed by the    *)
(* `apply_event` tail prune; here by the ReviewContinue add source       *)
(* (`SidecarQueueAddable \subseteq openNodes`), the AcceptWorker         *)
(* prune-image conjunct, and ApplySidecarClosure's consume.              *)
(***************************************************************************)
SidecarQueueSubsetOpen ==
    sidecarQueue \subseteq openNodes
