...
AcceptedValidWorkerOrphanContract(deletedNodes, orphanedByDelta, livePresent, liveCoverage, liveChallengeCoverage) ==
    ...
    \* waives only orphan-freeness of the accepted state, and not in the
    \* cleanup phase (a cleanup burst must end orphan-free whatever the
    \* window state; SupervisorProtocol.tla additionally refines the
    \* cleanup-VALIDATION carve-out for stating-phase cleanup bursts).
...
(* (ProofFormalization unconditionally; theorem_stating — which also    *)
(* models RevisionStating, cf. deviations §22 — iff every configured    *)
(* target is covered, i.e. the orphan-construction window is shut).      *)
...
ApplySidecarClosure(n) ==
    ...
    /\ inFlightRequestKind = "none"
    /\ \/ phase = "proof_formalization"
       \/ (phase = "theorem_stating" /\ ~OrphanConstructionWindowOpen)
    /\ n \in openNodes
    /\ n # activeNode
