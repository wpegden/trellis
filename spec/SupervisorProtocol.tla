...
CONSTANTS
    ...
    Deviations,
    NoDeviation,
    \* Definition nodes whose Lean body is a placeholder (the
    \* definition-analogue of a SKETCH proof): either the bare `True` body
    \* of a `def`/`abbrev`, or the contentless `where`-body of a
    \* `structure`/`inductive`/`class` data declaration (no fields or
    \* constructors). Observed off-disk by the kernel
    \* (`observe_placeholder_definition_nodes`); a node in this set
    \* can never reach correspondence Pass — `current_corr_state` forces a
    \* deterministic Fail (the `"placeholder_def_auto_fail"` correspondence
    \* auto-fail, mirroring the soundness `"sketch_auto_fail"` status value).
    \* Modelled as a constant external fact at the same abstraction level as
    \* the sketch mechanism (which is a status-VALUE the verifier/observation
    \* layer assigns, not a separate state variable). Configs that don't want
    \* to model placeholder definitions set PlaceholderDefinitionNodes <- {}.
    PlaceholderDefinitionNodes,
    ...
    \* ---- PV challenge-coverage parity (Step 1, 2026-06-25) ----------------
    \* `ChallengeTargets` is the closed set of byte-pinned challenge targets
    \* (kernel `ChallengeTargetSpec`). Disjoint from
    \* `Targets` (the paper-faithfulness bucket): a challenge target carries
    \* NO verifier lane, only a derived `BlockerKind::ChallengeCoverage`
    \* blocker. Configs that don't model challenge coverage set
    \* ChallengeTargets <- {}; the whole surface then degrades to no-ops.
    ChallengeTargets,
    \* Closed GOAL-owned resolution map. `prove` exposes only the positive
    \* result; `decide` exposes positive, negative, and ratified-conditional
    \* results. No execution/artifact fact can rewrite this map.
    ChallengeResolution,
    \* `GoalMode` is the run-wide goal mode (kernel `goal_mode` / `PvGoalMode`):
    \* "lean" is mode-B (Lean-goals / byte-pinned challenge specs, no paper-
    \* faithfulness bucket); "prose" is mode-A (math autoformalization, paper-
    \* grounded). In mode-B the configured (paper-faithfulness) target bucket
    \* is empty — see `ModeBConfiguredTargetsEmpty`. The prove-or-disprove
    \* polarity machinery layered on top (Step 2) is FORWARD DESIGN ahead of
    \* Rust per the README "Rust Parity Rule" carve-out; the challenge
    \* substrate (ChallengeTargets / challengeCoverage / blocker) has kernel
    \* parity today.
    GoalMode,
    \* ---- PV mode-B prove-or-disprove axis (Step 2, forward design) -------
    ...
    \* times across a run. Mirrors SupervisorCore!MaxPolarityFlips.
    MaxPolarityFlips,
    \* Abstract truth model. `ChallengeTruth[t] \in {"prove","disprove"}` is
    \* which side of the Decide pair (T vs ~T) is ACTUALLY true. The axiom-
    \* floor closure gate (`CloseChallengeSidePermitted`) only lets a covering
    \* node close the side that matches `ChallengeTruth[t]` — so closing a
    \* FALSE statement is impossible on EITHER polarity (the abstraction of
    \* "passing the same clean-axiom gate"; a false Lean goal has no axiom-
    \* clean closed proof). Immutable for a run. Mirrors
    \* SupervisorCore!ChallengeTruth.
    ChallengeTruth,
    \* Dedicated PV under-model assumptions node (`Tablet/Assumptions`).
    \* Setup also seeds an approved uninterpreted `axiom` predicate hook naming
    \* Rust-valid raw Aeneas values. Worker-authored staged `.spec` / `.tex`
    \* blocks here must be conditional property axioms over that hook; ordinary
    \* Correspondence gates the pair before the assumptions sub-lane gates
    \* Rust-language eligibility. Eligibility is limited to hook-conditional
    \* property axioms.
    AssumptionsNode
...
\* Placeholder-`True` bodies only occur on definition nodes (a theorem body
\* is a proof, never the bare type `True`), mirroring the kernel's
\* `NodeKind::Definition` gate in `observe_placeholder_definition_nodes`.
ASSUME \A n \in PlaceholderDefinitionNodes: NodeKinds[n] = "definition"
...
\* Challenge targets are a distinct bucket from the paper-faithfulness
\* targets and from nodes (the carrier disjointness keeps the blocker
\* taxonomy unambiguous; cf. SupervisorCore ASSUME ChallengeTargets \cap
\* Targets = {}).
...
ASSUME ChallengeResolution \in [ChallengeTargets -> {"decide", "prove"}]
ASSUME AssumptionsNode \in Nodes
ASSUME (GoalMode = "lean") => (NodeKinds[AssumptionsNode] = "definition")
...
StageValues ==
    {
        ...
        "HumanGate",
        "Complete",
        \* Pre-bootstrap Phase 0 uses its own state record; the global stage
        \* begins at Check and remains there until the sealed-root production
        \* handoff enters theorem-stating Start.
        "Check",
        "SeamAudit",
        "CorrespondenceAudit",
        "Sealed",
...
\* transition and differs only in out-of-spec setup (import, inherited
\* approvals, frozen nodes). See CORE_SPEC_DEVIATIONS.md §22.
PhaseValues == {"source_adaptation", "theorem_stating", "proof_formalization", "cleanup", "complete"}

Phase0StageValues == {"Worker", "Check", "SeamAudit", "CorrespondenceAudit", "Sealed", "Incomplete"}
MaxPhase0Handbacks == 32
ASSUME Fingerprints \ {NoFingerprint} # {}
Phase0InitialRoot == CHOOSE fp \in Fingerprints \ {NoFingerprint}: TRUE

CorrStates == {"unknown", "pass", "fail"}
CorrUpdates == CorrStates \cup {"same"}

SubstantivenessStates == CorrStates \cup {"false_as_stated"}
SubstantivenessUpdates == SubstantivenessStates \cup {"same"}
...
\* Per-target outcome protocol. The existing `challengeClosedSide` variable
\* carries this record so the large protocol keeps one atomic challenge-state
\* cell per target while distinguishing unconditional proof/disproof closure,
\* the conditional lifecycle, and optional artifact disclosure.
ConditionalStages ==
    {"none", "proposed", "correspondence_pass", "human_approved", "open", "closed"}
ConditionalStageRank(stageValue) ==
    CASE stageValue = "none" -> 0
      [] stageValue = "proposed" -> 1
      [] stageValue = "correspondence_pass" -> 2
      [] stageValue = "human_approved" -> 3
      [] stageValue = "open" -> 4
      [] stageValue = "closed" -> 5

ChallengeOutcomeValues ==
    [
        positiveSide: {"open", "closed"},
        negativeSide: {"open", "closed"},
        conditionalStage: ConditionalStages,
        conditionalRejected: BOOLEAN,
        conditionalTarget: ChallengeTargets,
        conditionIdentity: Fingerprints,
        sealIdentity: Fingerprints,
        ratifiedConditionIdentity: Fingerprints,
        ratifiedSealIdentity: Fingerprints,
        closedConditionIdentity: Fingerprints,
        closedSealIdentity: Fingerprints,
        artifactIdentity: Fingerprints
    ]

InitialChallengeOutcome(t) ==
    [
        positiveSide |-> "open",
        negativeSide |-> "open",
        conditionalStage |-> "none",
        conditionalRejected |-> FALSE,
        conditionalTarget |-> t,
        conditionIdentity |-> NoFingerprint,
        sealIdentity |-> NoFingerprint,
        ratifiedConditionIdentity |-> NoFingerprint,
        ratifiedSealIdentity |-> NoFingerprint,
        closedConditionIdentity |-> NoFingerprint,
        closedSealIdentity |-> NoFingerprint,
        artifactIdentity |-> NoFingerprint
    ]
...
WorkerOutcomes ==
    {"none", "valid", "invalid", "stuck", "needs_restructure", "target_false_under_model"}
...
RetryOutcomeKinds ==
    {"none", "invalid", "stuck", "needs_restructure", "target_false_under_model", "transport"}
...
GateKinds == {"none", "advance", "needinput", "protected_reapproval", "assumption_review"}
...
\* TheoremStating Restructure: the third legal target-edit mode, the
\* analog of proof `restructure`. The reviewer names a cross-cone
\* authorizedNodes set (the coordinated statements to restate) and a
\* next_active focus within it; the candidate / authorization envelope is
\* `presentNodes \ frozen` rather than the cone.
TargetEditModes == {"global", "targeted", "restructure"}
ProofEditModes == {"local", "restructure", "coarse_restructure"}
\* "restructure" is shared by TargetEditModes and ProofEditModes, so the
\* union deduplicates it — TaskModes is unchanged in membership.
...
InitialPresentNodes ==
    {n \in Nodes : NodeKinds[n] = "preamble"}
        \cup (IF GoalMode = "lean" THEN {AssumptionsNode} ELSE {})
InitialNodeKinds ==
    [n \in Nodes |->
        IF n \in {m \in Nodes : NodeKinds[m] = "preamble"} THEN
            "preamble"
        ELSE
            "definition"]
ProjectInvariants ==
...
NoRequest ==
    [
        ...
        theoremRestructureNextActiveNodes |-> {},
        allowTargetedWithoutNextActive |-> FALSE,
        allowedResets |-> {},
...
\* Fourth object variant, otype "challenge", carrying the challenge-target
\* id. Carrier for the kernel-derived `BlockerKind::ChallengeCoverage`
\* blocker (model.rs). The byte-conformance of the claimed declaration text
\* is observation-layer; the spec models only claim-update legality and the
\* coverage/decision gate.
ChallengeObject(t) == [otype |-> "challenge", challenge |-> t]
...
BlockerUniverse ==
    ...
    \cup
    \* ChallengeCoverage blockers (kernel `BlockerKind::ChallengeCoverage`).
    \* One per challenge target; the coverage blocker carries no meaningful
    \* fingerprint (it is a derived structural blocker, not a verifier
    \* verdict), so the fp slot is pinned to `NoFingerprint`. See
    \* `ChallengeBlockers` / `ChallengeTargetBlocked` for the membership rule.
    {Blocker("challenge_coverage", ChallengeObject(t), NoFingerprint) : t \in ChallengeTargets}

BlockerChoiceId(b) ==
    b.kind \o ":" \o b.object.otype \o ":" \o
        (IF b.object.otype = "node" THEN b.object.node
         ELSE IF b.object.otype = "target" THEN b.object.target
         ELSE IF b.object.otype = "deviation" THEN b.object.deviation
         ELSE b.object.challenge)
        \o ":" \o b.fp
...
VARIABLES
    ...
    committedCoverage,
    approvedCoverage,
    \* ---- PV challenge-coverage (Step 1) + prove-or-disprove (Step 2) ----
    \* `challengeCoverage[t]` : the LIVE side's covering node-set for
    \*   challenge target t (kernel `challenge_coverage` derived from
    \*   `challenge_claims`). Modeled as direct live state (a single copy,
    \*   like SupervisorCore!challengeCoverage); the committed mirror that
    \*   ordinary `currentCoverage`/`committedCoverage` maintain is abstracted
    \*   away here — challenge claims have no separate live/committed claim-map
    \*   split in this model, so there is no drift to reconcile at quiescence.
    \* `approvedChallengeCoverage[t]` : frozen at advance / protected
    \*   reapproval (kernel `ApprovedTargetSnapshot.challenge_coverage`).
    \* `challengePolarity[t]` : the LIVE side (default "prove"); only the
    \*   StuckMathAudit accept flips it, and only for `decide` targets.
    \* `polarityFlips[t]` : per-target flip counter (anti-thrash bound).
    \* `challengeClosedSide[t]` : structured outcome record. It carries
    \*   separate positive/negative closure, the exact conditional packet and
    \*   monotone lifecycle, plus an optional frozen artifact identity. The
    \*   artifact field has no authority over closure or completion.
    challengeCoverage,
    ...
    \* PV under-model assumptions sub-lane. TRUE while a Corr-passed
    \* Assumptions node is being checked by the StuckMathAudit assumptions
    \* lane. `pendingUnderModelAssumptions` mirrors the kernel's pending
    \* PROPOSED_ASSUMPTIONS count. Each counted item is abstractly a
    \* hook-conditional property axiom over the setup Rust-validity hook.
    ...
    \* everShallowCoarseClosed: subset of Nodes; history of coarse
    \*     nodes observed shallow-closed against committed. Monotone
    \*     except for accepted deletions (retired nodes are pruned).
    ...
    inFlightRequest,
    response,
    \* Finite abstraction of the pre-bootstrap Rust Phase0State. Concrete
    \* manifests, receipt sets, ledger rows, and bundle bytes refine these
    \* roots/booleans; the generation and handback counter are exact.
    phase0State
...
Vars == <<CampaignVars, phase0State>>
\* Every campaign variable except phase and stage.  Written out as a
\* syntactic tuple: TLC evaluates UNCHANGED of a non-tuple expression
\* (such as Tail(Tail(CampaignVars))) by evaluating it in the primed
\* state, which fails on the variables the action leaves unassigned.
CampaignDataVars ==
    <<
        cycle,
        attempt,
...
\* All protocol state except the structured per-target outcome map. New
\* abstract conditional/artifact actions use this projection so they cannot
\* accidentally mutate any unrelated supervisor state.
VarsExceptChallengeOutcome ==
    <<
        phase,
        stage,
        cycle,
...
\* `everShallowCoarseClosed` is the closed-history refresh tracked in
\* `commit_live` against the COMMITTED baseline (S8 invariant) —
\* monotone except for accepted deletions (retired nodes are pruned).
...
\* `RequestGlobalRepairAudit` (Step A — sets request, drops any
\* still-pending grant it supersedes, dispatches),
...
DepClosureFor(depMap, seed, livePresent) ==
    {
        n \in livePresent :
            \A s \in SUBSET livePresent:
                (seed \subseteq s /\ \A m \in s: depMap[m] \subseteq s) => n \in s
    }
...
OrphanNodesFor(liveCoverage, livePresent, depMap) ==
    {n \in livePresent :
        /\ n # "Preamble"
        /\ n \notin DepClosureFor(depMap, TargetCoveringNodes(liveCoverage), livePresent)}

OrphanCleanupActive ==
    pendingTask.orphanCleanupNodes # {}

\* Uncovered-target orphan window (kernel:
\* `orphan_construction_window_open`): open iff some configured paper
\* target has an empty covering set in the live coverage.  Consumed by
\* `WorkerAcceptedResponsesSatisfyContract` on the unprimed (pre-burst)
\* state; challenge targets are deliberately excluded.
OrphanConstructionWindowOpen ==
    \E t \in configuredTargets : currentCoverage[t] = {}
...
SubstantivenessLaneMapsWithFalseAt(laneMaps, node) ==
    [l \in VerifierLanes |-> [laneMaps[l] EXCEPT ![node] = "false_as_stated"]]
...
\* Defined after CorrLaneReportNeighbors because SANY does not permit the
\* former forward reference. This is a definition-order repair only.
SubstantivenessLaneMapNeighbors(verifyNodes) ==
    LET base ==
            {CorrNodeLaneMapsFromReports(reports, verifyNodes)
                : reports \in CorrLaneReportNeighbors}
    IN base
       \cup IF GoalMode = "prose" THEN
               {SubstantivenessLaneMapsWithFalseAt(laneMaps, node)
                   : laneMaps \in base, node \in verifyNodes}
           ELSE
               {}
...
\* `NotDoneYet` value that the kernel-level reconciler maps onto "same"
\* (no status update — node remains Unknown). `FalseAsStated` persists as
\* its own substantiveness value and derives a non-Pass current state. See
...
ReconcileSubstantivenessLaneMaps(laneMaps) ==
    [n \in Nodes |->
        ...
        IN IF votes = {"same"} THEN
            "same"
        ELSE IF \E s \in SubstantivenessStates: votes = {s} THEN
            CHOOSE s \in SubstantivenessStates: votes = {s}
        ELSE
            "same"
    ]
...
SubstantivenessLaneMapsSplit(laneMaps) ==
    \E n \in Nodes:
        LaneMapSplit({laneMaps[l][n] : l \in VerifierLanes}, SubstantivenessStates)
...
WorkerStagesUnderModelDraft(nextPresent, nextCorrCurrent) ==
    /\ GoalMode = "lean"
    /\ AssumptionsNode \in nextPresent
    /\ nextCorrCurrent[AssumptionsNode] # corrApprovedFp[AssumptionsNode]
...
WorkerRemovedNodes ==
    inFlightRequest.currentPresentNodes \ response.present

WorkerResponseDeps(livePresent) ==
    ApplyNodeSetUpdates(currentDeps, response.depMap, livePresent)

WorkerResponseCoverage(livePresent) ==
    NormalizedWorkerCoverage(
        currentTargetClaims,
        response.targetClaimMap,
        livePresent,
...
WorkerPostResponseOrphans ==
    OrphanNodesFor(response.coverage, response.present, WorkerResponseDeps(response.present))

WorkerBaselineOrphans ==
    OrphanNodesFor(currentCoverage, inFlightRequest.currentPresentNodes, currentDeps)

WorkerSameBurstOrphansBeforeDeletion ==
    LET syntheticPresent == response.present \cup WorkerRemovedNodes
    IN OrphanNodesFor(
        WorkerResponseCoverage(syntheticPresent),
        syntheticPresent,
        WorkerResponseDeps(syntheticPresent)
    )

WorkerCleanupValidation ==
    inFlightRequest.workerAcceptance.validationKind \in {"cleanup", "final_cleanup"}
...
WorkerAcceptedResponsesSatisfyContract ==
    ...
    \/ /\ WorkerStructuralMapsMatchObserved
       ...
       /\ response.deletedNodes = WorkerRemovedNodes
       \* Uncovered-target orphan window (kernel:
       \* `orphan_construction_window_open` in
       \* `tablet_deletion_contract_errors`): while some configured paper
       \* target has an empty PRE-burst covering set, the waiver is
       \* NARROW — deletion LEGALITY is unconditional (removed nodes must
       \* lie in the same-burst orphan set, which includes pre-existing
       \* orphans, so orphan sweeping stays legal but supported nodes may
       \* never be deleted); the window waives only the must-delete-own-
       \* new-orphans requirement (the removed = same-burst equality) and,
       \* for NON-CLEANUP bursts only, the leaves-NEW-orphans rejection.
       \* Evaluated on the unprimed `currentCoverage` (the window the
       \* worker was dispatched under), so a burst that itself covers the
       \* final uncovered target while leaving unrelated orphans is still
       \* accepted. Cleanup validation keeps the post-response gate
       \* window-independently, but judges the burst against its baseline.
       /\ (\/ WorkerCleanupValidation
           \/ WorkerRemovedNodes \subseteq WorkerSameBurstOrphansBeforeDeletion)
       /\ (\/ WorkerCleanupValidation
           \/ OrphanConstructionWindowOpen
           \/ WorkerRemovedNodes = WorkerSameBurstOrphansBeforeDeletion)
       /\ (\/ (OrphanConstructionWindowOpen /\ ~WorkerCleanupValidation)
           \/ WorkerPostResponseOrphans \ WorkerBaselineOrphans = {})
...
ReviewResetChoices(currentPhase, retryKind) ==
    ...
    \* Bug 2 (live-run incident 2026-06-26): this reviewer-driven menu is the
    \* SOLE producer of "lastClean" — a SEMANTIC, reviewer-driven rollback.
    \* No state-integrity / divergence fault auto-selects LastClean. Round-2:
    \* the operational state-divergence recovery (startup
    \* "state diverges from disk") AUTO-REWINDS to lastCommit
    \* (runtime.rs:restore_worktree_to_head, `git reset --hard HEAD`, ~1 cycle
    \* back) and fails loud only if divergence persists after that rewind; it
    \* never reaches restore_repo_worktree_to_last_clean (see
    \* CORE_SPEC_DEVIATIONS.md #24). The on-disk LastClean rewind target is
    \* commit-pinned (runtime.rs:resolve_last_clean_commitish) — the more-recent
    \* of the commit pointer / nearest HEAD-ancestor clean tag, never the
    \* lexically-highest clean-tag; that selection is a runtime/git concern
    \* below this spec's abstraction.
...
CurrentNodeCorrState(n) ==
    \* Placeholder-`True` definition auto-fail: forced Fail before the
    \* stored-status match, mirroring the kernel `current_corr_state`
    \* short-circuit (`"placeholder_def_auto_fail"`).
    IF /\ n \in presentNodes
       /\ n \in PlaceholderDefinitionNodes
    THEN
        "fail"
...
UnderModelAssumptionDraftReady ==
    /\ GoalMode = "lean"
    /\ AssumptionsNode \in presentNodes
    /\ stagedUnderModelAssumptionDraft
    /\ CurrentNodeCorrUnknown(AssumptionsNode)
    /\ UnderModelAssumptionShapeOK

PostNodeCorrState(n, corrMap, corrApprovedMap) ==
    \* Placeholder-`True` definition auto-fail (see CurrentNodeCorrState).
    IF /\ n \in presentNodes
       /\ n \in PlaceholderDefinitionNodes
    THEN
        "fail"
...
\* True iff the node claims at least one deviation whose current state is
\* specifically Fail (the Deviation lane ran and rejected it), as opposed
\* to a still-verifying Unknown claim. Mirror of kernel
\* `node_has_failed_deviation_claim`. Drives the recovery edge in
\* `CurrentSubstantivenessState`: a rejected-deviation claim reads Fail
\* (worker frontier) rather than Unknown (verifier frontier).
NodeHasFailedDeviationClaim(n) ==
    \E id \in nodeDeviationClaims[n] : CurrentDeviationFail(id)
...
\*
\* Audit follow-up (2026-05-27, kernel commit 4e83783): when the node
\* claims a deviation whose authorization is not currently Pass, the
\* substantiveness state short-circuits — the lane cannot advance until
\* the deviation is authorized or the claim is dropped. Mirror of
\* `current_substantiveness_state`'s deviation-claim branch, split on the
\* deviation's state to give a rejected claim a recovery edge:
\*   - a Fail-deviation claim (`NodeHasFailedDeviationClaim`) reads "fail":
\*     drops the node off `SubstantivenessVerifyNodes` and onto the worker
\*     frontier, where the worker drops the claim or revises the deviation
\*     (re-opening the Deviation lane). Without this a Fail deviation with
\*     live claims has no recovery edge.
\*   - an Unknown-deviation (still verifying) claim reads "unknown",
\*     keeping the node on the verifier frontier until the lane settles.
CurrentSubstantivenessState(n) ==
    IF phase \notin {"theorem_stating", "proof_formalization"} THEN
        "pass"
    ELSE IF n \notin presentNodes THEN
        "unknown"
    ELSE IF GoalMode = "lean" /\ n = AssumptionsNode THEN
        "pass"
    ELSE IF NodeHasFailedDeviationClaim(n) THEN
        "fail"
    ...
    ELSE IF /\ substantivenessStatus[n] = "fail"
            /\ substantivenessCurrentFp[n] = substantivenessApprovedFp[n]
    THEN "fail"
    ELSE IF /\ substantivenessStatus[n] = "false_as_stated"
            /\ substantivenessCurrentFp[n] = substantivenessApprovedFp[n]
    THEN "fail"
    ELSE "unknown"
...
\* The kernel also requires an unbound worker-authored Decide primary. This
\* abstraction lacks binding and provenance state, so it over-approximates
\* within the sole current claimant of a prose challenge target.
CurrentSubstantivenessAllowsCorr(n) ==
    \/ CurrentSubstantivenessPass(n)
    \/ /\ GoalMode = "prose"
       /\ \E t \in ChallengeTargets: challengeCoverage[t] = {n}
       /\ substantivenessStatus[n] = "false_as_stated"
       /\ substantivenessCurrentFp[n] = substantivenessApprovedFp[n]
...
CorrVerifyNodes ==
    IF phase \in {"theorem_stating", "proof_formalization"} THEN
        {n \in presentNodes :
            /\ CurrentNodeCorrUnknown(n)
            /\ CurrentSubstantivenessAllowsCorr(n)
            /\ (n # AssumptionsNode \/ UnderModelAssumptionDraftReady)}
    ELSE
        {n \in presentNodes :
            /\ CurrentNodeCorrUnknown(n)
            /\ (n # AssumptionsNode \/ UnderModelAssumptionDraftReady)}
...
SubstantivenessVerifyNodes ==
    IF phase \in {"theorem_stating", "proof_formalization"} THEN
        {n \in presentNodes : CurrentSubstantivenessUnknown(n) /\ n # AssumptionsNode}
    ELSE
        {}
...
\* ---- C4' auto-dispatch fallback scan + C1 first-review backlog ------
\*
\* Kernel `select_theorem_sound_verify_node` (model.rs) arm 3 no longer
\* defers to the held target; it scans `live.present_nodes ∩
\* sound_auto_dispatch_eligible ∩ current_sound_unknown`.
\* `select_theorem_held_target` itself is unchanged (it still defines
\* the reviewer next_active envelope), so `SelectedTheoremHeldTarget`
\* above stays byte-identical.
\*
\* `SoundScanEligibleFor` is the fully parameterized per-node gate —
\* mirror of kernel `sound_auto_dispatch_eligible` = needs_sound +
\* non-SKETCH + `sound_repair_ready` (own + direct-dep corr AND
\* substantiveness Pass) — restricted to a currently-Unknown sound
\* state. Accept actions call it with the post-response maps they
\* compute locally (matching the kernel, which consults
\* `sound_verify_nodes()` on the mutated state); the convenience form
\* `SoundAutoDispatchEligible` reads the unprimed state. The kernel
\* keys the dep set on the recorded noderef fingerprint parts, falling
\* back to `deps`; the spec's dep map is that abstraction.
SoundScanEligibleFor(n, proofSet, openSet, depsMap,
                     corrMap, corrCurrentMap, corrApprovedMap,
                     substMap, substCurrentMap, substApprovedMap,
                     assessMap, soundMap, soundCurrentMap, soundApprovedMap) ==
    /\ n \in proofSet
    /\ n \in openSet
    /\ assessMap[n] # "sketch_auto_fail"
    /\ ~(soundMap[n] = "pass" /\ soundCurrentMap[n] = soundApprovedMap[n])
    /\ ~(soundMap[n] \in {"fail", "structural"}
         /\ soundCurrentMap[n] = soundApprovedMap[n])
    /\ corrMap[n] = "pass" /\ corrCurrentMap[n] = corrApprovedMap[n]
    /\ substMap[n] = "pass" /\ substCurrentMap[n] = substApprovedMap[n]
    /\ \A d \in depsMap[n] :
        /\ corrMap[d] = "pass" /\ corrCurrentMap[d] = corrApprovedMap[d]
        /\ substMap[d] = "pass" /\ substCurrentMap[d] = substApprovedMap[d]

SoundAutoDispatchEligible(n) ==
    SoundScanEligibleFor(n, currentProofNodes, openNodes, currentDeps,
                         corrStatus, corrCurrentFp, corrApprovedFp,
                         substantivenessStatus, substantivenessCurrentFp,
                         substantivenessApprovedFp,
                         soundAssessmentStatus, soundStatus, soundCurrentFp,
                         soundApprovedFp)

\* C4' ordering leg 1 — assessment class: FreshUnknown (never reviewed)
\* first, then SelfEditUnknown/SplitUnknown, then
\* DepEditOnlyStalePassDeferred, then everything else the eligibility
\* filter admits (reviewer_accepted_pass). Mirrors kernel
\* `sound_auto_dispatch_class_rank`.
SoundAutoDispatchClassRank(n) ==
    IF soundAssessmentStatus[n] = "fresh_unknown" THEN 0
    ELSE IF soundAssessmentStatus[n] \in {"self_edit_unknown", "split_unknown"} THEN 1
    ELSE IF soundAssessmentStatus[n] = "dep_edit_only_stale_pass_deferred" THEN 2
    ELSE 3

\* C4' ordering leg 2 — open-dependent fanout: number of open proof
\* nodes transitively depending on `n` (reverse dep reachability,
\* excluding `n` itself). Mirrors kernel `open_dependent_fanout`
\* (computed on the fly, never persisted).
OpenDependentFanout(n) ==
    Cardinality({m \in (ReverseDepClosure({n}, presentNodes) \ {n}) :
        m \in currentProofNodes /\ m \in openNodes})

TheoremSoundAutoDispatchCandidates ==
    IF CorrespondenceBlockersExist THEN
        {}
    ELSE
        {n \in presentNodes : SoundAutoDispatchEligible(n)}

\* Total order over the scan: class rank ascending, then fanout
\* DESCENDING, then NodeOrder ascending (NodeOrder stands in for the
\* kernel's node-name-ascending tie-break, exactly as
\* SelectedTheoremHeldTarget uses it for the name-descending pick).
SoundAutoDispatchPrecedes(n, m) ==
    \/ SoundAutoDispatchClassRank(n) < SoundAutoDispatchClassRank(m)
    \/ /\ SoundAutoDispatchClassRank(n) = SoundAutoDispatchClassRank(m)
       /\ OpenDependentFanout(n) > OpenDependentFanout(m)
    \/ /\ SoundAutoDispatchClassRank(n) = SoundAutoDispatchClassRank(m)
       /\ OpenDependentFanout(n) = OpenDependentFanout(m)
       /\ NodeOrder[n] <= NodeOrder[m]

SelectedTheoremSoundVerifyNode ==
    IF TheoremSoundAutoDispatchCandidates = {} THEN
        NoNode
    ELSE
        CHOOSE n \in TheoremSoundAutoDispatchCandidates :
            \A m \in TheoremSoundAutoDispatchCandidates :
                SoundAutoDispatchPrecedes(n, m)

\* C1 stability-gated first-review backlog. Membership = never
\* reviewed (`fresh_unknown`) + the per-node gate WITHOUT the global
\* `CorrespondenceBlockersExist` gate. The kernel additionally requires
\* the node's own-tex fingerprint to have been stable for K committed
\* snapshots (`sound_backlog_stability` tracker, K =
\* TRELLIS_SOUND_BACKLOG_STABILITY_CYCLES, default 8); the snapshot
\* clock is below this spec's abstraction (no snapshot counter is
\* modeled), so the spec over-approximates stability as always
\* satisfied — every stability-gated schedule the kernel produces is a
\* refinement of the behaviors admitted here. Parameterized over the
\* corr / substantiveness maps so accept actions can evaluate it
\* against their locally computed post-response maps.
SoundBacklogNodesFor(corrMap, corrApprovedMap, substMap, substApprovedMap) ==
    IF phase \in {"theorem_stating", "proof_formalization"} THEN
        {n \in presentNodes :
            /\ soundAssessmentStatus[n] = "fresh_unknown"
            /\ SoundScanEligibleFor(n, currentProofNodes, openNodes, currentDeps,
                                    corrMap, corrCurrentFp, corrApprovedMap,
                                    substMap, substantivenessCurrentFp,
                                    substApprovedMap,
                                    soundAssessmentStatus, soundStatus,
                                    soundCurrentFp, soundApprovedFp)}
    ELSE
        {}

SoundBacklogNodes ==
    SoundBacklogNodesFor(corrStatus, corrApprovedFp,
                         substantivenessStatus, substantivenessApprovedFp)

\* A placeholder-`True` definition node is forced to correspondence Fail
\* (the `"placeholder_def_auto_fail"` mechanism) regardless of its stored
\* status / fingerprint, mirroring the kernel's `current_corr_state`
\* short-circuit. Every other node uses the ordinary "not (pass /\
\* fp-matched)" rule.
NodeCorrBlockersFor(corrMap, corrCurrentMap, corrApprovedMap, livePresent) ==
    {Blocker("node_corr", NodeObject(n), corrCurrentMap[n]) :
        n \in {m \in livePresent :
            \/ m \in PlaceholderDefinitionNodes
            \/ ~(corrMap[m] = "pass" /\ corrCurrentMap[m] = corrApprovedMap[m])}}
...
SubstantivenessBlockersFor(nodeMap, nodeCurrentMap, nodeApprovedMap, livePresent) ==
    ...
    ELSE
        ...
        {Blocker("substantiveness", NodeObject(n), nodeCurrentMap[n]) :
            n \in {m \in livePresent :
                /\ ~(GoalMode = "lean" /\ m = AssumptionsNode)
...
\* ===================================================================== *
\* PV challenge-coverage blocker (Step 1) + prove-or-disprove gate (Step 2)
\* ===================================================================== *
\* CloseChallengeSidePermitted(t, side) is the abstract AXIOM-FLOOR gate:
\* a side may close only if it is TRUE under the abstract reality
\* ChallengeTruth[t]. The truth check is polarity-symmetric for a `decide`
\* target; a `prove` target additionally excludes the negative arm. Mirrors
\* SupervisorCore!CloseSidePermitted apart from the new resolution map.
CloseChallengeSidePermitted(t, side) ==
    /\ ChallengeTruth[t] = side
    /\ (side = "prove" \/ ChallengeResolution[t] = "decide")

ConditionalIdentityStarted(outcome) ==
    /\ outcome.conditionIdentity # NoFingerprint
    /\ outcome.sealIdentity # NoFingerprint

ConditionalRatificationExact(outcome) ==
    /\ ConditionalIdentityStarted(outcome)
    /\ outcome.ratifiedConditionIdentity = outcome.conditionIdentity
    /\ outcome.ratifiedSealIdentity = outcome.sealIdentity

ConditionalClosureExact(outcome) ==
    /\ outcome.conditionalStage = "closed"
    /\ ConditionalRatificationExact(outcome)
    /\ outcome.closedConditionIdentity = outcome.conditionIdentity
    /\ outcome.closedSealIdentity = outcome.sealIdentity

ConditionalNoLongerPending(outcome) ==
    \/ outcome.conditionalStage = "none"
    \/ outcome.conditionalRejected

ConditionalRatificationReadyTargets ==
    {t \in ChallengeTargets :
        /\ ChallengeResolution[t] = "decide"
        /\ challengeClosedSide[t].positiveSide = "open"
        /\ challengeClosedSide[t].negativeSide = "open"
        /\ challengeClosedSide[t].conditionalStage = "correspondence_pass"
        /\ ~challengeClosedSide[t].conditionalRejected
        /\ ConditionalIdentityStarted(challengeClosedSide[t])}

RatifyChallengeConditional(outcomes, t) ==
    [outcomes EXCEPT
        ![t].conditionalStage = "human_approved",
        ![t].ratifiedConditionIdentity = outcomes[t].conditionIdentity,
        ![t].ratifiedSealIdentity = outcomes[t].sealIdentity]

ChallengePositiveClosed(t) ==
    challengeClosedSide[t].positiveSide = "closed"

ChallengeNegativeClosed(t) ==
    /\ ChallengeResolution[t] = "decide"
    /\ challengeClosedSide[t].negativeSide = "closed"

ChallengeConditionalClosed(t) ==
    /\ ChallengeResolution[t] = "decide"
    /\ ~challengeClosedSide[t].conditionalRejected
    /\ ConditionalClosureExact(challengeClosedSide[t])

\* Deterministic displayed-result precedence matches the kernel classifier:
\* positive proof, then exact ratified conditional, then negative proof.
\* A live pending proposal suppresses an unconditional display; rejection
\* makes only an independently closed unconditional side displayable.
DisplayedResultClassesIn(outcomes, t) ==
    IF /\ outcomes[t].positiveSide = "closed"
       /\ ConditionalNoLongerPending(outcomes[t]) THEN
        {"proved"}
    ELSE IF /\ ChallengeResolution[t] = "decide"
            /\ ~outcomes[t].conditionalRejected
            /\ ConditionalClosureExact(outcomes[t]) THEN
        {"conditional"}
    ELSE IF /\ ChallengeResolution[t] = "decide"
            /\ outcomes[t].negativeSide = "closed"
            /\ ConditionalNoLongerPending(outcomes[t]) THEN
        {"disproved"}
    ELSE
        {}

DisplayedResultClasses(t) ==
    DisplayedResultClassesIn(challengeClosedSide, t)

ChallengeTargetDecidedIn(outcomes, t) ==
    /\ challengeCoverage[t] # {}
    /\ DisplayedResultClassesIn(outcomes, t) # {}

\* Per-target completion is independent of live polarity and artifact state.
\* A `prove` target can reach only the positive arm by ResolutionPolicy.
ChallengeTargetDecided(t) ==
    ChallengeTargetDecidedIn(challengeClosedSide, t)

\* A challenge target is a kernel-derived `BlockerKind::ChallengeCoverage`
\* blocker. It has NO verifier lane. PHASE-GATING: it is a blocker in
\* proof_formalization and cleanup (where proving happens and completion is
\* gated), but NOT in theorem_stating — the prove-vs-disprove DIRECTION is
...
\* false. This mirrors the Substantiveness phase-dormancy carve-out.
\* Coverage/decision is still required for COMPLETION (cleanup -> complete,
\* which requires GlobalBlockers = {} with the challenge target active).
\*
\* Step 2: clears only when the LIVE side is DECIDED (covered AND closed) —
\* never via the dormant side. (Step 1 used a coverage-only gate; see the
\* SPEC_TODO §0 / git history.)
ChallengeTargetBlocked(t) ==
    /\ phase \in {"proof_formalization", "cleanup"}
    /\ ~ ChallengeTargetDecided(t)

ChallengeBlockersFor(coverageMap) ==
    {Blocker("challenge_coverage", ChallengeObject(t), NoFingerprint) :
        t \in {u \in ChallengeTargets : ChallengeTargetBlocked(u)}}
...
GlobalBlockers ==
    ...
    \cup
    ChallengeBlockersFor(challengeCoverage)
...
\* `CleanupBlockersAreAdmissible` invariant would be discovered only after the
...
\* are still committed-present but NOT currently shallow-closed
...
EverShallowCoarseClosedRegressed ==
    ...
    ELSE
        {n \in everShallowCoarseClosed :
            /\ n \in coarseDagNodes
            \* Retirement: a history entry absent from the COMMITTED
            \* present set has been (accepted-)deleted — retired, not
            \* regressed. Committed (not live) presence, matching the
            \* snapshot the closure check runs against. Mirrors the
            \* kernel's committed.present_nodes conjunct.
            /\ n \in committedPresentNodes
...
ProofAggregateNodeBlockerFocusCandidates ==
    ...
    ELSE
        LET
            \* The kernel bails out on the FIRST blocker whose object is not
            \* node-scoped (`let BlockerObject::Node { node } = ... else
            \* return`), so every non-node otype closes the candidate set —
            \* `deviation` and `challenge` as much as `target`. Testing
            \* `= "target"` alone also made `b.object.node` a field-select on
            \* a deviation blocker.
            blockedNodes ==
                IF \E b \in GlobalBlockers : b.object.otype # "node"
                THEN {}
...
\* PARITY GAP (sound_verifier_eligible): kernel commit 4ab1c1be extended
\* model.rs `theorem_targeted_mode_legal` with `|| sound_verifier_eligible(node)`
\* so a FreshUnknown sound-backlog node is a legal Targeted anchor even though
\* it carries no current Fail blocker. This spec does not model soundness
\* fingerprints / FreshUnknown state, so the extra disjunct is omitted here; as
\* a result this spec's targeted candidate set stays a subset of the cone-based
\* `RequestAllowedNextActiveNodes` and the cone-floor relaxation in
\* `ReviewResponseLegal` (skip-floor-for-targeted) is currently redundant in the
\* model but kept for structural faithfulness to the kernel.
...
\* TheoremStating Restructure frozen set: present nodes whose statement a
\* Restructure must never authorize restating. A node is frozen iff it is
\* in the approved-target coverage closure, or is the kernel-managed
\* Preamble / Axioms node. (The kernel additionally freezes challenge
\* byte-pinned nodes; this spec does not model challenge coverage as a
\* state variable, so this is a sound under-approximation of the frozen
\* set — every node frozen here is frozen in the kernel.)
\*
\* PARITY GAP (PV / challenge coverage / mode-B prove-or-disprove):
\* The kernel HAS challenge-target coverage (model.rs ChallengeTargetSpec /
\* BlockerKind::ChallengeCoverage / ApprovedTargetSnapshot.challenge_coverage),
\* a goal_mode discriminant (PvGoalMode), the byte-pin
\* (worker_normalization::challenge_conformance_errors), and the StuckMathAudit
\* + advance-gate machinery (engine.rs). This faithful-kernel spec does NOT yet
\* model any of it (a `challengeCoverage` state variable would have to thread
\* through every action's UNCHANGED frame across ~17k lines, validated sim-only
\* — the RAM-heavy harness). The PV mode-B + prove-or-disprove DESIGN is fully
\* modeled and bounded-sim-validated in the INTENT spec SupervisorCore.tla
\* (CONSTANT GoalMode / ChallengeTargets / ChallengeTruth; VARIABLES
\* challengePolarity / polarityFlips / challengeClosedSide; predicates
\* CloseSidePermitted / ChallengeTargetDecided / ChallengeTargetBlocked;
\* AcceptStuckMathAudit sole-flipper; ReviewAdvancePhase vacuous auto-advance;
\* invariants SoundnessFloor / NotBothSidesClosed / NoHumanInModeB /
\* CoverageOnLiveOnly / DecidedResolves / AuditSoleFlipper). Bringing THIS spec
\* to challenge-coverage parity (then adding the same polarity layer, keeping
\* BigSpec => SupervisorCore!Spec) is a tracked follow-up; it was deferred here
\* to avoid leaving the 17k-line spec mid-edit / non-parsing under a sim-only
\* validation budget. See spec/SPEC_TODO.md.
TheoremRestructureFrozenNodes ==
    (UNION {approvedCoverage[t] : t \in approvedConfiguredTargets})
        \cup ({"Preamble", "Axioms"} \cap presentNodes)

\* Restructure candidate / authorization envelope = present \ frozen.
TheoremRestructureEnvelope ==
    presentNodes \ TheoremRestructureFrozenNodes
...
SoundVerifyNodes ==
    IF phase = "theorem_stating" THEN
        \* C4': the auto-dispatch fallback is the ordered scan over
        \* eligible current-Unknown nodes (class > fanout desc > name),
        \* no longer the held target filtered to Unknown. The
        \* reviewer-requested and active-node arms of
        \* `select_theorem_sound_verify_node` remain unmodeled here
        \* (as before); the held target itself still governs the
        \* reviewer next_active envelope only.
        IF SelectedTheoremSoundVerifyNode # NoNode THEN
            {SelectedTheoremSoundVerifyNode}
        ELSE
            {}
    ELSE IF phase = "proof_formalization" THEN
...
PostSubstantivenessUnknownNodes(nodeMap, nodeApprovedMap) ==
    IF phase \in {"theorem_stating", "proof_formalization"} THEN
        {n \in presentNodes :
            ...
            /\ ~(nodeMap[n] = "false_as_stated" /\ substantivenessCurrentFp[n] = nodeApprovedMap[n])}
    ELSE
        {}

PostSubstantivenessAllowsCorr(n, nodeMap, nodeApprovedMap) ==
    \/ /\ nodeMap[n] = "pass"
       /\ substantivenessCurrentFp[n] = nodeApprovedMap[n]
    \/ /\ GoalMode = "prose"
       /\ \E t \in ChallengeTargets: challengeCoverage[t] = {n}
       /\ nodeMap[n] = "false_as_stated"
       /\ substantivenessCurrentFp[n] = nodeApprovedMap[n]
...
PostSoundVerifyNodes(selectedHeldTarget) ==
    IF phase = "theorem_stating" THEN
        \* C4': the post-accept sound frontier is the eligibility scan.
        \* `selectedHeldTarget = NoNode` is the gate proxy: the accept
        \* actions compute it as NoNode exactly when their post-response
        \* blocker set has corr/paper/substantiveness members (the
        \* kernel's `corr_blockers_exist` gate on the mutated state) or
        \* when no proof-open-corrPass-soundNotPass candidate exists —
        \* and the scan set is a subset of that candidate set, so the
        \* proxy is exact. Callers whose action mutates corr or
        \* substantiveness maps must inline `SoundScanEligibleFor` with
        \* their post maps instead (see AcceptCorrArtifactTheorem /
        \* AcceptSubstantivenessArtifactTheorem).
        IF selectedHeldTarget = NoNode THEN
            {}
        ELSE
            {n \in presentNodes : SoundAutoDispatchEligible(n)}
    ELSE IF phase = "proof_formalization" THEN
...
RequestSoundVerifyNode(kind) ==
    ...
    ELSE IF kind \in {"sound"} /\ SoundBacklogNodes # {} THEN
        \* C1 backlog dispatch: a Sound request issued while the ordinary
        \* frontier is empty (global gate closed) carries the backlog
        \* head — fanout descending, then NodeOrder (name) ascending.
        \* The kernel persists the chosen node in
        \* `sound_backlog_dispatch` at the service point and re-derives
        \* the request from that field (byte-equality invariant,
        \* Malformed reissues); the spec re-derives from the same
        \* deterministic ordering, which agrees as long as membership is
        \* unchanged while the request is in flight (verifier lanes do
        \* not mutate the tablet, so it is).
        CHOOSE n \in SoundBacklogNodes :
            \A m \in SoundBacklogNodes :
                \/ OpenDependentFanout(n) > OpenDependentFanout(m)
                \/ /\ OpenDependentFanout(n) = OpenDependentFanout(m)
                   /\ NodeOrder[n] <= NodeOrder[m]
    ELSE
...
RequestVerifyNodes(kind) ==
    ...
    ELSE IF kind = "sound" THEN
        \* Mirror kernel `request_verify_nodes` → `request_sound_verify_nodes`:
        \* the per-dispatch singleton, which under C1 also covers a
        \* backlog dispatch issued while `SoundVerifyNodes` is empty.
        RequestSoundVerifyNodes(kind)
    ELSE
        {}
...
\* TheoremStating Restructure candidate / authorization envelope surfaced
\* on the Review request (present \ frozen). Distinct from the cone-based
\* allowed/targeted sets so broadening Restructure does not widen the
\* Global/Targeted next_active check.
RequestTheoremRestructureNextActiveNodes(kind) ==
    IF /\ kind = "review"
       /\ phase = "theorem_stating"
       /\ retryOutcomeKind # "invalid"
    THEN
...
RequestAllowedNextModes(kind) ==
    ...
    ELSE IF phase = "theorem_stating" THEN
        IF retryOutcomeKind = "invalid" THEN
            {CurrentMode}
        ELSE
            \* Global is always offered; Targeted when its cone candidate
            \* set is non-empty; Restructure (free, no audit gate) when the
            \* present-minus-frozen envelope is non-empty.
            {"global"}
                \cup (IF RequestTargetedNextActiveNodes(kind) = {}
                        THEN {} ELSE {"targeted"})
                \cup (IF RequestTheoremRestructureNextActiveNodes(kind) = {}
                        THEN {} ELSE {"restructure"})
    ELSE IF phase = "proof_formalization" THEN
        ProofEditModes
    ELSE IF phase = "cleanup" THEN
        {"cleanup"}
...
\* Theorem-stating offers every failed blocker kind. Proof-formalization
\* offers substantiveness alone: that lane's fingerprint is node-local while
\* its rubric is relational, so an importer-side repair leaves a failed child
\* off the verifier frontier with no other route back to it. The reset is not
\* a clearing action in either phase — Unknown is non-Pass, so the blocker
\* stays in GlobalBlockers until a verifier passes on the node.
\*
\* Both offers drop a substantiveness Fail the reset cannot move. Only the
\* status/fingerprint arm at the tail of `CurrentSubstantivenessState` is
\* reachable by a reset; a rejected-deviation claim
\* (`NodeHasFailedDeviationClaim`) answers ahead of that arm, so resetting
\* such a node rewrites maps the verdict never reads and it stays off
\* `SubstantivenessVerifyNodes`. The worker is its recovery edge, and the
\* Deviation blocker riding alongside stays offered.
IsDerivedSubstantivenessFail(b) ==
    /\ b.kind = "substantiveness"
    /\ ~ /\ substantivenessStatus[b.object.node] \in {"false_as_stated", "fail"}
         /\ substantivenessCurrentFp[b.object.node] = substantivenessApprovedFp[b.object.node]

RequestAllowedResetBlockers(kind) ==
    LET movable == {b \in CurrentFailedBlockers : ~IsDerivedSubstantivenessFail(b)}
    IN
        IF kind # "review" THEN
            {}
        ELSE IF phase = "theorem_stating" THEN
            movable
        ELSE IF phase = "proof_formalization" THEN
            {b \in movable : b.kind = "substantiveness"}
        ELSE
            {}
...
ReviewScopeEnvelope(nextMode, nextActive) ==
    ...
    IN
        ...
        ELSE IF phase = "cleanup" THEN
            presentNodes
        \* TheoremStating Restructure: the envelope is present \ frozen, NOT
        \* the focus impact region; the reviewer narrows it to an explicit
        \* authorizedNodes subset (EffectiveAuthorizedNodes).
        ELSE IF phase = "theorem_stating" /\ nextMode = "restructure" THEN
            TheoremRestructureEnvelope
...
WorkerResponse(currentCycle, outcome, livePresent, liveOpen, liveCoverage, liveCorrCurrent, livePaperCurrent, liveSoundCurrent, liveTargetFp) ==
    ...
    IN [
        ...
        corrPanelSplit |-> FALSE,
        soundPanelSplit |-> FALSE,
        \* WorkerResponse.deleted_nodes: the explicit same-burst Tablet
        \* node deletion report. Disk normalization remains authoritative:
        \* acceptance requires this field to equal current-present minus
        \* normalized-present.
        deletedNodes |-> presentNodes \ livePresent,
        proofNodeMap |->
            [n \in Nodes |->
...
ReviewScenarioFragments(kind) ==
    IF kind = "review" THEN
        (<<
            ...
            ELSE IF retryOutcomeKind = "target_false_under_model" THEN
...
RequestSoundContract(kind) ==
    IF kind \in {"sound"} THEN
        [
            ...
            \* Mirror kernel request_contracts.rs "target_nodes":
            \* request.sound_verify_nodes — the per-dispatch singleton
            \* (backlog-aware under C1), not the raw frontier.
            targetNodes |-> RequestSoundVerifyNodes(kind),
...
TypeOK ==
    ...
    /\ phase0State \in [
        stage: Phase0StageValues,
        generation: Nat,
        handbacks: 0..MaxPhase0Handbacks,
        genesisRoot: Fingerprints,
        genesisGoalRoot: Fingerprints,
        goalRoot: Fingerprints,
        rawRoot: Fingerprints,
        adaptedRoot: Fingerprints,
        replayedRoot: Fingerprints,
        ledgerRoot: Fingerprints,
        semanticBundleRoot: Fingerprints,
        sealedRoot: Fingerprints,
        diagnosticRoot: Fingerprints,
        productionRoot: Fingerprints,
        bootstrapRoot: Fingerprints,
        advanceGateRoot: Fingerprints,
        deltaComplete: BOOLEAN,
        discoveryComplete: BOOLEAN,
        ablationComplete: BOOLEAN,
        bindingComplete: BOOLEAN,
        seamClean: BOOLEAN,
        correspondenceClean: BOOLEAN,
        seamGeneration: Nat,
        correspondenceGeneration: Nat,
        unresolved: BOOLEAN
        ]
    /\ cycle \in 0..MaxCycle
    ...
    \* PV challenge coverage (Step 1) + prove-or-disprove cluster (Step 2).
    /\ challengeCoverage \in [ChallengeTargets -> SUBSET Nodes]
    /\ approvedChallengeCoverage \in [ChallengeTargets -> SUBSET Nodes]
    /\ \A t \in ChallengeTargets:
        /\ challengeCoverage[t] \subseteq presentNodes
        /\ approvedChallengeCoverage[t] \subseteq Nodes
        \* Claim exclusivity: at most one claiming node per challenge target,
        \* and no node claims two challenge targets.
        /\ Cardinality(challengeCoverage[t]) <= 1
    /\ \A t1, t2 \in ChallengeTargets:
        (t1 # t2) => (challengeCoverage[t1] \cap challengeCoverage[t2] = {})
    /\ challengePolarity \in [ChallengeTargets -> {"prove", "disprove"}]
    /\ polarityFlips \in [ChallengeTargets -> 0..MaxPolarityFlips]
    /\ challengeClosedSide \in [ChallengeTargets -> ChallengeOutcomeValues]
    ...
    /\ substantivenessStatus \in [Nodes -> SubstantivenessStates]
    ...
    /\ stuckMathAuditAssumptionsLaneActive \in BOOLEAN
    /\ stagedUnderModelAssumptionDraft \in BOOLEAN
    /\ pendingUnderModelAssumptions \in 0..MaxCycle
    /\ stuckMathAuditAssumptionsLaneActive => stuckMathAuditActive
    ...
    /\ inFlightRequest.theoremRestructureNextActiveNodes \subseteq Nodes
    ...
    \* NeedInputAuditor, deviation protocol prompts, NeedInput re-entry
    \* guard, reference papers, trust-route projection, structured give-up
    \* witnesses, opaque-inventory projection, Phase-0 upload origins, etc.); current value 63.
    ...
    /\ response.substantivenessMap \in [Nodes -> SubstantivenessUpdates]
    ...
    /\ response.substantivenessLaneMaps \in [VerifierLanes -> [Nodes -> SubstantivenessUpdates]]
    ...
    /\ response.deletedNodes \subseteq Nodes
...
\* Cleanup blocker contract. This used to read
\* `phase = "cleanup" => GlobalBlockers = {}`: at every protocol pause
\* point in Cleanup the run could legally terminate.
\*
\* The kernel's per-burst cleanup acceptance predicate has weakened from
\* `formalization_complete` to `formalization_valid`
\* (`kernel/src/model.rs`). It keeps every Lean-closure clause verbatim —
\* no open sorry, no unverified proof node, a fresh closure record for
\* every proof node — and admits exactly one class of open blocker: a
\* `node_corr` blocker on a node OUTSIDE the ratified statement surface.
\* That is what lets a cleanup task create a helper node whose NL gloss
\* has not been verified yet. Correspondence on a non-ratified node is a
\* scheduling or documentation matter, not a soundness one; Lean closure
\* is ground truth for what was proved.
\*
\* The weakening is a floor, not a redefinition of done. Every exit
\* remains gated on the EMPTY set: `ReviewDone`, `ReviewAdvancePhase`,
\* and the package barrier all require `GlobalBlockers = {}`, so nothing
\* blocker-carrying reaches `complete`.
\*
\* Scope note: this spec has no action that PRODUCES a cleanup state with
\* an admissible blocker (helper extraction and the correspondence-repair
\* route are not modelled here). The invariant is therefore weaker than
\* the reachable state space requires — deliberately, so the spec stops
\* asserting a property the kernel no longer guarantees without inventing
\* actions whose fidelity has not been checked.
CleanupProtectedStatementNodes ==
    (UNION {approvedCoverage[t] : t \in approvedConfiguredTargets})
        \cup ({"Preamble", "Axioms"} \cap presentNodes)

CleanupAdmissibleBlocker(b) ==
    /\ b.kind = "node_corr"
    /\ b.object.otype = "node"
    /\ b.object.node \notin CleanupProtectedStatementNodes

CleanupBlockersAreAdmissible ==
    phase = "cleanup" =>
        \A b \in GlobalBlockers : CleanupAdmissibleBlocker(b)
...
\*
\* Read together with `CleanupBlockersAreAdmissible`: the two together say that
...
\* ----------------------------------------------------------------------
\*
\* Together with the existing `CleanupBlockersAreAdmissible` and
...
FingerprintPinnedOnDecisiveStatus ==
    /\ \A n \in presentNodes :
         ...
         /\ substantivenessStatus[n] \in {"pass", "false_as_stated", "fail"}
...
GlobalBlockersExhaustive ==
    ...
    \* ChallengeCoverage (kernel `BlockerKind::ChallengeCoverage`): a
    \* challenge target is a blocker iff it is phase-active and not decided.
    \* Phase-gated to proof_formalization + cleanup (see
    \* `ChallengeTargetBlocked`).
    /\ \A t \in ChallengeTargets :
         ChallengeTargetBlocked(t)
           <=> (\E b \in GlobalBlockers :
                  b.kind = "challenge_coverage" /\ b.object.challenge = t)
...
\*     must be empty (by induction from CleanupBlockersAreAdmissible in the
...
\* Global-repair request/grant mutual exclusion: a pending Step A
\* request and a pending Step B grant never coexist. The Core spec
\* encodes this structurally (one `globalRepairStep` variable cannot
\* hold both); here it is maintained by Step A dropping any
\* still-pending grant it supersedes (`RequestGlobalRepairAudit`)
\* and Step B setting the grant only after consuming the request
\* (`ApplyStuckMathAuditGlobalRepairResponse`). Mirrors the kernel:
\* `route_global_repair_request_to_auditor` takes the grant at the
\* Step A accept point.
GlobalRepairRequestGrantMutex ==
    ~(/\ pendingGlobalRepairRequest # NoGlobalRepairRequest
      /\ pendingGlobalRepairGrant # NoGlobalRepairGrant)
...
Init ==
    ...
    /\ approvedChallengeCoverage = [t \in ChallengeTargets |-> {}]
    /\ challengePolarity = [t \in ChallengeTargets |-> "prove"]
       \* Every Decide target starts on the Prove side (default polarity);
       \* only the StuckMathAudit accept may flip it (Step 2).
    /\ polarityFlips = [t \in ChallengeTargets |-> 0]
    /\ challengeClosedSide = [t \in ChallengeTargets |-> InitialChallengeOutcome(t)]
    ...
    /\ phase0State = [
        ...
        generation |-> 0,
        handbacks |-> 0,
        genesisRoot |-> Phase0InitialRoot,
        genesisGoalRoot |-> Phase0InitialRoot,
        goalRoot |-> Phase0InitialRoot,
        rawRoot |-> Phase0InitialRoot,
        adaptedRoot |-> Phase0InitialRoot,
        replayedRoot |-> Phase0InitialRoot,
        ledgerRoot |-> Phase0InitialRoot,
        semanticBundleRoot |-> NoFingerprint,
        sealedRoot |-> NoFingerprint,
        diagnosticRoot |-> NoFingerprint,
        productionRoot |-> NoFingerprint,
        bootstrapRoot |-> NoFingerprint,
        advanceGateRoot |-> NoFingerprint,
        deltaComplete |-> TRUE,
        discoveryComplete |-> TRUE,
        ablationComplete |-> TRUE,
        bindingComplete |-> FALSE,
        seamClean |-> FALSE,
        correspondenceClean |-> FALSE,
        seamGeneration |-> 0,
        correspondenceGeneration |-> 0,
        unresolved |-> FALSE
        ]
...
ReviewDecisionLegal ==
    ...
    \* NeedInput re-entry guard (Defect 2): a NEED_INPUT escalation is
    \* illegal while human input is already outstanding and the reviewer
    \* is not consuming it in the same response. Mirror of the kernel
    \* check in `validate_review_response_against_request`
    \* (review_normalization.rs). Without it, a content-free human
    \* approve (which only sets `human_input_outstanding = TRUE` and
    \* re-issues a plain Review) lets the reviewer re-escalate against
    \* the same unresolved blocker, spinning the
    \* Review -> StuckMathAudit -> HumanGate -> Review livelock. The
    \* reviewer must instead CONTINUE (clearing the flag) to act on the
    \* human payload before NEED_INPUT becomes legal again.
    /\ (response.decision = "NEED_INPUT" /\ humanInputOutstanding)
        => response.clearHumanInput
    /\ \A n \in Nodes:
    ...
    /\ IF phase = "theorem_stating" THEN
            ...
            /\ IF invalidAttempt THEN
               ...
               ELSE
                    /\ response.decision \in RequestAllowedDecisions("review")
                    /\ response.reset \in RequestAllowedResets("review")
                    \* Restructure reaches beyond the cone-based candidate
                    \* set: its next_active is validated against the
                    \* present-minus-frozen envelope. Targeted likewise
                    \* reaches beyond the cone-based candidate set: since
                    \* kernel commit 4ab1c1be `TheoremTargetedModeLegal`
                    \* admits sound-verifier-eligible nodes (not modeled in
                    \* this spec's `TheoremTargetedModeLegal`, see the parity
                    \* note there), so its candidate set is not a subset of
                    \* the cone. The cone floor is therefore skipped for a
                    \* Targeted Continue; the targeted-set check below is the
                    \* governing gate. Mirrors the kernel's
                    \* review_response_legal.
                    /\ \/ response.nextActive = NoNode
                       \/ response.nextActive \in RequestAllowedNextActiveNodes("review")
                       \/ /\ response.nextMode = "restructure"
                          /\ response.nextActive \in
                                 RequestTheoremRestructureNextActiveNodes("review")
                       \/ /\ response.nextMode = "targeted"
                          /\ response.decision = "CONTINUE"
                          /\ response.reset = NoCheckpoint
                          /\ response.nextActive \in
                                 RequestTargetedNextActiveNodes("review")
                    /\ response.nextMode \in RequestAllowedNextModes("review")
                    \* TheoremStating Restructure: authorizedNodes non-empty
                    \* (cross-cone permission), every entry within the
                    \* present-minus-frozen envelope, and next_active among
                    \* them. Mirrors the kernel's review_response_legal.
                    /\ (response.nextMode = "restructure"
                        /\ response.decision = "CONTINUE"
                        /\ response.reset = NoCheckpoint)
                       => /\ response.authorizedNodes # {}
                          /\ response.authorizedNodes \subseteq
                                 RequestTheoremRestructureNextActiveNodes("review")
                          /\ (response.nextActive # NoNode
                              => response.nextActive \in response.authorizedNodes)
                    \* Defect A: the Targeted-mode next_active requirement
                    \* applies only to an ACTING decision — a Continue with no
                    \* reset that dispatches a worker to a chosen focus. Every
                    \* non-acting decision (AdvancePhase, NeedInput, and a
                    \* reset-carrying Continue) re-derives or has no next_active
                    \* and is waived. Generalizes the earlier AdvancePhase-only
                    \* waiver (kernel commit 58a1bd7).
                    /\ (/\ response.nextMode = "targeted"
                        /\ response.decision = "CONTINUE"
                        /\ response.reset = NoCheckpoint)
    ...
    \*       `lastReviewerGlobalRepairRequestCycle`: within
    \*       `StuckMathAuditDispatchCooldownCycles` cycles of the
    \*       previous Step A acceptance it is legal only when no
    \*       recent decline is in scope, and it must have no
    \*       in-flight `pendingGlobalRepairRequest`. A pending Step B
    \*       grant does not rate-limit Step A; the accepted Step A
    \*       supersedes it.
...
\*     (see RequestAllowedResetBlockers for the per-phase offer)
\*
...
\* and the blocker set must be empty by `CleanupBlockersAreAdmissible` plus
...
\* `RequestBlockers("review") = GlobalBlockers` and `CleanupBlockersAreAdmissible`
...
\*    deviation claim, its substantiveness state must NOT be Pass — even
\*    if the underlying `substantivenessStatus[n]` says Pass. The claim
\*    splits on the deviation's state: a rejected (Fail) claim reads
\*    "fail" (worker-frontier recovery edge); an in-flight (Unknown) claim
\*    reads "unknown" (verifier frontier). Mirror of kernel
\*    `current_substantiveness_state`'s deviation-claim branch.
DeviationUnauthorizedClaimSuppressesSubstantivenessPass ==
    \A n \in presentNodes :
        NodeHasUnauthorizedDeviationClaim(n)
            => /\ CurrentSubstantivenessState(n) # "pass"
               /\ NodeHasFailedDeviationClaim(n)
                    => CurrentSubstantivenessState(n) = "fail"
               /\ (NodeHasUnauthorizedDeviationClaim(n) /\ ~NodeHasFailedDeviationClaim(n))
...
\* ===================================================================== *
\* PV challenge-coverage (Step 1) + prove-or-disprove (Step 2) invariants. *
\* Big-spec analogs of the SupervisorCore invariants of the same name.    *
\* ===================================================================== *

\* ModeBConfiguredTargetsEmpty. In a mode-B (Lean-goals) run the paper-
\* faithfulness target bucket is always empty (ASSUME seeds it empty; no
\* action populates it in mode-B — EnvEditConfiguredTargets is mode-A only,
\* see its GoalMode # "lean" guard). Mirrors SupervisorCore.
ModeBConfiguredTargetsEmpty ==
    GoalMode = "lean" => configuredTargets = {}

\* CompleteRequiresChallengeCoverage. Completion requires every challenge
\* target covered. Enforcement is inherited: an uncovered (Step 2:
\* undecided) phase-active challenge target sits in GlobalBlockers, and the
\* advance / done gates require GlobalBlockers = {}. Mirrors SupervisorCore.
CompleteRequiresChallengeCoverage ==
    phase = "complete" =>
        \A t \in ChallengeTargets : challengeCoverage[t] # {}

\* CoverageOnLiveOnly. The PV cluster stays structurally well-formed: the
\* anti-thrash flip budget holds, polarity / closure stay in range, and a
\* closed record always belongs to a covered (live) side — no orphan
\* dormant-side closure survives. Mirrors SupervisorCore. (Step 1: the
\* closure record is always "open", so the last conjunct is vacuous; Step 2
\* makes it load-bearing.)
CoverageOnLiveOnly ==
    \A t \in ChallengeTargets :
        /\ polarityFlips[t] <= MaxPolarityFlips
        /\ challengePolarity[t] \in {"prove", "disprove"}
        /\ challengeClosedSide[t].positiveSide \in {"open", "closed"}
        /\ challengeClosedSide[t].negativeSide \in {"open", "closed"}
        /\ (ChallengePositiveClosed(t) \/ ChallengeNegativeClosed(t)
            \/ ChallengeConditionalClosed(t))
              => (challengeCoverage[t] # {})

\* ResolutionPolicy. GOAL.md fixes the closed resolution map. A `prove`
\* target never flips, closes the negative side, or starts a conditional;
\* only a `decide` target may use those alternatives.
ResolutionPolicy ==
    \A t \in ChallengeTargets :
        ChallengeResolution[t] = "prove" =>
            /\ challengePolarity[t] = "prove"
            /\ polarityFlips[t] = 0
            /\ challengeClosedSide[t].negativeSide = "open"
            /\ challengeClosedSide[t].conditionalStage = "none"

\* ConditionalLifecycleWellFormed. Target identity is fixed by the map key;
\* condition identity is frozen at proposal, seal identity at Corr pass,
\* ratification copies the exact pair, and closure copies that same sealed
\* pair. These shape constraints make stale/substituted packets impossible.
ConditionalLifecycleWellFormed ==
    \A t \in ChallengeTargets :
        LET outcome == challengeClosedSide[t]
        IN  /\ outcome.conditionalTarget = t
            /\ CASE outcome.conditionalStage = "none" ->
                        /\ ~outcome.conditionalRejected
                        /\ outcome.conditionIdentity = NoFingerprint
                        /\ outcome.sealIdentity = NoFingerprint
                        /\ outcome.ratifiedConditionIdentity = NoFingerprint
                        /\ outcome.ratifiedSealIdentity = NoFingerprint
                        /\ outcome.closedConditionIdentity = NoFingerprint
                        /\ outcome.closedSealIdentity = NoFingerprint
                    [] outcome.conditionalStage = "proposed" ->
                        /\ outcome.conditionIdentity # NoFingerprint
                        /\ outcome.sealIdentity = NoFingerprint
                        /\ outcome.ratifiedConditionIdentity = NoFingerprint
                        /\ outcome.ratifiedSealIdentity = NoFingerprint
                        /\ outcome.closedConditionIdentity = NoFingerprint
                        /\ outcome.closedSealIdentity = NoFingerprint
                    [] outcome.conditionalStage = "correspondence_pass" ->
                        /\ ConditionalIdentityStarted(outcome)
                        /\ outcome.ratifiedConditionIdentity = NoFingerprint
                        /\ outcome.ratifiedSealIdentity = NoFingerprint
                        /\ outcome.closedConditionIdentity = NoFingerprint
                        /\ outcome.closedSealIdentity = NoFingerprint
                    [] outcome.conditionalStage \in {"human_approved", "open"} ->
                        /\ ~outcome.conditionalRejected
                        /\ ConditionalRatificationExact(outcome)
                        /\ outcome.closedConditionIdentity = NoFingerprint
                        /\ outcome.closedSealIdentity = NoFingerprint
...
\* SoundnessFloor. An unconditional result closes only the true exact side.
\* A conditional result is available only to `decide` and only from the exact
\* human-ratified seal; its proof never upgrades either unconditional side.
SoundnessFloor ==
    \A t \in ChallengeTargets :
        /\ ChallengePositiveClosed(t) => (ChallengeTruth[t] = "prove")
        /\ ChallengeNegativeClosed(t) =>
              /\ ChallengeResolution[t] = "decide"
              /\ ChallengeTruth[t] = "disprove"
        /\ (challengeClosedSide[t].conditionalStage = "closed") =>
              /\ ChallengeResolution[t] = "decide"
              /\ ConditionalClosureExact(challengeClosedSide[t])
              /\ challengeClosedSide[t].positiveSide = "open"
              /\ challengeClosedSide[t].negativeSide = "open"

\* NotBothSidesClosed. Positive and negative closure are mutually exclusive;
\* exact conditional closure is also distinct from both unconditional sides.
\* Thus C -> T never implies T or ~T.
NotBothSidesClosed ==
    \A t \in ChallengeTargets :
        /\ ~(ChallengePositiveClosed(t) /\ ChallengeNegativeClosed(t))
        /\ ChallengeConditionalClosed(t) =>
...
\* ArtifactIrrelevance. Quantifying over every absent/present frozen artifact
\* identity proves that adding, removing, or replacing disclosure cannot alter
\* the displayed resolution or ChallengeTargetDecided.
ArtifactIrrelevance ==
    \A t \in ChallengeTargets, artifact \in Fingerprints :
        LET changed == [challengeClosedSide EXCEPT ![t].artifactIdentity = artifact]
        IN ChallengeTargetDecidedIn(challengeClosedSide, t)
             = ChallengeTargetDecidedIn(changed, t)

\* NoHumanInModeB. Mode-B keeps the ordinary human-stop lanes suppressed.
\* Besides assumption review, an advance/protected-reapproval episode may
\* carry exactly one current correspondence-passed conditional packet.
NoHumanInModeB ==
    GoalMode = "lean" =>
        /\ ((stage = "HumanGate") =>
              \/ gateKind \in {"none", "assumption_review"}
              \/ /\ gateKind \in {"advance", "protected_reapproval"}
                    /\ Cardinality(ConditionalRatificationReadyTargets) = 1)
        /\ (inFlightRequest.kind = "human_gate" =>
              \/ inFlightRequest.gateKind \in {"none", "assumption_review"}
              \/ /\ inFlightRequest.gateKind \in {"advance", "protected_reapproval"}
                    /\ Cardinality(ConditionalRatificationReadyTargets) = 1)
        /\ pendingProtectedReapprovalNodes = {}

\* UnderModelAssumptionLifecycleContract. Formula bodies stay abstract. The
\* assumptions sub-lane can be active only after the Assumptions node's
\* `.spec`/`.tex` pair has passed Correspondence, and any pending count denotes
\* a worker-authored conditional property axiom over the setup Rust-validity hook.
UnderModelAssumptionLifecycleContract ==
    /\ stuckMathAuditAssumptionsLaneActive =>
        /\ GoalMode = "lean"
        /\ AssumptionsNode \in presentNodes
        /\ stagedUnderModelAssumptionDraft
        /\ CurrentNodeCorrPass(AssumptionsNode)
        /\ UnderModelAssumptionShapeOK
    /\ pendingUnderModelAssumptions > 0 =>
        /\ GoalMode = "lean"
        /\ UnderModelAssumptionShapeOK

\* DecidedResolves. Every complete target has exactly one displayed class.
\* `prove` admits only Proved; `decide` admits Proved, Disproved, or the exact
\* ratified Conditional. A rejected/stalled candidate remains no result.
DecidedResolves ==
    phase = "complete" =>
        \A t \in ChallengeTargets :
            /\ ChallengeTargetDecided(t)
            /\ Cardinality(DisplayedResultClasses(t)) = 1
            /\ DisplayedResultClasses(t) \subseteq
                  IF ChallengeResolution[t] = "prove" THEN
                      {"proved"}
                  ELSE
                      {"proved", "disproved", "conditional"}

\* AuditSoleFlipper (ACTION property). `challengePolarity` changes only
\* across a StuckMathAudit accept step (AcceptStuckMathAuditBackToReviewer,
\* the sole flipper) — no worker / reviewer / verifier / human action
\* mutates it. TWO-STATE property: checked as `[][AuditSoleFlipper]_Vars`
\* under PROPERTIES, not as a single-state INVARIANT. Operationally it is
\* the machine-checked form of the UNCHANGED-challengePolarity discipline
\* every non-flipper action carries. Mirrors SupervisorCore.
AuditSoleFlipper ==
    (challengePolarity' # challengePolarity) =>
        /\ stage = "StuckMathAudit"
        /\ inFlightRequest.kind = "stuck_math_audit"

\* ConditionalProgressMonotone (ACTION property). Every target moves only
\* forward through the six-stage lifecycle. Once introduced, target,
\* condition, and seal identities are immutable; ratified and closed copies
\* likewise remain fixed. Artifact identity is deliberately excluded.
ConditionalProgressMonotone ==
    \A t \in ChallengeTargets :
        LET before == challengeClosedSide[t]
            after == challengeClosedSide'[t]
        IN  /\ ConditionalStageRank(after.conditionalStage)
                  >= ConditionalStageRank(before.conditionalStage)
            /\ ConditionalStageRank(after.conditionalStage)
                  <= ConditionalStageRank(before.conditionalStage) + 1
            /\ after.conditionalTarget = before.conditionalTarget
            /\ before.conditionalStage # "none" =>
                  after.conditionIdentity = before.conditionIdentity
            /\ ConditionalStageRank(before.conditionalStage)
                  >= ConditionalStageRank("correspondence_pass") =>
                  after.sealIdentity = before.sealIdentity
            /\ ConditionalStageRank(before.conditionalStage)
                  >= ConditionalStageRank("human_approved") =>
                  /\ after.ratifiedConditionIdentity = before.ratifiedConditionIdentity
                  /\ after.ratifiedSealIdentity = before.ratifiedSealIdentity
            /\ before.conditionalStage = "closed" =>
                  /\ after.closedConditionIdentity = before.closedConditionIdentity
                  /\ after.closedSealIdentity = before.closedSealIdentity
            /\ before.conditionalRejected => after.conditionalRejected
...
StartCycle ==
    ...
    /\ stage' =
        ...
        \* before the cone-clean force-review, before the verifier/worker
        \* priority ladder.
        ...
        ELSE IF pendingTask.taskBlockers # {} THEN
            "Worker"
        \* Sound-frontier yield (2026-07, mirrors the kernel's
        \* `theorem_start_request_kind` / `proof_start_request_kind`):
        \* the auto sound frontier fills the cycle-start slot only when
        \* NO pending worker task exists (`pendingTask = NoPendingTask`
        \* below). A pending empty-taskBlockers task previously lost to
        \* the sound arm and was silently cleared on the verifier issue
        \* (`pendingTask' = NoPendingTask` on non-Worker dispatch) —
        \* the reviewer-routed-task starvation shape. The kernel's
        \* reviewer-requested Sound arm carries the same pendingTask
        \* gate (lossless: the request set is persisted and served at
        \* the next sound slot); at theorem_stating that arm is
        \* unmodeled here (see SoundVerifyNodes), and at
        \* proof_formalization it folds into SoundVerifyNodes, so the
        \* single gate below covers it. Paper / corr keep their
        \* precedence over the pending task.
        ELSE IF phase = "theorem_stating" THEN
            ...
            THEN
                "VerifyCorr"
            ELSE IF SoundVerifyNodes # {} /\ pendingTask = NoPendingTask THEN
                "VerifySound"
            ELSE
                "Worker"
        ELSE IF phase = "proof_formalization" THEN
...
\* Two disjuncts (mirrors SupervisorCore!EditConfiguredTargets):
\*   1. the historical in-place edit during theorem_stating;
\*   2. add-targets mode (the offline `add_paper_targets` runtime-CLI
\*      action, 2026-07): superset-only growth on a COMPLETED run, with
\*      the phase flipped back to theorem_stating IN THE SAME STEP —
\*      CleanupDoneTerminal + GlobalBlockersExhaustive make in-place
\*      growth at complete illegal (a new configured target immediately
\*      carries a paper blocker). The kernel revives into RevisionStating,
\*      which the specs do not model; theorem_stating is the sanctioned
\*      abstraction (RevisionStating is theorem-stating-like for every
\*      gate modeled here). approvedConfiguredTargets is UNCHANGED in both
\*      disjuncts, and both preserve the mode-A GoalMode guard.
\* (`EnvEditConfiguredTargets` itself is defined after the two branch
\* actions — TLA+ requires definition before use.)
EnvEditConfiguredTargetsInPlace ==
    /\ phase = "theorem_stating"
    /\ stage = "Start"
    \* Mode-B (PV) has no paper-faithfulness bucket; the configured-target
    \* set stays empty (ModeBConfiguredTargetsEmpty). This operator edit is
    \* mode-A only. Mirrors SupervisorCore!EditConfiguredTargets.
    /\ pendingUnderModelAssumptions = 0
    /\ ~(GoalMode = "lean" /\ configuredTargets = {})
...
\* Add-targets mode (offline `add_paper_targets` runtime-CLI action).
\* Superset-only configured-target growth on a COMPLETED run, atomically
\* flipping complete -> theorem_stating in the same step. Kernel recipe
\* mirrored here:
\*   - added targets re-enter the paper lane fresh (paperStatus "unknown";
\*     the kernel also clears their approved fingerprints — none can exist
\*     for a never-configured target, so paperApprovedFp is UNCHANGED);
\*   - coverage recomputed against the grown target set per the existing
\*     EnvEditConfiguredTargetsInPlace machinery (the added targets get
\*     empty coverage: no claims name them yet);
\*   - pendingTask reset; active seat / held target / edit mode reset to
\*     their theorem-stating entry values;
\*   - the cleanup latch set returns to its dormant values, preserving
\*     PhaseDormancyContract's `phase \notin {"cleanup", "complete"}`
\*     clause after the flip;
\*   - cycle is UNCHANGED (the kernel keeps the cycle and the event log
\*     continuous across the revival);
\*   - approvedConfiguredTargets is UNCHANGED (only the AdvancePhase
\*     freeze moves it).
\* Mode-A only: GoalMode # "lean" AND a non-empty pre-add configured set.
EnvAddTargetsAtComplete ==
    /\ phase = "complete"
    /\ stage = "Complete"
    /\ GoalMode # "lean"
    /\ configuredTargets # {}
    /\ pendingUnderModelAssumptions = 0
    /\ \E added \in SUBSET (Targets \ configuredTargets):
        /\ added # {}
        /\ LET newTargets == configuredTargets \cup added
           IN
                /\ configuredTargets' = newTargets
                /\ paperStatus' =
                    [t \in Targets |-> IF t \in added THEN "unknown"
                                       ELSE paperStatus[t]]
                /\ currentCoverage' = CoverageFromClaims(currentTargetClaims, presentNodes, newTargets)
                /\ committedCoverage' = CoverageFromClaims(committedTargetClaims, committedPresentNodes, newTargets)
    /\ phase' = "theorem_stating"
    ...
    /\ cleanupActiveTask' = NoTask
    /\ cleanupAuditBurstCount' = 0
    \* The spec's theorem_stating has no coarse DAG (CoarseAnchorWellFormed
    \* pins a non-empty coarseDagNodes to {proof_formalization, cleanup,
    \* complete}), so the flip clears it. The KERNEL retains the field
    \* across the revival (mirroring import_revision_project) — benign
    \* there: RevisionStating is coarse-dormant and the next AdvancePhase
    \* re-snapshots coarseDagNodes from present nodes. Documented
    \* abstraction slack, caught by AddTargetsProtocolHarness.
    /\ coarseDagNodes' = {}
    /\ UNCHANGED
        <<
            cycle,
            attempt,
            invalidAttempt,
            \* Self-containment: `Next` also assigns retryOutcomeKind'
            \* globally (RetryOutcomeKindNext, whose complete-stage branch
            \* is the identity), but harness specs compose this action into
            \* their own Next — the explicit UNCHANGED keeps the action
            \* fully specified stand-alone.
            retryOutcomeKind,
            gateKind,
            gateFromInvalidAttempt,
            proofEditMode,
...
\* gate, or an in-flight retry. Transitions
...
MaybeIssueProtectedReapproval ==
    ...
    \* In mode-B this existing episode is reused only for one exact
    \* correspondence-passed conditional packet; ordinary protected-target
    \* drift remains absent because the byte-pinned target cannot change.
    /\ (\/ /\ GoalMode # "lean"
              /\ pendingProtectedReapprovalNodes # {}
           \/ /\ GoalMode = "lean"
              /\ pendingProtectedReapprovalNodes = {}
    ...
    /\ (\/ /\ GoalMode # "lean"
              /\ GlobalBlockers = {}
           \/ /\ GoalMode = "lean"
              /\ (GlobalBlockers \ ChallengeBlockersFor(challengeCoverage)) = {})
    /\ ~humanInputOutstanding
    /\ retryOutcomeKind = "none"
    /\ stage' = "HumanGate"
...
\* Automatic ProofFormalization -> Cleanup route. Mirrors kernel
\* `route_after_progress` / `start_cycle` calling
\* `enter_cleanup_phase_after_formalization_complete`: no reviewer or
\* human gate is required once local closure is fresh and all blockers
\* are gone.
ProofFormalizationAutoAdvance ==
    /\ phase = "proof_formalization"
    /\ stage \in {"Start", "Reviewer"}
    /\ inFlightRequest = NoRequest
    /\ response = NoResponse
    /\ pendingProtectedReapprovalNodes = {}
    /\ FormalizationComplete
...
HumanApproveProtectedReapproval ==
    ...
    \* Freeze the approved challenge-coverage snapshot (kernel
    \* `ApprovedTargetSnapshot.challenge_coverage`). The live polarity /
    \* unconditional closure cluster is inert. A mode-B conditional gate
    \* ratifies the unique exact correspondence-passed packet.
    /\ approvedChallengeCoverage' = challengeCoverage
    /\ UNCHANGED <<challengeCoverage, challengePolarity, polarityFlips>>
    /\ IF GoalMode = "lean" THEN
          \E t \in ConditionalRatificationReadyTargets :
              challengeClosedSide' = RatifyChallengeConditional(challengeClosedSide, t)
       ELSE
...
\* After Assumptions Corr Pass, the assumptions lane either records one
\* pending conditional property axiom over the setup Rust-validity hook for
\* human review or rejects the staged blocks.
...
AcceptStuckMathAuditBackToReviewer ==
    ...
    /\ response' = NoResponse
    /\ nativeHistoryKinds' = nativeHistoryKinds \cup {<<inFlightRequest.kind, inFlightRequest.phase>>}
    \* PV mode-B SOLE POLARITY FLIPPER (prove-or-disprove). The audit lane
    \* is the ONLY action that flips a Decide target's live polarity (the
    \* AuditSoleFlipper property machine-checks that no other action mutates
    \* challengePolarity). This plain audit-done arm is the natural flip
    \* site: it is reachable via the ReviewNeedInput* routings AND global-
    \* repair, NOT only via cone-clean — so a target whose default Prove
    \* side is false-and-unclosable can always be reached and flipped to its
    \* true Disprove side (the Decide direction becomes reachable). Flip is:
    \* reversible (toggles), bounded by MaxPolarityFlips (anti-thrash), and
    \* RESETS the new live side's closure record to "open" and clears its
    \* coverage (the prior claim was for the now-dormant side). Mode-B only
    \* and proof_formalization only (the polarity axis does not exist in
    \* mode-A; the direction is discovered during proving). Mirrors
    \* SupervisorCore!AcceptStuckMathAudit.
    /\ \/ \* No polarity change this audit burst.
          /\ UNCHANGED <<challengePolarity, polarityFlips,
                         challengeClosedSide, challengeCoverage>>
       \/ \* Flip one target's live side (reversible, bounded).
          /\ GoalMode = "lean"
          /\ phase = "proof_formalization"
          /\ \E t \in ChallengeTargets :
                 /\ ChallengeResolution[t] = "decide"
                 \* A closed conditional is already a final result and its
                 \* monotone identity cannot be reset by polarity selection.
                 /\ challengeClosedSide[t].conditionalStage # "closed"
                 /\ polarityFlips[t] < MaxPolarityFlips
                 /\ challengePolarity' =
                        [challengePolarity EXCEPT ![t] =
                            IF @ = "prove" THEN "disprove" ELSE "prove"]
                 /\ polarityFlips' = [polarityFlips EXCEPT ![t] = @ + 1]
                 /\ challengeClosedSide' =
                        [challengeClosedSide EXCEPT
                            ![t].positiveSide = "open",
                            ![t].negativeSide = "open"]
                 /\ challengeCoverage' = [challengeCoverage EXCEPT ![t] = {}]
    /\ UNCHANGED requestSeq
    /\ UNCHANGED <<
        phase,
        cycle,
...
RecordAuditPlan ==
    ...
    /\ UNCHANGED <<
        ...
        committedCoverage, approvedCoverage,
        challengeCoverage, approvedChallengeCoverage, challengePolarity,
        polarityFlips, challengeClosedSide, paperStatus,
...
AcceptStuckMathAuditRetry ==
    ...
    /\ stuckMathAuditAssumptionsLaneActive' = stuckMathAuditAssumptionsLaneActive
    /\ stagedUnderModelAssumptionDraft' = stagedUnderModelAssumptionDraft
    /\ pendingUnderModelAssumptions' = pendingUnderModelAssumptions
    /\ inFlightRequest' = NoRequest
    /\ response' = NoResponse
...
\*       `lastReviewerGlobalRepairRequestCycle` (S10) that binds only
\*       while the previous Step A's adjudication is in scope (an
\*       in-flight request or a recent decline), and a no-overlap
\*       check against `liveProtectedStatementNodeSet` (the latter not
\*       modeled at the spec level; deferred to the kernel validator).
\*       A still-pending Step B grant does NOT block Step A: the fresh
\*       accepted Step A supersedes it (the kernel drops the grant and
\*       records it on the new pending request).
\*
...
RequestGlobalRepairAudit ==
    ...
    /\ pendingGlobalRepairRequest = NoGlobalRepairRequest
    \* Cooldown gate (S10): binds only while a recent decline is in
    \* scope (the pendingGlobalRepairRequest = NoGlobalRepairRequest
    \* conjunct above already covers the in-flight-request arm).
    /\ \/ lastReviewerGlobalRepairRequestCycle = NoCycle
       \/ cycle - lastReviewerGlobalRepairRequestCycle >=
          StuckMathAuditDispatchCooldownCycles
       \/ latestGlobalRepairAuditDeclineReason = ""
    \* State: package the reviewer ask into the persisted slot, route
    \* to the audit lane, clear any prior decline reason. A fresh Step A
    \* supersedes any still-pending grant (kernel drops it at the accept
    \* point and surfaces it to the adjudicating audit).
    /\ pendingGlobalRepairRequest' =
...
EnvStageWorkerTargetFalseUnderModel ==
    /\ GoalMode = "lean"
    /\ phase \in {"theorem_stating", "proof_formalization"}
    /\ stage = "Worker"
    /\ inFlightRequest.kind = "worker"
...
AcceptValidWorkerTheorem ==
    ...
    /\ stuckMathAuditAssumptionsLaneActive' = stuckMathAuditAssumptionsLaneActive
    /\ stagedUnderModelAssumptionDraft' =
        IF WorkerStagesUnderModelDraft(presentNodes', corrCurrentFp')
        THEN TRUE
...
AcceptNeedsRestructureWorkerTheorem ==
    ...
    /\ response.status = "ok"
    /\ response.kind = "worker"
    /\ WorkerFinalOutcome \in {"needs_restructure", "target_false_under_model"}
    /\ response.cycle = cycle
    /\ stage' = "Reviewer"
    /\ attempt' = CurrentRetryAttempt(WorkerFinalOutcome)
    /\ invalidAttempt' = FALSE
    /\ retryOutcomeKind' = WorkerFinalOutcome
    /\ gateKind' = "none"
    /\ gateFromInvalidAttempt' = FALSE
    /\ reviewerComments' = reviewerComments
    /\ latestPaperEvidenceLanes' = latestPaperEvidenceLanes
...
AcceptValidWorkerProof ==
    ...
    /\ localClosureUnverified' \in
        LocalClosureUnverifiedNeighbors(presentNodes', openNodes', currentProofNodes')
    /\ currentCoverage' = CoverageFromClaims(currentTargetClaims', presentNodes', configuredTargets)
    \* PV challenge-claim legality (kernel `challenge_claims` /
    \* `worker_normalization::challenge_conformance_errors`): the worker may,
    \* in a proof burst, EITHER leave challenge coverage as-is OR add one new
    \* claim for a currently-uncovered challenge target. Claim-update
    \* legality is the same exclusivity rule the Core spec models: one
    \* claiming node per target and one challenge target per node (the
    \* byte-conformance of the declaration text is observation-layer). A
    \* claim must reference a node the worker actually emitted (present').
    \* PV worker challenge step (prove-or-disprove). One of:
    \*   (a) no challenge delta;
    \*   (b) ADD ONE CLAIM for an uncovered target (exclusive claim);
    \*   (c) CLOSE the live unconditional side of a covered target — gated by the
    \*       abstract AXIOM-FLOOR `CloseChallengeSidePermitted(t, live)`, which
    \*       permits closing only the side that is TRUE under ChallengeTruth.
    \*       Polarity-symmetric: closing a false Prove or a false Disprove is
    \*       impossible (SoundnessFloor). The dormant side is never closed
    \*       (CoverageOnLiveOnly / NotBothSidesClosed). A flip (the Decide
    \*       direction change) is the StuckMathAudit lane's sole job.
    /\ \/ /\ challengeCoverage' = challengeCoverage
          /\ challengeClosedSide' = challengeClosedSide
       \/ \E t \in ChallengeTargets, n \in presentNodes' :
              /\ challengeCoverage[t] = {}
              /\ \A t2 \in ChallengeTargets : n \notin challengeCoverage[t2]
              /\ challengeCoverage' = [challengeCoverage EXCEPT ![t] = {n}]
              /\ challengeClosedSide' = challengeClosedSide
       \/ \E t \in ChallengeTargets :
              /\ challengeCoverage[t] # {}
              /\ challengeClosedSide[t].positiveSide = "open"
              /\ challengeClosedSide[t].negativeSide = "open"
              \* A pending pre-ratification candidate blocks finalization, not
              \* independent Lean closure. It cannot be ratified after this
              \* side closes and must be rejected/withdrawn before display.
              /\ (\/ challengeClosedSide[t].conditionalStage
                        \in {"none", "proposed", "correspondence_pass"}
                   \/ challengeClosedSide[t].conditionalRejected)
              /\ CloseChallengeSidePermitted(t, challengePolarity[t])
              /\ challengeClosedSide' =
                    IF challengePolarity[t] = "prove" THEN
                        [challengeClosedSide EXCEPT ![t].positiveSide = "closed"]
                    ELSE
                        [challengeClosedSide EXCEPT ![t].negativeSide = "closed"]
              /\ challengeCoverage' = challengeCoverage
    /\ corrCurrentFp' = response.corrCurrent
    /\ paperCurrentFp' = response.paperCurrent
    /\ soundCurrentFp' = response.soundCurrent
    ...
    /\ UNCHANGED
        <<
            ...
            committedCoverage,
            approvedCoverage,
            \* challengeCoverage + challengeClosedSide are assigned above
            \* (claim / close paths); the live polarity selector and flip
            \* counter are the StuckMathAudit lane's job and held UNCHANGED.
            approvedChallengeCoverage,
            challengePolarity,
            polarityFlips,
            paperStatus,
...
\* response shape mirrors that bifurcation. The per-node response extends
\* the correspondence-shaped report neighborhood with the prose-only
\* `false_as_stated` outcome.
EnvStagePaperArtifact ==
    ...
    /\ \E laneReports \in CorrLaneReportNeighbors:
        ...
        IN
        /\ \E nodeLaneMaps \in SubstantivenessLaneMapNeighbors(inFlightRequest.substantivenessVerifyNodes):
            response' =
                [
                    NoResponse EXCEPT
                        !.status = "ok",
    ...
    /\ LET newPaperStatus ==
       ...
       IN
            ...
            /\ stage' =
                ...
                ELSE IF CorrVerifyNodes # {} THEN
                    "VerifyCorr"
                \* C4': the sound arm is the post-accept eligibility scan
                \* (this action leaves corr/substantiveness/sound maps
                \* unchanged, so PostSoundVerifyNodes' unprimed
                \* eligibility reads are the post-state reads).
                ELSE IF PostSoundVerifyNodes(newHeldTarget) # {} THEN
                    "VerifySound"
                \* C1 service point (kernel issue_sound_backlog_or_review
                \* at the apply_theorem_paper_accept step-5 Reviewer
                \* handoff): a stability-backlog Sound panel may replace
                \* the Review. The kernel's once-per-cycle guard
                \* (last_sound_response_cycle) is vacuously open here — a
                \* Sound response never routes to VerifyPaper within a
                \* cycle, so no Sound response can precede this accept.
                ELSE IF SoundBacklogNodes # {} THEN
                    "VerifySound"
                ELSE
                    "Reviewer"
            /\ pendingTask' = NoPendingTask
...
AcceptSubstantivenessArtifactTheorem ==
    ...
    /\ LET newSubstantivenessStatus ==
           ...
           newSubstantivenessApproved ==
            [n \in Nodes |->
                IF response.substantivenessMap[n] \in {"pass", "false_as_stated", "fail"} THEN
           ...
           substantivenessFail ==
            \E n \in presentNodes :
                /\ newSubstantivenessStatus[n] \in {"false_as_stated", "fail"}
       ...
       IN
            ...
            /\ stage' =
                ...
                ELSE IF CorrVerifyNodes # {} THEN
                    "VerifyCorr"
                \* C4': post-accept eligibility scan against the
                \* substantiveness maps this action just computed
                \* (newHeldTarget is the gate proxy — NoNode when the
                \* post-response blocker set has corr/paper/
                \* substantiveness members).
                ...
                THEN
                    "VerifySound"
                \* C1 service point (kernel issue_sound_backlog_or_review;
                \* the substantiveness accept routes through
                \* apply_theorem_paper_accept step 5). Once-per-cycle
                \* guard vacuously open — see AcceptPaperArtifactTheorem.
                ELSE IF SoundBacklogNodesFor(corrStatus, corrApprovedFp,
                                             newSubstantivenessStatus,
                                             newSubstantivenessApproved) # {}
                THEN
                    "VerifySound"
...
AcceptCorrArtifactTheorem ==
    ...
    /\ response.cycle = cycle
    /\ ~(GoalMode = "lean"
         /\ AssumptionsNode \in inFlightRequest.corrVerifyNodes
         /\ response.corrMap[AssumptionsNode] = "pass")
    /\ LET newCorrStatus ==
            [n \in Nodes |->
       ...
       IN
            ...
            /\ stage' =
                ...
                ELSE IF nonAdjudicableCorrFrontier THEN
                    "VerifyCorr"
                \* C4': post-accept eligibility scan against the corr maps
                \* this action just computed (newHeldTarget is the gate
                \* proxy — NoNode when the post-response blocker set has
                \* corr/paper/substantiveness members).
                ELSE IF /\ newHeldTarget # NoNode
                        /\ \E n \in presentNodes :
                            SoundScanEligibleFor(n, currentProofNodes, openNodes, currentDeps,
                                                 newCorrStatus, corrCurrentFp, newCorrApproved,
                                                 substantivenessStatus,
                                                 substantivenessCurrentFp,
                                                 substantivenessApprovedFp,
                                                 soundAssessmentStatus, soundStatus,
                ...
                THEN
                    "VerifySound"
                \* C1 service point (kernel issue_sound_backlog_or_review
                \* at the apply_theorem_corr_accept tail). Once-per-cycle
                \* guard vacuously open — see AcceptPaperArtifactTheorem.
                ELSE IF SoundBacklogNodesFor(newCorrStatus, newCorrApproved,
                                             substantivenessStatus,
                                             substantivenessApprovedFp) # {}
                THEN
                    "VerifySound"
                    ...
                    CHOOSE n \in eligibleHeldTargets:
                        \A m \in eligibleHeldTargets:
                            ...
                            \/ /\ NodeRank[n] = NodeRank[m]
                               /\ NodeOrder[n] >= NodeOrder[m]
            /\ stage' =
                \* Intra-cycle sound cap (2026-07): the pre-cap Pass
                \* drain (self-loop VerifySound over every eligible
                \* Unknown, the C4' post-verdict scan) is retired — the
                \* kernel's `last_sound_response_cycle` guard, stamped
                \* by THIS response, caps the cycle-start / accept-drain
                \* chain at one frontier Sound per cycle, so a theorem
                \* Sound accept always hands off to the Reviewer.
                \* Remaining Unknowns are served one per cycle by
                \* StartCycle / the pre-response accept drains. (The
                \* kernel's non-adjudicable-Unknown routing sites sit
                \* outside the cap and can add a Sound on rare
                \* Fail-escalation traces; those sites are modeled
                \* separately by the Post* frontier operators.)
                \* C1: the Reviewer handoff here is a kernel service
                \* point (issue_sound_backlog_or_review), but the same
                \* just-stamped guard means the backlog never fires
                \* after a same-cycle sound response.
                "Reviewer"
    /\ pendingTask' = NoPendingTask
    /\ humanInputOutstanding' = humanInputOutstanding
    /\ nodeDifficulty' = nodeDifficulty
...
AcceptAssumptionsCorrPass ==
    ...
    /\ LET newCorrStatus ==
            [n \in Nodes |->
                IF response.corrMap[n] = "same" THEN corrStatus[n]
                ELSE response.corrMap[n]]
           newCorrApproved ==
            [n \in Nodes |->
                IF response.corrMap[n] \in {"pass", "fail"} THEN corrCurrentFp[n]
                ELSE corrApprovedFp[n]]
           newPaperStatus ==
            [t \in Targets |->
                IF response.paperMap[t] = "same" THEN paperStatus[t]
                ELSE response.paperMap[t]]
           newPaperApproved ==
            [t \in Targets |->
                IF response.paperMap[t] \in {"pass", "fail"} THEN paperCurrentFp[t]
                ELSE paperApprovedFp[t]]
       IN
            /\ corrStatus' = newCorrStatus
            /\ corrApprovedFp' = newCorrApproved
            /\ paperStatus' = newPaperStatus
            ...
            /\ pendingTask' = NoPendingTask
            /\ stuckMathAuditActive' = TRUE
            /\ stuckMathAuditAssumptionsLaneActive' = TRUE
            /\ stuckMathAuditNeedInputAudit' = NoNeedInputAuditContext
            /\ stuckMathAuditBurstRetryCount' = 0
            /\ lastStuckMathAuditDispatchedCycle' = cycle
            /\ stagedUnderModelAssumptionDraft' = stagedUnderModelAssumptionDraft
...
EnvStageReviewArtifact ==
    ...
    /\ \E decision \in ReviewDecisionChoices(phase, GlobalBlockers, retryOutcomeKind),
          ...
          \* Reset-blocker choices track the offer set, so the
          \* proof-formalization substantiveness reset is reachable here.
          rb \in IF phase \in {"theorem_stating", "proof_formalization"}
                 THEN TaskBlockerChoices(RequestAllowedResetBlockers("review"))
                 ELSE {{}},
          clearHI \in BOOLEAN,
          ...
          \* Quantified as a node set rather than over
          \* `{NoGlobalRepairRequest} \cup {records}`: TLC cannot enumerate a
          \* set holding both the string sentinel and a record, so the union
          \* form made the whole action unevaluable (every attempt to stage a
          \* reviewer artifact raised "equality of string with non-string").
          \* The successor set is unchanged — the empty set carries the
          \* no-request case, exactly the element the old union excluded.
          grNodes \in SUBSET presentNodes,
        ...
        /\ response' =
            [
                NoResponse EXCEPT
                    ...
                    !.globalRepairRequest =
                        IF grNodes = {}
                        THEN NoGlobalRepairRequest
                        ELSE [proposedExtensionNodes |-> grNodes,
                              dispatchedAtCycle |-> cycle],
...
ReviewContinueAfterInvalid ==
    ...
    /\ LET
            ...
            nextStage ==
                ...
                THEN
                    "VerifyCorr"
                \* Sound-frontier yield (2026-07): this Continue arm
                \* always installs a pending worker task (the kernel
                \* sets `pending_task` unconditionally before re-running
                \* `theorem_start_request_kind` at this site), so the
                \* auto sound frontier — the retired C4' post-review
                \* scan that stood here — can no longer fire: the
                \* frontier arm yields whenever a pending task exists.
                \* Reviewer-requested Sound dispatch at this site
                \* remains an under-approximation (as before; the scan
                \* never modeled it either).
                ELSE
                    "Worker"
       IN
...
ReviewNeedInputAfterInvalid ==
    ...
    \* Mode-B (PV): a disproof never routes to a human; the NeedInput
    \* human-stop lane is suppressed (NoHumanInModeB). A genuine stuck
    \* state in mode-B is handled by the StuckMathAudit polarity-flip lane.
    /\ GoalMode # "lean"
    /\ LET
...
ReviewNeedInputAfterValid ==
    ...
    /\ ReviewDecisionLegal
    /\ response.decision = "NEED_INPUT"
    \* Mode-B (PV): NeedInput human-stop lane suppressed (NoHumanInModeB).
    /\ GoalMode # "lean"
    /\ LET
            nextCorrStatus == ApplyReviewCorrStatusResets(corrStatus, response.resetBlockers)
            nextTargetCorrStatus == ApplyReviewPaperStatusResets(paperStatus, response.resetBlockers)
...
ReviewAdvancePhase ==
    ...
    \* Mode-A (or any non-vacuous gate): the advance gate carries real
    \* meaning-review and routes to the human. Mode-B's vacuous gate auto-
    \* advances with no human turn (ReviewAdvancePhaseVacuousModeB below).
    /\ (\/ GoalMode # "lean"
       \/ /\ GoalMode = "lean"
          /\ Cardinality(ConditionalRatificationReadyTargets) = 1)
    /\ stage' = "HumanGate"
...
ReviewContinueProof ==
    ...
    /\ cyclesInCoarseRepairMode' =
           ...
           ELSE
               0
    \* The nine the action used to leave unassigned, resolved against the
    \* kernel and matching what every sibling review action already holds
    \* (`ReviewContinueAfterValid`, `ReviewAdvancePhase`):
    \*
    \*   coarseDagNodes — one production writer, the theorem-stating ->
    \*     proof-formalization advance (engine.rs, `state.coarse_dag_nodes =
    \*     state.live.present_nodes.clone()`). It is not committed-tier, so
    \*     even a Continue carrying `lastCommit` leaves it alone.
    \*   forceReviewAfterConeClean — set at the cone-clean accept and
    \*     CONSUMED at issue time: `start_cycle` clears it in the same step
    \*     that issues the forced Review, one step before the response
    \*     arrives. `StartCycle` above mirrors that, so holding it here is
    \*     the faithful reading rather than a sticky latch, and
    \*     `apply_proof_review_response` never mentions it.
    \*   StuckMathAuditVars — an ordinary proof Continue writes only
    \*     `stuck_math_audit.last_reviewer_lean_product`
    \*     (`record_stuck_math_audit_review`) and clears
    \*     `pending_worker_audit_request`; neither is a variable of this
    \*     spec. The shapes that DO move the audit lane — NeedInput, a
    \*     Continue carrying `global_repair_request`, a Continue carrying
    \*     `audit_request` — are separate kernel paths with their own
    \*     actions here.
    /\ UNCHANGED
        <<
            coarseDagNodes,
...
ReviewNeedInputProof ==
    ...
    /\ response.decision = "NEED_INPUT"
    \* Mode-B (PV): NeedInput human-stop lane suppressed (NoHumanInModeB);
    \* the StuckMathAudit polarity-flip lane handles a stuck Decide target.
    /\ GoalMode # "lean"
    /\ LET retryReview == (retryOutcomeKind # "none")
           resetRequested == (response.reset \in {"lastCommit", "lastClean"})
       IN
...
HumanApproveAdvance ==
    ...
    /\ approvedCoverage' = currentCoverage
    \* Freeze the approved challenge-coverage snapshot (kernel
    \* `ApprovedTargetSnapshot.challenge_coverage`) at the advance gate. The
    \* live polarity / unconditional closure cluster is inert. A non-vacuous
    \* mode-B advance ratifies the unique exact conditional packet.
    /\ approvedChallengeCoverage' = challengeCoverage
    /\ UNCHANGED <<challengeCoverage, challengePolarity, polarityFlips>>
    /\ IF GoalMode = "lean" THEN
          \E t \in ConditionalRatificationReadyTargets :
...
\* AdvanceGateVacuous (mode-B). The theorem_stating advance HumanGate
\* certifies the informal->formal faithfulness of worker-authored target
\* MEANING. In mode-B (GoalMode = "lean") the targets are author-given +
\* byte-pinned, so there is no faithfulness lane and the gate's meaning-
\* review set is EMPTY — the gate is vacuous. Mirrors
\* SupervisorCore!AdvanceGateVacuous.
AdvanceGateVacuous ==
    /\ GoalMode = "lean"
    /\ configuredTargets = {}

\* ReviewAdvancePhaseVacuousModeB. In mode-B the vacuous advance gate auto-
\* advances theorem_stating -> proof_formalization with NO human turn
\* (NoHumanInModeB), folding the reviewer ADVANCE_PHASE decision and the
\* HumanApproveAdvance phase-advance effect into a single step. Mirrors the
\* AdvanceGateVacuous branch of SupervisorCore!ReviewAdvancePhase. Legality
\* still requires GlobalBlockers = {} (via ReviewDecisionLegal). The
\* approved-target snapshot freeze and post-advance routing latch match
\* HumanApproveAdvance exactly.
ReviewAdvancePhaseVacuousModeB ==
    /\ phase = "theorem_stating"
    /\ stage = "Reviewer"
    /\ ~invalidAttempt
    /\ ReviewDecisionLegal
...
RetryOutcomeKindNext ==
    ...
    ELSE IF /\ stage = "Worker"
               ...
               /\ response.status = "ok"
               /\ response.cycle = cycle
               /\ WorkerFinalOutcome = "target_false_under_model"
    THEN
        "target_false_under_model"
    ...
    ELSE IF /\ stage = "HumanGate"
               /\ gateKind \in {"advance", "assumption_review"}
               /\ response.kind = "human_gate"
               /\ response.status = "ok"
               /\ response.humanChoice \in {"approve", "feedback"}
    THEN
...
\* ===================================================================== *
\* Step-4 abstract conditional and optional-artifact outcome transitions. *
\* ===================================================================== *

ChallengeOutcomeMutationReady ==
    /\ GoalMode = "lean"
    /\ phase \in {"theorem_stating", "proof_formalization"}
    /\ stage \in {"Start", "Reviewer"}
    /\ inFlightRequest = NoRequest
...
ProposeChallengeConditional ==
    /\ ChallengeOutcomeMutationReady
    /\ \E t \in ChallengeTargets,
          condition \in (Fingerprints \ {NoFingerprint}) :
          /\ ChallengeResolution[t] = "decide"
          /\ challengeClosedSide[t].positiveSide = "open"
          /\ challengeClosedSide[t].negativeSide = "open"
          /\ challengeClosedSide[t].conditionalStage = "none"
          /\ challengeClosedSide' =
                [challengeClosedSide EXCEPT
                    ![t].conditionalStage = "proposed",
                    ![t].conditionIdentity = condition]
    /\ UNCHANGED VarsExceptChallengeOutcome

PassChallengeConditionalCorrespondence ==
    /\ ChallengeOutcomeMutationReady
    /\ \E t \in ChallengeTargets,
          seal \in (Fingerprints \ {NoFingerprint}) :
          /\ ChallengeResolution[t] = "decide"
          /\ challengeClosedSide[t].conditionalStage = "proposed"
          /\ ~challengeClosedSide[t].conditionalRejected
          /\ challengeClosedSide' =
                [challengeClosedSide EXCEPT
                    ![t].conditionalStage = "correspondence_pass",
                    ![t].sealIdentity = seal]
    /\ UNCHANGED VarsExceptChallengeOutcome

RejectChallengeConditional ==
    /\ ChallengeOutcomeMutationReady
    /\ \E t \in ChallengeTargets :
          /\ challengeClosedSide[t].conditionalStage
                \in {"proposed", "correspondence_pass"}
          /\ ~challengeClosedSide[t].conditionalRejected
          /\ challengeClosedSide' =
                [challengeClosedSide EXCEPT ![t].conditionalRejected = TRUE]
    /\ UNCHANGED VarsExceptChallengeOutcome

OpenChallengeConditional ==
    /\ ChallengeOutcomeMutationReady
    /\ \E t \in ChallengeTargets :
          /\ challengeClosedSide[t].conditionalStage = "human_approved"
          /\ ConditionalRatificationExact(challengeClosedSide[t])
          /\ challengeClosedSide' =
                [challengeClosedSide EXCEPT ![t].conditionalStage = "open"]
    /\ UNCHANGED VarsExceptChallengeOutcome

CloseChallengeConditional ==
    /\ ChallengeOutcomeMutationReady
    /\ \E t \in ChallengeTargets :
          /\ challengeCoverage[t] # {}
          /\ challengeClosedSide[t].positiveSide = "open"
          /\ challengeClosedSide[t].negativeSide = "open"
          /\ challengeClosedSide[t].conditionalStage = "open"
          /\ ConditionalRatificationExact(challengeClosedSide[t])
          /\ challengeClosedSide' =
                [challengeClosedSide EXCEPT
                    ![t].conditionalStage = "closed",
                    ![t].closedConditionIdentity = challengeClosedSide[t].conditionIdentity,
                    ![t].closedSealIdentity = challengeClosedSide[t].sealIdentity]
    /\ UNCHANGED VarsExceptChallengeOutcome

\* Artifact disclosure may be absent or carry any frozen fingerprint. It can
\* be added, removed, or replaced before finalization, but is never a guard or
\* effect of proof/disproof/conditional closure.
ChangeChallengeArtifactDisclosure ==
    /\ ChallengeOutcomeMutationReady
    /\ \E t \in ChallengeTargets, artifact \in Fingerprints :
          /\ artifact # challengeClosedSide[t].artifactIdentity
          /\ challengeClosedSide' =
                [challengeClosedSide EXCEPT ![t].artifactIdentity = artifact]
    /\ UNCHANGED VarsExceptChallengeOutcome

\* ----------------------------------------------------------------------
\* Pre-bootstrap source adaptation.  Candidate proposals may contain bad
\* roots or missing facts; only Phase0CheckSuccess has authority to admit a
\* generation to the audit stages.  Transport/malformed retries are outside
\* this semantic machine and therefore never increment handbacks.
\* ----------------------------------------------------------------------

Phase0InitialCheckerFailure ==
    /\ phase = "source_adaptation"
    /\ phase0State.stage = "Check"
    /\ phase0State.generation = 0
    /\ phase0State.diagnosticRoot = NoFingerprint
    /\ phase0State' = [phase0State EXCEPT
        !.stage = "Worker",
        !.diagnosticRoot = phase0State.adaptedRoot,
        !.unresolved = TRUE]
    /\ UNCHANGED CampaignVars

Phase0WorkerRevision ==
    /\ phase = "source_adaptation"
    /\ phase0State.stage = "Worker"
    /\ phase0State.handbacks < MaxPhase0Handbacks
    /\ \E adapted, replayed, ledger \in Fingerprints,
          logged, discovered \in BOOLEAN:
        /\ adapted # NoFingerprint
        /\ ledger # NoFingerprint
        /\ phase0State' = [phase0State EXCEPT
            !.stage = "Check",
            !.generation = @ + 1,
            !.adaptedRoot = adapted,
            ...
            !.semanticBundleRoot = NoFingerprint,
            !.sealedRoot = NoFingerprint,
            !.deltaComplete = logged,
            !.discoveryComplete = discovered,
            !.ablationComplete = FALSE,
            !.bindingComplete = FALSE,
            !.seamClean = FALSE,
            !.correspondenceClean = FALSE,
            !.unresolved = FALSE]
...
Phase0CheckerFailure ==
    /\ phase = "source_adaptation"
    /\ phase0State.stage = "Check"
    /\ ~(phase0State.generation = 0 /\ phase0State.diagnosticRoot = NoFingerprint)
    /\ phase0State' = [phase0State EXCEPT
        !.stage = "Worker",
        !.diagnosticRoot = phase0State.adaptedRoot,
        !.unresolved = TRUE]
...
Phase0CheckSuccess ==
    ...
    /\ phase0State.stage = "Check"
    /\ phase0State.diagnosticRoot # NoFingerprint
    /\ phase0State.deltaComplete
    /\ phase0State.discoveryComplete
    /\ phase0State.adaptedRoot = phase0State.replayedRoot
    /\ phase0State.adaptedRoot # NoFingerprint
    /\ phase0State.ledgerRoot # NoFingerprint
    /\ \E bundle \in Fingerprints \ {NoFingerprint}:
        phase0State' = [phase0State EXCEPT
            !.stage = "SeamAudit",
            !.semanticBundleRoot = bundle,
            !.ablationComplete = TRUE,
...
Phase0SeamAuditPass ==
    /\ phase = "source_adaptation"
    /\ phase0State.stage = "SeamAudit"
    /\ phase0State' = [phase0State EXCEPT
        !.stage = "CorrespondenceAudit",
        !.seamClean = TRUE,
        !.seamGeneration = phase0State.generation]
    /\ UNCHANGED CampaignVars

Phase0AuditFindings ==
    /\ phase = "source_adaptation"
    /\ phase0State.stage \in {"SeamAudit", "CorrespondenceAudit"}
    /\ phase0State.handbacks < MaxPhase0Handbacks
    /\ phase0State' = [phase0State EXCEPT
        !.handbacks = @ + 1,
        !.stage = IF phase0State.handbacks + 1 = MaxPhase0Handbacks THEN "Incomplete" ELSE "Worker",
        !.seamClean = FALSE,
        !.correspondenceClean = FALSE,
        !.semanticBundleRoot = NoFingerprint,
        !.bindingComplete = FALSE,
...
Phase0CorrespondenceAuditPassAndSeal ==
    /\ phase = "source_adaptation"
    /\ phase0State.stage = "CorrespondenceAudit"
    /\ phase0State.seamClean
    /\ phase0State.seamGeneration = phase0State.generation
    /\ phase0State' = [phase0State EXCEPT
        !.stage = "Sealed",
        !.correspondenceClean = TRUE,
        !.correspondenceGeneration = phase0State.generation,
        !.sealedRoot = phase0State.adaptedRoot,
        !.unresolved = FALSE]
    /\ UNCHANGED CampaignVars

Phase0ProductionExtractionAndBootstrap ==
    /\ phase = "source_adaptation"
    /\ phase0State.stage = "Sealed"
    /\ phase0State.sealedRoot # NoFingerprint
    /\ phase0State' = [phase0State EXCEPT
        !.productionRoot = phase0State.sealedRoot,
        !.bootstrapRoot = phase0State.sealedRoot,
        !.advanceGateRoot = phase0State.sealedRoot]
    /\ phase' = "theorem_stating"
    /\ stage' = "Start"
    /\ UNCHANGED CampaignDataVars

Phase0IncompleteStop ==
    /\ phase = "source_adaptation"
    /\ phase0State.stage = "Incomplete"
    /\ UNCHANGED <<CampaignVars, phase0State>>
...
Phase0GoalFrozen ==
    /\ phase0State.goalRoot = phase0State.genesisGoalRoot
    /\ phase0State.goalRoot # NoFingerprint

AdaptedTreeEqualsLedgerReplay ==
    phase0State.stage \in {"SeamAudit", "CorrespondenceAudit", "Sealed"}
        => phase0State.adaptedRoot = phase0State.replayedRoot

NoUnloggedAdaptationBytes ==
    phase0State.stage \in {"SeamAudit", "CorrespondenceAudit", "Sealed"}
        => phase0State.deltaComplete

EveryEntryHasFailureAndAblation ==
    phase0State.stage \in {"SeamAudit", "CorrespondenceAudit", "Sealed"}
        => /\ phase0State.discoveryComplete
           /\ phase0State.ablationComplete

AuditPassBindsCurrentGeneration ==
    /\ phase0State.seamClean => phase0State.seamGeneration = phase0State.generation
    /\ phase0State.correspondenceClean
        => /\ phase0State.correspondenceGeneration = phase0State.generation
           /\ phase0State.seamGeneration = phase0State.generation

DiagnosticExtractionCannotSeedCampaign ==
    phase0State.productionRoot # NoFingerprint
        => /\ phase0State.sealedRoot # NoFingerprint
           /\ phase0State.productionRoot = phase0State.sealedRoot

NoProductionExtractionFromUnsealedTree ==
    /\ phase = "source_adaptation" => phase0State.productionRoot = NoFingerprint
    /\ phase0State.productionRoot # NoFingerprint
        => phase0State.productionRoot = phase0State.sealedRoot

BootstrapConsumesSealedBundle ==
    phase # "source_adaptation"
        => /\ phase0State.stage = "Sealed"
           /\ phase0State.productionRoot = phase0State.sealedRoot
           /\ phase0State.bootstrapRoot = phase0State.sealedRoot
           /\ phase0State.advanceGateRoot = phase0State.sealedRoot

SealedGenerationIsCleanAndComplete ==
    phase0State.stage = "Sealed"
        => /\ phase0State.sealedRoot = phase0State.adaptedRoot
           /\ phase0State.semanticBundleRoot # NoFingerprint
           /\ phase0State.bindingComplete
           /\ phase0State.discoveryComplete
           /\ phase0State.ablationComplete
           /\ phase0State.seamClean
           /\ phase0State.correspondenceClean
...
AdvanceGateBindsSealedGeneration ==
    phase0State.advanceGateRoot # NoFingerprint
        => phase0State.advanceGateRoot = phase0State.sealedRoot

Phase0BoundOnlyStops ==
    /\ phase0State.handbacks <= MaxPhase0Handbacks
    /\ phase0State.handbacks = MaxPhase0Handbacks
        => /\ phase0State.stage = "Incomplete"
           /\ phase = "source_adaptation"
