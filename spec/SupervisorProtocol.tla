...
CONSTANTS
    ...
    \* (kernel `ChallengeTargetSpec`; the per-target `kind` Prove/Decide and
    \* the pinned declaration text are observation-layer and not modeled —
    \* only claim-update legality and coverage gating are). Disjoint from
...
VARIABLES
    ...
    \*   "prove"; only the StuckMathAudit accept flips it).
    \* `polarityFlips[t]` : per-target flip counter (anti-thrash bound).
    \* `challengeClosedSide[t]` : closure record of the live side ("open" /
    \*   "closed"); closing is gated on the abstract truth model so a false
    \*   statement can never record "closed".
    challengeCoverage,
...
WorkerAcceptedResponsesSatisfyContract ==
    ...
    \/ /\ WorkerStructuralMapsMatchObserved
       ...
       \* window-independently, mirroring the kernel; the cleanup-phase
       \* absolute gate (`AcceptValidWorkerCleanup`) is likewise not
       \* window-gated.
       /\ (\/ WorkerCleanupValidation
...
ReviewResetChoices(currentPhase, retryKind) ==
    ...
    \* Bug 2 (designs incident 2026-06-26): this reviewer-driven menu is the
...
\* ChallengeTruth[t]. Polarity-symmetric — identical for "prove" and
\* "disprove" — which is what makes the disprove side as sound as the prove
\* side. Mirrors SupervisorCore!CloseSidePermitted. (Step 2 reads this in
\* the worker challenge-close path; in Step 1 it is unreferenced.)
...
\* Mirrors SupervisorCore!ChallengeTargetDecided.
\*
\* The live side has a covering node AND that covering proof has CLOSED
\* under the axiom-floor (`CloseChallengeSidePermitted`). A target whose
\* live side is false-and-thus-unclosable stays undecided until the
\* StuckMathAudit lane flips its polarity to the true side.
...
IsDerivedSubstantivenessFail(b) ==
    /\ b.kind = "substantiveness"
    /\ ~ /\ substantivenessStatus[b.object.node] = "fail"
...
RequestRecord(reqId, kind) ==
    [
        ...
        projectInvariants |-> ProjectInvariants,
        freshContext |-> RequestFreshContext(kind),
                    promptContractVersion |-> IF kind = "none" THEN 0 ELSE 41,
...
TypeOK ==
    ...
    /\ challengeClosedSide \in [ChallengeTargets -> {"open", "closed"}]
    ...
    \* guard, reference papers, etc.); current value 41.
    /\ inFlightRequest.promptContractVersion \in 0..41
    /\ inFlightRequest.promptContractVersion =
        IF inFlightRequest.kind = "none" THEN 0 ELSE 41
...
CoverageOnLiveOnly ==
    \A t \in ChallengeTargets :
        ...
        /\ challengeClosedSide[t] \in {"open", "closed"}
        /\ (challengeClosedSide[t] = "closed") => (challengeCoverage[t] # {})

\* SoundnessFloor / SymmetricAxioms. No reachable state has a Decide target
\* whose LIVE side recorded "closed" while that side is FALSE under the
\* abstract truth model. The closure gate (CloseChallengeSidePermitted) is
\* polarity-symmetric, so this holds identically for "prove" and "disprove":
\* a worker can never resolve a target by closing a false T or a false ~T.
\* This is the axiom-floor soundness wall. Mirrors SupervisorCore.
SoundnessFloor ==
    \A t \in ChallengeTargets :
        (challengeClosedSide[t] = "closed")
            => (ChallengeTruth[t] = challengePolarity[t])

\* NotBothSidesClosed. For a Decide pair {T, ~T}, the two sides are never
\* both closed. Structurally guaranteed: only the LIVE side has a closure
\* record, a polarity flip RESETS the record to "open", and SoundnessFloor
\* forbids the closed side being false. Since ChallengeTruth[t] is single-
\* valued, at most one side is ever closable. Mirrors SupervisorCore.
NotBothSidesClosed ==
    \A t \in ChallengeTargets :
        (challengeClosedSide[t] = "closed")
            => (challengePolarity[t] = ChallengeTruth[t])

\* NoHumanInModeB. In a mode-B run the ordinary advance /
\* protected-reapproval / need-input HumanGates remain unreachable: the
\* advance gate is vacuous and auto-advances when no under-model assumptions
\* are pending, and the ProtectedReapproval / NeedInput human-stop lanes are
\* guarded off. The only HumanGate exception is `assumption_review`, which
\* ratifies or rejects assumptions that already passed Assumptions Corr and
\* the assumptions lane. Mirrors SupervisorCore.
NoHumanInModeB ==
    GoalMode = "lean" =>
        /\ ((stage = "HumanGate") => gateKind \in {"none", "assumption_review"})
...
\* DecidedResolves. A disproved target (Disprove-side closed) counts as a
\* resolved / decided target for completion exactly as a proved one does:
\* ChallengeTargetDecided is polarity-agnostic, so a completed run may have
\* reached Done via the Disprove side. Mirrors SupervisorCore.
DecidedResolves ==
    phase = "complete" =>
        \A t \in ChallengeTargets :
            /\ ChallengeTargetDecided(t)
...
MaybeIssueProtectedReapproval ==
    ...
    \* monotonicity / protected-reapproval human gate never fires
    \* (NoHumanInModeB). Mirrors SupervisorCore!MaybeIssueProtectedReapprovalGate.
    /\ GoalMode # "lean"
    /\ inFlightRequest = NoRequest
    /\ response = NoResponse
    /\ pendingProtectedReapprovalNodes # {}
...
HumanApproveProtectedReapproval ==
    ...
    \* closure cluster is inert at a protected-reapproval gate.
...
AcceptValidWorkerProof ==
    ...
    \*   (c) CLOSE the live side of a covered-but-open target — gated by the
    ...
    /\ \/ /\ challengeCoverage' = challengeCoverage
       ...
       \/ \E t \in ChallengeTargets :
              ...
              /\ challengeClosedSide' = [challengeClosedSide EXCEPT ![t] = "closed"]
              /\ challengeCoverage' = challengeCoverage
    /\ corrCurrentFp' = response.corrCurrent
    /\ paperCurrentFp' = response.paperCurrent
    /\ soundCurrentFp' = response.soundCurrent
...
HumanApproveAdvance ==
    ...
    \* live polarity / closure cluster is inert at the advance gate.
    /\ approvedChallengeCoverage' = challengeCoverage
    /\ UNCHANGED <<challengeCoverage, challengePolarity, polarityFlips,
                   challengeClosedSide>>
    /\ paperApprovedFp' = paperCurrentFp
