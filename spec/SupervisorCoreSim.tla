...
SimFormalImports == {}
\* Prove-or-disprove axis. This DEFAULT harness runs mode-A (GoalMode =
\* "prose") so it keeps the existing paper-faithfulness behavior (non-empty
\* SimInitialConfiguredTargets) and existing invariant coverage unchanged.
\* The polarity vars are still
...
CleanupBlockersAreAdmissible == Core!CleanupBlockersAreAdmissible
