...
\* Placeholder-`True` definition nodes (correspondence auto-fail). Empty by
\* default: the mechanism is a conservative observation-layer forced-Fail,
\* exercised by the kernel's Rust unit tests; an empty set keeps the bounded
\* state space identical to the pre-placeholder model while still type-
\* checking the new constant. Set e.g. {"n5"} to exercise the auto-fail.
SimPlaceholderDefinitionNodes == {}
...
\* ---- PV challenge-coverage + prove-or-disprove sim instance -----------
\* One challenge target, disjoint from SimTargets. Its abstract truth is
\* "disprove" — so the DEFAULT Prove polarity is FALSE and the live side
\* cannot close until the StuckMathAudit lane flips it to Disprove. This is
\* the load-bearing direction-reachability / G1-G2 scenario: a covered-but-
\* false Prove side that completion can only resolve via a polarity flip.
\* (A single target keeps the bounded state space small; a second target
\* with truth "prove" to exercise both polarities in one run is a noted
\* follow-up.) NOTE: this is a mode-A (GoalMode "prose") base instance so
\* the existing mode-A regression invariants stay exercised; a mode-B
\* (GoalMode "lean", configuredTargets empty) variant lives in the
\* dedicated mode-B harness/cfg (follow-up).
SimChallengeTargets == {"c1"}
SimChallengeTruth == [t \in SimChallengeTargets |-> "disprove"]
SimChallengeResolution == [t \in SimChallengeTargets |-> "decide"]
SimGoalMode == "prose"
...
SP == INSTANCE SupervisorProtocol
    WITH
        ...
        PlaceholderDefinitionNodes <- SimPlaceholderDefinitionNodes,
        AllowReviewerPassOverride <- SimAllowReviewerPassOverride,
        ChallengeTargets <- SimChallengeTargets,
        ChallengeResolution <- SimChallengeResolution,
        ...
        approvedChallengeCoverage <- approvedChallengeCoverage,
...
CleanupBlockersAreAdmissible == SP!CleanupBlockersAreAdmissible
...
GlobalRepairRequestGrantMutex == SP!GlobalRepairRequestGrantMutex
...
ModeBConfiguredTargetsEmpty == SP!ModeBConfiguredTargetsEmpty
CompleteRequiresChallengeCoverage == SP!CompleteRequiresChallengeCoverage
CoverageOnLiveOnly == SP!CoverageOnLiveOnly
SoundnessFloor == SP!SoundnessFloor
NotBothSidesClosed == SP!NotBothSidesClosed
NoHumanInModeB == SP!NoHumanInModeB
UnderModelAssumptionLifecycleContract == SP!UnderModelAssumptionLifecycleContract
DecidedResolves == SP!DecidedResolves
ResolutionPolicy == SP!ResolutionPolicy
ConditionalLifecycleWellFormed == SP!ConditionalLifecycleWellFormed
ArtifactIrrelevance == SP!ArtifactIrrelevance
\* Pre-bootstrap source-adaptation invariants (operator single-gate model).
Phase0GoalFrozen == SP!Phase0GoalFrozen
AdaptedTreeEqualsLedgerReplay == SP!AdaptedTreeEqualsLedgerReplay
NoUnloggedAdaptationBytes == SP!NoUnloggedAdaptationBytes
EveryEntryHasFailureAndAblation == SP!EveryEntryHasFailureAndAblation
AuditPassBindsCurrentGeneration == SP!AuditPassBindsCurrentGeneration
DiagnosticExtractionCannotSeedCampaign == SP!DiagnosticExtractionCannotSeedCampaign
NoProductionExtractionFromUnsealedTree == SP!NoProductionExtractionFromUnsealedTree
BootstrapConsumesSealedBundle == SP!BootstrapConsumesSealedBundle
SealedGenerationIsCleanAndComplete == SP!SealedGenerationIsCleanAndComplete
AdvanceGateBindsSealedGeneration == SP!AdvanceGateBindsSealedGeneration
Phase0BoundOnlyStops == SP!Phase0BoundOnlyStops
\* AuditSoleFlipper is a TWO-STATE action property — checked under
\* PROPERTIES as a boxed action `[][A]_Vars`, not as an INVARIANT.
AuditSoleFlipperProp == [][SP!AuditSoleFlipper]_(SP!Vars)
ConditionalProgressMonotoneProp == [][SP!ConditionalProgressMonotone]_(SP!Vars)
