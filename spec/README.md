...
- `SupervisorProtocolSim.tla`
  - concrete harness for random simulation of the full Protocol spec
- `SupervisorProtocol.sim.cfg`
  - invariants for the Protocol simulation harness
- `SupervisorCoreSim.tla` / `SupervisorCoreSim.cfg`
  - small SupervisorCore harness checked exhaustively (full bounded BFS).
    This is where the GapResearch planner ↔ critic loop is modeled, so its
    termination invariants (`GapLoopTerminates`, `GapStageRejectConsistency`)
    live here, not in the Protocol cfg. The state space is small enough
    (~474k distinct states, depth 23, ≈25s) that exhaustive search is cheap.
    Runs mode-A (`SimGoalMode = "prose"`).
- `SupervisorCore.sim.cfg`
  - SupervisorCore random-simulation cfg (used with `SupervisorCoreSim.tla`
    in `-simulate` mode for `SimNodes > 1` or when staying sim-only). Carries
    the same invariant set plus the PV mode-B prove-or-disprove invariants.
- `SupervisorCoreSim_modeb.tla` / `SupervisorCoreSim_modeb.cfg`
  - PV **mode-B** (Lean-goals / prove-or-disprove) SupervisorCore harness,
    SIM-ONLY. `SimGoalMode = "lean"`, empty configured (faithfulness) bucket,
    two Decide challenge targets with OPPOSITE abstract truths (`c1` true on
    Prove, `c2` true on Disprove). Green cfg lists every PV mode-B invariant
    (`SoundnessFloor`, `NotBothSidesClosed`, `NoHumanInModeB`,
    `CoverageOnLiveOnly`, `DecidedResolves`, `ModeBConfiguredTargetsEmpty`).
    The `AuditSoleFlipper` action property is enforced structurally (the
    `UNCHANGED PolarityVars` frame on every non-audit action) and is BFS-only
    if checked explicitly. Run with `-simulate` only.
- `SupervisorCoreSim_modeb_reach.tla` + `SupervisorCoreSim_modeb_reach.cfg`
  / `SupervisorCoreSim_modeb_disprove.cfg`
  - mode-B REACHABILITY baits (expected-FAIL, NOT in the green suite): a
    larger `MaxCycle` lets a random `-simulate` run witness Done-via-disprove
    (`DoneNeverViaDisprove`) or the shallower disprove-side close milestone
    (`C2NeverDisproveClosed`). A reported invariant VIOLATION is the witness.

The previous *Protocol* BFS exhaustive harnesses (`SupervisorProtocolSmall.tla`,
...
run on the full Protocol spec is RAM-heavy enough that exhaustive search
regularly OOMs and even sim mode warrants a strict concurrency cap. Do not
re-add them. (The small SupervisorCore BFS above is a different, deliberately
bounded harness and is fine to run exhaustively.)

## Running TLC

`scripts/run_tlc.sh sim` runs both harnesses: first the small SupervisorCore
bounded BFS (carrying the gap-loop invariants), then the SupervisorProtocol
random simulation. The Protocol part is sim-only. Manual invocations:

```bash
# Gap-loop invariants — small exhaustive BFS (no -simulate):
java -XX:+UseParallelGC -jar tla2tools.jar -workers 2 \
    -config SupervisorCoreSim.cfg SupervisorCoreSim.tla

# Full Protocol spec — random simulation only:
