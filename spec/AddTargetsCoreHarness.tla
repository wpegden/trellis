...
HarnessNext ==
    \* Core!EditConfiguredTargets / Core!StartCycle are legacy actions;
    \* the sidecar-counter frame lives in Core!Next's wrapper, so the
    \* harness frames it here.
    \/ (Core!EditConfiguredTargets /\ UNCHANGED <<sidecarCloses, sidecarQueue>>)
