...
A run can put a small pool of Lean-specialized models — "grunts" — to work
...
The reviewer schedules the queue; an empty queue means idle grunts. Closures are
attributed per node in `closure_provenance` (`worker` or `sidecar`), and the
viewer's **Grunts** tab shows the pool, queue, and attempt history.
...
The driver calls the Mistral API directly, reading its key from `~/.vibe/.env`
(mode 0600) under `MISTRAL_API_KEY`. At <https://admin.mistral.ai/>: sign in
(the **free plan suffices**), **opt into "Enable Labs models"** under
[privacy](https://admin.mistral.ai/plateforme/privacy) — Leanstral is a Labs
model and requests fail without it, surfacing as a model-availability error —
then create a key under
[api-keys](https://admin.mistral.ai/organization/api-keys).

Mistral's `vibe` CLI ([install
guide](https://docs.mistral.ai/getting-started/quickstarts/vibe-code/install-cli))
stores the key at that path for you: run `vibe --setup`, pressing `m` to enter
the key manually if it cannot open a browser. Or write the file directly:

```bash
install -m 600 /dev/null ~/.vibe/.env
printf 'MISTRAL_API_KEY=%s\n' "$YOUR_KEY" >> ~/.vibe/.env
```

Enable it in `trellis.config.json`, where `grunts` is the pool size (the only
volume throttle) and `attempt_wall_seconds` caps one attempt:

```json
{"sidecar": {"enabled": true, "grunts": 2, "attempt_wall_seconds": 5400}}
```
