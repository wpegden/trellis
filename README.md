...
- [REFERENCE_PAPERS.md](REFERENCE_PAPERS.md): registering auxiliary papers as grounding authority for cited results (§11)
- [ADD_TARGETS.md](ADD_TARGETS.md): adding further targets from the same paper to a Complete run
...
To formalize into **Isabelle/HOL** instead of Lean, point setup at the shipped
Isabelle templates (`CONFIG_TEMPLATE=examples/trellis.isabelle.config.json`);
see INSTALLATION.md §2g ("Isabelle/HOL backend") for the distribution
requirement (Isabelle2025-2 or newer) and the `TRELLIS_ISABELLE_HOME`
override.
...
Its root URL is a landing page: one card per run found under the projects root,
with the run's lifecycle state read from disk (never started, live, paused,
halted, finished, abandoned) plus host RAM/swap/disk against the documented
requirements, supervisor/checker/sidecar liveness, and provider budget. Each
card links into that run's viewer.

To reach the viewer from another machine, forward its loopback port:

```bash
ssh -L 3301:127.0.0.1:3301 <host>   # then browse http://127.0.0.1:3301/trellis/
```

This is the supported way in: the viewer stays on loopback and SSH supplies
both encryption and authentication, so there is no TLS to set up (use plain
`-L`, never `-L '*:3301'` or `-g`, which republishes the viewer in cleartext on
your own LAN). SECURITY.md, "Deployment modes", covers the public-host
alternatives.

The viewer answers only to names this machine actually goes by (`localhost`,
loopback literals, its own short and fully-qualified hostnames, whatever it
bound), which is what stops a page you visit from re-pointing its own hostname
at the port and reading your runs. Nothing to configure — browsing by hostname
through nginx works as it always did — and `TRELLIS_VIEWER_ALLOWED_HOSTS` adds
a name if you reach it through an alias or a Host-rewriting proxy. Bound off
loopback, reads also require the token URL by default
(`TRELLIS_VIEWER_READ_TOKEN`).

Because the viewer can pause and relaunch runs, its controls live behind a
secret 8-character path segment. `<base>/<token>/` is the same landing page
with pause/resume and feedback enabled, `<base>/<token>/<project>` the same run
viewer with them enabled, and every plain URL keeps working read-only.
`start_viewer.sh` prints the whole control URL on its `viewer_control_url=`
line, and it works through the tunnel unchanged; writes additionally carry a
custom `X-Trellis-Control` header, which is what makes them unforgeable from
another site. See SECURITY.md for the tradeoff of putting the token in the URL.
If you expose the viewer publicly (§7 below), set `TRELLIS_VIEWER_CONTROL=0`
first.
...
- **`system_feedback` is logged by default; halting on it is opt-in.** When an
  ...
  `clear_instructions`), fix the underlying cause, then resume by *lifting*
  the marker — `./scripts/trellis_pause.sh resume <runtime_root> <repo_path>
  --clear-halt` renames it to a `.lifted-<unix_ts>.json` sibling and relaunches
  (§2), so the diagnostic outlives the resume. A marker already on disk always
  halts the run regardless of the knob. A lift settles that one instance; to
  stop a known-benign fingerprint halting on every recurrence, acknowledge it
  (`ack_system_feedback`) instead, which log-and-continues without disabling
  halts for novel feedback.
...
5. **Resume**: `./scripts/trellis_pause.sh resume <runtime_root> <repo_path>`.
   ...
   pause record only once the relaunch confirms. Resume refuses while a halt
   marker is on disk, naming it: add `--clear-halt` to lift a system-feedback
   marker and relaunch in one step, or `--clear-halt-any-kind` for a
   checker-disagreement or unparseable marker, where soundness is the open
   question and the triage belongs at the marker first.
...
Auto-dispatch stops refilling a node after the pool has failed it 3 times
...
is never told grunts exist, and no daemon runs. Leave it that way unless the
system requirements in `INSTALLATION.md` are comfortably exceeded: grunts buy
a modest speedup and some token savings, not a better chance of formalizing
anything, and each one costs a workspace clone and a concurrent Lean build.
...
When the paper cites results proved elsewhere, register those sources so the
verification lanes can read them. A registered document is grounding authority
only — the primary paper stays the sole source of targets. Workers claim the
documents a node relies on, and the substantiveness verifier reads the claimed
file as the authority for the cited result.
...
On an existing run, register the file and then admit it during a stop window
(§5); ids are immutable for the life of the run:

```bash
./scripts/add_reference_paper.sh <repo> <tex> --id smith2020 --source-id "Smith, JCTB 2020"
```

`REFERENCE_PAPERS.md` covers the kernel action that admits it, the quiescence
preconditions, composition with add-targets, and the deploy/rollback notes.
