...
def test_worker_model_ab_flag_randomizes_between_default_models(
    ...
    assert b_provider.model == "gpt-5.5"
    assert (
        ...
        == "proof_formalization:worker:codex:gpt-5.5:xhigh"
    )
    ...
    assert a_provider.model == "gpt-5.4"
    assert (
        bridge_module._session_scope(
            {"phase": "proof_formalization"},
            a_provider,
        ...
        == "proof_formalization:worker:codex:gpt-5.4:xhigh"
    )
...
def test_correspondence_prompt_renders_under_model_assumptions_rule_and_aeneas_path() -> None:
    tmpdir = Path(tempfile.mkdtemp())
    aeneas_path = "${TRELLIS_ROOT:-/path/to/trellis}/pv_spike/tools/aeneas/backends/lean/Aeneas"
