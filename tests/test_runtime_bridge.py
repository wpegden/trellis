...
# ----- fingerprint-acknowledgment ack policy on the Python writer ------
# The live emission path halts on EVERY non-empty system_feedback unless an
# operator has explicitly acknowledged that emission's STABLE fingerprint in
# `<runtime_root>/system_feedback_acks.json` (the same store the Rust CLI
# writes). An acked fingerprint is logged to the design-gap JSONL and the
# halt is suppressed; any NOVEL feedback still hard-halts.
...
def test_executor_system_feedback_default_marker_includes_fingerprint(
    tmp_path: Path,
) -> None:
    # Default path (no ack store on disk): marker IS written and now carries
    # the additive `fingerprint` field equal to the stable fingerprint.
    from trellis.agent_wrapper import executor as executor_module
    ...
    # No ack store → no design-gap log written.
    assert not (runtime_root / "system_feedback_design_gap_log.jsonl").exists()


def test_executor_system_feedback_acked_suppresses_marker_and_logs(
    tmp_path: Path,
) -> None:
    # Acked fingerprint: NO marker written (so the bridge gate would let
    # dispatch proceed) and a design-gap JSONL record IS appended.
    from trellis.agent_wrapper import executor as executor_module
    ...
    log = runtime_root / "system_feedback_design_gap_log.jsonl"
    assert log.exists()
    rec = json.loads(log.read_text().strip())
    assert rec["kind"] == "system_feedback_acknowledged"
...
def test_executor_system_feedback_novel_still_halts_when_other_acked(
    ...
    # An ack store that acks a DIFFERENT fingerprint must not suppress a
    # novel emission — novel feedback still hard-halts.
    from trellis.agent_wrapper import executor as executor_module
...
def test_executor_system_feedback_corrupt_store_fails_closed(tmp_path: Path) -> None:
    # Fail-closed: a corrupt/unparseable ack store is treated as EMPTY, so
    # the emission still halts (marker written).
    from trellis.agent_wrapper import executor as executor_module
