...
def test_worker_rejection_annotation_preserves_kind_feedback_and_judging_identity() -> None:
    annotated = bridge_module._annotate_worker_rejection(
        {
            "final_outcome": "invalid",
            "response": {
                "outcome": "Invalid",
                "invalid_kind": "contract_conflict",
        ...
        request={"acceptance_logic_identity": "trellis-worker-acceptance-v1:sha256:abc"},
        system_feedback="the prompt and checker require incompatible claim shapes",
    )
    reasons = annotated["response"]["deterministic_rejection_reasons"]
    assert reasons[0].startswith(
        "[worker_rejection] kind=contract_conflict phase=worker_report"
    )
    assert reasons[1] == (
        "[acceptance_logic_identity] trellis-worker-acceptance-v1:sha256:abc"
    )
    assert reasons[2].startswith("[worker_system_feedback]")
...
def _isolate_provider_home_seeding(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    """Never clone the operator's real provider state into bridge unit tests.

    Production bridge dispatch intentionally seeds a persistent burst home from
    ``Path.home()``.  On a development host that tree can contain many gigabytes
    of provider job history and build artifacts; materializing it once per test
    is both irrelevant to these bridge tests and capable of exhausting the test
    filesystem.  The burst-home implementation has its own focused tests.  Here
    we preserve the real seeding behavior while changing only its source to an
    empty, test-owned home.
    """
    source_home = tmp_path / "provider-source-home"
    source_home.mkdir()
    real_seed = bridge_module.seed_burst_home

    def seed_from_fixture_home(runtime_root, burst_id, **kwargs):
        return real_seed(
            runtime_root,
            burst_id,
    ...
    monkeypatch.setattr(bridge_module, "seed_burst_home", seed_from_fixture_home)
...
def _clear_worker_model_ab_env(monkeypatch: pytest.MonkeyPatch) -> None:
    for name in (
        bridge_module.WORKER_MODEL_AB_ENV,
        bridge_module.WORKER_MODEL_AB_A_MODEL_ENV,
        bridge_module.WORKER_MODEL_AB_B_MODEL_ENV,
        bridge_module.WORKER_MODEL_AB_B_PROB_ENV,
    ):
        monkeypatch.delenv(name, raising=False)


def test_worker_model_ab_is_disabled_by_default(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _clear_worker_model_ab_env(monkeypatch)
    ...
    effective, decision = bridge_module._maybe_apply_worker_model_ab(
        work_dir=tmp_path,
        provider=provider,
    )

    assert effective is provider
    assert decision is None


def test_worker_model_ab_flag_randomizes_between_default_models(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _clear_worker_model_ab_env(monkeypatch)
    (tmp_path / ".trellis").mkdir()
    (tmp_path / bridge_module.WORKER_MODEL_AB_FLAG_NAME).touch()
    provider = ProviderConfig(provider="codex", model="gpt-5.5", effort="xhigh")

    monkeypatch.setattr(bridge_module.random, "random", lambda: 0.25)
    b_provider, b_decision = bridge_module._maybe_apply_worker_model_ab(
        work_dir=tmp_path,
        provider=provider,
    )
    assert b_decision is not None
    assert b_decision.arm == "b"
    # The B arm is the run's OWN configured model — `b_model` prefers
    # `configured_model` and only falls back to the default when a run has
    # none. The A arm is the comparison model, which does come from the
    # default. Asserting literals hid this: the old expectations passed
    # because the configured model happened to equal the old default, so
    # the test could not tell the two paths apart.
    b_model = provider.model
    assert b_provider.model == b_model
    assert (
        bridge_module._session_scope(
            {"phase": "proof_formalization"},
            b_provider,
            "worker",
        )
        == f"proof_formalization:worker:codex:{b_model}:xhigh"
    )

    monkeypatch.setattr(bridge_module.random, "random", lambda: 0.75)
    a_provider, a_decision = bridge_module._maybe_apply_worker_model_ab(
        work_dir=tmp_path,
        provider=provider,
    )
    assert a_decision is not None
    assert a_decision.arm == "a"
    a_model = bridge_module.DEFAULT_WORKER_MODEL_AB_A_MODEL
    assert a_provider.model == a_model
    assert (
        bridge_module._session_scope(
            {"phase": "proof_formalization"},
            a_provider,
        ...
        == f"proof_formalization:worker:codex:{a_model}:xhigh"
    )
    # The arms must actually differ, or the harness compares a model to
    # itself and every A/B result is noise.
    assert a_model != b_model, "A/B arms must name different models"


def test_worker_model_ab_config_file_controls_models_and_probability(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _clear_worker_model_ab_env(monkeypatch)
    (tmp_path / ".trellis").mkdir()
    (tmp_path / bridge_module.WORKER_MODEL_AB_CONFIG_NAME).write_text(
        json.dumps(
            {
                "enabled": True,
                "a_model": "gpt-5.4-test",
                "b_model": "gpt-5.5-test",
                "b_prob": 1.0,
            }
        ),
        encoding="utf-8",
    )
    monkeypatch.setattr(bridge_module.random, "random", lambda: 0.99)

    effective, decision = bridge_module._maybe_apply_worker_model_ab(
        work_dir=tmp_path,
        provider=ProviderConfig(provider="codex", model="configured-worker", effort="high"),
    )

    assert decision is not None
    assert decision.arm == "b"
    assert decision.configured_model == "configured-worker"
    assert effective.model == "gpt-5.5-test"


def test_worker_model_ab_log_records_chosen_arm(tmp_path: Path) -> None:
    decision = bridge_module._WorkerModelAbDecision(
        arm="a",
    ...
    bridge_module._record_worker_model_ab(
        work_dir=tmp_path,
        request={"id": 17, "cycle": 41, "phase": "proof_formalization"},
        decision=decision,
        artifact_prefix="trellis_worker_17_result",
        session_scope="proof_formalization:worker:codex:gpt-5.4:xhigh",
    )

    log_path = tmp_path / bridge_module.WORKER_MODEL_AB_LOG_NAME
    ...
    assert records == [
        {
            ...
            "cycle": 41,
            "phase": "proof_formalization",
            "burst_prefix": "trellis_worker_17_result",
            "arm": "a",
            "configured_model": "gpt-5.5",
            "chosen_model": "gpt-5.4",
            "a_model": "gpt-5.4",
...
def test_sound_bridge_missing_artifact_is_retryable_not_fatal() -> None:
    """Bug 1: a sound verifier lane that finished WITHOUT a result artifact
    (ok=False) must NOT crash the bridge/supervisor. The bridge returns a
    Malformed verifier response (the engine re-dispatches on Malformed)
    instead of feeding the bad member to the kernel normalizer (which would
    Err → fatal `runtime step failed`)."""
    tmpdir = Path(tempfile.mkdtemp())
    config = _fake_config(tmpdir)
    request = BridgeCliRequest(
        config_path=tmpdir / "trellis.config.json",
    ...
    def _fake_kernel_cli(payload: dict[str, object]) -> dict[str, object]:
        if payload.get("action") == "build_malformed_response":
            assert payload.get("kind") == "sound"
            return {
        ...
        # normalize_* must never be reached — the missing-artifact member is
        # short-circuited to Malformed before normalization.
        raise AssertionError(
            f"normalizer must not run when an artifact is missing: {payload.get('action')!r}"
        )
    ...
    with patch("trellis.runtime.bridge.load_config", return_value=config), patch(
        ...
        return_value=PanelExecutionResponse(
            ...
            member_responses=[
                ...
                # v2 ran but produced no usable artifact.
                SingleAgentResponse(
                    request_id="sound-8-v2",
                    cycle=4,
                    kind="sound",
                    ...
                    ok=False,
                    payload=None,
                    error="…sound_8_v2.raw.json not found",
                ),
            ],
        ),
    ), patch(
...
def test_review_bridge_validates_blocker_ids_and_maps_to_protocol_shape() -> None:
    ...
    assert seen["session_name"] == "trellis-runtime-review-9-reviewer"
...
def test_human_gate_bridge_reads_file_response() -> None:
    ...
    runtime_root = tmpdir / "runtime"
    runtime_root.mkdir(parents=True, exist_ok=True)
    response_path = runtime_root / "human_gate_response.json"
    response_path.write_text(
        json.dumps({"choice": "feedback", "cycle": 6}),
        encoding="utf-8",
    )
    (runtime_root / "human_gate_response.viewer_meta.json").write_text(
        json.dumps({"choice": "feedback", "cycle": 6}),
        encoding="utf-8",
    )
    request = BridgeCliRequest(
    ...
    assert json.loads(call_payload["raw_payload_text"]) == {"choice": "feedback", "cycle": 6}
    # A fresh response is consumed so it cannot re-satisfy a later gate.
    assert not response_path.exists()
    assert not (runtime_root / "human_gate_response.viewer_meta.json").exists()
...
def test_human_gate_bridge_blocks_on_stale_response() -> None:
    # Defect 1: a valid Ok response that the kernel reports as NOT fresh
    # (its cycle stamp does not match the in-flight gate) must be treated
    # like a missing file so the supervisor keeps blocking and polling, and
    # the stale file must NOT be consumed.
    tmpdir = Path(tempfile.mkdtemp())
    runtime_root = tmpdir / "runtime"
    runtime_root.mkdir(parents=True, exist_ok=True)
    response_path = runtime_root / "human_gate_response.json"
    # Stamped for an earlier gate (cycle 212) while the live gate is cycle 9.
    response_path.write_text(
        json.dumps({"choice": "approve", "cycle": 212}),
        encoding="utf-8",
    )
    request = BridgeCliRequest(
        config_path=tmpdir / "trellis.config.json",
    ...
    # Must contain the substring the kernel's poll helper keys off, so the
    # supervisor keeps blocking rather than failing the run.
    assert "missing human gate response file" in str(excinfo.value)
    # The stale file is preserved (not consumed) — a future gate at cycle
    # 212 could legitimately match it.
...
def test_human_gate_bridge_skips_runtime_support_materialization() -> None:
    ...
    (runtime_root / "human_gate_response.json").write_text(
        json.dumps({"choice": "approve", "cycle": 8}),
        encoding="utf-8",
    )
    request = BridgeCliRequest(
        config_path=tmpdir / "trellis.config.json",
...
def test_commissioned_operation_without_declared_producer_is_refused_at_issuance(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """The bridge must reject an operation before attempting its producer."""
    tablet_dir = tmp_path / "Tablet"
    tablet_dir.mkdir(parents=True)
    (tablet_dir / "n1.lean").write_text(
        "theorem n1 : True := by sorry\n",
        encoding="utf-8",
    )
    (tablet_dir / "n1.tex").write_text("N1.\n", encoding="utf-8")
    ...
    request = BridgeCliRequest(
        ...
        request=_kernel_bound_request(config, {
            "kind": "worker",
            "work_kind": "unproducible_test_operation",
            "id": 41,
            "cycle": 9,
            "phase": "theorem_stating",
            "mode": "targeted",
            "active_node": "n1",
            "configured_targets": ["goal:n1"],
            "current_present_nodes": ["n1"],
            "current_proof_nodes": ["n1"],
            "current_deps": {"n1": []},
            "current_semantic_deps": {"n1": []},
            "current_target_claims": {"n1": ["goal:n1"]},
            "worker_context": {
                "active_difficulty": "Hard",
                "worker_profile": "Theorem",
                "validation_kind": "theorem_targeted",
    ...
    monkeypatch.setenv("TRELLIS_TRELLIS_BRIDGE_DRY_RUN", "1")
    with patch("trellis.runtime.bridge.load_config", return_value=config), patch(
        "trellis.runtime.bridge.PolicyManager"
    ) as mock_policy_manager:
        mock_policy_manager.return_value.current.return_value = _fake_policy()
        with pytest.raises(
            BridgeError,
            match="REFUSED commissioned operation.*no producer capability declaration",
        ):
            handle_bridge_request(request)
...
def test_worker_prompt_renders_process_memory_block_for_active_coarse_cone() -> None:
    tmpdir = Path(tempfile.mkdtemp())
    entry_dir = tmpdir / "process-memory" / "ConeA"
    entry_dir.mkdir(parents=True)
    (entry_dir / "pm-0001-route-x.md").write_text(
        "---\n"
        "id: pm-0001-route-x\n"
        "type: refuted-route\n"
        "status: active\n"
        ...
        "---\n\n"
        "Route X refuted: counterexample n=3 inline.\n",
        encoding="utf-8",
    )
    ...
    contract["prompt_fragments"] = list(contract["prompt_fragments"]) + [
    ...
    assert "## Process memory" in prompt
    assert "Route X refuted: counterexample n=3 inline." in prompt
    assert "memory_challenges" in prompt

    # Pre-migration repos (no process-memory/) render with the section
    # absent — the fragment collapses to an empty section.
    bare_repo = Path(tempfile.mkdtemp())
    bare_prompt = build_worker_prompt(
        request=_request(),
        worker_gate={"worker_acceptance": _worker_acceptance("proof_local", ["n1"])},
        repo_path=bare_repo,
        raw_output_path=bare_repo / "worker.raw.json",
        done_path=bare_repo / "worker.done",
        acceptance_context_path=bare_repo / "worker.acceptance.json",
    )
    assert "## Process memory" not in bare_prompt
...
def _write_reference_paper(tmpdir: Path) -> None:
    refs_dir = tmpdir / "paper" / "refs"
    refs_dir.mkdir(parents=True, exist_ok=True)
    ref_lines = [f"ref line {i:04d}" for i in range(1, 31)]
    ref_lines[9] = "ref line 0010: REF_MARKER_10"
    (refs_dir / "smith2020.tex").write_text("\n".join(ref_lines) + "\n", encoding="utf-8")


def test_worker_prompt_extracts_reference_paper_focus_ranges() -> None:
    # A `doc`-carrying range extracts from the STATE-carried registry's
    # tex_path (never config) and labels the fragment with the document
    # id; doc-free ranges keep the legacy primary-paper rendering; the
    # sidecar name is unchanged.
    tmpdir = _write_paper_grounding_repo(Path(tempfile.mkdtemp()))
    _write_reference_paper(tmpdir)
    raw_output_path = tmpdir / "worker.raw.json"
    request = _worker_request_with_paper_focus(
        ranges=[
            {"start_line": 41, "end_line": 42, "reason": "primary passage"},
            {"start_line": 9, "end_line": 11, "reason": "cited lemma", "doc": "smith2020"},
        ],
        tmpdir=tmpdir,
    )
    request["worker_contract"]["request_summary"]["reference_papers"] = {
        "smith2020": {"tex_path": "paper/refs/smith2020.tex", "source_id": "Smith 2020"}
    }
    prompt = build_worker_prompt(
        request=request,
        worker_gate={"worker_acceptance": _worker_acceptance("proof_local", ["n1"])},
    ...
    assert "paper/reference.tex:41-42" in prompt
    assert "paper/refs/smith2020.tex:9-11 [document: smith2020]" in prompt
    assert "THRESHOLD_MARKER_41" in prompt
    assert "REF_MARKER_10" in prompt
    assert "[document:" not in prompt.split("smith2020.tex:9-11", 1)[0], (
        "primary-paper fragments carry no document label (legacy rendering)"
    )
    sidecar = tmpdir / "worker.raw.paper_focus_fragments.md"
    assert sidecar.exists(), "sidecar keeps the legacy name"
    sidecar_text = sidecar.read_text(encoding="utf-8")
    assert "REF_MARKER_10" in sidecar_text
    assert "[document: smith2020]" in sidecar_text


def test_worker_prompt_reference_doc_unknown_id_fails_closed() -> None:
    tmpdir = _write_paper_grounding_repo(Path(tempfile.mkdtemp()))
    _write_reference_paper(tmpdir)
    request = _worker_request_with_paper_focus(
        ranges=[{"start_line": 1, "end_line": 2, "reason": "r", "doc": "nosuch"}],
        tmpdir=tmpdir,
    )
    request["worker_contract"]["request_summary"]["reference_papers"] = {
        "smith2020": {"tex_path": "paper/refs/smith2020.tex", "source_id": "Smith 2020"}
    }
    with pytest.raises(ValueError, match="reference_papers registry"):
        build_worker_prompt(
            request=request,
            worker_gate={"worker_acceptance": _worker_acceptance("proof_local", ["n1"])},
            repo_path=tmpdir,
...
def test_worker_prompt_reference_doc_out_of_range_names_reference_file() -> None:
    tmpdir = _write_paper_grounding_repo(Path(tempfile.mkdtemp()))
    _write_reference_paper(tmpdir)
    request = _worker_request_with_paper_focus(
        ranges=[{"start_line": 1, "end_line": 999, "reason": "r", "doc": "smith2020"}],
        tmpdir=tmpdir,
    )
    request["worker_contract"]["request_summary"]["reference_papers"] = {
        "smith2020": {"tex_path": "paper/refs/smith2020.tex", "source_id": "Smith 2020"}
...
def test_worker_prompt_clarifies_structure_scope_and_worker_fields() -> None:
    ...
    assert (
        "with `[]` when the node doesn't directly claim a configured paper target"
        " — likewise in `challenge_claim_updates` when challenge targets are configured"
    ) in prompt
...
def test_worker_prompt_uses_configurable_initial_theorem_dag_size_guidance() -> None:
    ...
    assert "a good target for the eventual proof-bearing DAG size, built up across cycles, is `20-60` proof-bearing nodes" in prompt
...
def test_proof_worker_prompt_includes_operational_guidance() -> None:
    ...
    assert "For the edit-compile-fix inner loop, run `.trellis/scripts/incremental-check Tablet.NodeName` first" in prompt
...
def test_correspondence_prompt_omits_no_acceptance_checker_line_when_json_validator_exists() -> None:
    tmpdir = Path(tempfile.mkdtemp())
...
def test_correspondence_prompt_renders_under_model_assumptions_rule_and_aeneas_path() -> None:
    tmpdir = Path(tempfile.mkdtemp())
    aeneas_path = "/path/to/trellis/pv_spike/tools/aeneas/backends/lean/Aeneas"
    contract = _corr_contract(
        verify_nodes=["Assumptions"],
        verify_targets=[],
    )
    contract["request_summary"]["under_model_assumptions"] = {
        "nodes": ["Assumptions"],
        "lean_form": "axiom",
        "correspondence": "lean_axiom_proposition_to_tex_statement",
    }
    contract["request_summary"]["tooling_hints"] = {
        "aeneas_lean_sources": aeneas_path,
    }
    expected_rule = (
    ...
    assert f"Aeneas Lean source: {aeneas_path}" in prompt

    ordinary_prompt = build_correspondence_prompt(
        request={
            "project_invariants": _project_invariants(),
            "kind": "corr",
            "verify_nodes": ["Assumptions"],
        ...
        repo_path=tmpdir,
        lane_id="v1",
        raw_output_path=tmpdir / "ordinary.raw.json",
        done_path=tmpdir / "ordinary.done",
    )

    assert expected_rule not in ordinary_prompt
...
def test_correspondence_prompt_scopes_per_node_previous_findings_to_nodes_and_lane() -> None:
    tmpdir = Path(tempfile.mkdtemp())
    contract = _corr_contract(verify_nodes=["main_node"], verify_targets=["thm:conn"])
    contract["previous_own_findings_by_lane"] = {
        "main_node": {
            "v1": {
                "node": "main_node",
                "overall": "REJECT",
                "summary": "main node same lane summary",
                "comments": "main node same lane comments",
                "correspondence": {
                    "decision": "FAIL",
                    "issues": [
                        {
                            "node": "main_node",
                            "description": "main node issue",
                        },
                        {
                            "node": "other_node",
                            "description": "leaked other node issue",
                        },
                    ],
                },
            },
            "v2": {
                "node": "main_node",
                "overall": "APPROVE",
                "summary": "main node other lane summary",
                "comments": "main node other lane comments",
        ...
        "other_node": {
            "v1": {
                "node": "other_node",
                "overall": "REJECT",
                "summary": "other node same lane summary",
                "comments": "other node same lane comments",
    ...
    assert "main node same lane summary" in prompt
    assert "main node same lane comments" in prompt
    assert "main node issue" in prompt
    assert "main node other lane summary" not in prompt
    assert "other node same lane summary" not in prompt
    assert "leaked other node issue" not in prompt
...
def test_worker_kernel_cli_failure_is_routed_as_infrastructure_fault() -> None:
    tmpdir = Path(tempfile.mkdtemp())
    runtime_root = tmpdir / "runtime"
    request = {"kind": "worker", "id": 23, "cycle": 4}

    with patch(
        "trellis.checking.run_kernel_cli",
        side_effect=bridge_module.KernelCliError("kernel process exited 101"),
    ):
        supplied = bridge_module.normalize_trellis_worker_result_data(
            {"outcome": "valid"},
            repo=tmpdir,
            acceptance_context={"request": request},
        )
    with patch(
        "trellis.runtime.bridge._build_malformed_response",
        return_value={
            "kind": "worker",
            "request_id": 23,
            "cycle": 4,
        ...
        response = bridge_module._save_worker_normalization_failure(
            runtime_root=runtime_root,
            request=request,
            raw_payload={"outcome": "valid"},
            acceptance_context_path=None,
            authoritative_repo_path=tmpdir,
            normalized_result=supplied,
        )

    assert response["transport_failure"] is True
    assert any(
        "kind=kernel_failure phase=kernel_validation" in reason
        for reason in response["deterministic_rejection_reasons"]
    )
    assert all(
        "kind=malformed_output" not in reason
        for reason in response["deterministic_rejection_reasons"]
    )


def test_worker_bridge_reuses_matching_latest_worker_response() -> None:
    tmpdir = Path(tempfile.mkdtemp())
    tablet_dir = tmpdir / "Tablet"
    tablet_dir.mkdir(parents=True, exist_ok=True)
    (tablet_dir / "Preamble.lean").write_text("theorem Preamble : True := by trivial\n")
    config = _fake_config(tmpdir)
    runtime_root = tmpdir / "runtime"
    bridge_dir = runtime_root / "bridge"
    bridge_dir.mkdir(parents=True, exist_ok=True)
    cached_response = {
        ...
        "comments": "",
        "snapshot": {"present_nodes": ["Preamble"], "open_nodes": [], "coverage": {}},
        "proof_node_updates": {},
        "node_kind_updates": {},
        "dep_updates": {},
        "semantic_dep_updates": {},
        ...
        json.dumps(
            {
                ...
                "tablet_snapshot": bridge_module._tablet_snapshot(tmpdir),
                "final_outcome": "valid",
                "errors": [],
                "validation_errors": [],
                "contract_errors": [],
    ...
    request = BridgeCliRequest(
        ...
        request=_kernel_bound_request(config, {
            ...
            "worker_context": {
                "active_difficulty": "Hard",
                "worker_profile": "cleanup",
                "validation_kind": "cleanup",
                "authorized_nodes": ["Preamble"],
            },
            "worker_acceptance": _worker_acceptance("cleanup", ["Preamble"]),
            "blocked_targets": [],
            "blockers": [],
            "protected_nodes": [],
            "current_present_nodes": ["Preamble"],
    ...
    with patch("trellis.runtime.bridge.load_config", return_value=config), patch(
        ...
        "trellis.runtime.bridge._restore_active_worker_base_via_kernel",
        side_effect=AssertionError("cached response should avoid restore"),
    ), patch(
        "trellis.runtime.bridge.build_trellis_worker_acceptance_context",
        side_effect=AssertionError("cached response should avoid context rebuild"),
    ), patch(
        "trellis.runtime.bridge.execute_agent_request",
        side_effect=AssertionError("cached response should avoid agent launch"),
    ):
        mock_policy_manager.return_value.current.return_value = _fake_policy()
        response = handle_bridge_request(request)
...
def test_worker_bridge_refuses_valid_cache_after_content_only_worktree_restore() -> None:
    """A cached Valid response is reusable only over the exact bytes it checked.

    Presence-only comparison cannot distinguish these two versions of the same
    node.  This is the manual-restore form of audit finding #2 and the surviving
    cache half of the two-rename interruption window.
    """
    tmpdir = Path(tempfile.mkdtemp())
    tablet_dir = tmpdir / "Tablet"
    tablet_dir.mkdir(parents=True, exist_ok=True)
    edited = "theorem Preamble : True := by\n  exact True.intro\n"
    restored = "theorem Preamble : True := by trivial\n"
    (tablet_dir / "Preamble.lean").write_text(edited, encoding="utf-8")
    post_burst_snapshot = bridge_module._tablet_snapshot(tmpdir)
    (tablet_dir / "Preamble.lean").write_text(restored, encoding="utf-8")
        ...
        json.dumps(
            {
                "raw": {"outcome": "valid"},
                "response": cached_response,
                "tablet_snapshot": post_burst_snapshot,
                "final_outcome": "valid",
            }
        ),
        encoding="utf-8",
    ...
    with patch("trellis.runtime.bridge.load_config", return_value=config), patch(
        ...
        "trellis.runtime.bridge.execute_agent_request",
        side_effect=AssertionError("a stale cache must fail closed before agent launch"),
    ):
        mock_policy_manager.return_value.current.return_value = _fake_policy()
        response = handle_bridge_request(request)
    ...
    assert response["transport_failure"] is True
    assert any(
        "exact Tablet byte snapshot" in reason
        for reason in response["deterministic_rejection_reasons"]
    )
...
def test_worker_bridge_refuses_done_recovery_without_post_burst_content_binding() -> None:
    """A bare `.done` cannot prove which source bytes the worker produced.

    The acceptance context authenticates only the pre-burst baseline.  If a
    restore happened after `.done` was written, re-normalizing the raw result
    would describe the restored bytes and silently lose a content-only edit.
    """
    tmpdir = Path(tempfile.mkdtemp())
    tablet_dir = tmpdir / "Tablet"
    tablet_dir.mkdir(parents=True, exist_ok=True)
    ...
    with patch("trellis.runtime.bridge.load_config", return_value=config), patch(
        ...
        "trellis.runtime.bridge.build_trellis_worker_acceptance_context",
        side_effect=AssertionError("bare done recovery must fail before context rebuild"),
    ), patch(
        "trellis.runtime.bridge.normalize_trellis_worker_result_data",
        side_effect=AssertionError("bare done recovery must not be normalized"),
    ), patch(
        "trellis.runtime.bridge.execute_agent_request",
    ...
    assert response["transport_failure"] is True
    assert any(
        "no authenticated post-burst source snapshot" in reason
        for reason in response["deterministic_rejection_reasons"]
    )
...
def test_worker_bridge_no_done_relaunch_restores_active_worker_base() -> None:
    ...
    def _capture_rebuild(
        repo, request_arg, *, collect_observations, paper_source_path=None, goal_prose_path=None
    ):
...
def test_review_prompt_renders_source_recourse_when_env_vars_set(
    ...
    for relative in (
        "kernel/src/artifact_validation.rs",
        "kernel/src/model.rs",
        "kernel/src/request_contracts.rs",
        "kernel/src/worker_normalization.rs",
        "kernel/src/runtime_cli_observations.rs",
        "kernel/src/bin/runtime_cli.rs",
    ):
    ...
    contract["request_summary"]["acceptance_logic_identity"] = (
        "trellis-worker-acceptance-v1:sha256:current"
    )
    fragments = list(contract["prompt_fragments"])
    ...
    assert "reviewer_source_matches_acceptance=false" in prompt
    assert "trellis-worker-acceptance-v1:sha256:current" in prompt
    assert "use it as historical context" in prompt
...
def test_executor_record_system_feedback_writes_halt_marker(tmp_path: Path) -> None:
    # Direct unit test on the wrapper's emission path in HALT mode
    # (opt-in via TRELLIS_SYSTEM_FEEDBACK_HALT=1): when an agent
    ...
    with patch.dict(
        os.environ,
        {
            "TRELLIS_KERNEL_CACHE_ROOT": str(runtime_root),
            "TRELLIS_SYSTEM_FEEDBACK_HALT": "1",
        },
    ):
    ...
    assert "system_feedback_halt" in data["clear_instructions"], (
        "clear_instructions must point the operator at the opt-in knob"
    )
    assert "unix_ts" in data and isinstance(data["unix_ts"], int)
    # Halt mode ALSO appends the unified log row (one durable stream
    # regardless of mode).
    log = runtime_root / "system_feedback_log.jsonl"
    assert log.exists()
    rec = json.loads(log.read_text().strip())
    assert rec["kind"] == "system_feedback"
    assert rec["acked"] is False
    assert rec["halted"] is True
    assert rec["system_feedback"] == "tool X mis-parsed argument Y"
...
# ----- opt-in halt + fingerprint-acknowledgment on the Python writer ---
# system_feedback halting is OPT-IN: with the knob absent (no top-level
# `system_feedback_halt` in trellis.config.json, no
# TRELLIS_SYSTEM_FEEDBACK_HALT env) every non-empty emission is appended to
# the unified `<runtime_root>/system_feedback_log.jsonl` and the run
# continues. With the knob on, every emission halts unless an operator has
# explicitly acknowledged that emission's STABLE fingerprint in
# `<runtime_root>/system_feedback_acks.json` (the same store the Rust CLI
# writes). An acked fingerprint gets a log row only in BOTH modes; any
# NOVEL feedback still hard-halts in halt mode.


def _feedback_request(tmp_path: Path, **overrides):
    from trellis.agent_wrapper.protocol import AgentLane, SingleAgentRequest
    ...
    kw = dict(
        ...
        state_dir=state_dir,
        session_name="s",
        lane=AgentLane(kind="reviewer", node_name="Foo"),
        timeout_seconds=0.0,
    )
    kw.update(overrides)
    return SingleAgentRequest(**kw)


def test_system_feedback_fingerprint_matches_rust_gold_vector() -> None:
    # Cross-language parity: the Python fingerprint for a fixed
    # (request_kind, burst_role, feedback) triple MUST equal the value the
    # Rust `system_feedback_fingerprint` produces for the same input.
    #
    # How the gold hex was obtained: a temporary `#[test]` was added to
    # `kernel/src/runtime_cli_observations.rs` that printed
    #   system_feedback_fingerprint(
    #     "reviewer", "reviewer",
    #     "Node `Foo` failed on cycle 809 with 3 targets.")
    # and run via `cargo test -p trellis-kernel --bin trellis_runtime_cli
    # zz_temp_print_parity_vector -- --nocapture`, printing the hex below,
    # then reverted. Independently cross-checked with
    #   printf 'sffp:v1\x1freviewer\x1freviewer\x1fnode <id> failed on cycle
    #   <n> with <n> targets.' | sha256sum
    # (hand-derived normalized material) yielding the identical digest.
    from trellis.agent_wrapper.executor import system_feedback_fingerprint

    gold = "53b24d5e48f08508d1755aad790e5f90d4b5a560f191ce08a3cc303dc33c696d"
    got = system_feedback_fingerprint(
        "reviewer", "reviewer", "Node `Foo` failed on cycle 809 with 3 targets."
    )
    assert got == gold, f"Python fingerprint {got} diverged from Rust gold {gold}"
    assert len(got) == 64 and got == got.lower()


def test_system_feedback_fingerprint_stability_and_separation() -> None:
    # Mirrors the Rust `system_feedback_fingerprint_is_stable_across_cycle_variation`
    # invariants: node-id / digit variation collapses to the SAME fingerprint;
    # genuinely different prose differs; request_kind/burst_role are part of key.
    from trellis.agent_wrapper.executor import system_feedback_fingerprint

    a = "reviewer flags a known gap in `NodeA` after 3 targets at cycle 809"
    b = "reviewer flags a known gap in `NodeB` after 17 targets at cycle 934"
    c = "worker cannot dispatch because the checker socket is unreachable"
    fp_a = system_feedback_fingerprint("reviewer", "reviewer", a)
    fp_b = system_feedback_fingerprint("reviewer", "reviewer", b)
    fp_c = system_feedback_fingerprint("reviewer", "reviewer", c)
    fp_a_worker = system_feedback_fingerprint("worker", "worker", a)
    assert fp_a == fp_b, "node-id/count/cycle variation MUST share a fingerprint"
    assert fp_a != fp_c, "different gap-class MUST differ"
    assert fp_a != fp_a_worker, "different request_kind/burst_role is a distinct class"


def test_executor_system_feedback_halt_mode_marker_includes_fingerprint(
    tmp_path: Path,
) -> None:
    # Halt mode (env=1), no ack store on disk: marker IS written and
    # carries the `fingerprint` field equal to the stable fingerprint.
    from trellis.agent_wrapper import executor as executor_module

    runtime_root = tmp_path / "runtime"
    runtime_root.mkdir()
    request = _feedback_request(tmp_path)
    feedback = "tool `X` mis-parsed argument `Y` on cycle 809"
    with patch.dict(
        os.environ,
        {
            "TRELLIS_KERNEL_CACHE_ROOT": str(runtime_root),
        ...
        executor_module._record_system_feedback(
            request, artifact_name="artifact.json", system_feedback=feedback
        )
    marker = runtime_root / "system_feedback_halt.json"
    assert marker.exists()
    data = json.loads(marker.read_text())
    expected_fp = executor_module.system_feedback_fingerprint(
        "reviewer", "reviewer", feedback
    )
    assert data["fingerprint"] == expected_fp
    assert expected_fp in data["clear_instructions"]
    # The unified log records the unacked halting emission too.
    rec = json.loads((runtime_root / "system_feedback_log.jsonl").read_text().strip())
    assert rec["acked"] is False
    assert rec["halted"] is True
    assert rec["fingerprint"] == expected_fp


def test_executor_system_feedback_default_logs_and_continues(tmp_path: Path) -> None:
    # DEFAULT path (knob absent everywhere): NO halt marker; the emission
    # is appended to the unified log with the full field set and the
    # writer returns without freezing the run. Pins the shipped
    ...
    request = _feedback_request(tmp_path)
    feedback = "tool `X` mis-parsed argument `Y` on cycle 809"
    env = {"TRELLIS_KERNEL_CACHE_ROOT": str(runtime_root)}
    with patch.dict(os.environ, env, clear=False):
        os.environ.pop("TRELLIS_SYSTEM_FEEDBACK_HALT", None)
        executor_module._record_system_feedback(
            request, artifact_name="artifact.json", system_feedback=feedback
        )
    assert not (runtime_root / "system_feedback_halt.json").exists(), (
        "default (knob absent) must NOT write a halt marker"
    )
    log = runtime_root / "system_feedback_log.jsonl"
    assert log.exists(), "default must append the unified jsonl row"
    rec = json.loads(log.read_text().strip())
    expected_fp = executor_module.system_feedback_fingerprint(
        "reviewer", "reviewer", feedback
    )
    ...
    assert rec["request_kind"] == "reviewer"
    assert rec["burst_role"] == "reviewer"
    assert rec["active_node"] == "Foo"
    assert rec["artifact"] == "artifact.json"
    assert rec["system_feedback"] == feedback
    assert rec["fingerprint"] == expected_fp
    assert isinstance(rec["unix_ts"], int)
    assert rec["acked"] is False
    assert rec["halted"] is False


def test_executor_system_feedback_config_knob_enables_halt(tmp_path: Path) -> None:
    # Opt-in via the config file: top-level `system_feedback_halt: true`
    # in <work_dir>/trellis.config.json (no env override) => marker.
    from trellis.agent_wrapper import executor as executor_module

    runtime_root = tmp_path / "runtime"
    runtime_root.mkdir()
    request = _feedback_request(tmp_path)
    (request.work_dir / "trellis.config.json").write_text(
        json.dumps({"system_feedback_halt": True})
    )
    with patch.dict(os.environ, {"TRELLIS_KERNEL_CACHE_ROOT": str(runtime_root)}):
        os.environ.pop("TRELLIS_SYSTEM_FEEDBACK_HALT", None)
        executor_module._record_system_feedback(
            request, artifact_name="artifact.json", system_feedback="config-mode halt"
        )
    assert (runtime_root / "system_feedback_halt.json").exists()
    rec = json.loads((runtime_root / "system_feedback_log.jsonl").read_text().strip())
    assert rec["halted"] is True


def test_executor_system_feedback_env_overrides_config_both_directions(
    tmp_path: Path,
) -> None:
    # Env beats config in BOTH directions (launcher-friendly override).
    from trellis.agent_wrapper import executor as executor_module

    runtime_root = tmp_path / "runtime"
    runtime_root.mkdir()
    request = _feedback_request(tmp_path)
    config = request.work_dir / "trellis.config.json"

    # env=0 + config true => NO marker, log row only.
    config.write_text(json.dumps({"system_feedback_halt": True}))
    with patch.dict(
        os.environ,
        {
            "TRELLIS_KERNEL_CACHE_ROOT": str(runtime_root),
            "TRELLIS_SYSTEM_FEEDBACK_HALT": "0",
        },
    ):
        executor_module._record_system_feedback(
            request, artifact_name="artifact.json", system_feedback="env forces off"
        )
    assert not (runtime_root / "system_feedback_halt.json").exists(), (
        "env=0 must override config true"
    )

    # env=1 + config false => marker.
    config.write_text(json.dumps({"system_feedback_halt": False}))
    with patch.dict(
        os.environ,
        {
            "TRELLIS_KERNEL_CACHE_ROOT": str(runtime_root),
        ...
        executor_module._record_system_feedback(
            request, artifact_name="artifact.json", system_feedback="env forces on"
        )
    assert (runtime_root / "system_feedback_halt.json").exists(), (
        "env=1 must override config false"
    )
    lines = (runtime_root / "system_feedback_log.jsonl").read_text().strip().splitlines()
    assert len(lines) == 2
    assert json.loads(lines[0])["halted"] is False
    assert json.loads(lines[1])["halted"] is True


def test_executor_system_feedback_malformed_config_fails_loud_toward_halt(
    tmp_path: Path,
) -> None:
    # Malformed knob (unparseable config JSON, or a non-bool value)
    # resolves LOUDLY toward halting — unreadable operator intent falls
    # back to the fail-loudly behavior, never silently disables it.
    from trellis.agent_wrapper import executor as executor_module
    ...
    request = _feedback_request(tmp_path)
    config = request.work_dir / "trellis.config.json"
    config.write_text(json.dumps({"system_feedback_halt": "yes"}))
    with patch.dict(os.environ, {"TRELLIS_KERNEL_CACHE_ROOT": str(runtime_root)}):
        os.environ.pop("TRELLIS_SYSTEM_FEEDBACK_HALT", None)
        assert executor_module._system_feedback_halt_enabled(request.work_dir) is True
        config.write_text("{ this is not valid json")
        assert executor_module._system_feedback_halt_enabled(request.work_dir) is True
        # Malformed env value also resolves toward halting.
        os.environ["TRELLIS_SYSTEM_FEEDBACK_HALT"] = "maybe"
        try:
            config.write_text(json.dumps({"system_feedback_halt": False}))
            assert (
                executor_module._system_feedback_halt_enabled(request.work_dir) is True
            )
        finally:
            os.environ.pop("TRELLIS_SYSTEM_FEEDBACK_HALT", None)


def test_executor_system_feedback_acked_suppresses_marker_and_logs(
    tmp_path: Path,
) -> None:
    # Acked fingerprint in HALT mode: NO marker written (so the bridge
    # gate would let dispatch proceed) and a unified-log JSONL record IS
    # appended (row only).
    from trellis.agent_wrapper import executor as executor_module
    ...
    runtime_root.mkdir()
    request = _feedback_request(tmp_path)
    feedback = "known design gap: reviewer cannot see source for `NodeA`"
    fp = executor_module.system_feedback_fingerprint("reviewer", "reviewer", feedback)
    (runtime_root / "system_feedback_acks.json").write_text(
        json.dumps(
            {
                "schema_version": 1,
                "acknowledged": {
                    fp: {
                        "fingerprint": fp,
                        "reason": "known gap, tracked in ISSUE-42",
                        "unix_ts": 1700000000,
    ...
    marker = runtime_root / "system_feedback_halt.json"
    assert not marker.exists(), "acked fingerprint must NOT write a halt marker"
    log = runtime_root / "system_feedback_log.jsonl"
    assert log.exists()
    rec = json.loads(log.read_text().strip())
    assert rec["kind"] == "system_feedback"
...
def test_executor_system_feedback_novel_still_halts_when_other_acked(
    tmp_path: Path,
) -> None:
    # Halt mode: an ack store that acks a DIFFERENT fingerprint must not
    # suppress a novel emission — novel feedback still hard-halts.
    from trellis.agent_wrapper import executor as executor_module
    ...
    other_fp = "0" * 64
    (runtime_root / "system_feedback_acks.json").write_text(
        json.dumps({"schema_version": 1, "acknowledged": {other_fp: {}}})
    )
    request = _feedback_request(tmp_path)
    with patch.dict(
        os.environ,
        ...
        executor_module._record_system_feedback(
            request,
            artifact_name="artifact.json",
            system_feedback="a brand new blocker never seen before",
        )
    assert (runtime_root / "system_feedback_halt.json").exists()


def test_executor_system_feedback_corrupt_store_fails_closed(tmp_path: Path) -> None:
    # Halt mode, fail-closed: a corrupt/unparseable ack store is treated
    # as EMPTY, so the emission still halts (marker written).
    from trellis.agent_wrapper import executor as executor_module

    runtime_root = tmp_path / "runtime"
    runtime_root.mkdir()
    (runtime_root / "system_feedback_acks.json").write_text("{not valid json")
    request = _feedback_request(tmp_path)
    with patch.dict(
        os.environ,
        {
        ...
        executor_module._record_system_feedback(
            request,
            artifact_name="artifact.json",
            system_feedback="feedback with a corrupt ack store present",
        )
    assert (runtime_root / "system_feedback_halt.json").exists()


def test_system_feedback_pre_existing_marker_blocks_dispatch_regardless_of_knob(
    tmp_path: Path,
) -> None:
    # Back-compat pin: a halt marker already on disk (operator-era halt)
    # is honored by the bridge gate REGARDLESS of the knob — even with
    # halting explicitly disabled via env, dispatch is refused until an
    # operator deletes the marker.
    bridge_request = _stub_bridge_request(tmp_path)
    marker = bridge_module.system_feedback_halt_marker_path(bridge_request.runtime_root)
    marker.write_text(json.dumps({"kind": "system_feedback"}))
    with patch.dict(os.environ, {"TRELLIS_SYSTEM_FEEDBACK_HALT": "0"}):
        with pytest.raises(BridgeError) as exc_info:
            handle_bridge_request(bridge_request)
    assert "system_feedback halt marker present" in str(exc_info.value)


def test_load_system_feedback_ack_store_missing_is_empty(tmp_path: Path) -> None:
    from trellis.agent_wrapper import executor as executor_module

    assert executor_module._load_system_feedback_ack_store(None) == set()
    assert executor_module._load_system_feedback_ack_store(tmp_path) == set()


def _protocol_state_for_verdicts() -> dict[str, object]:
    """Kernel state covering one node per lean_closure verdict plus the two
    fingerprint outcomes the lane maps distinguish.
    """
    return {
        "phase": "ProofFormalization",
        "proof_nodes": ["Closed", "Open", "Unverified", "Failed"],
        "local_closure_records": {"Closed": {"node": "Closed"}},
        "local_closure_unverified_nodes": ["Unverified", "Failed"],
        "local_closure_failures": {"Failed": {"axiom_violations": []}},
        "corr_status": {
            "Closed": "Pass",
            "Open": "Pass",
            "Unverified": "Fail",
        ...
        "sound_status": {"Open": "Fail", "Closed": "Fail"},
        "sound_approved_fingerprints": {"Open": "s1", "Closed": "s1"},
        "substantiveness_status": {"Defn": "Pass"},
        "substantiveness_approved_fingerprints": {"Defn": "b1"},
        "live": {
            "present_nodes": ["Closed", "Open", "Unverified", "Failed", "Defn"],
            "open_nodes": ["Open"],
            "sketch_proof_nodes": [],
            "corr_current_fingerprints": {
                "Closed": "c1",
            ...
            "sound_current_fingerprints": {"Open": "s1", "Closed": "s1"},
            "sound_current_fingerprint_parts": {},
            "substantiveness_current_fingerprints": {"Defn": "b1"},
        },
    }


def test_stuck_math_audit_node_lane_verdicts_mirror_kernel_state(tmp_path: Path) -> None:
    runtime_root = tmp_path / "runtime"
    runtime_root.mkdir()
    (runtime_root / "protocol_state.json").write_text(
        json.dumps(_protocol_state_for_verdicts()), encoding="utf-8"
    )

    verdicts = bridge_module._stuck_math_audit_node_lane_verdicts(runtime_root)

    assert verdicts["lean_closure"] == {
        "Closed": "closed",
        "Open": "open",
        "Unverified": "unverified",
        "Failed": "failed",
    }
    # `Defn` is not a proof node, so it carries no Lean-closure obligation.
    assert "Defn" not in verdicts["lean_closure"]
    assert verdicts["correspondence"] == {
        "Closed": "pass",
        "Open": "pass",
        "Unverified": "fail",
        "Failed": "pass",
        # Drifted fingerprint: the kernel re-verifies rather than trusting
        # the stored Pass.
        "Defn": "?",
    }
    # `needs_sound` is `present && open && proof_node`; everything else takes
    # the kernel's synthetic VerifierPass, which reads `not_required` here so
    # a stale stored Fail stays legible.
    assert verdicts["soundness"] == {
        "Closed": "not_required",
        "Open": "fail",
    ...
    assert verdicts["sound_recorded_fail"] == {
        "Closed": "VerifierFail",
        "Open": "VerifierFail",
    }
    assert verdicts["substantiveness"]["Defn"] == "pass"
    assert verdicts["available"] is True


def test_stuck_math_audit_node_lane_verdicts_without_protocol_state(tmp_path: Path) -> None:
    verdicts = bridge_module._stuck_math_audit_node_lane_verdicts(tmp_path / "absent")
    assert verdicts == {
        "available": False,
        "lean_closure": {},
        "correspondence": {},
...
def test_stuck_math_audit_bridge_context_carries_node_lane_verdicts() -> None:
    """The audit's request-context JSON is where the per-node lane verdicts
    reach the burst; `.request.json` stays the verbatim kernel request.
    """
    tmpdir = Path(tempfile.mkdtemp())
    config = _fake_config(tmpdir)
    runtime_root = tmpdir / "runtime"
    ...
    contract = {
        ...
        "request_summary": {
            "phase": "proof_formalization",
            "scenario": "stuck_math_audit",
            "resettable_theorem_stating_nodes": ["Open"],
        },
        "audit_latch": {"active": True, "trigger": "test"},
        "burst_role": "stuck_math_audit",
        "artifact_contract": {
            "result_type": "stuck_math_audit_result_v1",
            "prompt_schema_example": {
                "report": "audit report",
                "cone_clean_node": "Open",
                "tasks": [],
                "probe_paths": [],
            },
        },
    ...
    request = BridgeCliRequest(
        ...
        request=_kernel_bound_request(config, {
            ...
            "phase": "proof_formalization",
            "mode": "global",
            "stuck_math_audit_binding": _provider_binding(config.reviewer),
            "stuck_math_audit_contract": contract,
        }),
    )
    ...
    assert response["dry_run"] is True
    staging = tmpdir / ".trellis" / "runtime" / "runtime" / "staging"
    context = json.loads(
        (staging / "trellis_stuck_math_audit_41_result.context.json").read_text(
            encoding="utf-8"
        )
    )
    verdicts = context["node_lane_verdicts"]
    assert verdicts["lean_closure"]["Open"] == "open"
    assert verdicts["soundness"]["Open"] == "fail"
    assert verdicts["correspondence"]["Defn"] == "?"
    raw_request = json.loads(
        (staging / "trellis_stuck_math_audit_41_result.request.json").read_text(
            encoding="utf-8"
        )
    )
    assert "node_lane_verdicts" not in raw_request


def _verdicts_from_state(tmp_path: Path, state: dict[str, object]) -> dict[str, object]:
    runtime_root = tmp_path / "runtime"
    runtime_root.mkdir(exist_ok=True)
    (runtime_root / "protocol_state.json").write_text(
        json.dumps(state), encoding="utf-8"
    )
    return bridge_module._stuck_math_audit_node_lane_verdicts(runtime_root)


def _sound_lane_state(
    *,
    status: str,
    stored_parts: dict[str, object],
    current_parts: dict[str, object],
) -> dict[str, object]:
    """One open proof node carrying a stored soundness assessment."""
    return {
        "phase": "ProofFormalization",
        "proof_nodes": ["N"],
        "sound_assessments": {"N": {"status": status, "fingerprints": stored_parts}},
        "live": {
            "present_nodes": ["N"],
            "open_nodes": ["N"],
            "sound_current_fingerprint_parts": {"N": current_parts},
        },
    }


def test_substantiveness_verdict_fails_on_a_rejected_deviation_claim(
    tmp_path: Path,
) -> None:
    """`node_has_failed_deviation_claim` short-circuits
    `current_substantiveness_state` to Fail ahead of the stored Pass.
    """
    verdicts = _verdicts_from_state(
        tmp_path,
        {
            "phase": "ProofFormalization",
            "proof_nodes": [],
            "node_deviation_claims": {"N": ["dev"]},
            "deviation_status": {"dev": "Fail"},
            "deviation_approved_fingerprints": {"dev": "d1"},
            "substantiveness_status": {"N": "Pass"},
            "substantiveness_approved_fingerprints": {"N": "b1"},
            "live": {
                "present_nodes": ["N"],
                "open_nodes": [],
                "deviation_current_fingerprints": {"dev": "d1"},
                "substantiveness_current_fingerprints": {"N": "b1"},
            },
        },
    )
    assert verdicts["substantiveness"]["N"] == "fail"


def test_substantiveness_verdict_unknown_on_an_unauthorized_deviation_claim(
    tmp_path: Path,
) -> None:
    """A claim on a still-verifying deviation keeps
    `current_substantiveness_state` at Unknown ahead of the stored Pass.
    """
    verdicts = _verdicts_from_state(
        tmp_path,
        {
            ...
            "live": {
                "present_nodes": ["N"],
                "open_nodes": [],
                # Drifted deviation fingerprint: the Deviation lane owes a
                # fresh verdict, so the claim is not yet authorized.
                "deviation_current_fingerprints": {"dev": "d2"},
                "substantiveness_current_fingerprints": {"N": "b1"},
            },
        },
    )
    assert verdicts["substantiveness"]["N"] == "?"


def test_substantiveness_verdict_passes_preamble(tmp_path: Path) -> None:
    """`Preamble` is routed to the target-level lane, and
    `current_substantiveness_state` returns Pass for it with no stored entry.
    """
    verdicts = _verdicts_from_state(
        tmp_path,
        {
    ...
    assert verdicts["substantiveness"]["Preamble"] == "pass"


def test_correspondence_verdict_fails_on_a_placeholder_definition(
    tmp_path: Path,
) -> None:
    """A `live.placeholder_definition_nodes` entry is a deterministic corr Fail
    ahead of the stored Pass.
    """
    verdicts = _verdicts_from_state(
        tmp_path,
        {
            ...
            "proof_nodes": [],
            "corr_status": {"N": "Pass"},
            "corr_approved_fingerprints": {"N": "c1"},
            "live": {
                "present_nodes": ["N"],
                "open_nodes": [],
                "placeholder_definition_nodes": ["N"],
                "corr_current_fingerprints": {"N": "c1"},
            },
        },
    )
    assert verdicts["correspondence"]["N"] == "fail"


def test_correspondence_verdict_honors_the_empty_fingerprint_guard(
    tmp_path: Path,
) -> None:
    """An empty corr fingerprint under a stored Pass is Unknown, except on the
    `Preamble` node whose init pin `allow_empty_fingerprint` carves out.
    """
    verdicts = _verdicts_from_state(
        tmp_path,
        {
            "phase": "ProofFormalization",
            "proof_nodes": [],
            "corr_status": {"N": "Pass", "Preamble": "Pass"},
            "corr_approved_fingerprints": {"N": "", "Preamble": ""},
            "live": {
                "present_nodes": ["N", "Preamble"],
                "open_nodes": [],
                "corr_current_fingerprints": {"N": "", "Preamble": ""},
            },
        },
    )
    assert verdicts["correspondence"]["N"] == "?"
    assert verdicts["correspondence"]["Preamble"] == "pass"


def test_soundness_verdict_clears_an_empty_stored_own_tex_to_unknown(
    tmp_path: Path,
) -> None:
    """A reviewer-pinned Fail stored with no own-tex fingerprint, against a live
    proof that now has one, is `SelfEditUnknown` — the Soundness lane owes this
    node a fresh verdict.
    """
    verdicts = _verdicts_from_state(
        tmp_path,
        _sound_lane_state(
            status="ReviewerPinnedFail",
            stored_parts={
                "own_tex_hash": "",
            ...
            current_parts={
                "own_tex_hash": "h1",
                "dep_statement_hashes": {},
                "combined_sound_fp": "k1",
            },
        ),
    )
    assert verdicts["soundness"]["N"] == "?"
    assert verdicts["sound_recorded_fail"]["N"] == "ReviewerPinnedFail"


def test_soundness_verdict_clears_own_tex_drift_to_unknown(tmp_path: Path) -> None:
    """Own-tex drift is `SelfEditUnknown` whatever the stored verdict was."""
    verdicts = _verdicts_from_state(
        tmp_path,
        _sound_lane_state(
            status="VerifierFail",
...
def test_soundness_verdict_holds_dependency_drift_at_fail(tmp_path: Path) -> None:
    """Dependency drift re-keys a stored `VerifierStructural` onto
    `DepEditOnlyStaleFail`, which `current_sound_state` reads as Fail.
    """
    verdicts = _verdicts_from_state(
        tmp_path,
        _sound_lane_state(
            status="VerifierStructural",
            stored_parts={
                "own_tex_hash": "h1",
                "dep_statement_hashes": {"A": "d0"},
                "combined_sound_fp": "k0",
            },
            current_parts={
                "own_tex_hash": "h1",
                "dep_statement_hashes": {"A": "d1"},
                "combined_sound_fp": "k1",
            },
        ),
    )
    assert verdicts["soundness"]["N"] == "fail"
    assert verdicts["sound_recorded_fail"]["N"] == "VerifierStructural"


def test_soundness_verdict_marks_a_lean_closed_reviewer_pinned_fail(
    tmp_path: Path,
) -> None:
    """A Lean-closed node owes the Soundness lane nothing, and the reviewer's
    recorded rejection stays visible beside that.
    """
    verdicts = _verdicts_from_state(
        tmp_path,
        {
            "phase": "ProofFormalization",
            "proof_nodes": ["N"],
            "local_closure_records": {"N": {"node": "N"}},
            "sound_assessments": {
                "N": {
                    "status": "ReviewerPinnedFail",
                    "fingerprints": {
    ...
    assert verdicts["soundness"]["N"] == "not_required"
    assert verdicts["lean_closure"]["N"] == "closed"
    assert verdicts["sound_recorded_fail"] == {"N": "ReviewerPinnedFail"}


def test_node_lane_verdicts_report_unavailable_on_a_non_utf8_state(
    tmp_path: Path,
) -> None:
    runtime_root = tmp_path / "runtime"
    runtime_root.mkdir()
    (runtime_root / "protocol_state.json").write_bytes(b'{"phase": "\xff\xfe"}')

    verdicts = bridge_module._stuck_math_audit_node_lane_verdicts(runtime_root)

    assert verdicts["available"] is False
    assert verdicts["soundness"] == {}


def test_node_lane_verdicts_report_unavailable_on_unparseable_state(
    tmp_path: Path,
) -> None:
    runtime_root = tmp_path / "runtime"
    runtime_root.mkdir()
    ...
    assert (
        bridge_module._stuck_math_audit_node_lane_verdicts(runtime_root)["available"]
        is False
    )


def test_node_lane_verdicts_fail_closed_on_a_wrong_typed_node_set(
    tmp_path: Path,
) -> None:
    """A string where the kernel writes a node set iterates one entry per
    character, so the whole record degrades instead.
    """
    state = _protocol_state_for_verdicts()
    state["proof_nodes"] = "Closed"

    verdicts = _verdicts_from_state(tmp_path, state)
