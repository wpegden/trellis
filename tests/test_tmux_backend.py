...
from trellis.agents.tmux_backend import (  # noqa: E402
    ...
    per_lane_tmpdir,
    _raw_path_for_done,
    _raw_artifact_is_recoverable,
    _forgotten_done_recovery_seconds,
...
def test_raw_path_for_done_matches_artifacts_helpers():
    """`_raw_path_for_done` must derive the same sibling that
    trellis.artifacts produces, including stems with internal dots."""
    from trellis.artifacts import done_marker_path, raw_json_path

    state_dir = Path("/state")
    for canonical in ("worker_PaperFocus_42.json", "reviewer_Corr.v3.json", "x"):
        dpath = done_marker_path(state_dir, canonical)
        assert _raw_path_for_done(dpath) == raw_json_path(state_dir, canonical), (
            f"raw-path derivation diverged for {canonical}: "
            f"{_raw_path_for_done(dpath)} != {raw_json_path(state_dir, canonical)}"
        )


def test_raw_artifact_is_recoverable_gating(tmp_path: Path):
    """Only a parseable JSON dict with a non-empty `outcome` is recoverable."""
    valid = tmp_path / "valid.raw.json"
    valid.write_text(json.dumps({"outcome": "Pass", "x": 1}), encoding="utf-8")
    assert _raw_artifact_is_recoverable(valid)

    no_outcome = tmp_path / "no_outcome.raw.json"
    no_outcome.write_text(json.dumps({"x": 1}), encoding="utf-8")
    assert not _raw_artifact_is_recoverable(no_outcome)

    empty_outcome = tmp_path / "empty_outcome.raw.json"
    empty_outcome.write_text(json.dumps({"outcome": "  "}), encoding="utf-8")
    assert not _raw_artifact_is_recoverable(empty_outcome)

    not_json = tmp_path / "partial.raw.json"
    not_json.write_text('{"outcome": "Pas', encoding="utf-8")  # truncated write
    assert not _raw_artifact_is_recoverable(not_json)

    not_object = tmp_path / "list.raw.json"
    not_object.write_text(json.dumps([{"outcome": "Pass"}]), encoding="utf-8")
    assert not _raw_artifact_is_recoverable(not_object)

    missing = tmp_path / "nope.raw.json"
    assert not _raw_artifact_is_recoverable(missing)


def test_forgotten_done_recovery_seconds_env(monkeypatch=None):
    """Threshold defaults to 120s and honors the env override."""
    import trellis.agents.tmux_backend as tmb
    from unittest.mock import patch

    with patch.dict(os.environ, {}, clear=False):
        os.environ.pop("TRELLIS_FORGOTTEN_DONE_RECOVERY_SECONDS", None)
        assert tmb._forgotten_done_recovery_seconds() == 120.0
    with patch.dict(os.environ, {"TRELLIS_FORGOTTEN_DONE_RECOVERY_SECONDS": "45"}):
        assert tmb._forgotten_done_recovery_seconds() == 45.0
    with patch.dict(os.environ, {"TRELLIS_FORGOTTEN_DONE_RECOVERY_SECONDS": "bogus"}):
        assert tmb._forgotten_done_recovery_seconds() == 120.0


def _forgotten_done_harness(tmp_path: Path):
    """Shared fake-clock harness for the wait_until_idle recovery tests.

    Returns (tmb, done_file, raw_path, now_state, run). The simulated pane is a
    frozen *busy* gemini screen (never goes stable), so the ONLY way the loop
    exits cleanly is the done_file path — recovery or a real marker. If neither
    fires the loop runs to `total_timeout` and returns (False, 'timeout').
    """
    from unittest.mock import patch
    import trellis.agents.tmux_backend as tmb
    ...
    done_file = tmp_path / "worker_PaperFocus_7.done"
    raw_path = tmp_path / "worker_PaperFocus_7.raw.json"
    ...
    def run(**overrides):
        ...
        kwargs = dict(
            ...
            liveness_probe=None,
            apparent_stall_seconds=0.0,
            api_error_idle_seconds=0.0,
        )
        kwargs.update(overrides)
        with (
            patch.object(tmb, "capture", lambda _s, history=True: busy_screen),
            patch.object(tmb, "pane_dead", lambda _session: False),
            patch.object(tmb, "agent_is_busy", lambda _screen, _provider: True),
            patch.object(tmb.time, "monotonic", fake_monotonic),
            patch.object(tmb.time, "monotonic_ns", fake_monotonic_ns),
            patch.object(tmb.time, "sleep", fake_sleep),
        ):
            return tmb.wait_until_idle(FakeHandle(), **kwargs)

    return tmb, done_file, raw_path, now_state, run


def test_wait_until_idle_recovers_forgotten_done_marker(tmp_path: Path):
    """(a) A stable, valid raw.json with NO .done marker → the wait recovers:
    it writes the marker itself and returns done_file, instead of hanging until
    total_timeout. This is the core antigravity rescue."""
    from unittest.mock import patch
    tmb, done_file, raw_path, now_state, run = _forgotten_done_harness(tmp_path)
    # Valid finished artifact, written once (real mtime fixed); no .done.
    raw_path.write_text(json.dumps({"outcome": "Pass", "result": {}}), encoding="utf-8")
    assert not done_file.exists()

    with patch.dict(os.environ, {"TRELLIS_FORGOTTEN_DONE_RECOVERY_SECONDS": "120"}):
        done, reason = run(total_timeout=600.0)

    assert done and reason == "done_file", (
        f"expected recovery via done_file, got done={done} reason={reason}"
    )
    assert done_file.exists(), "recovery must write the .done marker"
    # Recovered only AFTER the stability window elapsed, not on the first poll.
    assert now_state[0] >= 120.0, (
        f"recovered too early at t={now_state[0]:.0f}s (threshold 120s)"
    )


class _StatWithMtime:
    """Minimal stat-result shim that overrides st_mtime_ns only."""
    def __init__(self, base, mtime_ns):
        self._base = base
        self.st_mtime_ns = int(mtime_ns)
    def __getattr__(self, name):
        return getattr(self._base, name)


def test_wait_until_idle_does_not_recover_growing_raw(tmp_path: Path):
    """(b1) A raw.json whose mtime keeps advancing (write still in flight)
    must NOT be recovered — the stability window resets every bump, so the
    loop never recovers and runs out to total_timeout."""
    from unittest.mock import patch
    tmb, done_file, raw_path, now_state, run = _forgotten_done_harness(tmp_path)
    raw_path.write_text(json.dumps({"outcome": "Pass"}), encoding="utf-8")
    ...
    def growing_stat(self, *a, **k):
        ...
        if self == raw_path:
            bump[0] += 5_000_000_000  # mtime advances every poll
            return _StatWithMtime(st, bump[0])
        return st

    with (
        patch.dict(os.environ, {"TRELLIS_FORGOTTEN_DONE_RECOVERY_SECONDS": "120"}),
        patch.object(Path, "stat", growing_stat),
    ):
        done, reason = run(total_timeout=600.0)

    assert not done_file.exists(), "must NOT write marker while raw is still growing"
    assert reason == "timeout", (
        f"expected fall-through to timeout, got done={done} reason={reason}"
    )


def test_wait_until_idle_does_not_recover_invalid_raw(tmp_path: Path):
    """(b2) A stable raw.json that is unparseable / has no `outcome` must NOT
    be recovered, even after the stability window — validity gates it out."""
    from unittest.mock import patch
    tmb, done_file, raw_path, now_state, run = _forgotten_done_harness(tmp_path)
    # Stable (single write) but missing `outcome`.
    raw_path.write_text(json.dumps({"result": {"x": 1}}), encoding="utf-8")

    with patch.dict(os.environ, {"TRELLIS_FORGOTTEN_DONE_RECOVERY_SECONDS": "120"}):
        done, reason = run(total_timeout=400.0)

    assert not done_file.exists(), "must NOT recover an outcome-less raw artifact"
    assert reason == "timeout", (
        f"expected timeout (no recovery on invalid raw), got reason={reason}"
    )


def test_wait_until_idle_does_not_recover_absent_raw(tmp_path: Path):
    """(b3) No raw.json at all and no marker → keep waiting to timeout exactly
    as before; recovery must not fabricate a result from nothing."""
    tmb, done_file, raw_path, now_state, run = _forgotten_done_harness(tmp_path)
    assert not raw_path.exists()
    done, reason = run(total_timeout=300.0)
    assert not done_file.exists()
    assert reason == "timeout", f"expected timeout with no raw, got {reason}"


def test_wait_until_idle_marker_present_path_unchanged(tmp_path: Path):
    """(c) When the .done marker is already present, the very first poll returns
    done_file immediately — recovery logic must not perturb the normal path."""
    tmb, done_file, raw_path, now_state, run = _forgotten_done_harness(tmp_path)
    done_file.write_text("done", encoding="utf-8")
    done, reason = run(total_timeout=600.0)
    assert done and reason == "done_file"
    # Returned on the first iteration — no time advanced.
    assert now_state[0] == 0.0, (
        f"marker-present path should return immediately, advanced to {now_state[0]}s"
    )


def test_wait_until_idle_recovery_disabled_when_threshold_zero(tmp_path: Path):
    """Threshold <= 0 disables recovery entirely; a stable valid raw with no
    marker then falls through to timeout (legacy behavior preserved)."""
    from unittest.mock import patch
    tmb, done_file, raw_path, now_state, run = _forgotten_done_harness(tmp_path)
    raw_path.write_text(json.dumps({"outcome": "Pass"}), encoding="utf-8")
    with patch.dict(os.environ, {"TRELLIS_FORGOTTEN_DONE_RECOVERY_SECONDS": "0"}):
        done, reason = run(total_timeout=300.0)
    assert not done_file.exists(), "threshold 0 must disable recovery"
    assert reason == "timeout"
def test_write_chat_artifacts_canonical_prefix_is_id_keyed():
    # A canonical kernel artifact name (`trellis_<kind>_<id>_<suffix>`) names
    # the chat dir DIRECTLY (no `<role>_` prefix), so the viewer's request-id
    # resolver surfaces the worker burst. request_id is parsed from the name.
    with tempfile.TemporaryDirectory() as td:
        cwd = Path(td)
        target = write_chat_artifacts(
            cwd, role="worker",
            artifact_prefix="trellis_worker_77_result",
            prompt="P", final_pane_text="PANE", transcript_last_assistant="R",
            provider="claude", model="claude-opus-4-8",
            scope="theorem_stating:worker:claude:claude-opus-4-8:max",
        )
        assert target.name == "trellis_worker_77_result", target.name
        assert (target / "pane.txt").read_text() == "PANE"
        assert (target / "prompt.txt").read_text() == "P"
        call = json.loads((target / "call.json").read_text())
        assert call["request_id"] == 77, call
        assert call["artifact_id"] == "trellis_worker_77_result"


def test_write_chat_artifacts_scope_prefix_keeps_role_prefix():
    # A legacy session-scope prefix (no `trellis_` head) keeps the `<role>_`
    # prefix — unchanged behavior for any non-canonical caller.
    with tempfile.TemporaryDirectory() as td:
        cwd = Path(td)
        target = write_chat_artifacts(
            cwd, role="worker",
            artifact_prefix="theorem_stating:worker:claude:claude-opus-4-8:max",
            prompt="P", final_pane_text="PANE", transcript_last_assistant="R",
            provider="claude",
        )
        assert target.name == "worker_theorem_stating:worker:claude:claude-opus-4-8:max", target.name
