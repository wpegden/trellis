...
def test_multiple_axiom_verdicts_fail_closed(monkeypatch, tmp_path):
    """A forged clean line plus Lean's real line must never be disambiguated."""
    axioms, _ = _probe(
        monkeypatch,
        tmp_path,
        "'forged' does not depend on any axioms\n"
...
def test_sidecar_axiom_probe_uses_the_same_root_qualified_declaration(
    monkeypatch, tmp_path
):
    """The advisory precheck must not reject work the authoritative probe accepts."""
    from types import SimpleNamespace
    from pathlib import Path
    from trellis.sidecar import prevalidate as pv
    from trellis.sidecar.config import SidecarConfig
    ...
    def _fake_run(*args, **kwargs):
        ...
        seen_source = (repo / ".trellis-sidecar-axioms-probe.lean").read_text(
            encoding="utf-8"
        )
        return SimpleNamespace(
            returncode=0,
            stdout="'singleton' does not depend on any axioms\n",
            stderr="",
        )

    monkeypatch.setattr(pv.subprocess, "run", _fake_run)
    axioms, _ = pv.run_axioms_probe(
        Path(tmp_path), "singleton", SidecarConfig(sandbox_role="")
    )
...
# Live-run node linegraph2_5, 2026-07-31: clean body, 4 open imports,
