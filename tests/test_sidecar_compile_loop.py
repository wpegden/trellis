...
mathlib), toolchain ``v4.33.0`` from ``~/.elan``, a Preamble and a
...
def _lake_available() -> bool:
    if shutil.which("lake") is None:
        return False
    toolchain_dir = Path.home() / ".elan" / "toolchains" / "leanprover--lean4---v4.33.0"
    return toolchain_dir.exists()


pytestmark = pytest.mark.skipif(
    not _lake_available(), reason="lake / v4.33.0 toolchain unavailable"
)
