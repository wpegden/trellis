...
(documented offline escape hatch); a scripted fake model submits a
failing body then the correct one. Exercises splice → didChange →
diagnostics → confirm-lake-build → probes → spooled ``success``
record, with zero API tokens.
"""
...
def _scripted_client(bodies: List[str]) -> ModelClient:
    replies = [
        {
    ...
    return ModelClient(_cfg(), "sk-test", post=post, sleep=lambda s: None)
...
def test_end_to_end_fail_then_success_over_real_lean(
    ...
    try:
        ...
        client = _scripted_client(["  exact nonexistent_lemma_xyz\n", "  rfl\n"])
        result = run_attempt(
            config=config,
            client=client,
            system_prompt="SYS",
        ...
        assert pre.local_closure_status == "skipped"  # no script in micro-ws
