...
def _stub_observations(monkeypatch: pytest.MonkeyPatch) -> dict[str, list[dict[str, Any]]]:
    ...
    def _fake_lean_semantic(
        repo: Path,
        nodes: list[str],
        *,
        timeout_secs: float,
        bwrap_role: Any = None,
        principal_names: Any = None,
...
def stubbed_server(
    ...
    # These are transport/response-shape tests: their observation stubs do
    # not create real oleans or provenance.  The replay precondition itself
    # is exercised in the checker-server and sidecar E2E suites, so bypass
    # that trusted-boundary prerequisite here rather than pretending the
    # synthetic observation payload attested artifacts it never wrote.
    monkeypatch.setattr(
        server,
...
def test_client_local_closure_scan_only_sets_request_field(raw_socket_path: Path) -> None:
    """FIX 2 coverage: ``scan_only=True`` must add ``scan_only`` to the wire
    request so the server appends ``--scan-only`` to the Lean script CLI.
    The default (``scan_only`` absent) is pinned by
    ``test_client_local_closure_envelope_shape``."""
    captured: dict[str, object] = {}
    ...
    try:
        try:
            client_local_closure_axioms(
                raw_socket_path, "Foo", timeout_secs=2.0, scan_only=True
            )
        except CheckerRpcError:
            pass
    finally:
    ...
    assert isinstance(request, dict)
    assert request.get("scan_only") is True, request


def test_client_local_closure_preamble_module_owner_sets_request_field(
    raw_socket_path: Path,
) -> None:
    """Preamble has module identity but no fabricated FILESPEC principal."""
    captured: dict[str, object] = {}
    ...
    request = captured.get("request", {})
    assert isinstance(request, dict)
    assert request.get("module_owner") is True, request
    assert "principal_name" not in request


def test_client_local_closure_rejects_module_owner_for_ordinary_node(
    tmp_path: Path,
) -> None:
    with pytest.raises(ValueError, match="reserved for Preamble"):
        client_local_closure_axioms(
