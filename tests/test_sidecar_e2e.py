...
def _lake_available() -> bool:
    if shutil.which("lake") is None:
        return False
    return (
        Path.home() / ".elan" / "toolchains" / "leanprover--lean4---v4.33.0"
    ).exists()
...
if sys.argv[1:2] == ["sync-supervisor-workspace"]:
    ...
    # `_seed_tablet` writes this shim into BOTH repos, so check.py runs
    # either from the worker (<w>/.trellis/scripts/) or from the supervisor
    # (<w>/.trellis/supervisor/repo/.trellis/scripts/). parents[2] is
    # whichever repo we are in; ONLY the supervisor copy needs the extra
    # three hops back down to the worker. Applying those hops
    # unconditionally resolved the worker repo to $HOME — and
    # sync_supervisor_workspace then mirrored the entire home directory
    # into ~/.trellis/supervisor/repo on every run of this test.
    repo_root = Path(__file__).resolve().parents[2]
    in_supervisor_copy = repo_root.parts[-3:] == (".trellis", "supervisor", "repo")
    worker_repo = repo_root.parents[2] if in_supervisor_copy else repo_root
    if worker_repo == Path.home():
        raise SystemExit(
            "rig check.py: refusing to sync $HOME as a worker repo "
            "(path arithmetic overshot; see the comment above)"
        )
...
def _init_runtime(rig: Dict[str, Any], queue: List[Dict[str, Any]] | None = None) -> None:
    ...
    _run_kernel(
        rig,
        {
            ...
            # S6 fixes (Claude 6): init refuses an already-initialized root
            # by default; this rig deliberately re-stages the runtime root
            # it just seeded, so it opts in explicitly.
            "allow_reinit": True,
            "state": state,
            "metadata": {
    ...
    # Certificate v2 deliberately refuses a non-genesis state whose existing
    # owners have no records. This fixture injects a cycle-1 state directly,
    # so bring it through the same stopped-runtime migration required of a
    # legacy production state before exercising the sidecar boundary.
    _run_kernel(
        rig,
        {
...
def _scripted_codex(bodies: List[str]):
    """Stands in for ``run_attempt_codex``: the generator is faked, the
    rest of the attempt is real.

    Only the agent is scripted — the callback it submits bodies to is the
    one ``run_grunt_attempt`` built, so the warm server, the splice, the
    confirming build, prevalidation and the spool publish all run for
    real. Bodies are consumed one per round; running out ends the attempt
    on its budget, as the wall does live."""
    ...
    def run(
        *,
        ...
        system_prompt,
        initial_body,
        compile_body_factory,
        codex_home=None,
        log=lambda _m: None,
        **_kwargs,
    ) -> "AttemptResult":
        compile_body = compile_body_factory()
        result = AttemptResult(
            status="failed", reasoning_effort=config.reasoning_effort or ""
        )
        for body in queued:
            result.iterations += 1
            result.prompt_tokens += 10
            result.completion_tokens += 5
            if compile_body(body).ok:
...
def test_grunt_attempt_good_and_bad_runs(rig: Dict[str, Any], monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        attempt_mod, "run_attempt_codex", _scripted_codex(["  rfl\n"])
    )
    ...
    monkeypatch.setattr(
        attempt_mod, "run_attempt_codex", _scripted_codex(["  exact bogus_xyz\n"] * 5)
    )
    outcome = _run_rig_attempt(
...
def test_manager_n2_assigns_from_real_export_and_cancels_on_removal(
    ...
    try:
        ...
        # let the real binary export the post-remove queue. `entry_seq`
        # 9, not 3: the generation counter must land the kernel lane's
        # re-mint of Lift ABOVE the generation the in-flight attempt
        # holds, which is what a live monotone counter does anyway (this
        # rig resets state between images).
        shutil.rmtree(rig["worker"] / ".trellis-history", ignore_errors=True)
        _init_runtime(
            rig, queue=[{"node": "Rung", "entry_seq": 9, "queued_at_cycle": 1}]
        )
        _run_kernel(
            rig,
            {"action": "run", "root": str(runtime), "max_steps": 1},
        ...
        # Removed from the REVIEWER lane is not removed from the world:
        # Lift is still eligible, so the kernel lane picks it up at a
        # FRESH generation. The cancel is keyed on the generation, so
        # the running attempt still dies.
        kernel_lane = {row["node"]: row["entry_seq"] for row in export["kernel_queue"]}
        assert kernel_lane.get("Lift", 0) > 4, export["kernel_queue"]

        lift_attempt = procs["Lift"]
        daemon.run_once()
        assert lift_attempt.poll() is not None, (
        ...
        # The SAME pass that cancelled the stale generation refills the
        # freed grunt from the kernel lane — which is the whole point of
        # the standing list. Lift is back, on the fresh generation.
        busy_now = {s.node: s for s in daemon.slots.values() if s is not None}
        assert set(busy_now) == {"Rung", "Lift"}
        assert busy_now["Lift"].entry_seq == kernel_lane["Lift"]
        rows = [
            json.loads(line)
...
def test_drain_restart_adopt_and_expire_one_generation_end_to_end(
    ...
    config = SidecarConfig(
        ...
        poll_seconds=0.01,
        export_stale_after_seconds=3600.0,
        # ONE grunt: this test is about a single generation's journey
        # across a restart, and the kernel lane would otherwise fill the
        # second slot with Lift and give the story two attempts to
        # follow.
        grunts=1,
    )
    ...
    try:
        ...
        export = json.loads((runtime / "sidecar" / "candidates.json").read_text())
        queue_nodes = [row["node"] for row in export["queue"]]
        assert queue_nodes == [], "the one adopted generation expired"
        # Kernel-queue refill: the reviewer lane is empty and the kernel
        # lane holds the standing ranked list. Rung — whose generation
        # was spent THIS cycle — is held back for a cycle; Lift, the
        # other eligible node, is resident.
        assert [row["node"] for row in export["kernel_queue"]] == ["Lift"]
...
def test_kernel_ingest_applies_through_real_checker(rig: Dict[str, Any]) -> None:
    ...
    export = json.loads((runtime / "sidecar" / "candidates.json").read_text())
    assert export["schema"] == 3
    assert all(row["node"] != "Rung" for row in export["queue"])
    assert all(row["node"] != "Rung" for row in export.get("kernel_queue", []))
    assert all(row["node"] != "Rung" for row in export["eligible_now"])
    # The reviewer's copy is the same document minus the kernel lane.
    reviewer = json.loads(
        (runtime / "sidecar" / "reviewer_candidates.json").read_text()
    )
    assert "kernel_queue" not in reviewer
    assert reviewer["queue"] == export["queue"]
