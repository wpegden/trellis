...
def test_sync_copies_thy_into_supervisor_tablet(tmp_path: Path) -> None:
    """The Isabelle worker theory ``Tablet/<Node>.thy`` mirrors supervisor-side
    exactly as a Lean node file does (the ``.thy`` allowlist clause). A non-node
    extension (``.txt``) under Tablet/ is still rejected by the positive-suffix
    filter, so the ``.thy`` clause is additive — it does not widen the filter to
    arbitrary files."""
    worker, supervisor, cache_path = _seed_worker_repo(tmp_path)
    thy_body = (
    ...
    (worker / "Tablet" / "Foo.thy").write_text(thy_body, encoding="utf-8")
    (worker / "Tablet" / "Bar.txt").write_text("not a theory\n", encoding="utf-8")

    result = sync_tablet_dir(worker, supervisor, cache_path)

    assert "Foo.thy" in result["changed"]
    assert (supervisor / "Tablet" / "Foo.thy").read_text(encoding="utf-8") == thy_body
    # The worker .thy body is mirrored byte-for-byte (the projection rename to
    # isabelle/Tablet_Foo.thy happens later in sync_session, not here).
    assert not (supervisor / "Tablet" / "Bar.txt").exists()
...
def test_sync_rejected_source_preserves_existing_supervisor_mirror(tmp_path: Path) -> None:
    worker, supervisor, cache_path = _seed_worker_repo(tmp_path)
    src = worker / "Tablet" / "LemmaA.lean"
    src.write_text("trusted prior bytes\n", encoding="utf-8")
    sync_tablet_dir(worker, supervisor, cache_path)

    src.unlink()
    target = tmp_path / "untrusted.txt"
    target.write_text("must not replace mirror\n", encoding="utf-8")
    os.symlink(target, src)
    result = sync_tablet_dir(worker, supervisor, cache_path)

    assert result["rejected"] == ["LemmaA.lean"]
    assert result["removed"] == []
    assert (
        supervisor / "Tablet" / "LemmaA.lean"
    ).read_text(encoding="utf-8") == "trusted prior bytes\n"
    assert "LemmaA.lean" in load_fingerprint_cache(cache_path)
...
def test_compute_cache_key_changes_when_only_private_artifact_part_changes(
    tmp_path: Path,
) -> None:
    """A private-only proof change must invalidate semantic closure caches."""
    repo = _seed_supervisor_repo_with_chain(tmp_path)
    exported = (
        repo / ".lake" / "build" / "lib" / "lean" / "Tablet" / "Dep1.olean"
    )
    Path(str(exported) + ".server").write_bytes(b"server-part")
    private = Path(str(exported) + ".private")
    private.write_bytes(b"private-proof-v1")
    sync_cache = _make_sync_cache(
        **{"Leaf.lean": "leafsha", "Dep1.lean": "dep1sha", "Dep2.lean": "dep2sha"}
    )
    before = compute_semantic_payload_cache_key(
        repo,
        "Leaf",
        sync_cache,
        "scriptsha",
    ...
    private.write_bytes(b"private-proof-v2")
    after = compute_semantic_payload_cache_key(
        repo,
        "Leaf",
        sync_cache,
        "scriptsha",
...
def test_unqualified_print_axioms_cache_version_is_a_cold_miss(
    tmp_path: Path,
) -> None:
    """A successful v1 result may contain the old ambiguous probe output."""
    cache_dir = print_axioms_cache_dir(tmp_path)
    key = "f" * 64
    store_print_axioms(
        cache_dir,
        key,
        ...
        returncode=0,
        stdout=(
            "'Singleton.singleton' does not depend on any axioms\n"
            "'singleton' does not depend on any axioms\n"
        ),
        stderr="",
        timed_out=False,
        spawn_error="",
    ...
    assert PRINT_AXIOMS_CACHE_VERSION > 1
    assert (
        load_print_axioms(
            cache_dir,
            key,
