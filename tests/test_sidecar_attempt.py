...
loop (both already exercised on their own) and the driver loop, leaving
exactly the runner's outcome/report surface under test.
"""
...
def _stub_runner(
    ...
    driver result; returns the loop it will use."""
    ...
    monkeypatch.setattr(
        compile_loop_mod, "make_goal_tool_handler", lambda *_a, **_k: (lambda _a2: "")
    )
    monkeypatch.setattr(attempt_mod, "read_api_key", lambda _cfg: "sk-test")
    monkeypatch.setattr(attempt_mod, "ModelClient", lambda *_a, **_k: object())
    monkeypatch.setattr(attempt_mod, "run_attempt", lambda **_kw: result)
    return loop
...
def test_failed_attempt_log_line_names_the_cause(
    ...
    it a `perfect`-class incident is diagnosable only by reconstructing
