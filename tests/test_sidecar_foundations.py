...
def test_config_default_budgets_are_wall_clock_regime() -> None:
    ...
    Per-attempt token/iteration caps and cumulative volume caps are all
    DISABLED (0). Throughput is bounded only by the grunt pool + wall.
    Compaction fires at the 200k live-context threshold (vibe's
    auto_compact analog).

    The wall is 60 min by operator decision, down from the 90 min the
    bench evidence was collected under (whose decisive close took 64.9
    min). Longer walls buy few extra closures for the slot-time they
    hold, and the wall is settable per-run (`--grunt-wall`), so the
    default is a throughput choice rather than a correctness one."""
    cfg = SidecarConfig()
    assert cfg.attempt_wall_seconds == 3600.0
    assert cfg.max_iterations == 0
    assert cfg.attempt_tokens == 0
...
def test_kernel_feedback_rewrites_grunt_result_and_log_idempotently(
    tmp_path: Path,
) -> None:
    gdir = tmp_path / "sidecar" / "grunts" / "2"
    feedback_dir = tmp_path / "sidecar" / "feedback"
    gdir.mkdir(parents=True)
    feedback_dir.mkdir(parents=True)
    attempt_id = "sc-feedback-Rung"
    result = gdir / f"result-{attempt_id}.json"
    log = gdir / f"attempt-{attempt_id}.log"
    result.write_text(json.dumps({"status": "success", "detail": "closed"}))
    log.write_text(f"sidecar[{attempt_id}]: attempt success\n")
    feedback = {
        "schema": 1,
    ...
    (feedback_dir / "feedback-feedback-1.json").write_text(json.dumps(feedback))

    assert deliver_kernel_feedback(tmp_path) == 1
    rewritten = json.loads(result.read_text())
    assert rewritten["status"] == "rejected"
    assert rewritten["detail"] == "closure_probe (axiom violation)"
    assert rewritten["grunt_outcome"] == {"status": "success", "detail": "closed"}
    assert rewritten["kernel_feedback"] == [feedback]
    assert "kernel rejected: closure_probe" in log.read_text()
    assert "[feedback-1]" in log.read_text()
    assert deliver_kernel_feedback(tmp_path) == 0
    assert log.read_text().count("kernel rejected") == 1
