...
def test_build_nodes_renders_pv_waived_corr_and_substantiveness_solid(tmp_path: Path) -> None:
    ...
    for name, declaration in {
        ...
        "GoalStmt": "theorem GoalStmt : True := by\n-- BODY\n  sorry\n",
        "GoalStmt__Refutation": "theorem GoalStmt__Refutation : True := by\n-- BODY\n  sorry\n",
    }.items():
    ...
    present = [
        ...
        "WorkerDef",
        "GoalStmt",
        "GoalStmt__Refutation",
    ]
    protocol_state = {
        "phase": "TheoremStating",
        "live": {
            "present_nodes": list(present),
        ...
        "challenge_claims": {
            ...
            # D6 re-key: a claim on a worker_authored target does NOT waive
            # the lanes; a claim on a kernel_derived twin still does.
            "GoalStmt": ["goal:f"],
            "GoalStmt__Refutation": ["goal:f__refutation"],
        },
        "configured_challenge_targets": {
            "goal:f": {
                "kind": "Theorem",
                "resolution": "decide",
                "statement_provenance": "worker_authored",
            },
            "goal:f__refutation": {
                "kind": "Theorem",
                "statement_provenance": "kernel_derived",
            },
        },
    ...
    # D6 re-key: the worker-authored GOAL statement node is NOT waived —
    # both lanes render unverified — while the kernel-derived refutation
    # twin's claimant keeps the waiver.
    assert nodes["GoalStmt"]["verification"]["correspondence"] == "?"
    assert nodes["GoalStmt"]["verification"]["substantiveness"] == "?"
    assert nodes["GoalStmt__Refutation"]["verification"]["correspondence"] == "pass"
    assert nodes["GoalStmt__Refutation"]["verification"]["substantiveness"] == "pass"


def test_gate_view_separates_routine_advance_from_fail_loud_need_input() -> None:
    # The viewer used to key the human-review panel off the reviewer decision
    # string, which renders a routine phase-advance checkpoint and the
    # fail-loud NeedInput escalation identically. `gate_kind` is the kernel's
    # own discriminator; project it so the viewer never has to guess.
    advance = _gate_view({"gate_kind": "Advance"})
    assert advance["kind"] == "advance"
    assert advance["reason"] == ""
    assert advance["reason_source"] == ""

    need_input = _gate_view(
        {
            "gate_kind": "NeedInput",
            "gate_from_invalid_attempt": True,
            "latest_stuck_math_audit_rejection_reason": "NeedInputAuditor failed twice in a row",
        }
    )
    assert need_input["kind"] == "need_input"
    assert need_input["from_invalid_attempt"] is True
    assert need_input["reason"] == "NeedInputAuditor failed twice in a row"
    assert need_input["reason_source"] == "latest_stuck_math_audit_rejection_reason"

    assert _gate_view({"gate_kind": "None"})["kind"] == "none"
    assert _gate_view({"gate_kind": "ProtectedReapproval"})["kind"] == "protected_reapproval"
    assert _gate_view({"gate_kind": "AssumptionReview"})["kind"] == "assumption_review"


def test_gate_view_never_decorates_a_routine_gate_with_a_stale_incident() -> None:
    # The reason carriers are not cleared on every transition. Surfacing them
    # at an advance gate would turn a normal checkpoint into a false incident.
    gate = _gate_view(
        {
            "gate_kind": "Advance",
            "latest_stuck_math_audit_rejection_reason": "stale escalation text",
            "gap_human_escalation": {"reason": "planner_human_required"},
        }
    )
...
def test_gate_view_reason_priority_and_gap_research_payload() -> None:
    gate = _gate_view(
        {
            "gate_kind": "NeedInput",
            "stuck_math_audit": {"need_input_audit": {"reviewer_reason": "reviewer could not proceed"}},
            "gap_human_escalation": {
                "reason": "planner_human_required",
                "precise_question_tex": "Is the constant 2 or 3?",
                "minimal_unblocking_input": "the intended constant",
                "what_was_ruled_out": ["induction on n"],
                "escalated_at_cycle": 941,
            },
        }
    )
    # No `latest_stuck_math_audit_rejection_reason`, so the audit's reviewer
    # reason wins over the bare GapResearch reason code.
    assert gate["reason"] == "reviewer could not proceed"
    assert gate["reason_source"] == "stuck_math_audit.need_input_audit.reviewer_reason"
    assert gate["question"] == "Is the constant 2 or 3?"
    assert gate["unblocking_input"] == "the intended constant"
    assert gate["ruled_out"] == ["induction on n"]
    assert gate["escalated_at_cycle"] == 941


def test_gate_view_degrades_on_a_runtime_predating_the_field() -> None:
    # Empty kind (not "none") is the "this runtime has no gate_kind" signal:
    # the viewer falls back to its legacy reviewer-decision heuristic.
    assert _gate_view({})["kind"] == ""
    # ...but the in-flight HumanGate request carries the same field, so a
    # partial state still classifies.
    inferred = _gate_view({"in_flight_request": {"kind": "HumanGate", "gate_kind": "NeedInput"}})
    assert inferred["kind"] == "need_input"


def test_live_viewer_state_projects_the_gate_for_the_frontend(monkeypatch, tmp_path) -> None:
    # The gate classification is useless unless it reaches the wire. Deleting
    # the `"gate": _gate_view(...)` line from `_build_live_viewer_state` must
    # fail something; without this test the whole suite stays green.
    runtime_root = tmp_path / "runtime"
    runtime_root.mkdir()
    protocol_state_path = runtime_root / "protocol_state.json"
    protocol_state_path.write_text(
        json.dumps(
            {
                "cycle": 12,
                "stage": "HumanGate",
                "gate_kind": "NeedInput",
                "in_flight_request": {"kind": "HumanGate"},
                "latest_stuck_math_audit_rejection_reason": "lane could not progress",
            }
        ),
        encoding="utf-8",
    )
    ...
    state = viewer_adapter._build_live_viewer_state(tmp_path)["state"]

    assert state["awaiting_human_input"] is True
    assert state["gate"]["kind"] == "need_input"
    assert state["gate"]["reason"] == "lane could not progress"
    assert state["gate"]["reason_source"] == "latest_stuck_math_audit_rejection_reason"
