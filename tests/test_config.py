...
from trellis.config import TEX_STATEMENT_ENVS, ConfigError, PolicyManager, load_config
...
_RESOLVE_OK = {
    "status": "resolve_main_result_targets_ok",
    "output": {
        "targets": [{"tex_label": "thm:main", "start_line": 3, "end_line": 5}],
        "available_labels": ["thm:main"],
        "preview": [],
    },
}
...
def test_load_config_threads_main_result_envs(tmp_path: Path) -> None:
    config_path = _write_config(
        tmp_path / "repo",
        {
            "paper_tex_path": "paper.tex",
            "main_result_envs": [" Theorem ", "PROPOSITION", "theorem"],
        },
    )

    with patch("trellis.config.run_kernel_cli", return_value=_RESOLVE_OK) as mock_kernel:
        config = load_config(config_path)

    payload = mock_kernel.call_args.args[0]
    assert payload["main_result_envs"] == ["theorem", "proposition"]
    assert config.workflow.main_result_envs == ["theorem", "proposition"]


def test_load_config_omits_main_result_envs_when_unconfigured(tmp_path: Path) -> None:
    config_path = _write_config(tmp_path / "repo", {"paper_tex_path": "paper.tex"})

    with patch("trellis.config.run_kernel_cli", return_value=_RESOLVE_OK) as mock_kernel:
        config = load_config(config_path)

    payload = mock_kernel.call_args.args[0]
    assert payload["main_result_envs"] is None
    assert config.workflow.main_result_envs == []


def test_load_config_rejects_non_canonical_main_result_env(tmp_path: Path) -> None:
    config_path = _write_config(
        tmp_path / "repo",
        {"paper_tex_path": "paper.tex", "main_result_envs": ["theorem", "conjecture"]},
    )

    with patch("trellis.config.run_kernel_cli", return_value=_RESOLVE_OK):
        with pytest.raises(ConfigError) as excinfo:
            load_config(config_path)

    assert "'conjecture'" in str(excinfo.value)
    assert "normalize_paper_envs.py" in str(excinfo.value)


@pytest.mark.parametrize("raw_envs", [[], ["theorem", ""], "theorem"])
def test_load_config_rejects_malformed_main_result_envs(tmp_path: Path, raw_envs: object) -> None:
    config_path = _write_config(
        tmp_path / "repo",
        {"paper_tex_path": "paper.tex", "main_result_envs": raw_envs},
    )
    ...
    assert "config.workflow.main_result_envs" in str(excinfo.value)


def test_tex_statement_envs_mirrors_the_kernel_constant() -> None:
    """The subset check in config.py duplicates the kernel's canonical env
    list; this pins the two spellings together.
    """
    source = (REPO_ROOT / "kernel" / "src" / "paper_targets.rs").read_text(encoding="utf-8")
    match = re.search(r"const TEX_STATEMENT_ENVS: &\[&str\] = &\[(.*?)\];", source, re.DOTALL)
    assert match, "kernel TEX_STATEMENT_ENVS constant not found"
    kernel_envs = tuple(re.findall(r'"([^"]+)"', match.group(1)))
    assert kernel_envs == TEX_STATEMENT_ENVS


def _write_challenge_registry(repo: Path) -> Path:
    registry_path = repo / "challenge" / "challenge_targets.json"
...
def test_load_config_parses_challenge_targets_path(tmp_path: Path) -> None:
    repo = tmp_path / "repo"
    config_path = _write_config(
        repo,
        {
            "paper_tex_path": "paper.tex",
            "challenge_targets_path": "challenge/challenge_targets.json",
        },
    )
    registry_path = _write_challenge_registry(repo)
    ...
    assert config.workflow.challenge_targets_path == registry_path.resolve()


def test_load_config_defaults_challenge_targets_path_to_none(tmp_path: Path) -> None:
    config_path = _write_config(tmp_path / "repo", {"paper_tex_path": "paper.tex"})
    ...
    assert config.workflow.challenge_targets_path is None


def test_load_config_rejects_missing_challenge_targets_file(tmp_path: Path) -> None:
    config_path = _write_config(
        tmp_path / "repo",
        {
            "paper_tex_path": "paper.tex",
    ...
    with patch(
        ...
        with pytest.raises(ConfigError, match="challenge_targets_path"):
            load_config(config_path)


def test_load_config_rejects_challenge_registry_without_targets(tmp_path: Path) -> None:
    repo = tmp_path / "repo"
    config_path = _write_config(
        repo,
        {
    ...
    registry_path = repo / "challenge" / "challenge_targets.json"
    registry_path.parent.mkdir(parents=True, exist_ok=True)
    registry_path.write_text(json.dumps({"schema_version": 1, "targets": []}), encoding="utf-8")
    ...
    with patch(
        ...
        with pytest.raises(ConfigError, match="non-empty `targets` array"):
            load_config(config_path)
...
def test_parse_provider_config_accepts_antigravity() -> None:
    from trellis.config import _parse_provider_config

    pc = _parse_provider_config(
        {"provider": "antigravity", "model": "Gemini 3.1 Pro (High)"},
        "config.worker",
    )
    assert pc.provider == "antigravity"
    assert pc.model == "Gemini 3.1 Pro (High)"


def test_parse_provider_config_rejects_unknown_provider() -> None:
    from trellis.config import _parse_provider_config

    with pytest.raises(ConfigError):
        _parse_provider_config({"provider": "bogus"}, "config.worker")


def test_forbidden_keywords_default_mirrors_kernel_lean_list() -> None:
    """FORBIDDEN_KEYWORDS_DEFAULT is the Python mirror of the kernel's
    LEAN_FORBIDDEN_KEYWORDS (kernel/src/backend.rs). Order is load-bearing
    (first-hit reporting), so the mirror must match entry-for-entry."""
    from trellis.config import FORBIDDEN_KEYWORDS_DEFAULT

    backend_rs = (REPO_ROOT / "kernel" / "src" / "backend.rs").read_text(
        encoding="utf-8"
    )
    match = re.search(
        r"pub const LEAN_FORBIDDEN_KEYWORDS: &\[&str\] = &\[(.*?)\];",
        backend_rs,
        re.DOTALL,
    )
    assert match is not None, "LEAN_FORBIDDEN_KEYWORDS not found in backend.rs"
    rust_list = re.findall(r'"([^"]+)"', match.group(1))
    assert list(FORBIDDEN_KEYWORDS_DEFAULT) == rust_list
    assert rust_list[0] == "sorryAx"


def _write_history_config(repo: Path, history: object) -> Path:
    """A minimal loadable config, optionally carrying a `history` section."""
    repo.mkdir(parents=True, exist_ok=True)
    raw: dict[str, object] = {
        "repo_path": ".",
        "worker": {"provider": "codex"},
...
def test_history_shared_state_defaults_to_true(tmp_path: Path) -> None:
    """The writer flip is on unless a config turns it off — the plain form
    grows past GitHub's 100 MB per-file limit on a long run, and every reader
    already decodes both forms."""
    config_path = _write_history_config(tmp_path / "repo", None)
    with patch("trellis.config.run_kernel_cli", return_value=_RESOLVE_OK):
        config = load_config(config_path)
    assert config.history.shared_state is True


def test_history_shared_state_can_be_turned_off(tmp_path: Path) -> None:
    config_path = _write_history_config(tmp_path / "repo", {"shared_state": False})
    with patch("trellis.config.run_kernel_cli", return_value=_RESOLVE_OK):
        config = load_config(config_path)
    assert config.history.shared_state is False


def test_history_shared_state_rejects_a_non_bool(tmp_path: Path) -> None:
    config_path = _write_history_config(tmp_path / "repo", {"shared_state": "yes"})
    with patch("trellis.config.run_kernel_cli", return_value=_RESOLVE_OK):
        with pytest.raises(ConfigError, match="history.shared_state"):
            load_config(config_path)
