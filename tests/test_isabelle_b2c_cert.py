...
def test_cert_omission_server_injects_probe(fake_db, tmp_path: Path) -> None:
    ...
    # (a) The SERVER physically wrote the checker-owned probe theory.
    probe_path = master / "Tablet_Foo__Cert.thy"
...
# These two construct a real ``IsabelleSession`` (not the fully-mocked
# ``_session_against_fake`` harness), so session cold-start resolves
# ``ISABELLE_HOME_USER`` by shelling out to the Isabelle binary. On a clean
# checkout without a resolvable ``isabelle`` (``TRELLIS_ISABELLE_BIN`` unset and
# no install at the default path) there is no binary to probe, so skip rather
# than fail; the version-pin logic itself is covered by
# ``test_parse_isabelle_version_forms``.
_needs_resolvable_isabelle = pytest.mark.skipif(
    not os.path.exists(isa.isabelle_bin()),
    reason="Isabelle binary not resolvable in this checkout (set TRELLIS_ISABELLE_BIN)",
)


@_needs_resolvable_isabelle
def test_start_rejects_old_isabelle_version(monkeypatch: pytest.MonkeyPatch) -> None:
...
@_needs_resolvable_isabelle
def test_version_pin_accepts_2025_2(monkeypatch: pytest.MonkeyPatch) -> None:
