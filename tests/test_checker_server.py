...
def test_resolve_worker_repo_explicit_metadata_repo_path_wins(tmp_path: Path) -> None:
    ...
    # sibling "designs-resume" (which here does not exist → would raise), but
    ...
    real_repo = tmp_path / "designs"
    (real_repo / ".trellis").mkdir(parents=True)
    runtime = tmp_path / "designs-resume-runtime"
    runtime.mkdir()
...
def _record_olean_provenance(repo: Path, node_name: str) -> None:
    """Mark a fake-built olean current with its source (what real
    materialize does in its post-build walk)."""
...
def test_start_prewarms_oleans_known_current_for_current_nodes(
    ...
    """Optimization #3 (content-freshness): ``start()`` scans the
    supervisor's olean tree and seeds ``_oleans_known_current`` with nodes
    whose olean is current with its source CONTENT — i.e. a provenance
    sidecar matching the current source-closure hash. Currency is decided by
    content hash, never mtime, so an operator ``touch`` / ``git reset`` that
    makes a content-stale olean look mtime-fresh cannot seed it here.
    """
    server = CheckerServer(runtime_root, parallelism=1, socket_group_gid=None)
    # Seed supervisor Tablet/ + .lake/build/.../Tablet/ with a mix of
    # current and stale-olean nodes so we can verify the filter. check.py +
    # Preamble.lean make the source-closure hash computable.
    ...
    # Fresh: olean with a provenance sidecar matching its source closure.
    # mtime is deliberately set OLDER than the source to prove the decision
    # is content-based, not mtime-based.
    (super_tablet / "Fresh.lean").write_text("-- src\n", encoding="utf-8")
    (olean_dir / "Fresh.olean").write_bytes(b"olean-fresh")
    _record_olean_provenance(server.supervisor_repo, "Fresh")
    fresh_src_mtime = (super_tablet / "Fresh.lean").stat().st_mtime
    os.utime(olean_dir / "Fresh.olean", (fresh_src_mtime - 100, fresh_src_mtime - 100))

    # Stale: olean with a WRONG provenance sidecar (content drift), frozen
    # mtime-newer than the source. Must NOT be pre-warmed — this is the
    # operator-touch shape the fix guards against.
    (super_tablet / "Stale.lean").write_text("-- src\n", encoding="utf-8")
    (olean_dir / "Stale.olean").write_bytes(b"olean-stale")
    _obs.write_olean_srcclosure(server.supervisor_repo, "Stale", "deadbeef" * 8)
    stale_src_mtime = (super_tablet / "Stale.lean").stat().st_mtime
    os.utime(olean_dir / "Stale.olean", (stale_src_mtime + 100, stale_src_mtime + 100))

    # No provenance sidecar at all: fail-closed, must NOT be pre-warmed even
    # though the olean is mtime-newer.
    (super_tablet / "Unvouched.lean").write_text("-- src\n", encoding="utf-8")
    (olean_dir / "Unvouched.olean").write_bytes(b"olean-unvouched")
    uv_src_mtime = (super_tablet / "Unvouched.lean").stat().st_mtime
    os.utime(olean_dir / "Unvouched.olean", (uv_src_mtime + 100, uv_src_mtime + 100))
    ...
    _record_olean_provenance(server.supervisor_repo, "Empty")
    ...
    try:
        ...
        assert "Unvouched" not in server._oleans_known_current
...
def test_handle_local_closure_axioms_scan_only_appends_flag_and_bypasses_cache(
    runtime_root: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """FIX 2 coverage: a ``scan_only`` request must append ``--scan-only``
    to the Lean script CLI and bypass the olean-keyed sidecar cache (the
    scan is parse-only and not olean-keyed). Asserts the flag reaches the
    ...
    # Scan-only must bypass the cache — a clean scan-only result is never
    # persisted (it is parse-only and not olean-keyed).
