    ...
    def _fake_run_lake_command(
        ...
        *,
        ...
        if (
            ...
            if "lean_module_manifest.lean" in str(argv[3]):
                nodes = argv[4:]
                return {
                    "returncode": 0,
                    "stdout": "\n".join(
                        json.dumps(
                            {
                                ...
                                "artifact_parts": [{
                                    "level": "exported",
                                    "path": str(Path(repo) / ".lake/build/lib/lean/Tablet" / f"{node}.olean"),
...
def stubbed_server(
    ...
    # function then walks the closure off these files. Preamble.lean is
    # included so the content-freshness source-closure hash is computable
    # (it reads Tablet/Preamble.lean unconditionally).
    for node_name in ("A", "B", "LemmaA", "Stub", "Preamble"):
        (server.worker_repo / "Tablet" / f"{node_name}.lean").write_text(
            "", encoding="utf-8"
        )
    # The check.py dispatch script is part of the source-closure hash and is
    # NOT synced (sync mirrors Tablet/ only), so seed it on the supervisor
    # side directly.
    check_script = server.supervisor_repo / ".trellis" / "scripts" / "check.py"
...
def test_print_axioms_root_qualifies_a_colliding_node_in_the_real_probe_path(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """The checker-authored tempfile must select the tablet's root theorem.

    ``singleton`` is a real collision shape: Lean can also expose
    ``Singleton.singleton`` under the short name.  The fake lake boundary
    reads the actual production tempfile before ``print_axioms`` removes it,
    so this test cannot pass via a hand-built expected probe disconnected from
    the supplying path.
    """
    repo = tmp_path / "repo"
    (repo / "Tablet").mkdir(parents=True)
    ...
    def _fake_run_lake_command(
        ...
        *,
        ...
        assert list(args[:2]) == ["env", "lean"]
        probe_path = called_repo / args[-1]
        seen_source = probe_path.read_text(encoding="utf-8")
        verdict = (
            "'singleton' does not depend on any axioms\n"
            if "#print axioms _root_.singleton" in seen_source
            else (
    ...
    assert seen_source == (
        "import Tablet.singleton\n#print axioms _root_.singleton\n"
    )
    assert payload["returncode"] == 0
    assert payload["stdout"].count("does not depend on any axioms") == 1


def test_print_axioms_root_qualification_preserves_an_ordinary_node_probe(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """A non-colliding ordinary node keeps the same successful audit result."""
    repo = tmp_path / "repo"
    (repo / "Tablet").mkdir(parents=True)
    seen_source = ""
    ...
    def _fake_run_lake_command(
        ...
        *,
        ...
        return {
            "returncode": 0,
            "stdout": "'OrdinaryNode' depends on axioms: [propext]\n",
            "stderr": "",
            "timed_out": False,
            "spawn_error": "",
        }
    ...
    assert seen_source == (
        "import Tablet.OrdinaryNode\n#print axioms _root_.OrdinaryNode\n"
    )
    assert payload["returncode"] == 0
    assert payload["stdout"] == "'OrdinaryNode' depends on axioms: [propext]\n"
...
def test_lake_compiler_bwrap_prepends_elan_bin_to_path(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(observations, "_Wait4Popen", _FakePopen)
