...
For each node on the frontier, use the verdict values in the emitted contract. PV prose requests additionally admit `FalseAsStated` for a substantive, nontrivial, faithfully intended candidate that is refuted under the byte-pinned extraction model.
...
Follow the emitted rubric on strength. PV prose treats unjustified strengthening of `GOAL.md` as `Fail`.
...
- `FalseAsStated` — when the emitted contract admits it, the candidate satisfies the substantive and faithfulness criteria but is refuted under the pinned model. Comment required.
...
- Render `decision: PASS` iff every verdict is Pass, an admitted FalseAsStated, or NotDoneYet; otherwise `FAIL`. NotDoneYet alone does not Fail the lane.
...
- Every FalseAsStated and Fail must carry a non-empty `comment`. Comments on Pass and NotDoneYet are optional.
